...
The standalone CMake build is primarily intended for developers and, as such,
has many build requirements. If embedding BoringSSL into another project
with a pre-existing build system, additionally see the
[INCORPORATING.md](/INCORPORATING.md) document.

Unless otherwise noted, build tools must be up to date within five years,
matching [Abseil guidelines](https://abseil.io/about/compatibility). If in
doubt, use the most recent stable version of each tool.
