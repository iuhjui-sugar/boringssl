 ...
 */
use crate::digest::{Digest, Sha256, Sha512};
...
/// wrapper type for Hmac SHA-256
pub type HmacSha256 = Hmac<32, Sha256>;

/// wrapper type for Hmac SHA-256
pub type HmacSha512 = Hmac<64, Sha512>;
...
#[inline]
fn hmac<const N: usize, D: Digest<N>>(key: &[u8], data: &[u8]) -> Result<[u8; N], InvalidLength> {
    let mut buf = [0_u8; N];
...
pub struct Hmac<const N: usize, D: Digest<N>> {
    ctx: *mut HMAC_CTX,
...
impl<const N: usize, D: Digest<N>> Hmac<N, D> {
    ...
    pub fn verify(self, tag: [u8; N]) -> Result<(), MacError> {
    ...
    pub fn verify_slice(self, tag: &[u8]) -> Result<(), MacError> {
        tag.len().eq(&N).then(|| ()).ok_or(MacError)?;
...
impl<const N: usize, D: Digest<N>> Drop for Hmac<N, D> {
    fn drop(&mut self) {
...
fn validate_key_len<const N: usize>(len: usize) -> bool {
...
mod tests {
    ...
    fn run_hmac_test_vectors<const N: usize, D: Digest<N>>(hash: HashAlg) {
        ...
        for test_group in test_set.test_groups {
            for test in test_group.tests {
                ...
                if let Some(desc) =
                    run_test::<N, D>(key.as_slice(), msg.as_slice(), tag.as_slice(), valid)
                {
                    panic!(
                        "\n\
    ...
    fn run_test<const N: usize, D: Digest<N>>(
        key: &[u8],
        input: &[u8],
        tag: &[u8],
        valid_data: bool,
    ) -> Option<&'static str> {
        let mut mac: Hmac<N, D> = Hmac::new_from_slice(key).unwrap();
        mac.update(input);
