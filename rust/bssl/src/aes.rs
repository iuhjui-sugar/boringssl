...
impl Aes128Key {
    /// configures the aes key to encrypt returning a new `AesEncryptKey` and consuming self
    pub fn new_encrypt_key(self) -> AesEncryptKey {
        AesKeyImpl::new_encrypt_key(self.0)
    }

    /// configures the aes key to decrypt returning a new `AesDecryptKey` and consuming self
