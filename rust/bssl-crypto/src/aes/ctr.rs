...
use super::{AesEncryptKey, BLOCK_SIZE};
use crate::CSliceMut;

/// Implementation of CTR128 mode for an AES block cipher.
pub struct AesCtr {
    key: AesEncryptKey,
    iv: [u8; BLOCK_SIZE],
    ecount_buf: [u8; BLOCK_SIZE],
    num: u8,
}

impl AesCtr {
    /// Instantiates a new AES-CTR stream cipher instance from the provided AesKey and iv
    pub fn new(key: AesEncryptKey, iv: [u8; BLOCK_SIZE]) -> Self {
        Self {
            key,
            iv,
            ecount_buf: [0u8; BLOCK_SIZE],
            num: 0,
        }
    }
    ...
    pub fn apply_keystream(&mut self, data: &mut [u8]) {
        let mut data_cslice = CSliceMut::from(data);
        // Safety:
        // -
        unsafe {
            bssl_sys::AES_ctr128_encrypt(
                data_cslice.as_mut_ptr(),
                data_cslice.as_mut_ptr(),
                data_cslice.len(),
                &self.key.0,
                self.iv.as_mut_ptr(),
                self.ecount_buf.as_mut_ptr(),
                &mut core::ffi::c_uint::from(self.num),
            )
        }
    }
}
...
#[cfg(test)]
mod test {
    use crate::aes::ctr::{AesCtr, AesEncryptKey};
