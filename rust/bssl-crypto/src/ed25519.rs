...
impl KeyPair {
    ...
    /// Builds this key-pair from a `seed`, which corresponds to the private key in this
    /// implementation
    pub fn new_from_seed(seed: [u8; SECRET_KEY_LENGTH]) -> Self {
        let mut public_key = [0u8; PUBLIC_KEY_LENGTH];
        let mut private_key = [0u8; KEYPAIR_LENGTH];
...
mod test {
    ...
    fn ed25519_empty_msg() {
        ...
        let kp = KeyPair::new_from_seed(sk);
        let sig = kp.sign(&msg);
