...
macro_rules! unsafe_iuf_algo {
    ($name:ident, $output_len:expr, $output_block_len:expr, $evp_md:ident, $one_shot:ident, $init:ident, $update:ident, $final_func:ident) => {
        impl Algorithm for $name {
            const OUTPUT_LEN: usize = $output_len as usize;
            const OUTPUT_BLOCK_LEN: usize = $output_block_len as usize;
            ...
            fn digest(mut self) -> alloc::vec::Vec<u8> {
                ArraySizedAlgorithm::<$output_len>::digest(self).to_vec()
            }
        ...
        impl<const OUTPUT_LEN: usize> ArraySizedAlgorithm<OUTPUT_LEN> for $name {
            /// Finish the hashing and return the digest.
