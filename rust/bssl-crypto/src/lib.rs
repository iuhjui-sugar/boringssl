...
///
/// (Tagged `unsafe` because otherwise this function could be used to
    ...
    F: FnOnce(*mut u8, usize),
{
    ...
    func(out_uninit.as_mut_ptr() as *mut u8, N);
...
/// Wrap a closure that writes at most `max_output` bytes to fill a vector.
/// If successfsul, it must return the number of bytes written.
...
fn cbb_to_buffer<F: FnOnce(*mut bssl_sys::CBB)>(initial_capacity: usize, func: F) -> Buffer {
    ...
    let mut cbb =
        unsafe { initialized_struct_fallible(|cbb| bssl_sys::CBB_init(cbb, initial_capacity)) }
