...
//!
//! let key: [u8; 32] = HkdfSha256::derive(b"secret", b"salt", b"info");
//! ```
//!
...
//! use bssl_crypto::hkdf::HkdfSha256;
//!
//! let prk = HkdfSha256::extract(b"secret", b"salt");
//! let key1 : [u8; 32] = prk.expand(b"info1");
//! let key2 : [u8; 32] = prk.expand(b"info2");
//!
//! assert_eq!(key1, HkdfSha256::derive(b"secret", b"salt", b"info1"));
//! assert_eq!(key2, HkdfSha256::derive(b"secret", b"salt", b"info2"));
//! ```
//!
...
//! use bssl_crypto::hkdf::HkdfSha256;
//!
//! let key: [u8; 256] = HkdfSha256::derive(b"secret", b"", b"info");
//! ```
//!
...
//!
//! let mut key = [0u8; 50];
//! HkdfSha256::derive_into(b"secret", b"salt", b"info", &mut key).expect(
//!    "HKDF can't produce that much");
//!
//! assert_eq!(key, HkdfSha256::derive(b"secret", b"salt", b"info"));
//! ```
...
impl<MD: digest::Algorithm> Hkdf<MD> {
    ...
    pub fn derive<const N: usize>(secret: &[u8], salt: &[u8], info: &[u8]) -> [u8; N] {
    ...
    /// Double-check which parameter is the secret/IKM and which is
    /// the salt when using.
    pub fn extract(secret: &[u8], salt: &[u8]) -> Prk {
...
mod tests {
    ...
    fn sha256() {
        ...
        let okm: [u8; 42] = HkdfSha256::derive(ikm.as_slice(), salt.as_slice(), info.as_slice());
    ...
    fn sha512() {
        ...
        let okm: [u8; 42] = HkdfSha512::derive(ikm.as_slice(), salt.as_slice(), info.as_slice());
        let expected = decode_hex(
    ...
    #[test]
    fn max_output() {
        let hkdf = HkdfSha256::extract(b"", b"");
