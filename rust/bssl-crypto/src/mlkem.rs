...
use crate::{cbb_to_array, initialized_struct, with_output_array_fallible, FfiSlice};
use core::mem::MaybeUninit;

/// An ML-KEM-768 public key.
pub struct PublicKey768(bssl_sys::MLKEM768_public_key);

/// An ML-KEM-768 private key.
pub struct PrivateKey768(bssl_sys::MLKEM768_private_key);

/// An ML-KEM-1024 public key.
///
/// Use ML-KEM-768 unless you have a good reason to need this larger size.
pub struct PublicKey1024(bssl_sys::MLKEM1024_public_key);

/// An ML-KEM-1024 private key.
///
/// Use ML-KEM-768 unless you have a good reason to need this larger size.
pub struct PrivateKey1024(bssl_sys::MLKEM1024_private_key);
...
impl PublicKey768 {
    ...
    pub fn parse(encoded: &[u8]) -> Option<Self> {
        ...
        let result =
            unsafe { bssl_sys::MLKEM768_parse_public_key(public_key.as_mut_ptr(), &mut cbs) };

        if result != 1 || cbs.len != 0 {
            return None;
        }

        Some(Self(unsafe { public_key.assume_init() }))
    }

    /// Return the serialization of this public key.
    pub fn to_bytes(&self) -> [u8; PUBLIC_KEY_BYTES_768] {
        unsafe {
            cbb_to_array(|cbb| {
    ...
    pub fn encapsulate(&self) -> ([u8; CIPHERTEXT_BYTES_768], [u8; SHARED_SECRET_BYTES]) {
        let mut ciphertext = MaybeUninit::<[u8; CIPHERTEXT_BYTES_768]>::uninit();
        ...
        unsafe {
            ...
            (ciphertext.assume_init(), shared_secret.assume_init())
        }
    }
}
...
impl PrivateKey768 {
    ...
    pub fn generate() -> ([u8; PUBLIC_KEY_BYTES_768], PrivateKey768, [u8; SEED_BYTES]) {
        let mut public_key = MaybeUninit::<[u8; PUBLIC_KEY_BYTES_768]>::uninit();
        let mut private_key = MaybeUninit::<PrivateKey768>::uninit();
    ...
    pub fn from_seed(seed: &[u8; SEED_BYTES]) -> Self {
        Self(unsafe {
            initialized_struct(|priv_key| {
                // Safety: `priv_key` is always fully written.
...
impl PublicKey1024 {
    ...
    pub fn parse(encoded: &[u8]) -> Option<Self> {
        ...
        // is fully written if this function returns 1.
        let result =
            unsafe { bssl_sys::MLKEM1024_parse_public_key(public_key.as_mut_ptr(), &mut cbs) };
    ...
    /// Return the serialization of this public key.
    pub fn to_bytes(&self) -> [u8; PUBLIC_KEY_BYTES_1024] {
        unsafe {
            cbb_to_array(|cbb| {
    ...
    /// Generate a secret key and encrypt it to this public key, returning the
    /// ciphertext and the shared secret key.
    pub fn encapsulate(&self) -> ([u8; CIPHERTEXT_BYTES_1024], [u8; SHARED_SECRET_BYTES]) {
        let mut ciphertext = MaybeUninit::<[u8; CIPHERTEXT_BYTES_1024]>::uninit();
        let mut shared_secret = MaybeUninit::<[u8; SHARED_SECRET_BYTES]>::uninit();
...
impl PrivateKey1024 {
    ...
    /// regenerate the same private key in the future.
    pub fn generate() -> (
        [u8; PUBLIC_KEY_BYTES_1024],
        PrivateKey1024,
        [u8; SEED_BYTES],
    ) {
        let mut public_key = MaybeUninit::<[u8; PUBLIC_KEY_BYTES_1024]>::uninit();
        let mut private_key = MaybeUninit::<PrivateKey1024>::uninit();
        let mut seed = MaybeUninit::<[u8; SEED_BYTES]>::uninit();
