...
use crate::cipher::{Cipher, CipherError, CipherType, StreamCipher};

/// A BoringSSL implemented AES-CTR-128 cipher.
pub struct Aes128Ctr(Cipher<16, 16>);

impl StreamCipher<16, 16> for Aes128Ctr {
    /// Creates a new AES-128-CTR cipher instance from key material.
    fn new(key: [u8; 16], nonce_and_counter: [u8; 16]) -> Self {
        Self(Cipher::new(&key, &nonce_and_counter, CipherType::Aes128Ctr))
    }
...
/// A BoringSSL implemented AES-CTR-256 cipher.
pub struct Aes256Ctr(Cipher<32, 16>);

impl StreamCipher<32, 16> for Aes256Ctr {
    /// Creates a new AES-256-CTR cipher instance from key material.
    fn new(key: [u8; 32], nonce_and_counter: [u8; 16]) -> Self {
        Self(Cipher::new(&key, &nonce_and_counter, CipherType::Aes256Ctr))
    }
...
mod test {
    ...
    fn aes_128_ctr_test_encrypt() {
        ...
        let mut cipher = Aes128Ctr::new(decode_hex("2b7e151628aed2a6abf7158809cf4f3c"), iv);
        let mut block: [u8; 16];
