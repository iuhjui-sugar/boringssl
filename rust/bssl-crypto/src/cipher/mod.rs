...
impl<C: EvpCipherType> Cipher<C> {
    ...
    fn encrypt(self, buffer: &[u8]) -> Result<Vec<u8>, CipherError> {
        ...
        let update_out_len_usize = update_out_len
            .try_into()
        ...
        // Slice indexing here will not panic because we ensured `output_vec` is larger than what
        // `EncryptUpdate` will write.
        #[allow(clippy::indexing_slicing)]
        let mut cslice_finalize_buf_mut = CSliceMut::from(&mut output_vec[update_out_len_usize..]);
        ...
        if final_result == 1 {
            unsafe {
                // Safety:
                // - output_vec[..E] is initialized by `EncryptUpdate`.
                // - output_vec[E..E + F] is initialized by `EncryptFinal`.
                // - E + F < max_encrypt_update_output_size + max_encrypt_final_output_size
                //         = output_vec.capacity
                // where E = encrypt_out_len_usize and F = final_out_len.
                output_vec.set_len(update_out_len_usize + final_put_len_usize)
            };
            Ok(output_vec)
        } else {
            Err(CipherError)
    ...
    fn decrypt(self, in_buffer: &[u8]) -> Result<Vec<u8>, CipherError> {
        ...
        if final_result == 1 {
            unsafe {
                // Safety:
                // - output_vec[..E] is initialized by `DecryptUpdate`.
                // - output_vec[E..E + F] is initialized by `DecryptFinal`.
                // - E + F < max_decrypt_update_output_size + max_decrypt_final_output_size
                //         = output_vec.capacity
                // where E = decrypt_out_len_usize and F = final_out_len.
                output_vec.set_len(update_out_len_usize + final_put_len_usize)
            };
            Ok(output_vec)
        } else {
