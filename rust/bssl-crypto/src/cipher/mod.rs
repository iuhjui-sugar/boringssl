...
impl<C: EvpCipherType> Cipher<C> {
    ...
    fn encrypt(self, buffer: &[u8]) -> Result<Vec<u8>, CipherError> {
        ...
        // EncryptFinal block
        {
            ...
            if final_result == 1 {
                // Will not panic because we ensured the vec's len() is initialized to the sum of
                // the max size that `EncryptUpdate` and `EncryptFinal` can write.
    ...
    fn decrypt(self, in_buffer: &[u8]) -> Result<Vec<u8>, CipherError> {
        ...
        // DecryptFinal block
        {
            ...
            if final_result == 1 {
                // Will not panic because we ensured the vec's len() is initialized to the sum of
                // the max size that `DecryptUpdate` and `DecryptFinal` can write.
                output_vec.truncate(update_out_len_usize + final_put_len_usize)
            } else {
                return Err(CipherError);
            }
