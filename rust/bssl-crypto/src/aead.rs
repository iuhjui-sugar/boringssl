...
impl<const N: usize, const T: usize> AeadImpl<N, T> {
    ...
    // tag. msg must contain at least `T` bytes for the auth tag
    fn decrypt(&self, msg: &[u8], aad: &[u8], nonce: &[u8; N]) -> Result<Vec<u8>, AeadError> {
        assert!(
            msg.len() >= T,
            "Invalid message length: {0}, must contain at least {1} bytes to hold an auth tag",
            msg.len(),
            T
        );
        let mut out = Vec::new();
...
mod test {
    ...
    fn test_invalid_data_length_decrypt() {
        ...
        let tag: [u8; 16] = decode_hex("00000000000000000000000000000000");
        let mut buf = Vec::from(&[] as &[u8]);
        let aes = new_aes_128_gcm_siv(&key);
        let _ = aes.decrypt(&buf, b"", &nonce);
    }
}
