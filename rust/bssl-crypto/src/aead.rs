...
use crate::{
    initialized_struct_fallible, with_output_array, with_output_vec, with_output_vec_fallible,
    FfiMutSlice, FfiSlice,
};
use alloc::vec::Vec;
    ...
    EvpAead<KEY_LEN, NONCE_LEN, TAG_LEN>
{
    ...
    unsafe fn new(key: &[u8; KEY_LEN], evp_aead: *const bssl_sys::EVP_AEAD) -> Self {
        Self(
            // Safety: type checking ensures that `ctx` is the correct size
            // for `EVP_AEAD_CTX_init`. `evp_aead` is assumed to be valid.
            unsafe {
                initialized_struct_fallible(|ctx| {
                    bssl_sys::EVP_AEAD_CTX_init(
                        ctx,
                        evp_aead,
                        key.as_ffi_ptr(),
                        key.len(),
            ...
            // Failure implies that the configured lengths were wrong.
            // Crashing is the best answer in that case.
            .unwrap(),
        )
    }
    ...
    for EvpAead<KEY_LEN, NONCE_LEN, TAG_LEN>
{
    fn drop(&mut self) {
        ...
        unsafe { bssl_sys::EVP_AEAD_CTX_cleanup(&mut self.0) }
    }
}
...
mod test {
    ...
    fn invariants() {
        let key16 = [0u8; 16];
        let key32 = [0u8; 32];
        check_aead_invariants(Aes128Gcm::new(&key16));
        check_aead_invariants(Aes256Gcm::new(&key32));
        check_aead_invariants(Aes128GcmSiv::new(&key16));
        check_aead_invariants(Aes256GcmSiv::new(&key32));
        check_aead_invariants(Chacha20Poly1305::new(&key32));
        check_aead_invariants(XChacha20Poly1305::new(&key32));
    }
