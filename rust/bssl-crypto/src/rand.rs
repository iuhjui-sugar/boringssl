...
///
/// BoringSSL crashes the address space if the PRNG experiences an error so
/// an error cannot be observed here.
pub fn rand_bytes(buf: &mut [u8]) {
    // Safety: `RAND_bytes` writes exactly `buf.len()` bytes.
    unsafe { bssl_sys::RAND_bytes(buf.as_mut_ffi_ptr(), buf.len()) };
}
...
pub fn rand_array<const N: usize>() -> [u8; N] {
    unsafe {
        with_output_array(|out, out_len| {
            ...
            bssl_sys::RAND_bytes(out, out_len);
        })
    }
}
