...
/// BoringSSL implemented AEAD operations where K is the size of the key and N is the size of the
/// nonce.
pub trait Aead<const K: usize, const N: usize> {
    ...
    /// Builds an AEAD from key material.
...
// Private generic Aead implementation.
struct AeadImpl<const K: usize, const N: usize, const T: usize>(*mut EVP_AEAD_CTX);
...
// Private  implementation of an Aead which is generic over Key size, Nonce size, and Tag size. This
// should only be exposed publicly by wrapper types which provide the correct generic arguments.
impl<const K: usize, const N: usize, const T: usize> AeadImpl<K, N, T> {
    fn new(key: [u8; K], aead_type: AeadType) -> Self {
        // Safety:
    ...
    // Decrypts msg in-place, on success msg will contain the plain text alone, without the auth tag.
    fn decrypt(&self, msg: &mut Vec<u8>, aad: &[u8], nonce: &[u8; N]) -> Result<(), AeadError> {
...
fn print_consts() {
    unsafe {
        println!("Alg is AES-128-GCM-SIV");
        let alg = bssl_sys::EVP_aead_aes_256_gcm_siv();
        let key_len = bssl_sys::EVP_AEAD_key_length(alg);
        let nonce_len = bssl_sys::EVP_AEAD_nonce_length(alg);
        let max_overhead = bssl_sys::EVP_AEAD_max_overhead(alg);
        let max_tag_len = bssl_sys::EVP_AEAD_max_tag_len(alg);
