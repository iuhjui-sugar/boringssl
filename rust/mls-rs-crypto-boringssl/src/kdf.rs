    ...
    async fn expand(&self, prk: &[u8], info: &[u8], len: usize) -> Result<Vec<u8>, KdfError> {
        ...
        match self.0 {
            KdfId::HkdfSha256 => {
                ...
                Ok(hkdf.expand::<255>(info)[0..len].to_vec())
            },
            KdfId::HkdfSha512 => {
...
mod test {
    ...
    use super::{KdfType, KdfWrapper, KdfError};
    ...
    fn sha256() {
        ...
        let kdf = KdfWrapper::new(CipherSuite::CURVE25519_AES128).unwrap();
    ...
    fn sha512() {
        ...
        let kdf = KdfWrapper::new(CipherSuite::CURVE448_CHACHA).unwrap();
        let prk = kdf.extract(&salt, &ikm).unwrap();
        assert_eq!(kdf.expand(&prk, &info, 42).unwrap(), expected_okm);
    }
    ...
    fn unsupported_cipher_suites() {
        ...
        assert_matches!(
            KdfWrapper::new(CipherSuite::P384_AES256).unwrap().extract(&salt, &ikm),
