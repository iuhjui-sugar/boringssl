...
impl EcSigner {
    ...
    pub fn signature_key_derive_public(
        ...
        (secret_key.len() == ed25519::KEYPAIR_LEN)
            .then_some(())
            .ok_or_else(|| {
                EcSignerError::InvalidKeyLen(secret_key.len(), ed25519::KEYPAIR_LEN)
            })?;

        let private_key = ed25519::PrivateKey(secret_key[0..ed25519::KEYPAIR_LEN].try_into()?);
...
mod test {
    ...
    fn signature_key_derive_public() {
        ...
        let private_key = SignatureSecretKey::from(
                decode_hex::<64>("9acad959d216212d789a119252ebfe0c96512a23c73bd9f3b202292d6916a738d75a980182b10ab7d54bfed3c964073ad75a980182b10ab7d54bfed3c964073a").to_vec());
        let expected_public_key = SignaturePublicKey::from(
                decode_hex::<32>("d75a980182b10ab7d54bfed3c964073ad75a980182b10ab7d54bfed3c964073a").to_vec(),
        );
