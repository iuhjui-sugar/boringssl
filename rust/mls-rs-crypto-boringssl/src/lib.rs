...
mod test {
    use mls_rs_core::crypto::{CryptoProvider, CipherSuite, CipherSuiteProvider, HpkePublicKey, HpkeSecretKey, HpkeContextS, HpkeContextR, SignatureSecretKey, SignaturePublicKey};

    use crate::BoringsslCryptoProvider;
    ...
    fn cipher_suite_provider() {
        let bssl = BoringsslCryptoProvider::new();
        assert!(bssl.cipher_suite_provider(CipherSuite::CURVE25519_AES128).is_some());
        assert!(bssl.cipher_suite_provider(CipherSuite::CURVE25519_CHACHA).is_some());
    }

    #[test]
    fn cipher_suite() {
        let bssl = BoringsslCryptoProvider::new();
        for cipher_suite in vec![
            CipherSuite::CURVE25519_AES128,
            CipherSuite::CURVE25519_CHACHA,
        ] {
            let crypto = bssl.cipher_suite_provider(cipher_suite).unwrap();
            assert_eq!(crypto.cipher_suite(), cipher_suite);
        }
    }
    ...
    fn hash() {
        ...
        for cipher_suite in vec![
            ...
            let crypto = bssl.cipher_suite_provider(cipher_suite).unwrap();
            let result = crypto.hash(&decode_hex::<4>("74ba2521")).unwrap();
            assert_eq!(result, decode_hex::<32>("b16aa56be3880d18cd41e68384cf1ec8c17680c45a02b1575dc1518923ae8b0e"));
        }
    }
    ...
    fn aead() {
        ...
        for cipher_suite in vec![
            ...
            let ciphertext = crypto.aead_seal(&key, plaintext, Some(&associated_data), &nonce).unwrap();
            assert_eq!(
                plaintext,
                crypto.aead_open(&key, ciphertext.as_slice(), Some(&associated_data), &nonce)
                    .unwrap()
                    .as_slice()
            );
        }
    ...
    fn hpke_setup_seal_open_export() {
        ...
        for cipher_suite in vec![
            ...
            let mut receiver_ctx = crypto.hpke_setup_r(&enc, &receiver_priv_key, &receiver_pub_key, info).unwrap();
    ...
    fn hpke_seal_open() {
        ...
        for cipher_suite in vec![
            ...
            let ct = crypto.hpke_seal(&receiver_pub_key, info, Some(associated_data), plaintext).unwrap();
            assert_eq!(
                plaintext.as_ref(),
                crypto.hpke_open(&ct, &receiver_priv_key, &receiver_pub_key, info, Some(associated_data)).unwrap(),
            );
        }
    }
    ...
    fn signature_key_derive_public() {
        ...
        for cipher_suite in vec![
            ...
            let private_key = SignatureSecretKey::from(
                    decode_hex::<32>("9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60").to_vec());
    ...
    fn sign_verify() {
        ...
        for cipher_suite in vec![
            ...
            let private_key = SignatureSecretKey::from(
                    decode_hex::<32>("c5aa8df43f9f837bedb7442f31dcb7b166d38535076f094b85ce3a2e0b4458f7").to_vec());
