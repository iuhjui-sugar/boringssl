...
static const std::vector<std::function<void(SSL_CTX*, CBS*)>> kAPIs = {
  [](SSL_CTX *ctx, CBS *cbs) {
    uint8_t b;
    if (!CBS_get_u8(cbs, &b)) {
      return;
  ...
  [](SSL_CTX *ctx, CBS *cbs) {
    ...
    const uint8_t *sct_data = CBS_data(cbs);
    if (!CBS_skip(cbs, len)) {
      return;
    }

    SSL_CTX_set_signed_cert_timestamp_list(ctx, sct_data, len);
  },
  [](SSL_CTX *ctx, CBS *cbs) {
    uint8_t len;
    if (!CBS_get_u8(cbs, &len)) {
    ...
    const uint8_t *ocsp_data = CBS_data(cbs);
    if (!CBS_skip(cbs, len)) {
      return;
    }

    SSL_CTX_set_ocsp_response(ctx, ocsp_data, len);
  },
  [](SSL_CTX *ctx, CBS *cbs) {
    std::string ciphers;
  ...
  [](SSL_CTX *ctx, CBS *cbs) {
    ...
    SSL_CTX_set_session_id_context(
        ctx, reinterpret_cast<const uint8_t *>(ciphers.data()), ciphers.size());
  },
  [](SSL_CTX *ctx, CBS *cbs) {
    uint32_t size;
...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *buf, size_t len) {
  ...
  for (;;) {
    ...
    kAPIs[index % kAPIs.size()](ctx.get(), &cbs);
  }
