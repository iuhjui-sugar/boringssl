...
static const CFB128TestCase kCFB128TestCases[] = {
  {
...
static const CFB256TestCase kCFB256TestCases[] = {
  {
    // This is the test case from
    // http://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-38a.pdf,
    // section F.3.17.
...
TEST(CFB128Test, TestVectors) {
  unsigned test_num = 0;
  for (const auto &test : kCFB128TestCases) {
    test_num++;
...
TEST(CFB256Test, TestVectors) {
  unsigned test_num = 0;
  for (const auto &test : kCFB256TestCases) {
    test_num++;
    SCOPED_TRACE(test_num);
