...
static inline void init_basis(crypto_word_t *gen, f2elm_t XP, f2elm_t XQ, f2elm_t XR) {
    fpcopy(gen,                  XP->c0);
...
static inline void fp2_encode(const f2elm_t x, uint8_t *enc) {
    f2elm_t t;
    from_fp2mont(x, t);
    ...
    const uint8_t pkB[SIKEp503_PUB_BYTESZ])
{
    ...
    SHA256_Init(&ctx);
    SHA256_Update(&ctx, temp, SIKEp503_MSG_BYTESZ);
    SHA256_Update(&ctx, ct, SIKEp503_CT_BYTESZ);
    SHA256_Final(secret, &ctx);
    // Generate shared secret ss = HMAC(m||ct, F)
    hmac_sum(ss, SIKEp503_SS_BYTESZ, H, secret);
    OPENSSL_cleanse(j, sizeof(j));
    ...
    const uint8_t skB[SIKEp503_PRV_BYTESZ])
{
    ...
    // Recover secret key A = G(m||pkB) mod
    secret[BITS_TO_BYTES(SIDHp503_PRV_A_BITSZ) - 1] &= (1 << (SIDHp503_PRV_A_BITSZ%8)) - 1;
