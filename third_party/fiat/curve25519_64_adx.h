...
static inline void table_select_4(ge_precomp_4 *t, const int pos,
  ...
  static_assert(sizeof(ge_precomp_4) == sizeof(k25519Precomp[pos][0]), "");
  for (int i = 0; i < 8; i++) {
    memcxor(t_bytes, k25519Precomp[pos][i], sizeof(t_bytes),
  ...
  memcmov(t->yplusx, t_bytes[1], sizeof(fe4), bnegative);
  memcmov(t->yminusx, t_bytes[0], sizeof(fe4), bnegative);
  memcmov(t->xy2d, xy2d_neg, sizeof(fe4), bnegative);
}
...
void x25519_ge_scalarmult_base_adx(uint8_t h[4][32], const uint8_t a[32]) {
  ...
  // satisfy precondition of fiat_25519_from_bytes; tests pass either way
  fe4_canon(r.X, r.X);
  fe4_canon(r.Y, r.Y);
  fe4_canon(r.Z, r.Z);
  fe4_canon(r.T, r.T);
