...
int tls13_handshake_write(SSL *ssl, SSL_HS_MESSAGE *msg) {
  CBB cbb;
  size_t length;
  if (!CBB_init_fixed(&cbb, ssl_handshake_start(ssl), msg->length) ||
      !CBB_add_bytes(&cbb, msg->data, msg->length) ||
      !CBB_finish(&cbb, NULL, &length) ||
      !ssl_set_handshake_header(ssl, msg->type, length) ||
      !ssl_do_write(ssl)) {
    return -1;
  }

  ssl3_update_handshake_hash(ssl, msg->raw, length + SSL3_HM_HEADER_LENGTH);

  printf("OUT: ");
  size_t i;
  for (i = 0; i < length + SSL3_HM_HEADER_LENGTH; i++) {
    printf("%02x", msg->raw[i]);
  }
  printf("\n");
