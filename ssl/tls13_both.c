...
int tls13_store_handshake_context(SSL *ssl) {
  ...
  ssl->s3->hs->hs_context_len = hs_context_len;
  EVP_MD_CTX_cleanup(&hh);
  return 1;
}
...
int tls13_finalize(SSL *ssl) {
  ...
  hs->exporter_secret_len = hs->key_len;
  hs->resumption_secret_len = hs->key_len;

  if (!derive_secret(ssl, &hs->exporter_secret, hs->exporter_secret_len,
                     hs->master_secret, hs->master_secret_len,
                     kTLS13LabelExporter, sizeof(kTLS13LabelExporter)) ||
      !derive_secret(ssl, &hs->resumption_secret, hs->resumption_secret_len,
                     hs->master_secret, hs->master_secret_len,
