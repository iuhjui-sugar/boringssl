...
int tls13_receive_certificate(SSL *ssl, SSL_HS_MESSAGE msg) {
  ...
  if (!ssl->server && !ssl3_check_leaf_certificate(ssl, leaf)) {
  ...
  ret = ssl_verify_cert_chain(ssl, ssl->session->cert_chain);
  ...
  ret = tls13_store_handshake_context(ssl);
...
int tls13_send_certificate_verify(SSL *ssl) {
  ...
  switch (sign_result) {
    ...
    case ssl_private_key_retry:
      /* TODO(svaldez): This doesn't actually work. */
