...
int tls13_handshake(SSL *ssl) {
  ...
  while (hs->handshake_state != HS_STATE_DONE) {
    ...
    if (hs->handshake_interrupt & HS_NEED_READ) {
      int ret;
      if (hs->handshake_interrupt & HS_NEED_DONT_HASH) {
        ret = ssl->method->ssl_get_message(ssl, -1, ssl_dont_hash_message);
      } else {
        ret = ssl->method->ssl_get_message(ssl, -1, ssl_hash_message);
      }
      if (ret <= 0) {
        return ret;
      }
      hs->handshake_interrupt &= ~HS_NEED_READ;
      hs->handshake_interrupt &= ~HS_NEED_DONT_HASH;
    }
