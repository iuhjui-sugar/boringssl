...
int tls13_handshake(SSL *ssl) {
  ...
  int result = 1;
  while (result == 1 && hs->handshake_state != HS_STATE_DONE) {
...
int tls13_store_handshake_context(SSL *ssl) {
  ...
  if (ssl->s3->hs->hash_context) {
    OPENSSL_free(ssl->s3->hs->hash_context);
  }

  ssl->s3->hs->hash_context = OPENSSL_malloc(EVP_MD_size(hh.digest) +
                                             EVP_MD_size(rh.digest));
  if (ssl->s3->hs->hash_context == NULL){
    goto end;
  }

  unsigned handshake_len, resumption_len;
  if (!EVP_DigestFinal_ex(&hh, ssl->s3->hs->hash_context,
                          &handshake_len)) {
    goto end;
  }

  EVP_DigestUpdate(&rh, ssl->s3->hs->resumption_ctx, ssl->s3->hs->resumption_ctx_len);
  if (!EVP_DigestFinal_ex(&rh, ssl->s3->hs->hash_context + handshake_len, &resumption_len)) {
    goto end;
  }

  ssl->s3->hs->hash_context_len = handshake_len + resumption_len;
  ret = 1;
...
static int tls13_send_key_update(SSL *ssl) {
  // assemble_handshake_message(out, SSL3_MT_KEY_UPDATE, NULL, 0);
  // send out
  // return tls13_rotate_traffic_secret(ssl, evp_aead_seal);
  return 0;
}
...
int tls13_receive_key_update(SSL *ssl, SSL_HS_MESSAGE msg) {
  ...
  if (!tls13_rotate_traffic_secret(ssl, evp_aead_open)) {
    return 0;
  }

  if (memcmp(ssl->s3->open_traffic_secret,
             ssl->s3->seal_traffic_secret, ssl->s3->traffic_secret_len) != 0) {
    return tls13_send_key_update(ssl);
  }
