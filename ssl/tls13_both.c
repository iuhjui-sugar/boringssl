...
int tls13_handshake(SSL *ssl) {
  ...
  while (hs->handshake_state != HS_STATE_DONE) {
    ...
    if (hs->handshake_interrupt & HS_NEED_READ) {
      int ret = tls13_handshake_read(ssl);
      if (ret <= 0) {
        return ret;
      }
    ...
    if (hs->handshake_interrupt & HS_NEED_CB) {
      return -1;
    }
  }
...
int tls13_handshake_read(SSL *ssl) {
  return ssl->method->ssl_get_message(ssl, -1, ssl_hash_message);
}
...
int tls13_receive_finished(SSL *ssl) {
  ...
  if (ssl->init_num != signature_len ||
      CRYPTO_memcmp(signature, ssl->init_msg, signature_len) != 0) {
    goto f_err;
  }
