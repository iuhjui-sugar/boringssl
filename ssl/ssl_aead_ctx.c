...
size_t SSL_AEAD_CTX_max_suffix_len(const SSL_AEAD_CTX *aead,
  ...
  return aead == NULL ? extra_in_len
                      : EVP_AEAD_max_overhead(aead->ctx.aead) + extra_in_len;
}
...
int SSL_AEAD_CTX_seal_scatter(SSL_AEAD_CTX *aead, uint8_t *out_prefix,
  ...
  if (aead->variable_nonce_included_in_record) {
    ...
    if (out < in + in_len && in < out_prefix + aead->variable_nonce_len) {
  ...
  if (!EVP_AEAD_CTX_seal_scatter(&aead->ctx, out, out_suffix, out_suffix_len,
