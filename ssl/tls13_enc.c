...
int tls13_init_key_schedule(SSL_HANDSHAKE *hs) {
  ...
  SSL_PRF_free_buffer(&hs->prf);
  return 1;
}

int tls13_advance_key_schedule(SSL_HANDSHAKE *hs, const uint8_t *in,
                               size_t len) {
  return HKDF_extract(hs->secret, &hs->hash_len, SSL_PRF_md(&hs->prf), in, len,
...
static int derive_secret(SSL_HANDSHAKE *hs, uint8_t *out, size_t len,
  ...
  if (!SSL_PRF_get_hash(&hs->prf, context_hash, &context_hash_len)) {
    return 0;
  }

  return hkdf_expand_label(out, SSL_PRF_md(&hs->prf), hs->secret, hs->hash_len,
...
int tls13_set_traffic_key(SSL *ssl, enum evp_aead_direction_t direction,
  ...
  const EVP_MD *digest =
      ssl_get_handshake_digest(SSL_get_session(ssl)->cipher->algorithm_prf);
...
int tls13_finished_mac(SSL_HANDSHAKE *hs, uint8_t *out, size_t *out_len,
  ...
  size_t context_hash_len;
  if (!SSL_PRF_get_hash(&hs->prf, context_hash, &context_hash_len) ||
      !tls13_verify_data(SSL_PRF_md(&hs->prf), out, out_len, traffic_secret,
...
int tls13_write_psk_binder(SSL_HANDSHAKE *hs, uint8_t *msg, size_t len) {
  SSL *const ssl = hs->ssl;
  size_t hash_len = SSL_PRF_digest_len(&hs->prf);
  ...
  if (!EVP_DigestInit_ex(&ctx, SSL_PRF_md(&hs->prf), NULL) ||
      !EVP_DigestUpdate(&ctx, hs->prf.buffer->data, hs->prf.buffer->length) ||
  ...
  if (!tls13_psk_binder(
          verify_data, SSL_PRF_md(&hs->prf), ssl->session->master_key,
          ssl->session->master_key_length, context, context_len, hash_len)) {
    return 0;
  }
...
int tls13_verify_psk_binder(SSL_HANDSHAKE *hs, SSL_SESSION *session,
  ...
  if (!EVP_Digest(CBS_data(&message), CBS_len(&message) - CBS_len(binders) - 2,
                  context, &context_len, SSL_PRF_md(&hs->prf), NULL)) {
    return 0;
  }

  uint8_t verify_data[EVP_MAX_MD_SIZE] = {0};
  CBS binder;
  if (!tls13_psk_binder(verify_data, SSL_PRF_md(&hs->prf), session->master_key,
