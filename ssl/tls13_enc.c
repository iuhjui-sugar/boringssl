...
int tls13_verify_psk_binder(SSL *ssl, SSL_SESSION *session,
  ...
  if (ssl->s3->handshake_buffer->length < CBS_len(binders) + 2) {
    return 0;
  }

  size_t data_len = ssl->s3->handshake_buffer->length - CBS_len(binders) - 2;
  ...
  unsigned context_len;
  if (!EVP_DigestInit_ex(&ctx, digest, NULL) ||
      !EVP_DigestUpdate(&ctx, ssl->s3->handshake_buffer->data, data_len) ||
      !EVP_DigestFinal_ex(&ctx, context, &context_len)) {
    EVP_MD_CTX_cleanup(&ctx);
    return 0;
  }
  ...
  return CRYPTO_memcmp(CBS_data(&binder), verify_data, hash_len) == 0;
}
