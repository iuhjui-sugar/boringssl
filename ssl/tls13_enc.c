...
static int set_traffic_key(SSL *ssl, enum tls_record_type_t type,
  ...
  if (direction == evp_aead_open) {
    ...
    ssl->method->set_write_state(ssl, traffic_aead);
  }
...
int tls13_derive_secrets(SSL *ssl) {
  ...
  if (!HKDF_extract(hs->early_secret, &hs->early_secret_len,
      ...
      !HKDF_extract(hs->handshake_secret, &hs->handshake_secret_len,
                    digest, hs->dhe_secret, hs->dhe_secret_len,
                    hs->early_secret, hs->early_secret_len)) {
    return 0;
  }

  uint8_t zero[EVP_MAX_MD_SIZE];
...
int tls13_finalize_keys(SSL *ssl) {
  ...
  memset(hs->early_secret, 0, hs->early_secret_len);
  memset(hs->handshake_secret, 0, hs->handshake_secret_len);
  memset(hs->master_secret, 0, hs->master_secret_len);
  return 1;
}
...
int tls13_finished_mac(SSL *ssl, uint8_t *out, size_t *out_len, int is_server) {
  ...
  const uint8_t *label;
  if (is_server) {
    label = (const uint8_t *)"server finished";
    if (ssl->server) {
    ...
    label = (const uint8_t *)"client finished";
    if (!ssl->server) {
      traffic_secret = ssl->s3->seal_traffic_secret;
    } else {
      traffic_secret = ssl->s3->open_traffic_secret;
