...
int tls13_update_handshake_keys(SSL *ssl, uint8_t *es, size_t es_len) {
  ...
  if (!HKDFExtract(secret, &secret_len, digest, es, es_len, NULL, 0)) {
    return 0;
  }
  ...
  tls13_label(&client_iv_label, &client_iv_label_len, key_type,
              tls13_record_client_iv, key_len, hs_hash, hs_hash_len);
  ...
  tls13_label(&server_iv_label, &server_iv_label_len, key_type,
              tls13_record_server_iv, key_len, hs_hash, hs_hash_len);


  if (!HKDFExpand(write_key, key_len, digest, secret, secret_len,
            client_key_label, client_key_label_len) ||
      !HKDFExpand(write_iv, key_len, digest, secret, secret_len,
            client_iv_label, client_iv_label_len) ||
      !HKDFExpand(read_key, key_len, digest, secret, secret_len,
            server_key_label, server_key_label_len) ||
      !HKDFExpand(read_iv, key_len, digest, secret, secret_len,
  ...
  printf("\n");
  printf("R_K_L: ");
  for (i = 0; i < server_key_label_len; i++) {
    printf("%02x", server_key_label[i]);
  }
  printf("\n");
  printf("R_K: ");
  for (i = 0; i < key_len; i++) {
