...
static int hkdf_expand_label(uint8_t *out, uint8_t *secret, size_t secret_len,
                             CBB *label, uint8_t *hash, size_t hash_len,
                             size_t len) {
  CBB hkdf_label;
  CBB cbb;
  if (!CBB_init(&cbb, 2 + 1 + CBB_len(label) + 1 + hash_len) ||
      !CBB_add_u16(&cbb, len) ||
      !CBB_add_u8_length_prefixed(&cbb, &hkdf_label) ||
      !CBB_add_bytes(&hkdf_label, kTLS13LabelVersion,
                     sizeof(kTLS13LabelVersion)) ||
      !CBB_add_bytes(&hkdf_label, CBB_data(label), CBB_len(label)) ||
      !CBB_add_u8(&cbb, hash_len) ||
      !CBB_add_bytes(&cbb, hash, hash_len)) {
    return 0;
  }

  return HKDF_expand(out, len, secret, secret_len, CBB_data(&cbb), CBB_len(&cbb));
}

int derive_secret(uint8_t *out, size_t len, uint8_t *secret, size_t secret_len,
                  CBB *label, uint8_t *hash, size_t hash_len, uint8_t *context,
                  size_t context_len) {
  CBB msg;
  ...
  return hkdf_expand_label(out, secret, secret_len, label, CBB_data(&msg),
                           CBB_len(&msg), len);
}
...
int update_traffic_key(SSL *ssl, uint8_t *secret, size_t secret_len,
  ...
  switch(type) {
    case type_early_handshake:
      type_label = "early handshake key expansion, ";
      type_label_len = 15;
    case type_early_data:
      type_label = "early application data key expansion, ";
      type_label_len = 22;
    case type_handshake:
      type_label ="handshake key expansion, ";
      type_label_len = 9;
    case type_data:
      type_label = "application data key expansion, ";
      type_label_len = 16;
  }
  ...
  if (!hkdf_expand_label(write_key, secret, secret_len, wk_label, NULL, 0,
                         key_len) ||
      !hkdf_expand_label(write_iv, secret, secret_len, wi_label, NULL, 0,
                         iv_len) ||
      !hkdf_expand_label(read_key, secret, secret_len, rk_label, NULL, 0,
                         key_len) ||
      !hkdf_expand_label(read_iv, secret, secret_len, ri_label, NULL, 0,
                         iv_len)) {
    return 0;
  }
