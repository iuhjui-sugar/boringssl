...
static int tls13_sign_verify_data(const EVP_MD *digest, uint8_t *out,
...
int tls13_finished_mac(SSL *ssl, uint8_t *out, size_t *out_len, int is_server) {
  ...
  size_t context_hash_len;
  if (!tls13_get_context_hash(ssl, context_hash, &context_hash_len) ||
      !tls13_sign_verify_data(digest, out, out_len, traffic_secret,
...
static int tls13_psk_binder(SSL *ssl, uint8_t *out, const EVP_MD *digest,
  ...
  if (!hkdf_expand_label(binder_key, digest, early_secret, hash_len,
      ...
      !tls13_sign_verify_data(digest, out, &len, binder_key, hash_len, context,
                              context_len)) {
    return 0;
  }
...
int tls13_write_psk_binder(SSL *ssl, uint8_t *msg, size_t len) {
  ...
  if (ssl->s3->handshake_buffer->length + len < hash_len + 3) {
    return 0;
  }
  ...
  int ret = tls13_psk_binder(ssl, signature, digest, ssl->session->master_key,
...
int tls13_verify_psk_binder(SSL *ssl, SSL_SESSION *session,
  ...
  size_t data_len = ssl->s3->handshake_buffer->length - binders_len;
  if (!tls13_psk_binder(ssl, signature, digest, session->master_key,
                        session->master_key_length,
                        (uint8_t *)ssl->s3->handshake_buffer->data, data_len,
                        hash_len)) {
    return 0;
  }

  return CRYPTO_memcmp(ssl->s3->handshake_buffer->data +
