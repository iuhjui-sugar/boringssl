...
int tls13_init_key_schedule(SSL *ssl, const uint8_t *resumption_ctx,
  ...
  hs->resumption_hash_len = resumption_hash_len;
...
int tls13_get_context_hashes(SSL *ssl, uint8_t *out, size_t *out_len) {
  ...
  memcpy(out + handshake_len, hs->resumption_hash, hs->resumption_hash_len);
  *out_len = handshake_len + hs->resumption_hash_len;
  return 1;
}
