...
int tls13_init_key_schedule(SSL *ssl, const uint8_t *resumption_ctx,
  ...
  hs->key_len = EVP_MD_size(digest);

  /* Save the hash of the resumption context.
   *
   * TODO(davidben): Hashing the resumption context seems unnecessary. Remove it
   * from the spec? */
  if (!EVP_Digest(resumption_ctx, resumption_ctx_len, hs->resumption_ctx, NULL,
                  digest, NULL)) {
    return 0;
  }
  hs->resumption_ctx_len = hs->key_len;

  /* Initialize the secret to the zero key. */
  memset(hs->secret, 0, hs->key_len);
  hs->secret_len = hs->key_len;
...
int tls13_advance_key_schedule(SSL *ssl, const uint8_t *in, size_t len) {
  ...
  return HKDF_extract(hs->secret, &hs->secret_len, digest, in, len, hs->secret,
                      hs->secret_len);
}
...
int tls13_get_context_hashes(SSL *ssl, uint8_t *out, size_t *out_len) {
  ...
  memcpy(out + handshake_len, hs->resumption_ctx, hs->resumption_ctx_len);
  *out_len = handshake_len + hs->resumption_ctx_len;
  return 1;
}
...
static int derive_secret(SSL *ssl, uint8_t *out, size_t len,
  ...
  return hkdf_expand_label(out, digest, hs->secret, hs->secret_len, label,
...
int tls13_set_handshake_traffic(SSL *ssl) {
  ...
  if (!derive_secret(ssl, traffic_secret, hs->key_len,
...
int tls13_finalize_keys(SSL *ssl) {
  ...
  if (!ssl->s3->exporter_secret_len) {
    ssl->s3->exporter_secret_len = hs->key_len;
  }

  if (!ssl->s3->resumption_secret_len) {
    ssl->s3->resumption_secret_len = hs->key_len;
  }
  ...
  if (!derive_secret(
      ...
      !derive_secret(ssl, ssl->s3->resumption_secret,
  ...
  /* TODO(davidben): Is that right? This would be the poison extension anyway. */
  memcpy(ssl->session->master_key, ssl->s3->resumption_secret,
         ssl->s3->resumption_secret_len);
  ssl->session->master_key_length = ssl->s3->resumption_secret_len;

  /* TODO(davidben): This should be done in ssl_handshake_free when we discard
   * the handshake object. */
  OPENSSL_cleanse(hs->secret, hs->secret_len);
  return 1;
}
