...
int tls13_update_master_secret(SSL *ssl) {
  ...
  if (!HKDF_expand(mSS, ssl->s3->hs->key_len, digest, ssl->s3->hs->xSS, ssl->s3->hs->key_len,
                   mSS_label, mSS_label_len) ||
      !HKDF_expand(mES, ssl->s3->hs->key_len, digest, ssl->s3->hs->xES, ssl->s3->hs->key_len,
                   mES_label, mES_label_len)) {
    return 0;
  }

  if (ssl->s3->hs->master_secret) {
    return 0;
  }

  ssl->s3->hs->master_secret = OPENSSL_malloc(EVP_MAX_MD_SIZE);
  if (!HKDF_extract(ssl->s3->hs->master_secret, &ssl->s3->hs->key_len,
                    digest, mES, ssl->s3->hs->key_len, mSS, ssl->s3->hs->key_len)) {
    return 0;
  }

  size_t i;
  printf("mSS: ");
  for (i = 0; i < ssl->s3->hs->key_len; i++) {
    printf("%02x", mSS[i]);
  }
  printf("\n");
  printf("mES: ");
  ...
  for (i = 0; i < ssl->s3->hs->key_len; i++) {
    printf("%02x", ssl->s3->hs->master_secret[i]);
  }
  printf("\n");


  ssl->s3->hs->traffic_secret = OPENSSL_malloc(EVP_MAX_MD_SIZE);
  ...
  if (!tls13_label(&traffic_label, &traffic_label_len, kTLS13LabelTraffic,
                   ...
                   ssl->s3->hs->key_len) ||
      !HKDF_expand(ssl->s3->hs->traffic_secret, ssl->s3->hs->key_len, digest,
                   ssl->s3->hs->master_secret, ssl->s3->hs->key_len,
  ...
  OPENSSL_free(mSS);
  OPENSSL_free(mES);
  OPENSSL_free(ssl->s3->hs->xSS);
  OPENSSL_free(ssl->s3->hs->xES);
  return 1;
}
...
int tls13_verify_finished(uint8_t *out, size_t *out_len, SSL *ssl,
  ...
  if (!HKDF_expand(key, key_len, digest, ssl->s3->hs->master_secret, ssl->s3->hs->key_len,
                   label, label_len)) {
    return 0;
  }

  unsigned len;
  if (HMAC(digest, key, key_len, ssl->s3->hs->hs_context, ssl->s3->hs->hs_context_len,
           out, &len) == NULL) {
    return 0;
  }
  *out_len = len;
