...
int tls13_write_psk_binder(SSL_HANDSHAKE *hs, uint8_t *msg, size_t len) {
  ...
  if (!tls13_psk_binder(verify_data, hs->prf.md, ssl->session->master_key,
...
int tls13_verify_psk_binder(SSL_HANDSHAKE *hs, SSL_SESSION *session,
  ...
  if (!EVP_Digest(CBS_data(&message), CBS_len(&message) - CBS_len(binders) - 2,
                  context, &context_len, hs->prf.md, NULL)) {
    return 0;
  }

  uint8_t verify_data[EVP_MAX_MD_SIZE] = {0};
  CBS binder;
  if (!tls13_psk_binder(verify_data, hs->prf.md, session->master_key,
