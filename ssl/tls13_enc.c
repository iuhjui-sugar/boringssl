...
static int derive_secret(SSL *ssl, SSL_HANDSHAKE *hs, uint8_t *out, size_t len,
                         const uint8_t *label, size_t label_len) {
  const EVP_MD *digest = ssl_get_handshake_digest(ssl_get_algorithm_prf(ssl));
...
int tls13_set_handshake_traffic(SSL *ssl, SSL_HANDSHAKE *hs) {
  ...
  if (!derive_secret(ssl, hs, client_traffic_secret, hs->hash_len,
      ...
      !derive_secret(ssl, hs, server_traffic_secret, hs->hash_len,
...
int tls13_derive_resumption_secret(SSL *ssl, SSL_HANDSHAKE *hs) {
  ...
  return derive_secret(ssl, hs, ssl->s3->new_session->master_key,
