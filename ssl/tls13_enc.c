...
int tls13_verify_psk_binder(SSL *ssl, SSL_SESSION *session,
  ...
  if (!ssl->method->init_message(ssl, &cbb, &body, SSL3_MT_CLIENT_HELLO) ||
      !CBB_add_bytes(&body, ssl->init_msg, ssl->init_num) ||
      !ssl->method->finish_message(ssl, &cbb, &data, &data_len) ||
      data_len < CBS_len(binders) + 2) {
    goto done;
  }
  ...
  ret = CRYPTO_memcmp(CBS_data(&binder), verify_data, hash_len) == 0;
