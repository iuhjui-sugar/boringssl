...
bool tls13_ech_accept_confirmation(
  ...
  // We hash |server_hello|, with the last 8 bytes of the random value zeroed.
  const size_t header_len =
      SSL_is_dtls(hs->ssl) ? DTLS1_HM_HEADER_LENGTH : SSL3_HM_HEADER_LENGTH;
  static const uint8_t kZeroes[8] = {0};
  const size_t offset = header_len + 2 + SSL3_RANDOM_SIZE - sizeof(kZeroes);
  if (server_hello.size() < offset + sizeof(kZeroes)) {
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return false;
  }

  auto before_zeroes = server_hello.subspan(0, offset);
  auto after_zeroes = server_hello.subspan(offset + sizeof(kZeroes));
  uint8_t context_hash[EVP_MAX_MD_SIZE];
  ...
  //
  // TODO(davidben): draft-11 will avoid this.
