...
static bool hkdf_expand_label_with_prefix(Span<uint8_t> out,
...
#ifdef FIPS_API_HAS_HKDF_EXPAND_LABEL_WITH_PREFIX
  return CRYPTO_tls13_hkdf_expand_label_with_prefix(
             out.data(), out.size(), digest, secret.data(), secret.size(),
             label_prefix.data(), label_prefix.size(),
             reinterpret_cast<const uint8_t *>(label.data()), label.size(),
             hash.data(), hash.size()) == 1;
#else   // FIPS_API_HAS_HKDF_EXPAND_LABEL_WITH_PREFIX
  // Here's a copy of CRYPTO_tls13_hkdf_expand_label_with_prefix to use if the
  // FIPS module is missing it:
  CBB cbb, child;
  ...
  return ret == 1;
#endif  // FIPS_API_HAS_HKDF_EXPAND_LABEL_WITH_PREFIX
}
