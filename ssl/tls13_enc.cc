...
static int hkdf_expand_label(uint8_t *out, uint16_t version,
  ...
  static const char *kTLS13LabelVersion =
...
static int derive_secret(SSL_HANDSHAKE *hs, uint8_t *out, size_t len,
  ...
  uint16_t version = hs->ssl->s3->have_version ? hs->ssl->session->ssl_version
                                               : hs->ssl->version;
  return hkdf_expand_label(out, version, hs->transcript.Digest(), hs->secret,
...
int tls13_derive_key_schedule(SSL_HANDSHAKE *hs) {
  ...
  return hkdf_expand_label(
      hs->secret, version, hs->transcript.Digest(), hs->secret, hs->hash_len,
      (const uint8_t *)kTLS13LabelDerived, strlen(kTLS13LabelDerived),
      derive_context, derive_context_len, hs->hash_len);
}
...
int tls13_set_traffic_key(SSL *ssl, enum evp_aead_direction_t direction,
  ...
  if (!hkdf_expand_label(iv, session->ssl_version, digest, traffic_secret, traffic_secret_len,
...
static const char kTLS132LabelExporter[] = "exp master";
static const char kTLS132LabelEarlyExporter[] = "e exp master";

static const char kTLS132LabelClientEarlyTraffic[] = "c e traffic";
static const char kTLS132LabelClientHandshakeTraffic[] = "c hs traffic";
static const char kTLS132LabelServerHandshakeTraffic[] = "s hs traffic";
static const char kTLS132LabelClientApplicationTraffic[] = "c ap traffic";
static const char kTLS132LabelServerApplicationTraffic[] = "s ap traffic";

int tls13_derive_early_secrets(SSL_HANDSHAKE *hs) {
  SSL *const ssl = hs->ssl;
  uint16_t version = ssl->version;
  if (!ssl->s3->have_version) {
    version = ssl->session->ssl_version;
  }

  const char *early_traffic_label = ssl_is_draft21(version)
                                        ? kTLS132LabelClientEarlyTraffic
                                        : kTLS13LabelClientEarlyTraffic;
  const char *early_exporter_label = ssl_is_draft21(version)
                                         ? kTLS132LabelEarlyExporter
...
int tls13_derive_application_secrets(SSL_HANDSHAKE *hs) {
  ...
  const char *client_label = ssl_is_draft21(ssl->version)
                                 ? kTLS132LabelClientApplicationTraffic
                                 : kTLS13LabelClientApplicationTraffic;
  const char *server_label = ssl_is_draft21(ssl->version)
                                 ? kTLS132LabelServerApplicationTraffic
                                 : kTLS13LabelServerApplicationTraffic;
  const char *exporter_label =
      ssl_is_draft21(ssl->version) ? kTLS132LabelExporter : kTLS13LabelExporter;
...
static const char kTLS132LabelApplicationTraffic[] =
    "traffic upd";
...
int tls13_rotate_traffic_key(SSL *ssl, enum evp_aead_direction_t direction) {
  ...
  const char *traffic_label = ssl_is_draft21(ssl->version)
                                  ? kTLS132LabelApplicationTraffic
...
static const char kTLS132LabelResumption[] = "res master";
...
static int tls13_verify_data(const EVP_MD *digest, uint16_t version, uint8_t *out,
...
int tls13_export_keying_material(SSL *ssl, uint8_t *out, size_t out_len,
  ...
  if (!ssl_is_draft21(ssl->version)) {
    ...
    if (use_context) {
      hash = context_in;
      hash_len = context_in_len;
    }

    const EVP_MD *digest = ssl_session_get_digest(SSL_get_session(ssl));
    return hkdf_expand_label(
        out, ssl->version, digest, ssl->s3->exporter_secret,
        ssl->s3->exporter_secret_len, (const uint8_t *)label, label_len, hash,
        hash_len, out_len);
  }
...
static const char kTLS132LabelPSKBinder[] = "res binder";
...
static int tls13_psk_binder(uint8_t *out, uint16_t version,
  ...
  const char *binder_label =
      ssl_is_draft21(version) ? kTLS132LabelPSKBinder : kTLS13LabelPSKBinder;
...
int tls13_write_psk_binder(SSL_HANDSHAKE *hs, uint8_t *msg, size_t len) {
  ...
  if (!ssl_is_draft21(ssl->version) || !hs->received_hello_retry_request) {
