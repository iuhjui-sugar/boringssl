...
static int hkdf_expand_label(uint8_t *out, const EVP_MD *digest,
  ...
  static const char kQUICLabelVersion[] = "quic ";

  const char *protocolLabel;
  if (use_quic_label) {
    protocolLabel = kQUICLabelVersion;
  } else {
    protocolLabel = kTLS13LabelVersion;
  }
  ...
  if (!CBB_init(cbb.get(),
                2 + 1 + strlen(protocolLabel) + label_len + 1 + hash_len) ||
      !CBB_add_u16(cbb.get(), len) ||
      !CBB_add_u8_length_prefixed(cbb.get(), &child) ||
      !CBB_add_bytes(&child, (const uint8_t *)protocolLabel,
                     strlen(protocolLabel)) ||
...
static int derive_secret(SSL_HANDSHAKE *hs, uint8_t *out, size_t len,
  ...
  return hkdf_expand_label(
      out, hs->transcript.Digest(), hs->secret, hs->hash_len, label, label_len,
      context_hash, context_hash_len, len, hs->ssl->stream_method != nullptr);
}
...
int tls13_set_traffic_key(SSL *ssl, enum ssl_encryption_level_t level,
  ...
  if (!hkdf_expand_label(key, digest, traffic_secret, traffic_secret_len, "key",
                         3, NULL, 0, key_len, ssl->stream_method != nullptr)) {
    return 0;
  }
  ...
  if (!hkdf_expand_label(iv, digest, traffic_secret, traffic_secret_len, "iv",
                         2, NULL, 0, iv_len, ssl->stream_method != nullptr)) {
    return 0;
  }
  ...
  if (ssl->stream_method) {
    if (!ssl->stream_method->set_encryption_secret(
...
int tls13_derive_early_secrets(SSL_HANDSHAKE *hs) {
  ...
  if (ssl->stream_method) {
    return ssl->stream_method->set_encryption_secret(
...
int tls13_derive_handshake_secrets(SSL_HANDSHAKE *hs) {
  ...
  if (ssl->stream_method) {
    return ssl->stream_method->set_encryption_secret(
               ...
               hs->hash_len) &&
           ssl->stream_method->set_encryption_secret(
...
int tls13_rotate_traffic_key(SSL *ssl, enum evp_aead_direction_t direction) {
  ...
  if (!hkdf_expand_label(secret, digest, secret, secret_len,
                         ...
                         secret_len, ssl->stream_method != nullptr)) {
    return 0;
  }
...
int tls13_finished_mac(SSL_HANDSHAKE *hs, uint8_t *out, size_t *out_len,
  ...
  if (!hs->transcript.GetHash(context_hash, &context_hash_len) ||
      !tls13_verify_data(hs->transcript.Digest(), hs->ssl->version, out,
                         ...
                         context_hash_len, hs->ssl->stream_method != nullptr)) {
    return 0;
  }
  return 1;
}
...
int tls13_export_keying_material(SSL *ssl, Span<uint8_t> out,
  ...
  return EVP_Digest(context.data(), context.size(), hash, &hash_len, digest,
         ...
         hkdf_expand_label(out.data(), digest, derived_secret,
                           ...
                           out.size(), ssl->stream_method != nullptr);
}
...
int tls13_write_psk_binder(SSL_HANDSHAKE *hs, uint8_t *msg, size_t len) {
  ...
  if (!tls13_psk_binder(verify_data, ssl->session->ssl_version, digest,
                        ...
                        hash_len, ssl->stream_method != nullptr)) {
    return 0;
  }
