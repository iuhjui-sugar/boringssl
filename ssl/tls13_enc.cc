...
bool tls13_derive_esni_secrets(SSL_HANDSHAKE *hs,
  ...
  hexdump(__FUNCTION__, "client_keyshare", ssl->config->esni_client_keyshare.data(), ssl->config->esni_client_keyshare.size());
  ...
  if (!CBB_init(esni_cbb.get(), 0) ||
      ...
      !CBB_add_u16(esni_cbb.get(), ssl->config->esni_group) ||
      !CBB_add_u16_length_prefixed(esni_cbb.get(), &keyshare) ||
      !CBB_add_bytes(&keyshare, ssl->config->esni_client_keyshare.data(),
                     ssl->config->esni_client_keyshare.size()) ||
      !CBB_add_bytes(esni_cbb.get(), ssl->s3->client_random, SSL3_RANDOM_SIZE) ||
      !CBBFinishArray(esni_cbb.get(), &esni_contents)) {
    return false;
  }

  hexdump(__FUNCTION__, "esni_contents", esni_contents.data(), esni_contents.size());
  ...
  hexdump(__FUNCTION__, "key", key.data(), key.size());
  hexdump(__FUNCTION__, "iv", iv.data(), iv.size());
