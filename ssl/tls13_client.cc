...
static enum ssl_hs_wait_t do_read_server_hello(SSL_HANDSHAKE *hs) {
  ...
  uint16_t expected_version =
      ssl_resumption_experiment(ssl->version) ? TLS1_2_VERSION : ssl->version;
  ...
  // TLS 1.3 version.
  if (have_supported_versions && !ssl_resumption_experiment(ssl->version)) {
  ...
  return ssl_resumption_experiment(ssl->version)
