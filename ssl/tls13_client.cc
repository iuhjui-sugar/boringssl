...
static enum ssl_hs_wait_t do_read_hello_retry_request(SSL_HANDSHAKE *hs) {
  ...
  //
  // TODO(davidben): A later draft will likely add an ECH signal to HRR and
  // change this.
...
static enum ssl_hs_wait_t do_read_server_hello(SSL_HANDSHAKE *hs) {
  ...
  //
  // TODO(davidben): This is a bit late in the process of parsing ServerHello.
  // |ssl->session| is only valid for ClientHelloInner, so the decisions made
  // based on PSK need to be double-checked.. draft-11 will fix this, at which
  // point this logic can be moved before any processing.
  if (hs->selected_ech_config) {
    uint8_t ech_confirmation[8];
    if (!hs->inner_transcript.InitHash(ssl_protocol_version(ssl),
                                       hs->new_cipher) ||
        !tls13_ech_accept_confirmation(hs, ech_confirmation,
                                       hs->inner_transcript, msg.raw)) {
      return ssl_hs_error;
    }
    ...
    if (CRYPTO_memcmp(ech_confirmation,
      ...
      // TODO(davidben): If the server declines ECH, we handshake with
      // ClientHelloOuter instead of ClientHelloInner. That path is not yet
      // implemented. For now, terminate the handshake with a distiguisable
      // error for testing.
