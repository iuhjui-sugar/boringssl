...
static enum ssl_hs_wait_t do_read_encrypted_extensions(SSL_HANDSHAKE *hs) {
  ...
  if (ssl->s3->early_data_accepted) {
    ...
    if (ssl->s3->channel_id_valid || ssl->s3->token_binding_negotiated ||
        ssl->s3->has_application_settings) {
      OPENSSL_PUT_ERROR(SSL, SSL_R_UNEXPECTED_EXTENSION_ON_EARLY_DATA);
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_ILLEGAL_PARAMETER);
      return ssl_hs_error;
    }
    ssl->s3->has_application_settings = hs->early_session->has_early_alps;
    if (!hs->local_application_settings.CopyFrom(
            hs->early_session->early_local_application_settings) ||
        !ssl->s3->peer_application_settings.CopyFrom(
            hs->early_session->early_peer_application_settings)) {
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
      return ssl_hs_error;
    }
  }

  // Store the ALPN and ALPS values in the session for 0-RTT.
  if (!hs->new_session->early_alpn.CopyFrom(ssl->s3->alpn_selected) ||
      ...
      !hs->new_session->early_peer_application_settings.CopyFrom(
          ssl->s3->peer_application_settings)) {
    ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
    return ssl_hs_error;
  }
...
static enum ssl_hs_wait_t do_send_client_encrypted_extensions(
  ...
  if (ssl->s3->has_application_settings && !ssl->s3->early_data_accepted) {
    ...
    if (!ssl->method->init_message(ssl, cbb.get(), &body,
        ...
        !CBB_add_bytes(&extension, hs->local_application_settings.data(),
