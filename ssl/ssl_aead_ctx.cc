...
bool SSLAEADContext::SuffixLen(size_t *out_suffix_len, const size_t in_len,
  ...
  if (SSL_CIPHER_is_AEAD(cipher())) {
      *out_suffix_len = extra_in_len + ctx_.get()->tag_len;
      return true;
  }
  ...
  if (SSL_CIPHER_is_NULL(cipher())) {
    *out_suffix_len = extra_in_len + digest_len;
    return true;
  }
  ...
  if (in_len + extra_in_len < in_len ||
         in_len + extra_in_len + digest_len < in_len + extra_in_len) {
    *out_suffix_len = 0;
    OPENSSL_PUT_ERROR(SSL, SSL_R_RECORD_TOO_LARGE);
    return false;
  }
  const size_t total_in_len = in_len + extra_in_len + digest_len;
  const size_t pad_len = block_size - (total_in_len % block_size);
  *out_suffix_len = extra_in_len + digest_len + pad_len;
  return true;
}
