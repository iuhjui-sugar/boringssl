...
size_t SSLAEADContext::SuffixLen(const size_t in_len,
                                 const size_t extra_in_len) const {
  if (is_null_cipher() || FUZZER_MODE) {
    return extra_in_len;
  }
  // TODO(martinkr): Plumb this all the way into AEAD API.
  if (SSL_CIPHER_is_AEAD(cipher())) {
      return extra_in_len + ctx_.get()->tag_len;
  }
  assert(SSL_CIPHER_is_block_cipher(cipher()));
  size_t block_size;
  ...
  const size_t total_len = in_len + extra_in_len + digest_len;
  const size_t pad_len = block_size - (total_len % block_size);
  return total_len + pad_len - in_len;
}
...
bool SSLAEADContext::SealScatter(uint8_t *out_prefix, uint8_t *out,
  ...
  const size_t suffix_len = SuffixLen(in_len, extra_in_len);
  ...
  /* TODO(martinkr): Make AEAD code not complain if max_out_suffix_len is lower
   * than |EVP_AEAD_max_overhead| but still sufficiently large. */
  const size_t fake_suffix_len = extra_in_len + MaxOverhead();

  bool result = !!EVP_AEAD_CTX_seal_scatter(
      ctx_.get(), out, out_suffix, &written_suffix_len, fake_suffix_len, nonce,
...
bool SSLAEADContext::Seal(uint8_t *out, size_t *out_len, size_t max_out_len,
                          ...
                          size_t in_len) {
  const size_t prefix_len = ExplicitNonceLen();
  const size_t suffix_len = SuffixLen(in_len, 0);
