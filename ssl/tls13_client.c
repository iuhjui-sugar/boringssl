...
int tls13_client_handshake(SSL *ssl) {
  ...
  switch (ssl->hs->handshake_state) {
    ...
    case HS_STATE_SEND_FINISHED:
      ...
      ssl->hs->handshake_state = HS_STATE_FINISHING;
      ssl->hs->handshake_interrupt = HS_NEED_WRITE;
      break;
    case HS_STATE_FINISHING:
...
int tls13_send_client_hello(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  CBB_cleanup(&cbb);
  int ret = set_hs_message(out, SSL3_MT_CLIENT_HELLO, data, length);
  OPENSSL_free(data);
  return ret;
...
int tls13_send_finished(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  int ret = set_hs_message(out, SSL3_MT_FINISHED, signature, signature_len);
  OPENSSL_free(signature);
  return ret;
