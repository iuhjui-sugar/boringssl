...
static enum ssl_hs_wait_t do_process_server_hello(SSL *ssl, SSL_HANDSHAKE *hs) {
  ...
  int reuse_session = 0;
  if (have_pre_shared_key) {
    if (!ssl_ext_pre_shared_key_parse_serverhello(ssl, &reuse_session, &alert,
                                                  &pre_shared_key)) {
  ...
  if (ssl->session == NULL) {
    ...
    if (ssl_cipher_get_resumption_cipher(ssl->session->cipher) !=
...
int tls13_process_new_session_ticket(SSL *ssl) {
  ...
  if (!CBS_get_u32(&cbs, &session->ticket_lifetime_hint) ||
      ...
      !CBS_stow(&ticket, &session->ticket, &session->ticket_len) ||
      CBS_len(&cbs) != 0) {
