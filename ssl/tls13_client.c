...
static enum ssl_hs_wait_t do_process_server_hello(SSL *ssl, SSL_HANDSHAKE *hs) {
  ...
  /* BoringSSL only supports PSK_AUTH and PSK_DHE_KE. */
  if (!have_key_share || have_sigalgs == have_pre_shared_key) {
    if (!have_key_share || !have_sigalgs) {
  ...
  uint8_t *dhe_secret;
  size_t dhe_secret_len = hash_len;
  if (have_key_share) {
    ...
    dhe_secret = OPENSSL_malloc(hash_len);
    memcpy(dhe_secret, kZeroes, hash_len);
  }
  ...
  if (have_sigalgs) {
    if (!ssl_ext_sigalgs_parse_serverhello(ssl, &alert, &sigalgs)) {
      ssl3_send_alert(ssl, SSL3_AL_FATAL, alert);
      return ssl_hs_error;
    }
  }
