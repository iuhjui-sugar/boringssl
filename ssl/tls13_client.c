...
static enum ssl_hs_wait_t do_select_client_certificate(SSL *ssl,
                                                       SSL_HANDSHAKE *hs) {
...
static enum ssl_hs_wait_t run_state(SSL *ssl, SSL_HANDSHAKE *hs) {
  enum client_hs_state_t state = hs->state;
  switch (state) {
    case state_process_server_hello:
      return do_process_server_hello(ssl, hs);
    case state_process_encrypted_extensions:
      return do_process_encrypted_extensions(ssl, hs);
    case state_process_certificate_request:
      return do_process_certificate_request(ssl, hs);
    case state_process_server_certificate:
      return do_process_server_certificate(ssl, hs);
    case state_process_server_certificate_verify:
      return do_process_server_certificate_verify(ssl, hs);
    case state_process_server_finished:
      return do_process_server_finished(ssl, hs);
    case state_select_client_certificate:
      return do_select_client_certificate(ssl, hs);
    case state_send_client_certificate:
      return do_send_client_certificate(ssl, hs);
    case state_send_client_certificate_verify:
      return do_send_client_certificate_verify(ssl, hs, 1 /* first run */);
    case state_complete_client_certificate_verify:
      return do_send_client_certificate_verify(ssl, hs, 0 /* complete */);
    case state_send_client_finished:
      return do_send_client_finished(ssl, hs);
    case state_flush:
      return do_flush(ssl, hs);
    case state_done:
      return ssl_hs_ok;
  }
...
enum ssl_hs_wait_t tls13_client_handshake(SSL *ssl, SSL_HANDSHAKE *hs) {
  while (hs->state != state_done) {
    enum ssl_hs_wait_t ret = run_state(ssl, hs);
    if (ret != ssl_hs_ok) {
      return ret;
    }
  }
