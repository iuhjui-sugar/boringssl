...
static int tls13_send_client_hello(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  CBB_cleanup(&cbb);
  int ret = tls13_assemble_handshake_message(out, SSL3_MT_CLIENT_HELLO, data,
                                             length);
  OPENSSL_free(data);
  return ret;
...
static int tls13_receive_server_hello(SSL *ssl, SSL_HS_MESSAGE msg) {
  ...
  if (CBS_len(&cbs) != 0) {
    ...
    while (CBS_len(&extensions) != 0) {
      ...
      if (type == TLSEXT_TYPE_pre_shared_key) {
        valid = ext_pre_shared_key_parse_serverhello(ssl, &ext_alert, &extension);
      } else if (type == TLSEXT_TYPE_key_share) {
        valid = ext_key_share_parse_serverhello(ssl, &ext_alert, &extension);
      } else if (type == TLSEXT_TYPE_early_data) {
        valid = ext_early_data_parse_serverhello(ssl, &ext_alert, &extension);
      } else {
...
int tls13_client_handshake(SSL *ssl, SSL_HANDSHAKE *hs) {
  ...
  switch (hs->handshake_state) {
    case HS_STATE_CLIENT_HELLO:
      result = tls13_send_client_hello(ssl, hs->out_message);
      if (hs->zero_rtt) {
    ...
    case HS_STATE_CLIENT_ENCRYPTED_EXTENSIONS:
      // Deal with Client Encrypted Extensions
    ...
    case HS_STATE_SERVER_HELLO:
      if (hs->in_message->type == SSL3_MT_HELLO_RETRY_REQUEST) {
        // TODO: Handle HelloRetryRequest (might kill 0-RTT)
        result = 0;
      ...
      result = tls13_receive_server_hello(ssl, *hs->in_message);
    ...
    case HS_STATE_SERVER_ENCRYPTED_EXTENSIONS:
      ...
      result = tls13_receive_encrypted_extensions(ssl, *hs->in_message);
    ...
    case HS_STATE_SERVER_CERTIFICATE_REQUEST:
      if (hs->in_message->type == SSL3_MT_CERTIFICATE_REQUEST) {
        result = tls13_receive_certificate_request(ssl, *hs->in_message);
        hs->handshake_interrupt = HS_NEED_READ;
      } else {
        result = 1;
        hs->handshake_interrupt = HS_NEED_NONE;
    ...
    case HS_STATE_SERVER_CERTIFICATE:
      ...
      hs->handshake_interrupt = HS_NEED_READ | HS_NEED_HASH;
      break;
...
static int tls13_receive_session_ticket(SSL *ssl, SSL_HS_MESSAGE msg) {
  ...
  // TODO: Actually implement NewSessionTicket
