...
static enum ssl_hs_wait_t do_process_server_hello(SSL *ssl, SSL_HANDSHAKE *hs) {
  ...
  if (ssl->session == NULL) {
    ...
    if (!ssl_cipher_get_ecdhe_psk_cipher(ssl->session->cipher, &resumption_cipher) ||
  ...
  /* The session is now stored in the mutable new_session, so we can clear the
   * saved copy in ssl->session. */
  SSL_set_session(ssl, NULL);
