...
int tls13_add_certificate(SSL_HANDSHAKE *hs) {
  ...
  if (hs->ocsp_stapling_requested && cert->ocsp_response != nullptr) {
  ...
  for (size_t i = 0;
       i < sk_CertCompressionAlg_num(ssl->ctx->cert_compression_algs); i++) {
    CertCompressionAlg *candidate =
        sk_CertCompressionAlg_value(ssl->ctx->cert_compression_algs, i);
  ...
  UniquePtr<CRYPTO_BUFFER> compressed_data;
  auto uncompressed_data(Span<const uint8_t>(msg).subspan(4));

  if (!alg->compress(ssl, &compressed_data, uncompressed_data)) {
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  }
  ...
  if (!ssl->method->init_message(ssl, cbb.get(), &body,
      ...
      !CBB_add_u24(&body, uncompressed_data.size()) ||
      !CBB_add_u24_length_prefixed(&body, &compressed) ||
      !CBB_add_bytes(&compressed,
                     CRYPTO_BUFFER_data(compressed_data.get()),
                     CRYPTO_BUFFER_len(compressed_data.get())) ||
