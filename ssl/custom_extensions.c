...
static int custom_ext_add_hello(SSL *ssl, CBB *extensions) {
  ...
  for (i = 0; i < sk_SSL_CUSTOM_EXTENSION_num(stack); i++) {
    ...
    if (ssl->server &&
      ...
      /* Server's cannot echo extensions that the client didn't send. */
      continue;
    }
    ...
    switch (ext->add_callback(ssl, ext->value, &contents, &contents_len, &alert,
                              ext->add_arg)) {
      case 1:
        if (contents_len > 0xffff) {
          OPENSSL_PUT_ERROR(SSL, SSL_R_CUSTOM_EXTENSION_CONTENTS_TOO_LARGE);
        ...
        if (!CBB_add_u16(extensions, ext->value) ||
            !CBB_add_u16(extensions, contents_len) ||
            !CBB_add_space(extensions, &space, contents_len)) {
        ...
        memcpy(space, contents, contents_len);
        if (ext->free_callback && 0 < contents_len) {
          ext->free_callback(ssl, ext->value, contents, ext->add_arg);
        }
