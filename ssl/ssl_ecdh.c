...
static int ssl_cecpq1_offer(SSL_ECDH_CTX *ctx, CBB *out) {
  ...
  uint8_t newhope_servermsg[NEWHOPE_OFFERMSG_LENGTH];
  NEWHOPE_offer(newhope_servermsg, data->newhope_sk);
...
static int ssl_cecpq1_accept(SSL_ECDH_CTX *ctx, CBB *cbb, uint8_t **out_secret,
  ...
  *out_alert = SSL_AD_INTERNAL_ERROR;
  ...
  uint8_t newhope_clientmsg[NEWHOPE_ACCEPTMSG_LENGTH];
  if (!NEWHOPE_accept(secret + 32, newhope_clientmsg, peer_key + 32,
