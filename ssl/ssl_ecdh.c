...
static int ssl_ec_point_generate_key(SSL_ECDH_CTX *ctx, CBB *out) {
  if (ctx->data != NULL) {
...
int ssl_ec_point_compute_premaster(SSL_ECDH_CTX *ctx, uint8_t **out_premaster,
...
static int ssl_x25519_generate_key(SSL_ECDH_CTX *ctx, CBB *out) {
...
static int ssl_x25519_compute_premaster(SSL_ECDH_CTX *ctx, uint8_t **out_premaster,
                                        size_t *out_premaster_len,
                                        uint8_t *out_alert,
  ...
  *out_alert = SSL_AD_INTERNAL_ERROR;
  ...
  if (peer_key_len != 32 || (peer_key[31] & 0x80) ||
      !X25519(premaster, (uint8_t *)ctx->data, peer_key)) {
    *out_alert = SSL_AD_DECODE_ERROR;
