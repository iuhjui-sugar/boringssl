...
#include "internal.h"
#include "../crypto/newhope/internal.h"
...
static int ssl_ec_point_client_key_exchange(SSL_ECDH_CTX *ctx,
                                            CBB *out_public_key,
                                            uint8_t **out_secret,
                                            size_t *out_secret_len,
                                            uint8_t *out_alert,
                                            ...
                                            size_t peer_key_len) {
  *out_alert = SSL_AD_INTERNAL_ERROR;
  if (!ssl_ec_point_generate_keypair(ctx, out_public_key) ||
      !ssl_ec_point_compute_secret(ctx, out_secret, out_secret_len, out_alert,
...
static int ssl_x25519_client_key_exchange(SSL_ECDH_CTX *ctx, CBB *out_public_key,
                                          uint8_t **out_secret,
                                          size_t *out_secret_len,
                                          uint8_t *out_alert,
                                          const uint8_t *peer_key,
                                          size_t peer_key_len) {
  *out_alert = SSL_AD_INTERNAL_ERROR;
  if (!ssl_x25519_generate_keypair(ctx, out_public_key) ||
      !ssl_x25519_compute_secret(ctx, out_secret, out_secret_len, out_alert,
                                 peer_key, peer_key_len)) {
    return 0;
  }
  return 1;
...
#define CECPQ1_CLIENTMSG_LENGTH (32 + NEWHOPE_CLIENTMSG_LENGTH)
#define CECPQ1_SERVERMSG_LENGTH (32 + NEWHOPE_SERVERMSG_LENGTH)
#define CECPQ1_SECRET_LENGTH (32 + SHA256_DIGEST_LENGTH)

static int ssl_cecpq1_server_generate_keypair(SSL_ECDH_CTX *ctx, CBB *out) {
  assert(ctx->data == NULL);
  ...
  uint8_t newhope_servermsg[NEWHOPE_SERVERMSG_LENGTH];
  NEWHOPE_keygen(newhope_servermsg, &data->newhope_sk);

  if (!CBB_add_bytes(out, x25519_public_key, sizeof(x25519_public_key)) ||
      !CBB_add_bytes(out, newhope_servermsg, sizeof(newhope_servermsg))) {
    return 0;
  }
  return 1;
}

static int ssl_cecpq1_server_finish(SSL_ECDH_CTX *ctx, uint8_t **out_secret,
                                    size_t *out_secret_len, uint8_t *out_alert,
                                    const uint8_t *peer_key,
                                    size_t peer_key_len) {
  if (peer_key_len != CECPQ1_CLIENTMSG_LENGTH) {
    *out_alert = SSL_AD_DECODE_ERROR;
    return 0;
  }
  ...
  *out_alert = SSL_AD_INTERNAL_ERROR;
  ...
  if (!NEWHOPE_server_compute_key(secret + 32, &data->newhope_sk, peer_key + 32,
...
static int ssl_cecpq1_client_key_exchange(SSL_ECDH_CTX *ctx, CBB *cbb,
                                          uint8_t **out_secret,
                                          size_t *out_secret_len,
                                          uint8_t *out_alert,
                                          const uint8_t *peer_key,
                                          size_t peer_key_len) {
  if (peer_key_len != CECPQ1_SERVERMSG_LENGTH) {
    *out_alert = SSL_AD_DECODE_ERROR;
    return 0;
  }
  ...
  *out_alert = SSL_AD_INTERNAL_ERROR;
  ...
  uint8_t newhope_clientmsg[NEWHOPE_CLIENTMSG_LENGTH];
  if (!NEWHOPE_client_compute_key(secret + 32, newhope_clientmsg, peer_key + 32,
                                  NEWHOPE_SERVERMSG_LENGTH)) {
    *out_alert = SSL_AD_DECODE_ERROR;
    goto err;
  }

  if (!CBB_add_bytes(cbb, x25519_public_key, sizeof(x25519_public_key)) ||
      !CBB_add_bytes(cbb, newhope_clientmsg, sizeof(newhope_clientmsg))) {
    goto err;
  }
...
static void ssl_cecpq1_cleanup(SSL_ECDH_CTX *ctx) {
  ...
  OPENSSL_cleanse(ctx->data, sizeof(cecpq1_data));
  OPENSSL_free(ctx->data);
}
...
static int ssl_dhe_client_key_exchange(SSL_ECDH_CTX *ctx, CBB *out_public_key,
                                       uint8_t **out_secret,
                                       size_t *out_secret_len,
                                       uint8_t *out_alert,
                                       const uint8_t *peer_key,
                                       size_t peer_key_len) {
  *out_alert = SSL_AD_INTERNAL_ERROR;
  if (!ssl_dhe_generate_keypair(ctx, out_public_key) ||
      !ssl_dhe_compute_secret(ctx, out_secret, out_secret_len, out_alert,
                              peer_key, peer_key_len)) {
    return 0;
  }
  return 1;
...
static const SSL_ECDH_METHOD kMethods[] = {
    {
        ...
        ssl_ec_point_client_key_exchange,
    },
    {
        NID_secp384r1,
        SSL_CURVE_SECP384R1,
    ...
    },
    {
        ...
        ssl_x25519_client_key_exchange,
    },
    {
        NID_cecpq1,
        SSL_CURVE_X25519_AND_NEWHOPE,
        "CECPQ1",
        ssl_cecpq1_cleanup,
        ssl_cecpq1_server_generate_keypair,
        ssl_cecpq1_server_finish,
        ssl_cecpq1_client_key_exchange,
    },
};
...
int SSL_ECDH_CTX_server_generate_keypair(SSL_ECDH_CTX *ctx,
                                         CBB *out_public_key) {
  return ctx->method->server_generate_keypair(ctx, out_public_key);
}
...
int SSL_ECDH_CTX_server_finish(SSL_ECDH_CTX *ctx, uint8_t **out_secret,
  ...
  return ctx->method->server_finish(ctx, out_secret, out_secret_len, out_alert,
                                    peer_key, peer_key_len);
}
...
int SSL_ECDH_CTX_client_key_exchange(SSL_ECDH_CTX *ctx, CBB *out_public_key,
                                     ...
                                     const uint8_t *peer_key,
                                     size_t peer_key_len) {
  return ctx->method->client_key_exchange(ctx, out_public_key, out_secret,
                                          out_secret_len, out_alert, peer_key,
                                          peer_key_len);
}
