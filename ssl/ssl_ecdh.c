...
static int ssl_ec_point_initiate(SSL_ECDH_CTX *ctx, CBB *out) {
...
static int ssl_ec_point_accept(SSL_ECDH_CTX *ctx, CBB *out_public_key,
                               ...
                               size_t peer_key_len) {
  *out_alert = SSL_AD_INTERNAL_ERROR;
  if (!ssl_ec_point_initiate(ctx, out_public_key) ||
...
static int ssl_x25519_initiate(SSL_ECDH_CTX *ctx, CBB *out) {
  assert(ctx->data == NULL);
...
static int ssl_x25519_accept(SSL_ECDH_CTX *ctx, CBB *out_public_key,
                             ...
                             size_t peer_key_len) {
  *out_alert = SSL_AD_INTERNAL_ERROR;
  if (!ssl_x25519_initiate(ctx, out_public_key) ||
...
static int ssl_dhe_initiate(SSL_ECDH_CTX *ctx, CBB *out) {
  DH *dh = (DH *)ctx->data;
...
static int ssl_dhe_accept(SSL_ECDH_CTX *ctx, CBB *out_public_key,
                          ...
                          size_t peer_key_len) {
  *out_alert = SSL_AD_INTERNAL_ERROR;
  if (!ssl_dhe_initiate(ctx, out_public_key) ||
...
int SSL_ECDH_CTX_initiate(SSL_ECDH_CTX *ctx, CBB *out_public_key) {
  return ctx->method->initiate(ctx, out_public_key);
}
