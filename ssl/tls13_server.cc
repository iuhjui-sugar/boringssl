...
static enum ssl_hs_wait_t do_select_session(SSL_HANDSHAKE *hs) {
  ...
  switch (select_session(hs, &alert, &session, &ssl->s3->ticket_age_skew,
    ...
    case ssl_ticket_aead_success:
      ...
      } else if (MakeConstSpan(ssl->s3->local_alps_settings) !=
                     session->early_local_alps ||
                 MakeConstSpan(ssl->s3->peer_alps_settings) !=
                     session->early_peer_alps) {
        // The negotiated ALPS must match the one in the ticket.
...
static enum ssl_hs_wait_t do_read_client_alps(SSL_HANDSHAKE *hs) {
  ...
  if (ssl->s3->sent_alps) {
    ...
    if (!ssl_check_message_type(ssl, msg, SSL3_MT_CLIENT_APPLICATION_SETTINGS) ||
        !ssl->s3->peer_alps_settings.CopyFrom(msg.body) ||
