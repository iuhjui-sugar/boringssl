...
static enum ssl_hs_wait_t do_send_server_hello(SSL_HANDSHAKE *hs) {
  ...
  if (hs->ech_is_inner_present) {
    ...
    if (!tls13_ech_accept_confirmation(hs, random_suffix, hs->transcript,
                                       server_hello)) {
      return ssl_hs_error;
    }
    ...
    const size_t offset = header_len + 2 + SSL3_RANDOM_SIZE - 8;
    Span<uint8_t> server_hello_out = MakeSpan(server_hello).subspan(offset, 8);
    OPENSSL_memcpy(server_hello_out.data(), random_suffix.data(), 8);
  }
