...
static enum ssl_hs_wait_t do_select_session(SSL_HANDSHAKE *hs) {
  ...
  switch (select_session(hs, &alert, &session, &ssl->s3->ticket_age_skew,
    ...
    case ssl_ticket_aead_success:
      ...
      } else if (MakeConstSpan(ssl->s3->alps_settings) != session->early_alps) {
  ...
  // Store the initial application settings in the session.
  if (!hs->new_session->early_alps.CopyFrom(ssl->s3->alps_settings)) {
    ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
    return ssl_hs_error;
  }
