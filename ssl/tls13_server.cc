...
static int resolve_ecdhe_secret(SSL_HANDSHAKE *hs, bool *out_need_retry,
  ...
  *out_need_retry = false;
  ...
  return tls13_derive_key_schedule(hs) &&
         tls13_advance_key_schedule(hs, dhe_secret.data(), dhe_secret.size());
}
...
static int add_new_session_tickets(SSL_HANDSHAKE *hs) {
  ...
  for (int i = 0; i < kNumTickets; i++) {
    ...
    if (!ssl->method->init_message(ssl, cbb.get(), &body,
        ...
        !tls13_derive_session_psk(session.get(), nonce, sizeof(nonce)) ||
...
static enum ssl_hs_wait_t do_select_session(SSL_HANDSHAKE *hs) {
  ...
  if (!resolve_ecdhe_secret(hs, &need_retry, &client_hello)) {
    if (need_retry) {
      ...
      if (ssl_is_draft21(ssl->version) && !hs->transcript.UpdateHRR()) {
        return ssl_hs_error;
      }
...
static enum ssl_hs_wait_t do_send_server_hello(SSL_HANDSHAKE *hs) {
  ...
  if (hs->cert_request) {
    if (ssl_is_draft21(ssl->version)) {
      ...
      CBB sigalg_contents, sigalgs_cbb;
      ...
      if (ssl->client_CA != nullptr || ssl->ctx->client_CA != nullptr) {
        CBB ca_contents;
        if (!CBB_add_u16(&cert_request_extensions,
...
static enum ssl_hs_wait_t do_send_server_finished(SSL_HANDSHAKE *hs) {
  ...
  if (ssl->early_data_accepted) {
    ...
    if (ssl_is_draft21(ssl->version)) {
      uint8_t eoed[4] = {SSL3_MT_END_OF_EARLY_DATA, 0, 0, 0};
      if (!hs->transcript.Update(eoed, sizeof(eoed))) {
...
static enum ssl_hs_wait_t do_process_end_of_early_data(SSL_HANDSHAKE *hs) {
  ...
  if (hs->early_data_offered) {
    ...
    if (ssl_is_draft21(ssl->version)) {
      SSLMessage msg;
      if (!ssl->method->get_message(ssl, &msg) ||
          !ssl_check_message_type(ssl, msg, SSL3_MT_END_OF_EARLY_DATA) ||
          CBS_len(&msg.body) != 0) {
        return ssl_hs_error;
      }
      ssl->method->next_message(ssl);
