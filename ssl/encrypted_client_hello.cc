...
bool ECHServerConfig::Init(Span<const uint8_t> ech_config,
  ...
  if (!EVP_HPKE_KEY_init(&key_, EVP_hpke_x25519_hkdf_sha256(),
  ...
  if (!EVP_HPKE_KEY_public_key(&key_, expected_public_key,
...
bool ECHServerConfig::SetupContext(EVP_HPKE_CTX *ctx, uint16_t kdf_id,
  ...
  return EVP_HPKE_CTX_setup_base_r(
      ctx, &key_, EVP_hpke_hkdf_sha256(), get_ech_aead(aead_id), enc.data(),
