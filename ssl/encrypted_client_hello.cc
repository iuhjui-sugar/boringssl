...
bool ssl_decode_client_hello_inner(
  ...
  for (uint8_t padding : Span<const uint8_t>(cbs)) {
    if (padding != 0) {
      OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
  ...
  auto extensions = MakeConstSpan(client_hello_inner.extensions,
                                  client_hello_inner.extensions_len);
  CBS extension;
  if (!ssl_client_hello_get_extension(&client_hello_inner, &extension,
                                      TLSEXT_TYPE_ech_outer_extensions)) {
    // No ech_outer_extensions. Copy everything.
    if (!CBB_add_bytes(&extensions_cbb, extensions.data(), extensions.size())) {
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return false;
    }
  } else {
    const size_t offset = CBS_data(&extension) - extensions.data();
    auto extensions_before =
        extensions.subspan(0, offset - 4 /* extension header */);
    auto extensions_after =
        extensions.subspan(offset + CBS_len(&extension));
    if (!CBB_add_bytes(&extensions_cbb, extensions_before.data(),
                       extensions_before.size())) {
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return false;
    }
    ...
    while (CBS_len(&ext_list) != 0) {
      ...
      if (!CBB_add_u16(&extensions_cbb, found) ||
          !CBB_add_u16_length_prefixed(&extensions_cbb, &ext_body_cbb) ||
          !CBB_add_bytes(&ext_body_cbb, CBS_data(&ext_body),
                         CBS_len(&ext_body))) {
        OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
        return false;
      }
    ...
    if (!CBB_add_bytes(&extensions_cbb, extensions_after.data(),
                       extensions_after.size())) {
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return false;
    }
  }
  ...
  if (!ssl_client_hello_init(ssl, &client_hello_inner,
      ...
      !ssl_client_hello_get_extension(&client_hello_inner, &extension,
                                      TLSEXT_TYPE_encrypted_client_hello) ||
      CBS_len(&extension) != 1 || CBS_data(&extension)[0] != ECH_CLIENT_INNER ||
      !ssl_client_hello_get_extension(&client_hello_inner, &extension,
...
bool ssl_client_hello_decrypt(EVP_HPKE_CTX *hpke_ctx, Array<uint8_t> *out,
  ...
  *out_is_decrypt_error = false;
  ...
  // |payload| must be inside |client_hello_outer|. We assert with |uintptr_t|
  // because the comparison would be UB if they didn't alias.
...
bool ssl_is_valid_ech_public_name(Span<const uint8_t> public_name) {
  // See draft-ietf-tls-esni-12, Section 4 and RFC5890, Section 2.3.1. The
