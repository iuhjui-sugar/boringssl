...
static bool parse_ech_config(CBS *cbs, ECHConfig *out, bool *out_supported,
  ...
  // TODO(davidben): Check validity of |public_name|.
  ...
  while (CBS_len(&extensions) != 0) {
    ...
    if (type & 0x8000) {
      ...
      // to enforce syntax, but we will ultimately ignore this ECHConfig.
  ...
  if (!hs->config->client_ech_config_list.empty()) {
    ...
    while (CBS_len(&child) > 0) {
      ...
      if (supported &&  //
        ...
        if (!CBB_init(info.get(), 8 + ech_config.raw.size()) ||
...
static size_t compute_extension_length(const EVP_HPKE_AEAD *aead,
  ...
  const size_t overhead = EVP_AEAD_max_overhead(EVP_HPKE_AEAD_aead(aead));
  size_t ret = 4;                // HpkeSymmetricCipherSuite cipher_suite
  ret++;                         // uint8 config_id
  ret += 2 + enc_len;            // opaque enc<1..2^16-1>
  ret += 2 + in_len + overhead;  // opaque payload<1..2^16-1>
  return ret;
}
...
bool ssl_encrypt_client_hello(SSL_HANDSHAKE *hs, Span<const uint8_t> enc) {
  ...
  // TODO(davidben): This ends up constructing the ClientHelloOuter twice.
  // Revisit this in the next draft, which uses a more forgiving construction.
  ...
  if (!CBB_init(cbb.get(), extension_len) ||
      ...
      !CBB_add_bytes(&enc_cbb, enc.data(), enc.size()) ||
      !CBB_add_u16_length_prefixed(cbb.get(), &payload_cbb) ||
      !CBB_reserve(&payload_cbb, &payload, payload_len) ||
...
int SSL_marshal_ech_config(uint8_t **out, size_t *out_len, uint8_t config_id,
  ...
  if (public_name_len == 0) {
    // TODO(davidben): Check |public_name| is a valid DNS name.
  ...
  if (!CBB_init(cbb.get(), 128) ||  //
      ...
      !CBB_add_bytes(&child, reinterpret_cast<const uint8_t *>(public_name),
                     public_name_len) ||
      // TODO(davidben): Reserve some GREASE extensions and include some.
...
int SSL_ech_accepted(const SSL *ssl) {
  if (SSL_in_early_data(ssl) && !ssl->server) {
    ...
    // ClientHelloInner.
    return ssl->s3->hs->selected_ech_config ? 1 : 0;
  }
