...
static int setup_buffer(SSL3_BUFFER *buf, size_t header_len, size_t cap) {
  ...
  buf->offset = (0 - header_len) & (SSL3_ALIGN_PAYLOAD - 1);
  buf->len = 0;
  buf->cap = cap;
  return 1;
}
...
static int dtls_read_buffer_extend(SSL *ssl) {
  SSL3_BUFFER *buf = &ssl->s3->read_buffer;
  ...
  /* Read a single packet from |ssl->rbio|. */
  ssl->rwstate = SSL_READING;
  int ret = BIO_read(ssl->rbio, buf->buf + buf->offset, buf->cap);
  if (ret <= 0) {
    return ret;
  }
...
static int tls_read_buffer_extend(SSL *ssl, size_t len) {
  SSL3_BUFFER *buf = &ssl->s3->read_buffer;
...
int ssl_read_buffer_extend(SSL *ssl, size_t len) {
  /* ssl_read_buffer_extend implicitly discards any consumed data. */
  ...
  if (SSL_IS_DTLS(ssl)) {
    ...
    ret = dtls_read_buffer_extend(ssl);
  } else {
    ret = tls_read_buffer_extend(ssl, len);
  }
