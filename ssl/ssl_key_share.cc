...
class CECPQ2KeyShare : public SSLKeyShare {
 public:
  CECPQ2KeyShare() {}
  ~CECPQ2KeyShare() override {
    OPENSSL_cleanse(x25519_private_key_, sizeof(x25519_private_key_));
    OPENSSL_cleanse(&hrss_public_key_, sizeof(hrss_public_key_));
    OPENSSL_cleanse(&hrss_private_key_, sizeof(hrss_private_key_));
  }
  ...
  bool Serialize(CBB *out) override {
    uint8_t hrss_public_key_bytes[HRSS_PUBLIC_KEY_BYTES];
    uint8_t hrss_private_key_bytes[HRSS_PRIVATE_KEY_BYTES];
    HRSS_marshal_public_key(hrss_public_key_bytes, &hrss_public_key_);
    HRSS_marshal_private_key(hrss_private_key_bytes, &hrss_private_key_);

    return (CBB_add_asn1_uint64(out, GroupID()) &&
            CBB_add_asn1_octet_string(out, x25519_private_key_,
                                      sizeof(x25519_private_key_)) &&
            CBB_add_asn1_octet_string(out, hrss_public_key_bytes,
                                      sizeof(hrss_public_key_bytes)) &&
            CBB_add_asn1_octet_string(out, hrss_private_key_bytes,
                                      sizeof(hrss_private_key_bytes)));
  }

  bool Deserialize(CBS *in) override {
    CBS key;
    if (!CBS_get_asn1(in, &key, CBS_ASN1_OCTETSTRING) ||
        CBS_len(&key) != sizeof(x25519_private_key_) ||
        !CBS_copy_bytes(&key, x25519_private_key_, sizeof(x25519_private_key_)) ||

        !CBS_get_asn1(in, &key, CBS_ASN1_OCTETSTRING) ||
        CBS_len(&key) != HRSS_PUBLIC_KEY_BYTES ||
        !HRSS_parse_public_key(&hrss_public_key_, CBS_data(&key)) ||

        !CBS_get_asn1(in, &key, CBS_ASN1_OCTETSTRING) ||
        CBS_len(&key) != HRSS_PRIVATE_KEY_BYTES ||
        !HRSS_parse_private_key(&hrss_private_key_, CBS_data(&key))) {
      return false;
    }
    return true;
  }
