...
class X25519Kyber768KeyShare : public SSLKeyShare {
 public:
  ...
  bool Generate(CBB *out) override {
    ...
    uint8_t kyber_public[KYBER_PUBLIC_KEY_BYTES];
    KYBER_generate_key(kyber_public, &kyber_private_key_);

    if (!CBB_add_bytes(out, x25519_public_key, sizeof(x25519_public_key)) ||
        !CBB_add_bytes(out, kyber_public,
                       sizeof(kyber_public))) {
      return false;
    }
  ...
  bool Encap(CBB *out_ciphertext, Array<uint8_t> *out_secret,
    ...
    if (peer_key.size() != 32 + KYBER_PUBLIC_KEY_BYTES ||
        !X25519(secret.data(), x25519_private_key_, peer_key.data()) ||
        !KYBER_public_key_from_bytes(&kyber_public_key, peer_key.data() + 32, peer_key.size() - 32)) {
      *out_alert = SSL_AD_DECODE_ERROR;
      OPENSSL_PUT_ERROR(SSL, SSL_R_BAD_ECPOINT);
      return false;
    }
    ...
    KYBER_encap(kyber_ciphertext, secret.data() + 32, secret.size() - 32, &kyber_public_key);

    if (!CBB_add_bytes(out_ciphertext, x25519_public_key,
                       sizeof(x25519_public_key)) ||
        !CBB_add_bytes(out_ciphertext, kyber_ciphertext, sizeof(kyber_ciphertext))) {
      return false;
    }
  ...
  bool Decap(Array<uint8_t> *out_secret, uint8_t *out_alert,
    ...
    *out_alert = SSL_AD_INTERNAL_ERROR;
    ...
    KYBER_decap(secret.data() + 32, secret.size() - 32, ciphertext.data() + 32, &kyber_private_key_);
    *out_secret = std::move(secret);
    return true;
  }
