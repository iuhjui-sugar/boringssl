...
bool MockQuicTransport::SetReadSecret(enum ssl_encryption_level_t level,
  ...
  read_secrets_[level].cipher = SSL_CIPHER_get_value(cipher);
  read_secrets_[level].secret.assign(secret, secret + secret_len);
  return true;
}
...
bool MockQuicTransport::SetWriteSecret(enum ssl_encryption_level_t level,
                                       ...
                                       size_t secret_len) {
  // TODO(davidben): Assert the various encryption secret invariants.
  write_secrets_[level].cipher = SSL_CIPHER_get_value(cipher);
  write_secrets_[level].secret.assign(secret, secret + secret_len);
  return true;
}
...
bool MockQuicTransport::ReadHeader(uint8_t *out_tag, size_t *out_len) {
  ...
  *out_tag = header[0];
  ...
  if (cipher_suite != read_secrets_[level].cipher) {
    return false;
  }
  const std::vector<uint8_t> &secret = read_secrets_[level].secret;
...
bool MockQuicTransport::WriteRecord(enum ssl_encryption_level_t level,
                                    ...
                                    size_t len) {
  uint16_t cipher_suite = write_secrets_[level].cipher;
  const std::vector<uint8_t> &secret = write_secrets_[level].secret;
