...
bool write_header(BIO *bio, uint8_t tag, size_t len) {
  ...
  header[3] = (len >> 8) & 0xff;
  header[4] = len & 0xff;
  return BIO_write_all(bio, header, sizeof(header)) == 1;
}
...
bool ReadAll(BIO *bio, uint8_t *out, size_t len) {
  while (len > 0) {
...
bool ReadHeader(BIO *bio, uint8_t *out_tag, size_t *out_len) {
  uint8_t header[5];
  if (!ReadAll(bio, header, sizeof(header))) {
    return false;
  }
...
bool MockQuicTransport::ReadHandshake() {
  ...
  if (!ReadAll(bio_.get(), read_secret.data(), read_secret.size())) {
    return false;
  }
  for (size_t i = 0; i < secret.size(); i++) {
    if (read_secret[i] != secret[i]) {
      return false;
    }
  }

  std::vector<uint8_t> buf(len);
  if (!ReadAll(bio_.get(), buf.data(), buf.size())) {
    return false;
  }
  int ret = SSL_provide_quic_data(ssl_, SSL_quic_read_level(ssl_), buf.data(),
                                  buf.size());
  return ret == 1;
}
...
bool MockQuicTransport::Flush() { return BIO_flush(bio_.get()) == 1; }
