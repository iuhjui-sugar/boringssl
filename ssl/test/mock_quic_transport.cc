...
bool MockQuicTransport::SetReadSecret(enum ssl_encryption_level_t level,
  ...
  read_secrets_[level].first = SSL_CIPHER_get_value(cipher);
  read_secrets_[level].second.assign(secret, secret + secret_len);
  return true;
}
...
bool MockQuicTransport::SetWriteSecret(enum ssl_encryption_level_t level,
                                       ...
                                       size_t secret_len) {
  // TODO(davidben): Assert the various encryption secret invariants.
  write_secrets_[level].first = SSL_CIPHER_get_value(cipher);
  write_secrets_[level].second.assign(secret, secret + secret_len);
  return true;
}
...
bool MockQuicTransport::ReadHeader(uint8_t *out_tag, size_t *out_len) {
  ...
  *out_tag = header[0];
  uint16_t cipher_suite = header[5] << 8 | header[6];
  ...
  if (cipher_suite != read_secrets_[level].first) {
    return false;
  }
  const std::vector<uint8_t> &secret = read_secrets_[level].second;
...
bool MockQuicTransport::WriteRecord(enum ssl_encryption_level_t level,
                                    ...
                                    size_t len) {
  uint16_t cipher_suite = write_secrets_[level].first;
  const std::vector<uint8_t> &secret = write_secrets_[level].second;
  size_t tlv_len = 2 + secret.size() + len;
  uint8_t header[7];
  header[0] = tag;
  header[1] = (tlv_len >> 24) & 0xff;
  header[2] = (tlv_len >> 16) & 0xff;
  header[3] = (tlv_len >> 8) & 0xff;
  header[4] = tlv_len & 0xff;
  header[5] = (cipher_suite >> 8) & 0xff;
  header[6] = cipher_suite & 0xff;
