...
bool Handshaker(const TestConfig *config, int rfd, int wfd,
                       bssl::Span<const uint8_t> input, int control) {
  bssl::UniquePtr<SSL_CTX> ctx = config->SetupCtx(/*old_ctx=*/nullptr);
  if (!ctx) {
    return false;
  }

  bssl::UniquePtr<SSL> ssl_handshake = config->NewSSL(
      ctx.get(), nullptr, false, std::unique_ptr<TestState>(new TestState));
  ...
  CBS cbs, handoff, state;
  CBS_init(&cbs, input.data(), input.size());
  if (!CBS_get_asn1_element(&cbs, &handoff, CBS_ASN1_SEQUENCE) ||
      !CBS_get_u24_length_prefixed(&cbs, &state) ||
      !DeserializeTestState(&state, GetTestState(ssl), ctx.get()) |
  ...
  if (!CBB_init(output.get(), 1024) ||
      ...
      !SerializeTestState(GetTestState(ssl), ssl->ctx, output.get()) ||
