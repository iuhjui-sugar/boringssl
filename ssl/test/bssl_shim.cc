...
static bool CheckHandshakeProperties(SSL *ssl, bool is_resume,
  ...
  if (config->expect_token_binding) {
    if (!SSL_get_token_binding_negotiated(ssl)) {
      fprintf(stderr, "no Token Binding negotiated\n");
      return false;
    } else if (SSL_get_negotiated_token_binding_param(ssl) != static_cast<uint8_t>(config->expected_token_binding_param)) {
...
static bool DoConnection(bssl::UniquePtr<SSL_SESSION> *out_session,
  ...
  if (!config->send_token_binding_params.empty()) {
    SSL_set_token_binding_params(ssl.get(), reinterpret_cast<const uint8_t*>(config->send_token_binding_params.data()), config->send_token_binding_params.length());
  }
