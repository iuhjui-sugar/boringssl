...
static bssl::unique_ptr<X509> LoadCertificate(const std::string &file) {
  bssl::unique_ptr<BIO> bio(BIO_new(BIO_s_file()));
  if (!bio || !BIO_read_filename(bio.get(), file.c_str())) {
    return nullptr;
  }
  return bssl::unique_ptr<X509>(PEM_read_bio_X509(bio.get(), NULL, NULL, NULL));
}

static bssl::unique_ptr<EVP_PKEY> LoadPrivateKey(const std::string &file) {
  bssl::unique_ptr<BIO> bio(BIO_new(BIO_s_file()));
  if (!bio || !BIO_read_filename(bio.get(), file.c_str())) {
    return nullptr;
  }
  return bssl::unique_ptr<EVP_PKEY>(PEM_read_bio_PrivateKey(bio.get(), NULL, NULL, NULL));
}
...
static bool GetCertificate(SSL *ssl, bssl::unique_ptr<X509> *out_x509,
                           bssl::unique_ptr<EVP_PKEY> *out_pkey) {
...
static bool InstallCertificate(SSL *ssl) {
  bssl::unique_ptr<X509> x509;
  bssl::unique_ptr<EVP_PKEY> pkey;
...
static bssl::unique_ptr<SSL_CTX> SetupCtx(const TestConfig *config) {
  bssl::unique_ptr<SSL_CTX> ssl_ctx(SSL_CTX_new(
  ...
  bssl::unique_ptr<DH> dh(DH_get_2048_256(NULL));
  if (!dh) {
    return nullptr;
  }
...
static bool RetryAsync(SSL *ssl, int ret) {
  ...
  switch (SSL_get_error(ssl, ret)) {
    ...
    case SSL_ERROR_WANT_CHANNEL_ID_LOOKUP: {
      bssl::unique_ptr<EVP_PKEY> pkey = LoadPrivateKey(GetTestConfig(ssl)->send_channel_id);
      if (!pkey) {
        return false;
      }
...
// previous exchange.
static bool DoExchange(bssl::unique_ptr<SSL_SESSION> *out_session, SSL_CTX *ssl_ctx,
                       const TestConfig *config, bool is_resume,
                       SSL_SESSION *session) {
  bssl::unique_ptr<SSL> ssl(SSL_new(ssl_ctx));
  if (!ssl) {
    return false;
  }
  ...
  if (!config->send_channel_id.empty()) {
    ...
    if (!config->async) {
      ...
      bssl::unique_ptr<EVP_PKEY> pkey = LoadPrivateKey(config->send_channel_id);
  ...
  bssl::unique_ptr<BIO> bio(BIO_new_socket(sock, BIO_NOCLOSE));
  if (!bio) {
    return false;
  }
  if (config->is_dtls) {
    bssl::unique_ptr<BIO> packeted = PacketedBioCreate(!config->async);
    if (!packeted) {
      return false;
    }
  ...
  if (config->async) {
    bssl::unique_ptr<BIO> async_scoped =
...
static int Main(int argc, char **argv) {
  ...
  bssl::unique_ptr<SSL_CTX> ssl_ctx = SetupCtx(&config);
  if (!ssl_ctx) {
  ...
  for (int i = 0; i < config.resume_count + 1; i++) {
    ...
    bssl::unique_ptr<SSL_SESSION> offer_session = std::move(session);
