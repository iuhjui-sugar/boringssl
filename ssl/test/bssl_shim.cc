...
static bssl::UniquePtr<EVP_PKEY> LoadPrivateKey(const std::string &file) {
  ...
  return bssl::UniquePtr<EVP_PKEY>(PEM_read_bio_PrivateKey(bio.get(), NULL, NULL, NULL));
}
...
static bool RetryAsync(SSL *ssl, int ret) {
  ...
  switch (SSL_get_error(ssl, ret)) {
    ...
    case SSL_ERROR_WANT_CHANNEL_ID_LOOKUP: {
      bssl::UniquePtr<EVP_PKEY> pkey = LoadPrivateKey(GetTestConfig(ssl)->send_channel_id);
      if (!pkey) {
        return false;
      }
...
// previous exchange.
static bool DoExchange(bssl::UniquePtr<SSL_SESSION> *out_session, SSL_CTX *ssl_ctx,
