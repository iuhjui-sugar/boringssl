...
static void ssl_ctx_add_session(SSL_SESSION *session, void *void_param) {
  SSL_CTX_add_session((SSL_CTX *)void_param, session);
}

static bssl::UniquePtr<SSL_CTX> SetupCtx(bssl::UniquePtr<SSL_CTX> old_ctx,
  ...
  if (old_ctx) {
    uint8_t keys[48];
    size_t keys_len = 48;
    if (!SSL_CTX_get_tlsext_ticket_keys(old_ctx.get(), &keys, keys_len) ||
        !SSL_CTX_set_tlsext_ticket_keys(ssl_ctx.get(), (const void *)keys,
                                        keys_len)) {
      return nullptr;
    }
    lh_SSL_SESSION_doall_arg(old_ctx->sessions, ssl_ctx_add_session, ssl_ctx.get());
    old_ctx->sessions = NULL;
  }
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < initial_config.resume_count + 1; i++) {
    ...
    ssl_ctx = SetupCtx(std::move(ssl_ctx), config);
    if (!ssl_ctx) {
