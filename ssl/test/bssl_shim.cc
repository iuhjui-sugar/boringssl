...
static int do_exchange(SSL_SESSION **out_session,
                       ...
                       int fd,
                       SSL_SESSION *session) {
  int async = 1 /* TODO(davidben): Drive with runner.go */;
  ...
  } while (ret < 0 && async && retry_async(ssl, ret, bio));
  if (ret != 1) {
    SSL_free(ssl);
    BIO_print_errors_fp(stdout);
    return 2;
  ...
  for (;;) {
    ...
    } while (n < 0 && async && retry_async(ssl, n, bio));
    if (n < 0) {
      SSL_free(ssl);
      BIO_print_errors_fp(stdout);
      return 3;
      ...
      } while (w < 0 && async && retry_async(ssl, w, bio));
      if (w != n) {
        SSL_free(ssl);
        BIO_print_errors_fp(stdout);
        return 4;
