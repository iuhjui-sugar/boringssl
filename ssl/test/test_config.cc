...
static int SetQuicEncryptionSecrets(SSL *ssl, enum ssl_encryption_level_t level,
                                    ...
                                    size_t secret_len) {
  return GetTestState(ssl)->quic_transport->SetSecrets(level, read_secret,
...
static int AddQuicHandshakeData(SSL *ssl, enum ssl_encryption_level_t level,
  ...
  return GetTestState(ssl)->quic_transport->WriteHandshakeData(level, data, len)
             ? 1
             : 0;
}

static int FlushQuicFlight(SSL *ssl) {
  return GetTestState(ssl)->quic_transport->Flush() ? 1 : 0;
}
