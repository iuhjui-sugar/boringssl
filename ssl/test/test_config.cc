...
bssl::UniquePtr<SSL> TestConfig::NewSSL(
  ...
  if (!server_esni_keypairs.empty()) {
    ...
    while (CBS_len(&cbs_priv_keys) >= k_priv_key_size) {
      CBS priv_key;
      if (!CBS_get_u16_length_prefixed(&cbs_priv_keys, &priv_key)) {
        return nullptr;
      }
      priv_key_ptrs.push_back(CBS_data(&priv_key));
      priv_key_sizes.push_back(CBS_len(&priv_key));
    }
  ...
  if (!client_esnikeys.empty()) {
    ...
    // Reconstitute the list of ESNIKeys in a single U16 length-prefixed stream.
    bssl::ScopedCBB esni_keys;
    CBB_init(esni_keys.get(), 0);
    for (const CBS &esni_keys_cbs : esni_keys_vec) {
      CBB_add_bytes(esni_keys.get(), CBS_data(&esni_keys_cbs),
                    CBS_len(&esni_keys_cbs));
    }

    if (!SSL_set_esni_keys(ssl.get(), CBB_data(esni_keys.get()),
  ...
  if (esni_force_enable) {
    SSL_set_enable_esni_grease(ssl.get(), true);
  }
