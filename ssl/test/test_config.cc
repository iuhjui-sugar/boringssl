...
bssl::UniquePtr<SSL> TestConfig::NewSSL(
  ...
  if (!ech_server_configs.empty()) {
    ...
    for (const auto &pair : ech_server_configs) {
      const std::string &ech_config = pair.first;
      const std::string &ech_private_key = pair.second;

      if (!SSL_ECH_SERVER_CONFIGS_add(
              configs.get(),
              /*is_retry_config=*/1,
    ...
    SSL_CTX_set1_ech_server_configs(ssl_ctx, configs.get());
  }
