...
static bool InstallCertificate(SSL *ssl) {
  ...
  if (config->use_raw_public_key_certificate) {
    ...
    SSL_set_server_raw_public_key_enabled(ssl);
    SSL_use_raw_public_key_certificate(ssl, spki, spki_len);
    return true;
  }
...
bssl::UniquePtr<SSL> TestConfig::NewSSL(
  ...
  if (!is_server && use_raw_public_key_certificate) {
    bssl::UniquePtr<EVP_PKEY> pkey;
    ...
    SSL_set_server_raw_public_key_enabled(ssl.get());

    SSL_set_custom_verify(ssl.get(),
