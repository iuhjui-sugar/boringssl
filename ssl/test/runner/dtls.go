...
func (c *Conn) dtlsDoReadRecord(want recordType) (recordType, *block, error) {
	...
	if !bytes.Equal(seq, c.in.seq[:]) {
		...
		return 0, nil, c.in.setErrorLocked(fmt.Errorf("dtls: oversized bad sequence number"))
	}
...
func (c *Conn) dtlsWriteRecord(typ recordType, data []byte) (n int, err error) {
	recordHeaderLen := dtlsRecordHeaderLen
	maxLen := c.config.MaxHandshakeFragmentSize
	if maxLen <= 0 {
		maxLen = 1024
	}
	...
	for firstRun || len(data) > 0 {
		...
		if typ == recordTypeHandshake {
			...
			// message_seq
			fragment = append(fragment, byte(c.sendHandshakeSeq>>8))
			fragment = append(fragment, byte(c.sendHandshakeSeq))
			// fragment_offset
			fragment = append(fragment, byte(n>>16))
			fragment = append(fragment, byte(n>>8))
			fragment = append(fragment, byte(n))
			// fragment_length
			fragment = append(fragment, byte(m>>16))
			fragment = append(fragment, byte(m>>8))
			fragment = append(fragment, byte(m))
			fragment = append(fragment, data[0:m]...)
		} else {
			fragment = data[0:m]
		}
		...
		if _, ok := c.out.cipher.(cipher.AEAD); ok {
			panic("no AEAD ciphers for DTLS until DTLS 1.2")
		}
...
func (c *Conn) dtlsDoReadHandshake() ([]byte, error) {
	...
	// behavior.
	for c.handMsg == nil || len(c.handMsg) < 4+c.handMsgLen {
