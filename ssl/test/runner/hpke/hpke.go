...
func keySchedule(mode uint8, kemID, kdfID, aeadID uint16, sharedSecret, info, psk, pskID []byte) (*Context, error) {
	...
	base_nonce := labeledExpand(kdfHash, secret, suiteID, []byte("base_nonce"), keyScheduleContext, aead.NonceSize())
...
func (c Context) computeNonce() []byte {
	nonce := make([]byte, len(c.base_nonce))
	// Write the big-endian |c.seq| value at the *end* of |base_nonce|.
	binary.BigEndian.PutUint64(nonce[len(nonce)-8:], c.seq)
	// XOR the big-endian |seq| with |c.base_nonce|.
	for i, b := range c.base_nonce {
		nonce[i] ^= b
	}
	return nonce
}
