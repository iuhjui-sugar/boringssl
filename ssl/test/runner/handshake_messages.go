...
func (m *helloRetryRequestMsg) unmarshal(data []byte) bool {
	...
	extLen := int(data[8])<<8 | int(data[9])
	data = data[10:]
	...
	if m.hasRequestContext {
		...
		for len(extensions) != 0 {
			...
			case extensionSignatureAlgorithms:
				...
				sigAlgsLen := int(contents[0])<<8 | int(contents[1])
				contents = contents[2:]
				if sigAlgsLen&1 != 0 {
					return false
				}
				if len(contents) < int(sigAlgsLen) {
					return false
				}
				numSigAlgs := sigAlgsLen / 2
				m.signatureAlgorithms = make([]signatureAlgorithm, numSigAlgs)
				for i := range m.signatureAlgorithms {
					m.signatureAlgorithms[i] = signatureAlgorithm(contents[0])<<8 | signatureAlgorithm(contents[1])
					contents = contents[2:]
				}
			case extensionCertificateAuthorities:
				...
				casLength := uint16(contents[0])<<8 | uint16(contents[1])
				contents = contents[2:]
				if len(contents) < int(casLength) {
					return false
				}
				cas := make([]byte, casLength)
				copy(cas, contents)
				contents = contents[casLength:]
