...
func (m *serverHelloMsg) marshal() []byte {
	...
	} else if m.vers == tls13ExperimentVersion || m.vers == tls13Experiment2Version {
	...
	hello.addBytes(m.random)
	if vers < VersionTLS13 || m.vers == tls13ExperimentVersion || m.vers == tls13Experiment2Version {
		sessionId := hello.addU8LengthPrefixed()
		sessionId.addBytes(m.sessionId)
	}
	...
	if vers >= VersionTLS13 {
		...
		if m.vers == tls13ExperimentVersion || m.vers == tls13Experiment2Version || m.supportedVersOverride != 0 {
			extensions.addU16(extensionSupportedVersions)
			extensions.addU16(2) // Length
...
func (m *serverHelloMsg) unmarshal(data []byte) bool {
	...
	if vers >= VersionTLS13 {
		for len(data) != 0 {
			...
			case extensionSupportedVersions:
				if m.vers != tls13ExperimentVersion && m.vers != tls13Experiment2Version {
					return false
				}
			default:
