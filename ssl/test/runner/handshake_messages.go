...
func parseSignatureAlgorithms(data []byte) ([]signatureAlgorithm, []byte) {
	if len(data) < 2 {
		return nil, nil
	}
	...
	return signatureAlgorithms, data
}

func parseCAs(data []byte) ([][]byte, []byte) {
	if len(data) < 2 {
		return nil, nil
	}
	...
	return certificateAuthorities, data
}
	...
	if m.hasRequestContext {
		...
		if isDraft21(m.vers) {
			...
			for len(extensions) != 0 {
				...
				switch extension {
				case extensionSignatureAlgorithms:
					sigAlgs, rest := parseSignatureAlgorithms(contents)
					if rest == nil || len(rest) != 0 {
						return false
					}
					m.signatureAlgorithms = sigAlgs
				case extensionCertificateAuthorities:
					cas, rest := parseCAs(contents)
					if rest == nil || len(rest) != 0 {
						return false
					}
			...
			if m.hasSignatureAlgorithm {
				sigAlgs, rest := parseSignatureAlgorithms(data)
				if rest == nil {
					return false
				}
				m.signatureAlgorithms = sigAlgs
