...
func (m *clientHelloMsg) unmarshal(data []byte) bool {
	...
	for len(extensions) > 0 {
		...
		case extensionTokenBinding:
			if !body.readU16(&m.tokenBindingVersion) || !body.readU8LengthPrefixedBytes(&m.tokenBindingParams) {
				return false
			}
...
func (m *serverExtensions) unmarshal(data byteReader, version uint16) bool {
	...
	*m = serverExtensions{}
	...
	for len(data) > 0 {
		...
		case extensionTokenBinding:
			if !body.readU16(&m.tokenBindingVersion) || !body.readU8LengthPrefixedBytes(&m.tokenBindingParams) || len(m.tokenBindingParams) != 1 {
				return false
			}
		case extensionExtendedMasterSecret:
			if len(body) != 0 {
