...
func (m *clientHelloMsg) marshal() []byte {
	...
	// Compute the order in which we will attempt to marshal extensions by
	// pulling all extensions in |m.prefixExtensions| to the front.
	var extensionOrder []uint16
	extensionOrder = append(extensionOrder, m.prefixExtensions...)
	specialExts := make(map[uint16]bool)
	for _, e := range m.prefixExtensions {
		specialExts[e] = true
	}
