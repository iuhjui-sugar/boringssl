...
func (m *serverHelloMsg) marshal() []byte {
	...
	if vers >= VersionTLS13 {
		...
		if m.omitExtensions || m.emptyExtensions {
			// Silently erasing server extensions will cause the client and server to
			// disagree on the fields. Instead, the test must explicit disable all
			// features which would write an extension. This panic ensures this happened
			// correctly.
			if extensions.len() != 0 {
