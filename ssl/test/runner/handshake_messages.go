...
func (p *paddedServerNameList) marshal(bb *byteBuilder, esniKeysPaddedLen uint16) {
	...
	// It seems that the length of the dns name should also
	// include the length prefix.
	var lenDnsName uint16 = uint16(len(p.dnsName) + 2)
	paddingSize := esniKeysPaddedLen - lenDnsName
...
func (c *clientEncryptedSNI) marshal(body *byteBuilder) {
	...
	//fmt.Printf("clientEncryptedSNI marshal -> %#X\n", body.data())
}
...
func (s *serverEncryptedSNI) unmarshal(br *byteReader) bool {
	...
	case serverESNIResponseRetryRequest:
		...
		for len(esniKeysListReader) > 0 {
			var esniKeysReader byteReader // for just one esniKeys
			if !esniKeysListReader.readU16LengthPrefixed(&esniKeysReader) {
				return false
			}
			var esniKeys EsniKeys
			if !esniKeys.unmarshal(&esniKeysReader) {
				return false
			}
			fmt.Printf("^^^ Read esniKeys: %#v\n", esniKeys)
