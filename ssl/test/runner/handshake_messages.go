...
func (bb *byteBuilder) createChild(size int) *byteBuilder {
	if bb.child != nil {
		bb.flush()
	}
	...
	*bb.buf = append(*bb.buf, make([]byte, size)...)
	return bb.child
}
...
func (m *clientHelloMsg) unmarshal(data []byte) bool {
	...
	for len(data) != 0 {
		...
		case extensionEarlyData:
			...
			m.hasEarlyData = true
			m.earlyDataContext = make([]uint8, l)
			copy(m.earlyDataContext, data[1:length])
...
func (m *serverHelloMsg) marshal() []byte {
	...
	if m.vers >= VersionTLS13 && enableTLS13Handshake {
		...
		m.extensions.marshal(extensions, false /* TLS 1.2 */)
		if extensions.len() == 0 {
			hello.discardChild()
		}
	}
...
func (m *serverHelloMsg) unmarshal(data []byte) bool {
	...
	} else if !m.extensions.unmarshal(data, false /* TLS 1.3 */) {
		return false
	}
...
func (m *encryptedExtensionsMsg) marshal() []byte {
	...
	m.extensions.marshal(extensions, true /* TLS 1.3 */)
...
func (m *encryptedExtensionsMsg) unmarshal(data []byte) bool {
	...
	return m.extensions.unmarshal(data, true /* TLS 1.3 */)
}
...
func (m *serverExtensions) marshal(extensions *byteBuilder, isTLS13 bool) {
	if m.duplicateExtension {
		// Add a duplicate bogus extension at the beginning and end.
		extensions.addU16(0xffff)
...
func (m *serverExtensions) unmarshal(data []byte, isTLS13 bool) bool {
	// Reset all fields.
	*m = serverExtensions{}
...
type encryptedExtensionsOldMsg struct {
	raw       []byte
	channelID []byte
}

func (m *encryptedExtensionsOldMsg) marshal() []byte {
	if m.raw != nil {
		return m.raw
	}
	...
	x := make([]byte, 4+length)
	x[0] = typeEncryptedExtensionsOld
	x[1] = uint8(length >> 16)
	x[2] = uint8(length >> 8)
	x[3] = uint8(length)
...
func (m *encryptedExtensionsOldMsg) unmarshal(data []byte) bool {
