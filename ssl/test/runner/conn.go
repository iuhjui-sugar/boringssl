...
func (hc *halfConn) decrypt(b *block) (ok bool, prefixLen int, contentType recordType, alertValue alert) {
	...
	if hc.cipher != nil {
		...
		case *tlsAead:
			...
			var err error
			payload, err = c.Open(payload[:0], nonce, payload, ad)
			if err != nil {
				return false, 0, 0, alertBadRecordMAC
			}
			if hc.version >= VersionTLS13 {
...
func (hc *halfConn) encrypt(b *block, explicitIVLen int, typ recordType) (bool, alert) {
	...
	if hc.cipher != nil {
		...
		case *tlsAead:
			...
			c.Seal(payload[:0], nonce, payload, ad)
		case cbcMode:
			blockSize := c.BlockSize()
			if explicitIVLen > 0 {
				c.SetIV(payload[:explicitIVLen])
...
func (c *Conn) doReadRecord(want recordType) (recordType, *block, error) {
	...
	if c.vers >= VersionTLS13 && encTyp != 0 {
		typ = encTyp
	}
	if !ok {
...
func (c *Conn) writeRecord(typ recordType, data []byte) (n int, err error) {
	...
	for len(data) > 0 || first {
		...
		if explicitIVLen == 0 {
			if aead, ok := c.out.cipher.(*tlsAead); ok && aead.explicitNonce && c.vers < VersionTLS13 {
				explicitIVLen = 8
