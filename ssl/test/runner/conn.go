...
func (hc *halfConn) writeRecordHeaderLen() int {
	if hc.isDTLS {
		if hc.version >= VersionTLS13 && hc.cipher != nil && !hc.config.Bugs.DTLSUsePlaintextRecordHeader {
...
func (hc *halfConn) encrypt(b *block, explicitIVLen int, typ recordType) (bool, alert) {
	...
	if hc.cipher != nil {
		...
		case *tlsAead:
			...
			if hc.isDTLS && hc.version >= VersionTLS13 && !hc.config.Bugs.DTLSUsePlaintextRecordHeader {
				nonce = make([]byte, 8)
