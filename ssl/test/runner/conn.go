...
func (c *Conn) SendNewSessionTicket() error {
	...
	if len(c.config.Bugs.SessionTicketKEMode) != 0 {
		m.keModes = c.config.Bugs.SessionTicketKEMode
	}
	if len(c.config.Bugs.SessionTicketAuthMode) != 0 {
		m.authModes = c.config.Bugs.SessionTicketAuthMode
	}
