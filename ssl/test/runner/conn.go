...
func (hc *halfConn) decrypt(b *block) (ok bool, prefixLen int, alertValue alert) {
	...
	if hc.mac != nil {
		...
		remoteMAC := payload[n:]
		var header []byte
		header = append(header, b.data[:3]...)
		header = append(header, b.data[recordHeaderLen-2:recordHeaderLen]...)
		localMAC := hc.mac.MAC(hc.inDigestBuf, seq, header, payload[:n])
...
func (hc *halfConn) encrypt(b *block, explicitIVLen int) (bool, alert) {
	...
	if hc.mac != nil {
		...
		header = append(header, b.data[:3]...)
		header = append(header, b.data[recordHeaderLen-2:recordHeaderLen]...)
		mac := hc.mac.MAC(hc.outDigestBuf, hc.seq[0:], header, b.data[recordHeaderLen+explicitIVLen:])
