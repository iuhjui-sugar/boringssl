...
func (c *Conn) esniClientSelectParams() (*ClientEsniSelection, error) {
	if len(c.config.EsniKeypairs) == 0 {
		return nil, nil
	}

	for _, serverEsniKeypair := range c.config.EsniKeypairs {
...
func (c *Conn) esniClientGrease() (*clientEncryptedSNI, *ClientEsniSelection, error) {
	greaseEsniKeypair := GetTestEsniKeypairs([]string{"foobarbaz"})
	greaseEsniKeys := greaseEsniKeypair[0].EsniKeys
	greaseKeyShareEntry := greaseEsniKeys.Keys[0]
	...
	greaseEncryptedSNI := randByteBuf(16 + int(greaseEsniKeys.PaddedLength) + greaseSuite.keyLen) // is tag_length = keyLen?
...
func (hs *clientHandshakeState) verifyCertificates(certMsg *certificateMsg) error {
	...
	if hs.esniRetry.enabled {
		...
		if retryEsniKeys == nil {
			hs.esniRetry.enabled = false
		} else {
			// If this config is reused, the retry keys will be
			// used to encrypt SNI.

			c.config.EsniKeypairs = []EsniKeypair{}
			if retryEsniKeys != nil {
				c.config.EsniKeypairs = append(c.config.EsniKeypairs, EsniKeypair{EsniKeys: *retryEsniKeys, PrivateKeys: nil})
			}
		}

		c.sendAlert(alertESNIRequired)
		return fmt.Errorf("ESNI required")
	}
