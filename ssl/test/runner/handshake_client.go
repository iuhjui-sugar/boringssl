...
NextCipherSuite:
	...
	if len(c.config.ClientECHConfigs) > 0 {
		innerHello, echHPKEContext, err = c.insertECH(hello, nil, c.config.Bugs.TruncateClientECHEnc)
		if err != nil {
			return err
		}
	...
	if haveHelloRetryRequest {
		...
		if innerHello != nil {
			secondInnerHello, _, err = c.insertECH(hello, echHPKEContext, c.config.Bugs.TruncateClientECHEnc)
			if err != nil {
				return err
			}
		}
...
func (c *Conn) insertECH(hello *clientHelloMsg, ctx *hpke.Context, truncateEnc bool) (innerHelloPtr *clientHelloMsg, ctxOut *hpke.Context, err error) {
...
func (hs *clientHandshakeState) doTLS13Handshake(
	...
	if hs.serverHello.hasPSKIdentity {
		...
		echFinishedHash.addEntropy(hs.session.masterSecret)
		c.didResume = true
	} else {
