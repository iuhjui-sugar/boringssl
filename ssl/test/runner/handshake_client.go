...
NextCipherSuite:
	...
	var ok bool
	c.vers, ok = c.config.mutualVersion(wireVersion, c.isDTLS)
	if !ok {
	...
	if haveHelloRetryRequest && (helloRetryRequest.vers != c.vers || helloRetryRequest.cipherSuite != serverHello.cipherSuite || helloRetryRequest.selectedGroup != serverHello.keyShare.group) {
		c.sendAlert(alertHandshakeFailure)
