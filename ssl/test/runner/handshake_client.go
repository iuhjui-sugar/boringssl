...
NextCipherSuite:
	...
	if ((len(hello.pskIdentities) > 0 && session.maxEarlyDataSize > 0 && c.config.Bugs.SendEarlyData != nil) || c.config.Bugs.SendFakeEarlyDataLength > 0) && !c.config.Bugs.OmitEarlyDataExtension {
