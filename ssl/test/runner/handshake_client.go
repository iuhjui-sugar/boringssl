...
func (c *Conn) esniClientSelectParams() (*ClientEsniSelection, error) {
	...
	for _, serverEsniKeys := range c.config.EsniKeys {
		...
		if !foundCurve {
			return nil, errors.New("tls: could not find supported curve for ESNI")
		}
...
func (c *Conn) esniClientBuildEncryptedSNI() (*clientEncryptedSNI, error) {
	...
	// Construct ClientESNIInner
	var clientEsniInner []byte

	// The value of nonce that is to be echoed by the server
	nonce := make([]byte, 16)
	rand.Read(nonce)
	clientEsniInner = append(clientEsniInner, nonce...)
	clientEsniInner = append(clientEsniInner, paddedPlaintextSni...)

	// TODO what value should be used for nonce?
	cipherNonce := make([]byte, cipher.NonceSize())
	encryptedSNI := cipher.Seal([]byte{}, cipherNonce, clientEsniInner, []byte{})
