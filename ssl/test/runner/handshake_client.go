...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if ok {
		...
		if ka, ok := keyAgreement.(*ecdheKeyAgreement); ok {
			if auth, ok := ka.auth.(*signedKeyAgreement); ok {
				c.peerSignatureAlgorithm = auth.sigAlg
			}
		}
	...
	if chainToSend != nil {
		...
		if certVerify.hasSignatureAlgorithm {
			certVerify.signatureAlgorithm, err = selectSignatureAlgorithm(privKey, certReq.signatureAlgorithms, c.config.signatureAlgorithmsForClient())
			if err != nil {
				c.sendAlert(alertInternalError)
				return err
			}
