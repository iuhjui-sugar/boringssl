...
func (hs *clientHandshakeState) doTLS13Handshake() error {
	...
	if encryptedExtensions.extensions.channelIDRequested {
		channelIDMsgBytes, err := hs.writeChannelIDMessage(false)
		if err != nil {
			return err
		}
...
func (hs *clientHandshakeState) sendFinished(out []byte, isResume bool) error {
	...
	if hs.serverHello.extensions.channelIDRequested {
		channelIDMsgBytes, err := hs.writeChannelIDMessage(isResume)
		if err != nil {
			return err
		}
...
func (hs *clientHandshakeState) writeChannelIDMessage(isResume bool) ([]byte, error) {
	c := hs.c
