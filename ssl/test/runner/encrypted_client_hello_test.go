...
func TestECHOuterExtensions(t *testing.T) {
	...
	innerCH := clientHelloMsg{
		...
		outerExtensions: &echOuterExtensions{
			...
			hash:            []byte("hash goes here"),
		},
	}
	fullInnerCHBytes, err := echIncorporateOuterExtensions(innerCH.marshal(), outerCH.marshal())
	if err != nil {
		t.Error(err)
	}
	var fullInnerCH clientHelloMsg
	if !fullInnerCH.unmarshal(fullInnerCHBytes) {
		t.Error("failed to unmarshal CH produced by echIncorporateOuterExtensions")
	}
