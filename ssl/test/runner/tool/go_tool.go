...
func EsniTester() {
	...
	flag.StringVar(&args.esniKeysOutFile, "esniKeysOutFile", "", "When non-empty, tool will write serialized ESNIKeys to this file")
	flag.StringVar(&args.publicName, "publicName", "", "The public name for generated ESNIKeys")
	flag.Parse()

	fmt.Printf("Invoked with args: %#v\n", args)

	esniKeypairs := runner.GetTestEsniKeypairs([]string{args.publicName})

	if args.generateEsniKeys {
		for _, esniKeypair := range esniKeypairs {
			var esniKeys runner.EsniKeys = esniKeypair.EsniKeys
			var privKeys []runner.EsniPrivKey = esniKeypair.PrivateKeys
			esniKeysBytes := esniKeys.Serialize()

			// TODO(dmcardle) write esniKeys and privKey to files on disk
			_ = privKeys // satisfy unused checker
	...
	if args.serverMode {
		// Assuming exactly one entry in esniKeypairs
		esniKeys := esniKeypairs[0].EsniKeys

		if len(args.esniKeysOutFile) > 0 {
			var err error
			err = esniKeys.WriteToFile(args.esniKeysOutFile, false)
			if err != nil {
				panic(err)
			}
			err = esniKeys.WriteToFile(args.esniKeysOutFile+".b64", true)
			if err != nil {
				panic(err)
			}
		}
		...
		wg.Add(1)
		go Server(&wg, msg_chan, esniKeypairs, args)
	}

	// TODO(dmcardle): Maybe read ESNIKeys from disk
	if args.clientMode {
		wg.Add(1)
		go Client(&wg, msg_chan, esniKeypairs, args)
	}
...
func Server(wg *sync.WaitGroup, msg_chan chan testerMsg, esniKeypairs []runner.EsniKeypair, args ToolArgs) {
	defer wg.Done()
...
func Client(wg *sync.WaitGroup, msg_chan chan testerMsg, esniKeypairs []runner.EsniKeypair, args ToolArgs) {
	defer wg.Done()
	...
	if args.useOldKeys {
		esniKeypairs = runner.GetTestEsniKeypairs([]string{"foo", "bar"})
	}
