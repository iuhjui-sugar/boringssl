...
func (e *x25519EcdhCurve) generateKeypair(rand io.Reader) (publicKey []byte, err error) {
	...
	var out [32]byte
	curve25519ScalarBaseMult(&out, &e.privateKey)
	return out[:], nil
}
...
func (e *x25519EcdhCurve) computeSecret(peerKey []byte) (preMasterSecret []byte, err error) {
	...
	// draft-ietf-tls-curve25519-01 section 2.3 recommends rejecting public
	// keys when the high order bit of last byte is set.
	if len(peerKey) != 32 || peerKey[31]&0x80 != 0 {
		return nil, errors.New("tls: invalid peer key")
	}
	var out, peerKeyCopy [32]byte
	copy(peerKeyCopy[:], peerKey)
	curve25519ScalarMult(&out, &e.privateKey, &peerKeyCopy)
