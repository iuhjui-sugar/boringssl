...
var cipherSuites = []*cipherSuite{
	...
	{TLS_CECPQ1_RSA_WITH_AES_256_GCM_SHA384, 32, 0, ivLenChaCha20Poly1305, cecpq1RSAKA, suiteCECPQ1 | suiteTLS12 | suiteSHA384, nil, nil, aeadAESGCM},
	{TLS_CECPQ1_ECDSA_WITH_AES_256_GCM_SHA384, 32, 0, ivLenChaCha20Poly1305, cecpq1ECDSAKA, suiteCECPQ1 | suiteECDSA | suiteTLS12 | suiteSHA384, nil, nil, aeadAESGCM},
...
func aeadAESGCM(version uint16, key, fixedNonce []byte) *tlsAead {
	if version >= VersionTLS13 {
		return xorNonceAeadAESGCM(key, fixedNonce)
	}
	return fixedNonceAeadAESGCM(key, fixedNonce)
}

func xorNonceAeadAESGCM(key, fixedNonce []byte) *tlsAead {
...
func fixedNonceAeadAESGCM(key, fixedNonce []byte) *tlsAead {
	aes, err := aes.NewCipher(key)
	if err != nil {
		panic(err)
	}
