...
ResendHelloRetryRequest:
	...
	// TODO(nharper): This does not check that SNI matches.
	if !sendHelloRetryRequest {
		if hs.clientHello.hasEarlyData {
			canEarlyData := hs.sessionState != nil
			if config.Bugs.AlwaysRejectEarlyData ||
				(c.usedALPN && c.clientProtocol != hs.sessionState.earlyALPN) ||
				(!c.usedALPN && hs.sessionState.earlyALPN != "") {
				canEarlyData = false
			}
