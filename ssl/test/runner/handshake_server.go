...
Curves:
	...
	if hs.suite.flags&suitePSK == 0 {
		...
		var err error
		certVerify.signatureAlgorithm, err = selectSignatureAlgorithm(c.vers, privKey, hs.clientHello.signatureAlgorithms, c.config.signatureAlgorithmsForServer())
		if err != nil {
			c.sendAlert(alertInternalError)
			return err
		}
	...
	if config.ClientAuth >= RequestClientCert {
		...
		if len(c.peerCertificates) > 0 {
			...
			if err := verifyMessage(c.vers, pub, certVerify.signatureAlgorithm, input, certVerify.signature); err != nil {
				c.sendAlert(alertBadCertificate)
				return err
			}
