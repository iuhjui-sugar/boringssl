...
func (hs *serverHandshakeState) decryptClientHello(helloOuter *clientHelloMsg) (helloInner *clientHelloMsg, err error) {
	...
	// encrypted_client_hello extension. Constructed this by piecing together
	...
	extensionsLength := len(helloOuter.raw) - helloOuter.extensionStart - 2
	extensionsLength -= helloOuter.echExtensionEnd - helloOuter.echExtensionStart
	outerHello.addU16(uint16(extensionsLength))
	outerHello.addBytes(helloOuter.raw[helloOuter.extensionStart+2 : helloOuter.echExtensionStart])
	outerHello.addBytes(helloOuter.raw[helloOuter.echExtensionEnd:])
