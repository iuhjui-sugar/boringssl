	...
	if foundKEMode {
		for i, pskIdentity := range pskIdentities {
			...
			if config.Bugs.AcceptAnySession {
				...
				realTicketAge := c.config.time().Sub(sessionState.ticketCreationTime).Seconds()
				if config.Bugs.ActualTicketAge != 0 {
					realTicketAge = config.Bugs.ActualTicketAge.Seconds()
				}
				...
				if !(realTicketAge-60 < clientTicketAge && clientTicketAge < realTicketAge+60) {
					continue
				}
