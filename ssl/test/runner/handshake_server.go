...
func (hs *serverHandshakeState) readClientHello() error {
	...
	if config.Bugs.ExpectNoTLS12Session {
		if len(hs.clientHello.sessionId) > 0 && c.wireVersion != tls13VariantDraftVersion {
...
ResendHelloRetryRequest:
	...
	if c.wireVersion == tls13VariantDraftVersion {
	...
	if c.wireVersion == tls13VariantDraftVersion && !c.skipEarlyData {
