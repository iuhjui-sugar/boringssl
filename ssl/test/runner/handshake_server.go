...
ResendHelloRetryRequest:
	...
	if sendHelloRetryRequest {
		...
		// The second ClientHello may drop PSK identities that are no longer
		// consistent with the cipher suite.
		if len(hs.clientHello.pskIdentities) < len(newClientHello.pskIdentities) {
			return errors.New("tls: second ClientHello has too many PSK identities")
		}
...
func checkClientHellosEqual(a, b []byte, isDTLS bool, ignoreExtensions []uint16) error {
	...
	if aVers != bVers {
		return errors.New("tls: ClientHello versions did not match")
	}
	if !bytes.Equal(aRandom, bRandom) {
		return errors.New("tls: ClientHello randoms did not match")
	}
	...
	if !bytes.Equal(aCipherSuites, bCipherSuites) {
		return errors.New("tls: cipher suites did not match")
	}
	if !bytes.Equal(aCompressionMethods, bCompressionMethods) {
		return errors.New("tls: compression methods did not match")
	}
