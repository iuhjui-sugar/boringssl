...
// TLV format. It uses a 1-byte tag to indicate the TLS record type and a
// 4-byte big-endian length indicating the length of the remaining payload.
// The first 2 bytes of the payload is the cipher suite ID of the cipher suite
// that would have been used to encrypt the record. This is followed by the key
// that would be used to encrypt the record, and finally followed by the
// plaintext of the record. Note that the 4-byte length covers the length of
// the cipher suite ID, key, and plaintext.
...
func (m *mockQUICTransport) read() (byte, []byte, error) {
	for {
		...
		if length < 2 {
			return 0, nil, fmt.Errorf("Record length %d is too short", length)
		}
		value := make([]byte, length)
		if _, err := io.ReadFull(m.Conn, value); err != nil {
			return 0, nil, fmt.Errorf("Error reading record")
		}
		cipherSuite := binary.BigEndian.Uint16(value[:2])
		value = value[2:]
...
func (m *mockQUICTransport) writeRecord(typ recordType, data []byte) (int, error) {
	...
	length := 2 + len(m.writeSecret) + len(data)
	payload := make([]byte, 1+4+length)
	payload[0] = tag
	binary.BigEndian.PutUint32(payload[1:5], uint32(length))
	binary.BigEndian.PutUint16(payload[5:7], m.writeCipherSuite)
