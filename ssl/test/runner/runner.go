...
func addESNITests() {
	...
	testEsniKeysOld, _ := GetTestEsniKeys(domainNames)
	testEsniKeys, testPrivateKeys := GetTestEsniKeys(domainNames)
	...
	esniTestCases := []testCase{
		...
		testCase{
			...
			flags: []string{
				"-host-name", "bar.foo.example",
				"-client-esnikeys",
				base64.StdEncoding.EncodeToString(shimEncodeClientEsniKeys(testEsniKeysOld)),
			},
			config: Config{
				MinVersion:      VersionTLS13,
				MaxVersion:      VersionTLS13,
		...
		// TODO(dmcardle): check that we actually authenticate
		// the public name "foo.example", not the "bar"
		// subdomain.
		testCase{
			testType: clientTest,
			name:     "Esni-Client-Grease",
			flags: []string{
				"-host-name", "bar.foo.example",
				"-esni-force-enable",
			},
