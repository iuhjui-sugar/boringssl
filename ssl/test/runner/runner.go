...
func shimEncodeServerKeyPairs(esniKeypairs []EsniKeypair) []byte {
	...
	for _, esniKeypair := range esniKeypairs {
		for i, privKey := range esniKeypair.PrivateKeys {
			bbOnePrivKey := bbPrivKeys.addU16LengthPrefixed()
			bbOnePrivKey.addU16(uint16(esniKeypair.EsniKeys.Keys[i].Group))
			bbOnePrivKey.addBytes(privKey[:])
		}
	}
