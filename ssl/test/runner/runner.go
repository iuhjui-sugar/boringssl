		...
		for _, resumeVers := range tlsVersions {
			...
			for _, protocol := range protocols {
				...
				if sessionVers.version == resumeVers.version {
					...
					// Offering a TLS 1.3 session sends an
					// empty session ID, so there is no way
					// to convince a non-lookahead client
					// the session was resumed. It will
					// appear to the client that a stray
