...
type ShimConfiguration struct {
	...
	// “:UNEXPECTED_MESSAGE:” (an OpenSSL/BoringSSL-stype error string) to
	// something like “unexpected TLS handshake message”.
	...
	for i := range testCases {
		...
		if len(*testToRun) != 0 {
			...
			for pattern := range shimConfig.DisabledTests {
				isSuppressed, err := filepath.Match(pattern, testCases[i].name)
				if err != nil {
					fmt.Fprintf(os.Stderr, "Error matching pattern from config file: %s\n", err)
					os.Exit(1)
				}
