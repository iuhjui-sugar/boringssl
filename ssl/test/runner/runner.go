...
func runTest(test *testCase, buildDir string) error {
	...
	err := doExchange(test, &config, conn, test.messageLen, false)
	conn.Close()
	if err == nil && test.resumeSession {
		...
		err = doExchange(test, &resumeConfig, connResume, test.messageLen, true)
	}
	connResume.Close()
...
func addResumptionVersionTests() {
	...
	// TODO(davidben): SSLv3 is omitted here because runner does not support
	// resumption with session IDs.
	for _, sessionVers := range tlsVersions[1:] {
		for _, resumeVers := range tlsVersions[1:] {
			suffix := "-" + sessionVers.name + "-" + resumeVers.name

			// It is legal to resume a session with a newer TLS
			// version, but not an older one.
			expectResume := sessionVers.version <= resumeVers.version
			var flags []string
			if !expectResume {
			...
			// If resumption is expected, also include tests where
			// resumption misses.
			if expectResume {
				testCases = append(testCases, testCase{
