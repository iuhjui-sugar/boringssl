...
func addResumptionVersionTests() {
			...
			for _, protocol := range protocols {
				...
				if sessionVers.version == resumeVers.version {
					...
					if resumeVers.version >= VersionTLS13 {
						// TLS 1.3 sessions are incompatible and are unresumable.
						localError = "didResume is false, but we expected the opposite"
						error = ""
					} else if sessionVers.version >= VersionTLS13 {
						// TLS 1.3 sessions are incompatible and are unresumable in lower versions.
				...
				// TLS 1.3 sessions are incompatible with mismatched resumption.
	...
	testCases = append(testCases, testCase{
		...
		shouldFail:         true,
		expectedLocalError: "no cipher suite supported by both client and server",
	})
}
