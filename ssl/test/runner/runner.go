...
func addEncryptedClientHelloTests() {
	...
	for _, protocol := range []protocol{tls, quic} {
		...
		testCases = append(testCases, testCase{
			testType: clientTest,
			protocol: protocol,
			name:     prefix + "ECH-Client-EarlyDataReject",
			config: Config{
				MinVersion:       VersionTLS13,
				MaxVersion:       VersionTLS13,
				ServerECHConfigs: []ServerECHConfig{echConfig},
		...
		// TODO(davidben): Once implemented, this handshake should complete.
		testCases = append(testCases, testCase{
			testType: clientTest,
			protocol: protocol,
		...
		testCases = append(testCases, testCase{
			...
			expectations:            connectionExpectations{echAccepted: true},
			resumeExpectations:      &connectionExpectations{echAccepted: false},
			// TODO(davidben): Once implemented, this should complete the handshake.
			shouldFail:    true,
			expectedError: ":CONNECTION_REJECTED:",
		})
		if protocol != quic {
	...
	for _, test := range testCases {
		...
		if !test.skipVersionNameCheck {
			for _, ver := range tlsVersions {
				...
				// Some tests specify a version including all the other exclude flags,
				// so disable this check if any exclude flags are found.
				if !found {
