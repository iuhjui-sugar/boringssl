...
func appendTranscript(path string, transcript []byte) error {
	...
	settings = append(settings, transcript...)
...
func doExchange(test *testCase, config *Config, conn net.Conn, isResume bool, transcripts *[][]byte) error {
...
func runTest(test *testCase, shimPath string, mallocNumToFail int64) error {
	...
	if err == nil {
		err = doExchange(test, &config, conn, false /* not a resumption */, &transcripts)
		conn.Close()
	}
	...
	for i := 0; err == nil && i < resumeCount; i++ {
		...
		if err == nil {
			err = doExchange(test, &resumeConfig, connResume, true /* resumption */, &transcripts)
			connResume.Close()
		}
	}
