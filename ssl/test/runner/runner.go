...
type testCase struct {
	...
	// expectedTokenBinding is the Token Binding parameter that should
	// have been negotiated (if expectTokenBinding is true).
	expectedTokenBinding uint8
...
func doExchange(test *testCase, config *Config, conn net.Conn, isResume bool, transcriptPrefix string, num int) error {
	...
	if test.expectTokenBinding {
		...
		if connState.TokenBindingParam != test.expectedTokenBinding {
			return fmt.Errorf("Expected param %02x, but got %02x", test.expectedTokenBinding, connState.TokenBindingParam)
		}
	...
	for _, ver := range tlsVersions {
		...
		testCases = append(testCases, testCase{
			...
			config: Config{
				...
				TokenBindingVersion: 15,
			},
			expectTokenBinding: true,
			expectedTokenBinding: 2,
			tls13Variant: ver.tls13Variant,
			flags: []string{
