	...
	for _, protocol := range []protocol{tls, quic} {
		...
		for _, hrr := range []bool{false, true} {
			...
			testCases = append(testCases, testCase{
				...
				flags: []string{
					...
					"-ech-server-config", fmt.Sprintf("%s,%s,true",
						base64.StdEncoding.EncodeToString(MarshalECHConfig(publicECHConfig2)),
						base64.StdEncoding.EncodeToString(secretKey2)),
					"-ech-server-config", fmt.Sprintf("%s,%s,true",
						base64.StdEncoding.EncodeToString(MarshalECHConfig(publicECHConfig3)),
						base64.StdEncoding.EncodeToString(secretKey3)),
		...
		for i, cipher := range echCiphers {
			...
			testCases = append(testCases, testCase{
				...
				flags: []string{
					"-ech-server-config", fmt.Sprintf("%s,%s,true",
						base64.StdEncoding.EncodeToString(MarshalECHConfig(config)),
		...
		// Test that after sending HRR, the ECH server can accept the second
		// ClientHelloInner.
		testCases = append(testCases, testCase{
			testType: serverTest,
			protocol: protocol,
			name:     prefix + "ECH-Server-HelloRetryRequest-OuterExtensions",
			config: Config{
				ServerName: "secret.example",
				// Require a HelloRetryRequest for every curve.
				DefaultCurves:   []CurveID{},
...
||||||| parent of 031b01d1d (Add ECH server (draft-ietf-tls-esni-09).)
		},
		resumeSession: true,
	})
=======
...
>>>>>>> 031b01d1d (Add ECH server (draft-ietf-tls-esni-09).)
