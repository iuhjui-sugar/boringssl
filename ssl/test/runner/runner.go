...
func addExtensionTests() {
	...
	for _, protocol := range []protocol{tls, dtls, quic} {
		for _, ver := range allVersions(protocol) {
			...
			if protocol == quic {
				// Client sends params
				for clientConfig := QuicUseCodepointStandard; clientConfig <= QuicUseCodepointLegacy; clientConfig++ {
					for serverSends := QuicUseCodepointStandard; serverSends <= QuicUseCodepointNeither; serverSends++ {
						name := "QUICTransportParams-Client-"
						serverTransportParams := []byte{3, 4}
						useCodepointFlag := "0"
						...
						if serverSends == QuicUseCodepointLegacy {
							name += "ServerLegacy-"
						}
						if serverSends == QuicUseCodepointBoth {
							name += "ServerBoth-"
						}
						if serverSends == QuicUseCodepointNeither {
							name += "ServerNeither-"
							serverTransportParams = nil
						}
						if serverSends != clientConfig {
							...
							flags = append(flags,
								"-expect-quic-transport-params",
								base64.StdEncoding.EncodeToString(serverTransportParams))
						}
						testCases = append(testCases, testCase{
							testType: clientTest,
							protocol: protocol,
							...
							config: Config{
								MinVersion:                            ver.version,
								MaxVersion:                            ver.version,
								QUICTransportParams:                   serverTransportParams,
				...
				// Server sends params
				for clientSends := QuicUseCodepointStandard; clientSends <= QuicUseCodepointNeither; clientSends++ {
					for serverConfig := QuicUseCodepointStandard; serverConfig <= QuicUseCodepointLegacy; serverConfig++ {
						name := "QUICTransportParams-Server-"
						clientTransportParams := []byte{1, 2}
						expectations := connectionExpectations{
						...
						if clientSends == QuicUseCodepointLegacy {
							name += "ClientLegacy-"
						}
						if clientSends == QuicUseCodepointBoth {
							name += "ClientBoth-"
						}
						if clientSends == QuicUseCodepointNeither {
							name += "ClientNeither-"
							clientTransportParams = nil
						}
						flags := []string{
							"-quic-transport-params",
							base64.StdEncoding.EncodeToString([]byte{3, 4}),
						...
						if clientSends != QuicUseCodepointBoth && clientSends != serverConfig {
							...
							flags = append(flags,
								"-expect-quic-transport-params",
								base64.StdEncoding.EncodeToString(clientTransportParams),
							)
						}
						testCases = append(testCases, testCase{
							testType: serverTest,
							...
							config: Config{
								MinVersion:                            ver.version,
								MaxVersion:                            ver.version,
								QUICTransportParams:                   clientTransportParams,
				...
				for clientConfig := QuicUseCodepointStandard; clientConfig <= QuicUseCodepointLegacy; clientConfig++ {
					name := "QUICTransportParams-Client-NotSentInNonQUIC-"
					useCodepointFlag := "0"
					if clientConfig == QuicUseCodepointLegacy {
						name += "Legacy-"
						useCodepointFlag = "1"
