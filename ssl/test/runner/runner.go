	...
	for _, protocol := range []protocol{tls, dtls, quic} {
		for _, ver := range allVersions(protocol) {
			...
			if ver.version >= VersionTLS13 {
				...
				for _, clientConfig := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
					for _, serverSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth} {
						flags := []string{}
						expectations := connectionExpectations{
						...
						expectedError := ""
						expectedLocalError := ""
						if clientConfig == ALPSUseCodepointNew {
						...
						// If server sends both or server and client ALPS code point matches,
						// we expect it should succeed, otherwise no application setting has
						// been negotiated, but the handshake should complete as normal.
						if serverSends == ALPSUseCodepointBoth || serverSends == clientConfig {
							flags = append(flags,
						...
						testCases = append(testCases, testCase{
							protocol:           protocol,
							testType:           clientTest,
							name:               fmt.Sprintf("ALPS-Basic-Client-Client%s-Server%s-%s", clientConfig, serverSends, suffix),
							skipQUICALPNConfig: true,
							config: Config{
								MaxVersion:          ver.version,
								NextProtos:          []string{"proto"},
								ApplicationSettings: map[string][]byte{"proto": []byte("runner1")},
								ALPSUseNewCodepoint: serverSends,
							},
							resumeConfig: &Config{
								MaxVersion:          ver.version,
								NextProtos:          []string{"proto"},
				...
				for _, clientSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth} {
					for _, serverConfig := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
						flags := []string{}
						expectations := connectionExpectations{
							peerApplicationSettingsOld: []byte("shim1"),
						}
						...
						expectedError := ""
						expectedLocalError := ""
						if serverConfig == ALPSUseCodepointNew {
							flags = append(flags, "-alps-use-new-codepoint")
							expectations = connectionExpectations{
								peerApplicationSettings: []byte("shim1"),
							}
						...
						// If client sends both or server and client ALPS code point matches,
						// we expect it should succeed, otherwise no application setting has
						// been negotiated, but the handshake should complete as normal.
						if clientSends == ALPSUseCodepointBoth || clientSends == serverConfig {
							flags = append(flags,
								"-select-alpn", "proto",
								"-on-initial-application-settings", "proto,shim1",
								"-on-initial-expect-peer-application-settings", "runner1",
							...
							expectations = connectionExpectations{}
							resumeExpectations = &connectionExpectations{}
							flags = append(flags, "-select-alpn", "proto")
						}
						...
						testCases = append(testCases, testCase{
							protocol:           protocol,
							testType:           serverTest,
							name:               fmt.Sprintf("ALPS-Basic-Server-Client%s-Server%s-%s", clientSends, serverConfig, suffix),
							skipQUICALPNConfig: true,
							config: Config{
								MaxVersion:          ver.version,
								NextProtos:          []string{"proto"},
				...
				for _, alpsCodePoint := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
					...
					// Test client declines application settings from the server when the
					// ALPS codepoint mismatch.
					bugs = ProtocolBugs{
						AlwaysNegotiateApplicationSettingsOld: true,
					}
					...
					testCases = append(testCases, testCase{
						protocol:           protocol,
						testType:           clientTest,
						name:               fmt.Sprintf("ALPS-Mismatch-Client-%s-%s", alpsCodePoint, suffix),
						skipQUICALPNConfig: true,
						config: Config{
							MaxVersion:          ver.version,
							NextProtos:          []string{"proto"},
				...
				for _, clientConfig := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
					for _, serverSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth} {
						...
						testCases = append(testCases, testCase{
							protocol: protocol,
							testType: clientTest,
							name:     fmt.Sprintf("ALPS-Reject-Client-Client%s-Server%s-%s", clientConfig, serverSends, suffix),
							config: Config{
								MaxVersion:          ver.version,
								NextProtos:          []string{"foo"},
				...
				for _, clientSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth} {
					for _, serverConfig := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
						...
						testCases = append(testCases, testCase{
							protocol: protocol,
							testType: clientTest,
							name:     fmt.Sprintf("ALPS-Reject-Client-Resume-Client%s-Server%s-%s", clientSends, serverConfig, suffix),
							config: Config{
								MaxVersion: ver.version,
							},
							resumeConfig: &Config{
				...
				for _, clientSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth} {
					for _, serverConfig := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
						...
						testCases = append(testCases, testCase{
							protocol: protocol,
							testType: serverTest,
							name:     fmt.Sprintf("ALPS-Decline-Server-Client%s-Server%s-%s", clientSends, serverConfig, suffix),
							config: Config{
								MaxVersion:          ver.version,
								NextProtos:          []string{"foo"},
								ApplicationSettings: map[string][]byte{"foo": []byte("runner")},
