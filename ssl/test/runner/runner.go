...
NextTest:
	for _, test := range tests {
		if test.protocol != tls ||
			...
			test.config.MaxVersion < VersionTLS11 ||
			(test.resumeConfig != nil && (test.resumeConfig.MaxVersion < VersionTLS11 || test.resumeConfig.MaxVersion >= VersionTLS13)) ||
		...
		shTest.flags = append(shTest.flags, "-handback")
