	...
	for _, alg := range testSignatureAlgorithms {
		for _, ver := range tlsVersions {
			...
			for _, signTestType := range []testType{clientTest, serverTest} {
				...
				// The existing RSA-PKCS1 code points do not exist in TLS 1.3.
				if ver.version >= VersionTLS13 && isTLS12PKCS1 {
					shouldFail = true
				}
				// The backported RSA-PKCS1 code points only exist for TLS 1.3
				// client certificates.
				if !(ver.version >= VersionTLS13 && signTestType == clientTest) && isTLS13PKCS1 {
					shouldFail = true
				}
