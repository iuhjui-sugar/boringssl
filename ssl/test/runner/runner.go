		...
		for _, ver := range allVersions(protocol) {
			...
			if ver.version >= VersionTLS13 {
				...
				for _, clientConfig := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
					for _, serverSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth, ALPSUseCodepointNeither} {
						flags := []string{}
						expectations := connectionExpectations{
				...
				for _, clientSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth, ALPSUseCodepointNeither} {
				...
				for _, clientSends := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld, ALPSUseCodepointBoth, ALPSUseCodepointNeither} {
					for _, serverConfig := range []ALPSUseCodepoint{ALPSUseCodepointNew, ALPSUseCodepointOld} {
						...
						testCases = append(testCases, testCase{
							protocol: protocol,
							testType: clientTest,
							name:     fmt.Sprintf("ALPS-Reject-Server-Client%s-Server%s-%s", clientSends, serverConfig, suffix),
							config: Config{
								MaxVersion: ver.version,
							},
							resumeConfig: &Config{
