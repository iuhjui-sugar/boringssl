...
func addTLS13HandshakeTests() {
	...
	// Test that the client reports the correct ALPN after a 0-RTT reject if ALPN was omitted from the first connection.
	testCases = append(testCases, testCase{
		testType: clientTest,
	...
	// Test that the client reports the correct ALPN after a 0-RTT reject if ALPN was omitted from the second connection.
	testCases = append(testCases, testCase{
		testType: clientTest,
	...
	// Test that the server correctly rejects 0-RTT when the previous session did not allow early data on resumption.
	testCases = append(testCases, testCase{
		testType: serverTest,
		name:     "TLS13-EarlyData-ZeroEarlyDataSize-Server",
		config: Config{
			MaxVersion: VersionTLS13,
		},
		resumeConfig: &Config{
	...
	// Test that we reject early data where ALPN is omitted from the first connection.
	testCases = append(testCases, testCase{
		testType: serverTest,
	...
	// Test that we reject early data where ALPN is omitted from the second connection.
	testCases = append(testCases, testCase{
		testType: serverTest,
