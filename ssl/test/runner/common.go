...
// ServerSessionCache is a cache of ServerSessionState objects that can be used
// by a client to resume a TLS session with a given server. ServerSessionCache
// implementations should expect to be called concurrently from different
// goroutines.
type ServerSessionCache interface {
	// Get searches for a ServerSessionState associated with the
	// given session ID.  On return, ok is true if one was found.
	Get(sessionId string) (session *ServerSessionState, ok bool)

	// Put adds the ServerSessionState to the cache with the given
	// session ID.
	Put(sessionId string, session *ServerSessionState)
}
...
type Config struct {
	...
	// ServerSessionCache is a cache of ServerSessionState entries
	// for TLS session resumption.
...
func (c *lruServerSessionCache) Put(sessionId string, session *ServerSessionState) {
	c.lruSessionCache.Put(sessionId, session)
}

func (c *lruServerSessionCache) Get(sessionId string) (*ServerSessionState, bool) {
	cs, ok := c.lruSessionCache.Get(sessionId)
	if !ok {
		return nil, false
	}
	return cs.(*ServerSessionState), true
}
