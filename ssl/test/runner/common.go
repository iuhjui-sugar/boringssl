...
// ESNIRecord extension numbers
const (
	esniRecordExtensionAddressSet uint16 = 0x1001 // draft-ietf-tls-esni-04
)
...
type Config struct {
	...
	// When EsniEnabled is true, the client will either send encrypted SNI
	// (if it has EsniKeys) or GREASE data.
	EsniEnabled bool

	// For clients, EsniKeypairs contains EsniKeys that correspond
	// to the intended origin, retrieved by some mechanism,
	// e.g. DNS. The PrivateKeys slice should be non-empty only on
	// servers.
	EsniKeypairs []EsniKeypair
