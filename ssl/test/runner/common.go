...
type ProtocolBugs struct {
	...
	// SRTPMasterKeyIdentifer, if not empty, is the SRTP MKI value
	// the client offers in negotiating SRTP. MKI support is still
	// missing so the peer must still send none.
	SRTPMasterKeyIdentifer string

	// SendSRTPProtectionProfile, if non-zero, is the SRTP profile
	// the server sends in ServerHello instead of the negotiated
	// one.
