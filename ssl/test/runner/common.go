...
type ProtocolBugs struct {
	...
	// SendExtensionOnCertificate, if not nil, causes the server to send the
	...
	// SendDuplicateCertExtensions, if true, causes the client to send an extra
	// copy of the OCSP/SCT extensions in the Certificate message.
	SendDuplicateCertExtensions bool

	// SendExtensionOnClientCertificate, if not nil, causes the client to send
	// the supplied bytes in the extensions on the Certificate message.
	SendExtensionOnClientCertificate []byte
