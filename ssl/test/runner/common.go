...
const tls13DraftVersion = 0x130f
...
const (
	...
	extensionSupportedVersions          uint16 = 0xff03 // draft-ietf-tls-tls13-16
...
type ProtocolBugs struct {
	...
	// ClientVersionsExtension, if non-empty, causes the client to send a
	// supported versions extension with the values from ClientVersionExtensions.
	ClientVersionsExtension []uint16
	...
	// SessionTicketKEMode, if present, determines the PSK key exchange modes to
	// send with the session ticket.
	SessionTicketKEMode []byte

	// SessionTicketAuthMode, if present, determines the PSK auth modes to send
	// with the session ticket.
	SessionTicketAuthMode []byte
