...
func selectSignatureAlgorithm(key crypto.PrivateKey, peerSigAlgs, ourSigAlgs []signatureAlgorithm) (signatureAlgorithm, error) {
	...
	for _, sigAlg := range ourSigAlgs {
		...
		signer := signerFromSignatureAlgorithm(sigAlg)
		if signer == nil {
			continue
		}
...
func signMessage(version uint16, key crypto.PrivateKey, config *Config, sigAlg signatureAlgorithm, msg []byte) ([]byte, error) {
	...
	if version < VersionTLS12 {
		switch key.(type) {
		case *rsa.PrivateKey:
			signer = &rsaPKCS1Signer{crypto.MD5SHA1}
		case *ecdsa.PrivateKey:
			signer = &ecdsaSigner{crypto.SHA1}
		default:
...
func verifyMessage(version uint16, key crypto.PublicKey, sigAlg signatureAlgorithm, msg, sig []byte) error {
	...
	if version < VersionTLS12 {
		...
		signer = signerFromSignatureAlgorithm(sigAlg)
		if signer == nil {
			return fmt.Errorf("unsupported signature algorithm %04x", sigAlg)
		}
	}
...
func (r *rsaPKCS1Signer) signMessage(key crypto.PrivateKey, config *Config, msg []byte) ([]byte, error) {
	...
	return rsaKey.Sign(config.rand(), r.computeHash(msg), r.hash)
}
...
func signerFromSignatureAlgorithm(sigAlg signatureAlgorithm) signer {
	// TODO(davidben): Implement RSA-PSS for TLS 1.3.
	switch sigAlg {
	case signatureRSAPKCS1WithMD5:
		return &rsaPKCS1Signer{crypto.MD5}
	case signatureRSAPKCS1WithSHA1:
		return &rsaPKCS1Signer{crypto.SHA1}
	case signatureRSAPKCS1WithSHA256:
		return &rsaPKCS1Signer{crypto.SHA256}
	case signatureRSAPKCS1WithSHA384:
		return &rsaPKCS1Signer{crypto.SHA384}
	case signatureRSAPKCS1WithSHA512:
		return &rsaPKCS1Signer{crypto.SHA512}
	case signatureECDSAWithSHA1:
		return &ecdsaSigner{crypto.SHA1}
	case signatureECDSAWithP256AndSHA256:
		return &ecdsaSigner{crypto.SHA256}
	case signatureECDSAWithP384AndSHA384:
		return &ecdsaSigner{crypto.SHA384}
	case signatureECDSAWithP521AndSHA512:
		return &ecdsaSigner{crypto.SHA512}
	default:
		return nil
	}
}
