...
func (h finishedHash) clientSum(baseKey []byte) []byte {
	...
	finishedHMAC.Write(h.contextHashes(nil))
...
func hkdfExpandLabel(hash crypto.Hash, secret, label, hashValue []byte, length int) []byte {
	...
	hkdfLabel := make([]byte, 4+9+len(label)+len(hashValue))
	x := hkdfLabel
	x[0] = byte(length >> 8)
	x[1] = byte(length)
...
// contextHashes returns the concatenation of the handshake hash and the
// resumption context hash, as used in TLS 1.3.
func (h *finishedHash) contextHashes(b []byte) []byte {
	b = h.client.Sum(b)
...
func (h *finishedHash) deriveSecret(secret, label []byte) []byte {
	...
	return hkdfExpandLabel(h.hash, secret, label, h.contextHashes(nil), h.hash.Size())
}
...
type trafficSide int

const (
	clientWrite trafficSide = iota
	serverWrite
)

// deriveTrafficAEAD derives traffic keys and constructs an AEAD given a traffic
// secret.
func deriveTrafficAEAD(version uint16, suite *cipherSuite, secret, phase []byte, side trafficSide) *tlsAead {
	...
	label = append(label, phase...)
	if side == clientWrite {
		label = append(label, []byte(", client write iv")...)
	} else {
		label = append(label, []byte(", server write iv")...)
	}
