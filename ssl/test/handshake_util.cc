...
static bool Proxy(BIO *socket, bool async, int control, int rfd, int wfd) {
  for (;;) {
    ...
    char buf[64];
    long bytes;
    if (FD_ISSET(wfd, &rfds) && (bytes = read(wfd, buf, sizeof(buf))) > 0) {
      char *b = buf;
      while (bytes) {
...
static bool RunHandshaker(BIO *bio, const TestConfig *config, bool is_resume,
  ...
  if (!handshaker_pid) {
    ...
    args.push_back(strdup(config->handshaker_path.c_str()));
  ...
  if (write(control[0], input.data(), input.size()) == -1) {
    perror("write");
    return false;
  }
  ...
  int len = read(control[0], buf.get(), 1024*1024);
  if (len == -1) {
    perror("read");
    return false;
  }
