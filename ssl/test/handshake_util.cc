...
static bool PrepareHandoff(SSL *ssl, SettingsWriter *writer,
                           bssl::Array<uint8_t> *out_handoff) {
  const TestConfig *config = GetTestConfig(ssl);
  bssl::UniquePtr<SSL> ssl_handoff =
      config->NewSSL(ssl->ctx, nullptr, false, nullptr);
  if (!ssl_handoff || !MoveTestConfig(ssl_handoff.get(), ssl) ||
      !MoveTestState(ssl_handoff.get(), ssl)) {
    return false;
  }
  SSL_set_handoff_mode(ssl_handoff.get(), 1);
  ...
  do {
    ret = CheckIdempotentError(
        "SSL_do_handshake", ssl_handoff.get(),
        [&]() -> int { return SSL_do_handshake(ssl_handoff.get()); });
  } while (!HandoffReady(ssl_handoff.get(), ret) && config->async &&
  ...
  if (!CBB_init(cbb.get(), 512) ||
      ...
      !SerializeTestState(GetTestState(ssl_handoff.get()), ssl_handoff->ctx, cbb.get())) {
...
bool DoSplitHandshake(bssl::UniquePtr<SSL> *ssl, SettingsWriter *writer,
                      bool is_resume) {
  bssl::Array<uint8_t> handshaker_input;
  ...
  // Also, the assert is dumb and I should remove it after investigating.
  const TestConfig *config = GetTestConfig(ssl->get());
  BIO *socket = SSL_get_rbio(ssl->get());
  ...
  bssl::Array<uint8_t> out;
  if (!RunHandshaker(socket, config, is_resume, handshaker_input, &out)) {
    fprintf(stderr, "Handshaker failed\n");
    return false;
  }

  CBS output, handback, state;
  CBS_init(&output, out.data(), out.size());
  if (!CBS_get_u24_length_prefixed(&output, &handback) ||
      !CBS_get_u24_length_prefixed(&output, &state) ||
      !DeserializeTestState(&state, GetTestState(ssl->get()), (*ssl)->ctx)) {
    fprintf(stderr, "Handshaker deserialization failed\n");
    return false;
  }
  bssl::Array<uint8_t> handback_bytes;
  handback_bytes.CopyFrom(handback);

  bssl::UniquePtr<SSL> ssl_handback =
      config->NewSSL((*ssl)->ctx, nullptr, false, nullptr);
  if (!ssl_handback ||
      !writer->WriteHandback(handback_bytes) ||
      !SSL_apply_handback(ssl_handback.get(), handback_bytes) ||
      !MoveTestConfig(ssl_handback.get(), ssl->get()) ||
      !MoveTestState(ssl_handback.get(), ssl->get())) {
