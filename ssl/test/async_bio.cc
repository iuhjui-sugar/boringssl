...
bssl::unique_ptr<BIO> AsyncBioCreate() {
  return bssl::unique_ptr<BIO>(BIO_new(&g_async_bio_method));
}

bssl::unique_ptr<BIO> AsyncBioCreateDatagram() {
  bssl::unique_ptr<BIO> ret(BIO_new(&g_async_bio_method));
  if (!ret) {
    return nullptr;
  }
