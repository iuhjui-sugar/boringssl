...
TEST_P(SSLVersionTest, SSLWriteRetry) {
  ...
  for (bool enable_partial_write : {false, true}) {
    ...
    ASSERT_EQ(SSL_get_error(client_.get(),
                            SSL_write(client_.get(), data, 2 * kChunkLen + 1)),
              SSL_ERROR_WANT_WRITE);
    ...
    char data3[] = "_____!!!!!";
    if (enable_partial_write) {
      ASSERT_EQ(SSL_write(client_.get(), data3, 2 * kChunkLen), kChunkLen);
      ASSERT_EQ(SSL_write(client_.get(), data3 + kChunkLen, kChunkLen),
                kChunkLen);
    } else {
      ASSERT_EQ(SSL_write(client_.get(), data3,2 * kChunkLen), 2 *kChunkLen);
    }
    ...
    int ret = SSL_write(client_.get(), data, 2 * kChunkLen);
    if (enable_partial_write) {
      ...
      ASSERT_EQ(SSL_write(client_.get(), data + kChunkLen, kChunkLen),
                kChunkLen);
    } else {
      ...
      // Finish writing the input.
      ASSERT_EQ(SSL_write(client_.get(), data, 2 * kChunkLen), 2 * kChunkLen);
    }
