...
TEST(SSLTest, StreamMethod) {
  ...
  auto writeMessage = [](SSL *ssl, enum ssl_encryption_level_t level,
    ...
    Packet *packet = reinterpret_cast<Packet *>(SSL_get_ex_data(ssl, 1));
  ...
  auto connect = [&](SSL *ssl, Packet* in_packet, Packet* out_packet) {
    while (1) {
      SSL_set_ex_data(ssl, 1, out_packet);
      int ret = SSL_do_handshake(ssl);
      if (ret == -1) {
        ASSERT_EQ(SSL_get_error(ssl, ret), SSL_ERROR_WANT_READ);
        while (!in_packet->has_data) {
          sleep(0.1);
        }
        ASSERT_TRUE(SSL_provide_data(ssl, in_packet->level, in_packet->data, in_packet->len));
        in_packet->has_data = false;
      } else {
        break;
      }
    }
  ...
  Packet cs_packet, sc_packet;
  cs_packet.has_data = false;
  sc_packet.has_data = false;

  std::vector<std::thread> threads;
  threads.emplace_back([&] { connect(client.get(), &sc_packet, &cs_packet); });
  threads.emplace_back([&] { connect(server.get(), &cs_packet, &sc_packet); });
  for (auto &thread : threads) {
    thread.join();
  }
