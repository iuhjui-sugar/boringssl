...
static void ssl_test_ticket_aead_ex_index_free(void *parent, void *ptr,
                                               ...
                                               long argl, void *argp) {
  struct ssl_test_ticket_aead_state *state =
      (struct ssl_test_ticket_aead_state *)ptr;
  if (state == nullptr) {
    return;
  }

  OPENSSL_free(state->plaintext);
  OPENSSL_free(state);
}

static void ssl_test_ticket_aead_ex_index_init() {
...
static int ssl_test_ticket_aead_get_ex_index() {
  CRYPTO_once(&g_ssl_test_ticket_aead_ex_index_once,
              ssl_test_ticket_aead_ex_index_init);
...
static ssl_ticket_aead_result_t ssl_test_ticket_aead_finish(
    const ssl_test_ticket_aead_state *state, uint8_t *out, size_t *out_len,
    size_t max_out_len, const uint8_t *plaintext, size_t plaintext_len) {
  ...
  if (plaintext_len == 0 || plaintext[plaintext_len - 1] != 0xff) {
    return ssl_ticket_aead_ignore_ticket;
  }

  if (max_out_len < plaintext_len - 1) {
    return ssl_ticket_aead_error;
  }

  OPENSSL_memmove(out, plaintext, plaintext_len - 1);
  *out_len = plaintext_len - 1;
...
static ssl_ticket_aead_result_t ssl_test_ticket_aead_open(
  ...
  if (state->retry_count > 0) {
    assert(state->plaintext == nullptr);
    state->plaintext = reinterpret_cast<uint8_t *>(OPENSSL_malloc(in_len));
    if (state->plaintext == nullptr) {
      return ssl_ticket_aead_error;
    }
    OPENSSL_memcpy(state->plaintext, in, in_len);
    state->plaintext_len = in_len;

    state->retry_count--;
    return ssl_ticket_aead_retry;
  }

  return ssl_test_ticket_aead_finish(state, out, out_len, max_out_len, in,
                                     in_len);
}

static ssl_ticket_aead_result_t ssl_test_ticket_aead_open_complete(
    SSL *ssl, uint8_t *out, size_t *out_len, size_t max_out_len) {
  auto state = reinterpret_cast<ssl_test_ticket_aead_state *>(
      SSL_get_ex_data(ssl, ssl_test_ticket_aead_get_ex_index()));
  ...
  return ssl_test_ticket_aead_finish(state, out, out_len, max_out_len,
                                     state->plaintext, state->plaintext_len);
}
...
static const SSL_TICKET_AEAD_METHOD kSSLTestTicketMethod = {
  ...
  ssl_test_ticket_aead_open_complete,
};
...
static void ConnectClientAndServerWithTicketMethod(
  ...
  struct ssl_test_ticket_aead_state *state =
      (struct ssl_test_ticket_aead_state *)OPENSSL_malloc(
          sizeof(struct ssl_test_ticket_aead_state));
  OPENSSL_memset(state, 0, sizeof(struct ssl_test_ticket_aead_state));
  ...
  SSL_set_session(client.get(), session);
  SSL_set_ticket_aead_method(server.get(), &kSSLTestTicketMethod);
