...
TEST(SSLTest, SignatureAlgorithmProperties) {
  ...
  EXPECT_EQ(NID_undef, SSL_get_signature_algorithm_digest(0x1234));
  EXPECT_FALSE(SSL_signature_algorithm_is_rsa_pss(0x1234));

  EXPECT_EQ(EVP_PKEY_RSA,
            SSL_get_signature_algorithm_key_type(SSL_SIGN_RSA_PKCS1_MD5_SHA1));
  EXPECT_EQ(NID_md5_sha1,
            SSL_get_signature_algorithm_digest(SSL_SIGN_RSA_PKCS1_MD5_SHA1));
  EXPECT_FALSE(SSL_signature_algorithm_is_rsa_pss(SSL_SIGN_RSA_PKCS1_MD5_SHA1));

  EXPECT_EQ(EVP_PKEY_EC, SSL_get_signature_algorithm_key_type(
                             SSL_SIGN_ECDSA_SECP256R1_SHA256));
  EXPECT_EQ(NID_sha256, SSL_get_signature_algorithm_digest(
                            SSL_SIGN_ECDSA_SECP256R1_SHA256));
  EXPECT_FALSE(
      SSL_signature_algorithm_is_rsa_pss(SSL_SIGN_ECDSA_SECP256R1_SHA256));

  EXPECT_EQ(EVP_PKEY_RSA,
            SSL_get_signature_algorithm_key_type(SSL_SIGN_RSA_PSS_SHA384));
  EXPECT_EQ(NID_sha384,
            SSL_get_signature_algorithm_digest(SSL_SIGN_RSA_PSS_SHA384));
  EXPECT_TRUE(SSL_signature_algorithm_is_rsa_pss(SSL_SIGN_RSA_PSS_SHA384));
}
