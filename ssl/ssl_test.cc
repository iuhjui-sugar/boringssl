...
static bool TestLeafInChainSession() {
  ...
  if (X509_cmp(session->x509_peer, sk_X509_value(session->x509_chain, 0)) != 0) {
    fprintf(stderr,
            "TestLeafInChainSession: leaf is not first element of chain.\n");
    return false;
  }

  if (sk_X509_num(session->x509_chain) != sk_CRYPTO_BUFFER_num(session->certs)) {
    fprintf(
        stderr,
  ...
  if (X509_cmp(session2->x509_peer, sk_X509_value(session2->x509_chain, 0)) != 0 ||
