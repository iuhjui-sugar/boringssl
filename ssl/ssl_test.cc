...
// and sending the correct codepoint extension on different combination of ALPS
// codepoint settings. More integration tests on runner.go will cover different
// situations to verify application_setting.
class AlpsNewCodepointTest : public testing::Test {
 protected:
  ...
  void SetUpApplicationSetting() {
    ...
    ASSERT_TRUE(SSL_add_application_settings(client_.get(), alpn, sizeof(alpn),
                                             nullptr, 0));
    ASSERT_TRUE(SSL_add_application_settings(server_.get(), alpn, sizeof(alpn),
                                             alps, sizeof(alps)));
  }
...
TEST_F(AlpsNewCodepointTest, ClientOnly) {
  // If client set new codepoint but server doesn't set.
  SetUpExpectedNewCodePoint();
