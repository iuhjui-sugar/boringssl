...
TEST(SSLTest, Handoff) {
  ...
  uint8_t *handoff_bytes;
  size_t handoff_len;
  ASSERT_TRUE(CBB_finish(cbb.get(), &handoff_bytes, &handoff_len));
  bssl::UniquePtr<uint8_t> handoff_bytes_storage(handoff_bytes);

  CBS handoff;
  CBS_init(&handoff, handoff_bytes, handoff_len);
  ...
  size_t handback_len;
  ASSERT_TRUE(CBB_finish(cbb_handback.get(), &handback_bytes, &handback_len));
