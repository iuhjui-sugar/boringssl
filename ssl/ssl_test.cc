 ...
 * Test cases call |CreateContext| to create a context with the protocol version
 * fixed to the current version under test. */
class SSLVersionTest : public ::testing::TestWithParam<VersionParam> {
 protected:
  SSLVersionTest()
      : server_ctx_(CreateContext()),
        client_ctx_(CreateContext()),
        cert_(GetTestCertificate()),
        key_(GetTestKey()) {}
  ...
  void SetUp() {
    ...
    ASSERT_TRUE(cert_);
    ASSERT_TRUE(key_);
    ASSERT_TRUE(SSL_CTX_use_certificate(server_ctx_.get(), cert_.get()));
    ASSERT_TRUE(SSL_CTX_use_PrivateKey(server_ctx_.get(), key_.get()));
  }
...
TEST_P(SSLVersionTest, TestSequenceNumber) {
  ASSERT_TRUE(Connect());
...
TEST_P(SSLVersionTest, TestOneSidedShutdown) {
...
TEST_P(SSLVersionTest, TestGetPeerCertificate) {
  ASSERT_TRUE(SSL_CTX_use_certificate(client_ctx_.get(), cert_.get()));
  ASSERT_TRUE(SSL_CTX_use_PrivateKey(client_ctx_.get(), key_.get()));
  ...
  EXPECT_EQ(sk_X509_num(SSL_get_peer_cert_chain(client_.get())), 1);
  EXPECT_EQ(sk_CRYPTO_BUFFER_num(SSL_get0_peer_certificates(client_.get())), 1);

  EXPECT_EQ(sk_X509_num(SSL_get_peer_cert_chain(server_.get())), 0);
  EXPECT_EQ(sk_CRYPTO_BUFFER_num(SSL_get0_peer_certificates(server_.get())), 1);
}

TEST_P(SSLVersionTest, TestNoPeerCertificate) {
...
TEST_P(SSLVersionTest, TestRetainOnlySHA256OfCerts) {
  ...
  SSL_CTX_set_verify(
      client_ctx_.get(), SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, nullptr);
  SSL_CTX_set_verify(
      server_ctx_.get(), SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, nullptr);
  SSL_CTX_set_cert_verify_callback(client_ctx_.get(), VerifySucceed, NULL);
  SSL_CTX_set_cert_verify_callback(server_ctx_.get(), VerifySucceed, NULL);
  ...
  EXPECT_EQ(
      OPENSSL_memcmp(cert_sha256, session->peer_sha256, SHA256_DIGEST_LENGTH),
      0);
}
...
TEST_P(SSLVersionTest, TestSessionIDContext) {
  ...
  EXPECT_TRUE(ExpectSessionReused(client_ctx_.get(), server_ctx_.get(),
                                  session.get(),
...
TEST_P(SSLVersionTest, TestSessionTimeout) {
  SSL_CTX_set_session_cache_mode(client_ctx_.get(), SSL_SESS_CACHE_BOTH);
  SSL_CTX_set_session_cache_mode(server_ctx_.get(), SSL_SESS_CACHE_BOTH);
...
TEST_P(SSLVersionTest, DefaultTicketKeyRotation) {
  ...
  ASSERT_TRUE(ExpectTicketKeyChanged(server_ctx_.get(), ticket_key,
                                     true /* changed */));
...
TEST_P(SSLVersionTest, TestSNICallback) {
...
TEST_P(SSLVersionTest, TestVersion) {
  ASSERT_TRUE(Connect());
...
// selection callback.
TEST_P(SSLVersionTest, TestALPNCipherAvailable) {
  // SSL 3.0 lacks extensions.
  if (version() == SSL3_VERSION) {
    return;
  }
...
TEST_P(SSLVersionTest, TestSSLClearSessionResumption) {
...
TEST_P(SSLVersionTest, TestAutoChain) {
...
TEST_P(SSLVersionTest, TestSSLWriteRetry) {
  if (is_dtls()) {
    return;
  }
...
TEST_P(SSLVersionTest, TestRecordCallback) {
  ...
  SSL_CTX_set_msg_callback(
      server_ctx_.get(), [](int is_write, int cb_version, int cb_type, const void *buf,
                    size_t len, SSL *ssl, void *arg) {
        CallbackType *cb_ptr = reinterpret_cast<CallbackType *>(arg);
        (*cb_ptr)(is_write, cb_version, cb_type, buf, len, ssl);
      });
  SSL_CTX_set_msg_callback_arg(server_ctx_.get(), &cb);
