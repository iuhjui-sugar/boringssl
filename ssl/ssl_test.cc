...
static bool TestSSLWriteRetry(bool is_dtls, const SSL_METHOD *method,
  ...
  for (bool enable_partial_write : std::vector<bool>{false, true}) {
    ...
    // write. Keep a count so we may reread it again later.
    char data[] = "hello world";
    unsigned count = 0;
    for (;;) {
      int ret = SSL_write(client.get(), data, 5);
      if (ret <= 0) {
    ...
    if (SSL_get_error(client.get(), SSL_write(client.get(), data, 5)) !=
    ...
    if (SSL_get_error(client.get(), SSL_write(client.get(), data, 3)) !=
            SSL_ERROR_SSL ||
        !ExpectBadWriteRetry()) {
    ...
    char data2[] = "hello";
    if (SSL_get_error(client.get(), SSL_write(client.get(), data2, 5)) !=
            SSL_ERROR_SSL ||
        !ExpectBadWriteRetry()) {
      fprintf(stderr, "SSL_write retry did not fail as expected.\n");
      return false;
    ...
    if (SSL_get_error(client.get(), SSL_write(client.get(), data2, 3)) !=
            SSL_ERROR_SSL ||
        !ExpectBadWriteRetry()) {
      fprintf(stderr, "SSL_write retry did not fail as expected.\n");
      return false;
    ...
    if (SSL_get_error(client.get(), SSL_write(client.get(), data, 11)) !=
        SSL_ERROR_WANT_WRITE) {
      fprintf(stderr, "SSL_write retry unexpectedly failed.\n");
      return false;
    }
    ...
    for (unsigned i = 0; i < count; i++) {
      if (SSL_read(server.get(), buf, sizeof(buf)) != 5 ||
          OPENSSL_memcmp(buf, "hello", 5) != 0) {
    ...
    char data3[] = "????? world";
    if (enable_partial_write) {
      if (SSL_write(client.get(), data3, 11) != 5 ||
          SSL_write(client.get(), data3 + 5, 6) != 6) {
        fprintf(stderr, "SSL_write retry failed.\n");
        return false;
      }
    } else if (SSL_write(client.get(), data3, 11) != 11) {
      fprintf(stderr, "SSL_write retry failed.\n");
      return false;
    }
    ...
    if (SSL_read(server.get(), buf, sizeof(buf)) != 5 ||
        OPENSSL_memcmp(buf, "hello", 5) != 0 ||
        SSL_read(server.get(), buf, sizeof(buf)) != 6 ||
        OPENSSL_memcmp(buf, " world", 6) != 0) {
