...
static bool TestCipherRule(const CipherTest &t) {
  std::unique_ptr<SSL_CTX> ctx(SSL_CTX_new(TLS_method()));
  if (!ctx) {
    return false;
  }
...
static bool TestRuleDoesNotIncludeNull(const char *rule) {
  std::unique_ptr<SSL_CTX> ctx(SSL_CTX_new(SSLv23_server_method()));
  if (!ctx) {
    return false;
  }
...
static bool TestSSL_SESSIONEncoding(const char *input_b64) {
  ...
  std::unique_ptr<SSL_SESSION> session(SSL_SESSION_from_bytes(input.data(), input.size()));
  if (!session) {
...
static bool TestDefaultVersion(uint16_t min_version, uint16_t max_version,
  ...
  std::unique_ptr<SSL_CTX> ctx(SSL_CTX_new(method()));
  if (!ctx) {
    return false;
  }
...
static std::unique_ptr<SSL_SESSION> CreateSessionWithTicket(size_t ticket_len) {
  std::vector<uint8_t> der;
  if (!DecodeBase64(&der, kOpenSSLSession)) {
    return nullptr;
  }
  std::unique_ptr<SSL_SESSION> session(SSL_SESSION_from_bytes(der.data(), der.size()));
  if (!session) {
    return nullptr;
  }
...
static bool GetClientHello(SSL *ssl, std::vector<uint8_t> *out) {
  std::unique_ptr<BIO> bio(BIO_new(BIO_s_mem()));
  if (!bio) {
    return false;
  }
...
static size_t GetClientHelloLen(size_t ticket_len) {
  std::unique_ptr<SSL_CTX> ctx(SSL_CTX_new(TLS_method()));
  std::unique_ptr<SSL_SESSION> session = CreateSessionWithTicket(ticket_len);
  if (!ctx || !session) {
    return 0;
  }
  std::unique_ptr<SSL> ssl(SSL_new(ctx.get()));
...
static std::unique_ptr<SSL_SESSION> CreateTestSession(uint32_t number) {
  std::unique_ptr<SSL_SESSION> ret(SSL_SESSION_new());
  if (!ret) {
    return nullptr;
  }
...
static bool TestInternalSessionCache() {
  ...
  std::vector<std::unique_ptr<SSL_SESSION>> sessions;
  for (int i = 0; i < 10; i++) {
    std::unique_ptr<SSL_SESSION> session = CreateTestSession(i);
    if (!session) {
      return false;
    }
  ...
  std::unique_ptr<SSL_SESSION> collision(CreateTestSession(7));
...
static std::unique_ptr<X509> GetTestCertificate() {
  static const char kCertPEM[] =
      ...
      "-----END CERTIFICATE-----\n";
  std::unique_ptr<BIO> bio(BIO_new_mem_buf(kCertPEM, strlen(kCertPEM)));
  return std::unique_ptr<X509>(PEM_read_bio_X509(bio.get(), nullptr, nullptr, nullptr));
}

static std::unique_ptr<EVP_PKEY> GetTestKey() {
  static const char kKeyPEM[] =
      ...
      "-----END RSA PRIVATE KEY-----\n";
  std::unique_ptr<BIO> bio(BIO_new_mem_buf(kKeyPEM, strlen(kKeyPEM)));
  return std::unique_ptr<EVP_PKEY>(
      PEM_read_bio_PrivateKey(bio.get(), nullptr, nullptr, nullptr));
}

static bool ConnectClientAndServer(std::unique_ptr<SSL> *out_client, std::unique_ptr<SSL> *out_server,
                                   SSL_CTX *client_ctx, SSL_CTX *server_ctx,
                                   SSL_SESSION *session) {
  std::unique_ptr<SSL> client(SSL_new(client_ctx)), server(SSL_new(server_ctx));
  if (!client || !server) {
    return false;
  }
...
static bool TestSequenceNumber(bool dtls) {
  std::unique_ptr<SSL_CTX> client_ctx(SSL_CTX_new(dtls ? DTLS_method() : TLS_method()));
  std::unique_ptr<SSL_CTX> server_ctx(SSL_CTX_new(dtls ? DTLS_method() : TLS_method()));
  if (!client_ctx || !server_ctx) {
    return false;
  }

  std::unique_ptr<X509> cert = GetTestCertificate();
  std::unique_ptr<EVP_PKEY> key = GetTestKey();
  if (!cert || !key ||
  ...
  std::unique_ptr<SSL> client, server;
...
static bool TestOneSidedShutdown() {
  std::unique_ptr<SSL_CTX> client_ctx(SSL_CTX_new(TLS_method()));
  std::unique_ptr<SSL_CTX> server_ctx(SSL_CTX_new(TLS_method()));
  if (!client_ctx || !server_ctx) {
    return false;
  }
...
static bool TestSessionDuplication() {
  ...
  std::unique_ptr<SSL_SESSION> session1(SSL_SESSION_dup(session0, SSL_SESSION_DUP_ALL));
  if (!session1) {
    return false;
  }
...
static bool TestSetBIO() {
  ...
  std::unique_ptr<SSL> ssl(SSL_new(ctx.get()));
  std::unique_ptr<BIO> bio1(BIO_new(BIO_s_mem())), bio2(BIO_new(BIO_s_mem())),
...
static bool TestGetPeerCertificate() {
  ...
  for (uint16_t version : kVersions) {
    ...
    std::unique_ptr<X509> peer(SSL_get_peer_certificate(server.get()));
...
static std::unique_ptr<SSL_SESSION> g_last_session;
...
static std::unique_ptr<SSL_SESSION> CreateClientSession(SSL_CTX *client_ctx,
                                             SSL_CTX *server_ctx) {
