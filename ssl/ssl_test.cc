...
void VerifyHandoff(bool alps_codepoint) {
  ...
  for (bool early_data : {false, true}) {
    ...
    for (bool is_resume : {false, true}) {
      ...
      SSL_set_alps_use_new_codepoint(client.get(), alps_codepoint);
      ...
      SSL_set_alps_use_new_codepoint(handshaker.get(), alps_codepoint);
      SSL_CTX_set_alpn_select_cb(
          handshaker_ctx.get(),
          [](SSL *ssl, const uint8_t **out, uint8_t *out_len, const uint8_t *in,
              unsigned in_len, void *arg) -> int {
...
TEST(SSLTest, Handoff) {
  for (bool alps_codepoint : {false, true}) {
    SCOPED_TRACE(alps_codepoint);
    VerifyHandoff(alps_codepoint);
  }
}
