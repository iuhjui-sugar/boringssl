...
void GenerateEsniKeypairs(CBB *out_key_struct, std::vector<CBB> *out_privs,
                          ...
                          bool bug_unsupported_groups = false) {
  CBB key_struct, public_name, keys;
  if (!CBB_init(out_key_struct, 0) ||
      !CBB_add_u16_length_prefixed(out_key_struct, &key_struct) ||
  ...
  std::rotate(rotated_group_ids.begin(),
              rotated_group_ids.begin() + k_rotate_amount,
  ...
  for (const uint16_t group_id : rotated_group_ids) {
    if (!bug_unsupported_groups) {
      // Skip this group if it's not in kDefaultGroups
      bool supported = false;
      for (const uint16_t supp : kDefaultGroups) {
        if (group_id == supp) {
          supported = true;
          break;
        }
  ...
  CBB cipher_suites;
  if (!CBB_add_u16_length_prefixed(&key_struct, &cipher_suites) ||
      !CBB_add_u16(&cipher_suites, 0x1303) ||
      !CBB_add_u16(&key_struct, 32) || !CBB_add_u16(&key_struct, 0) ||
...
bool ESNIHelperAddEsniPrivateKeys(SSL *ssl, CBB *key_struct,
  ...
  // Assuming |key_struct| is U16 length-prefixed, containing only one ESNIKeys.
  CBS esni_keys_list, esni_keys;
  CBS_init(&esni_keys, CBB_data(key_struct), CBB_len(key_struct));
  CBS_get_u16_length_prefixed(&esni_keys_list, &esni_keys);

  return SSL_add_esni_private_keys(ssl, CBS_data(&esni_keys),
                                   CBS_len(&esni_keys), privs, privs_len);
}
...
TEST(SSLTest, ESNIRetry) {
  ...
  GenerateEsniKeypairs(&key_struct_old, &priv_keys_old);
  ...
  ASSERT_TRUE(CompleteHandshakes(client.get(), server.get()));
  Array<uint8_t> retry_keys_copy;
  uint8_t *retry_keys = nullptr;
  size_t retry_keys_len = 0;
  SSL_get_retry_keys(client.get(), &retry_keys, &retry_keys_len);
  ASSERT_NE(retry_keys, nullptr);
  ASSERT_NE(retry_keys_len, static_cast<size_t>(0));
  retry_keys_copy.CopyFrom(Span<uint8_t>(retry_keys, retry_keys_len));

  // Give retry keys to the client and try handshake again. We should NOT get
  // retry keys.
  ASSERT_TRUE(SSL_set_esni_keys(client.get(), retry_keys_copy.data(),
                                retry_keys_copy.size()));
  ASSERT_TRUE(CompleteHandshakes(client.get(), server.get()));
...
TEST(SSLTest, ESNIKeysDeserialize) {
  ...
  CBB esni_keys_list, esni_keys;
  CBB_init(&esni_keys_list, 0);
  CBB_add_u16_length_prefixed(&esni_keys_list, &esni_keys);
  CBB_add_bytes(&esni_keys, kEsniKeysBytes, sizeof(kEsniKeysBytes));
  CBB_flush(&esni_keys_list);
  ASSERT_TRUE(
      SSL_set_esni_keys(client.get(), CBB_data(&esni_keys_list), CBB_len(&esni_keys_list)));
