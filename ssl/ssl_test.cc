...
TEST(SSLTest, KeyUpdateSpew) {
  ...
  // 32 key updates should work again after data.
  for (int i = 0; i < 32; i++) {
    EXPECT_TRUE(SSL_key_update(server.get(), SSL_KEY_UPDATE_NOT_REQUESTED));
    EXPECT_EQ(0, SSL_write(server.get(), nullptr, 0));
  }
  ...
  // 33 key updates should work with data interspersed
  for (int i = 0; i < 33; i++) {
    EXPECT_TRUE(SSL_key_update(server.get(), SSL_KEY_UPDATE_NOT_REQUESTED));
    EXPECT_EQ(0, SSL_write(server.get(), nullptr, 0));
    EXPECT_EQ(5, SSL_write(server.get(), "world", 5));
  ...
  // 33 key updates should work with 256 records written to allow for an extra
  // key update.
  for (int i = 0; i < 33; i++) {
    EXPECT_TRUE(SSL_key_update(server.get(), SSL_KEY_UPDATE_NOT_REQUESTED));
    EXPECT_EQ(0, SSL_write(server.get(), nullptr, 0));
  ...
  // 34 key updates should work with 512 records written to allow for an extra
  // key update.
  for (int i = 0; i < 34; i++) {
    EXPECT_TRUE(SSL_key_update(server.get(), SSL_KEY_UPDATE_NOT_REQUESTED));
    EXPECT_EQ(0, SSL_write(server.get(), nullptr, 0));
  ...
  // 33 key updates should not work alone, once a read has consumed the pending
  // "credit"
  for (int i = 0; i < 512; i++) {
    EXPECT_EQ(5, SSL_write(client.get(), "world", 5));
    ASSERT_EQ(5, SSL_read(server.get(), buf, sizeof(buf)));
