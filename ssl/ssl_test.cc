...
static void SetBoolOnFree(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
...
TEST(SSLTest, ExDataThreads) {
  ...
  int first_index =
      SSL_get_ex_new_index(0, nullptr, nullptr, nullptr, SetBoolOnFree);
  ...
  for (size_t i = 0; i < kNumSSLs; i++) {
    threads.emplace_back([&, i] {
      bssl::UniquePtr<SSL> ssl(SSL_new(ctx.get()));
      ASSERT_TRUE(ssl);
      ASSERT_TRUE(SSL_set_ex_data(ssl.get(), first_index, &flags[i]));
    });
  }
  for (auto &thread : threads) {
    thread.join();
  ...
  static_assert(kNumIndices <= kNumSSLs, "flags buffer too small");
  std::fill(std::begin(flags), std::end(flags), false);
  bssl::UniquePtr<SSL> ssl(SSL_new(ctx.get()));
  ASSERT_TRUE(ssl);
  for (size_t i = 0; i < kNumIndices; i++) {
    ...
    ASSERT_TRUE(SSL_set_ex_data(ssl.get(), index[i], &flags[i]));
  }
  ssl = nullptr;
  for (size_t i = 0; i < kNumIndices; i++) {
    EXPECT_TRUE(flags[i]);
