...
void ESNISetupClient(SSL_CTX *client_ctx, bssl::UniquePtr<SSL> *out_client) {
  ...
  SSL_set_enable_esni_grease(client.get(), true);
...
void ESNISetupServer(SSL_CTX *server_ctx, bssl::UniquePtr<SSL> *out_server) {
  ...
  ASSERT_TRUE(server);
  SSL_set_accept_state(server.get());
  SSL_set_enable_esni_grease(server.get(), true);
...
// TODO(dmcardle): Fix this broken test!
//
//   (1) We are using EsniKeypairTool, which we originally expected to generate
//       two different ESNIKeys.
//
//   (2) Since we're using the same ESNIKeys twice, we should actually be
//       checking that there were NO retry keys returned.
//
TEST(SSLTest, ESNIRetry) {
  ...
  ASSERT_TRUE(retry_keys_copy.CopyFrom(MakeConstSpan(retry_keys, retry_keys_len)));
}
...
TEST(SSLTest, ESNIKeysDeserialize) {
  ...
  ASSERT_EQ(kExpectedPublicName.size(),
            client->config->esni->esni_public_name.size());
  ASSERT_EQ(0, OPENSSL_memcmp(client->config->esni->esni_public_name.data(),
                              kExpectedPublicName.c_str(),
                              client->config->esni->esni_public_name.size()));
  ASSERT_EQ(client->config->esni->esni_group, kExpectedKeyShareGroup_0);

  ASSERT_EQ(sizeof(kExpectedKeyExchange_0),
            client->config->esni->esni_server_keyshare.size());
  ASSERT_EQ(
      0, OPENSSL_memcmp(client->config->esni->esni_server_keyshare.data(),
                        kExpectedKeyExchange_0, sizeof(kExpectedKeyExchange_0)));

  ASSERT_EQ(
      0, strcmp(client->config->esni->esni_cipher->name, "TLS_AES_128_GCM_SHA256"));
  ASSERT_EQ(client->config->esni->esni_padded_length, kExpectedPaddedLength);
}
