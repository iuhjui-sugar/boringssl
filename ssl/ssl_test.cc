...
static bool TestSessionTimeoutCertCallback(bool is_dtls,
  ...
  /* Set both context and connection (via callback) default session timeout.
   * The connection uses the default value, so should be ignored. */
  ...
  bssl::UniquePtr<SSL_SESSION> ctx_session =
      CreateClientSession(client_ctx.get(), server_ctx.get());
  if (!ctx_session) {
    fprintf(stderr, "Error getting session.\n");
    return false;
  ...
  if (!ExpectSessionReused(client_ctx.get(), server_ctx.get(), ctx_session.get(),
                           true /* expect session not reused */)) {
    fprintf(stderr, "Error resuming session with timeout of 0.\n");
    return false;
  }

  // Advance the clock just behind the timeout.
  g_current_time.tv_sec += timeout - 10 - 1;
