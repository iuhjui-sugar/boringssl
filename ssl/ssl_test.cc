...
void GenerateEsniKeypairs(CBB *out_key_struct,
  ...
  for (const NamedGroup &group : NamedGroups()) {
    CBB kse_bytes;
    if (!CBB_add_u16(&keys, group.group_id) ||
    ...
    CBB pub_key;
    CBB_init(&pub_key, 0);
    UniquePtr<SSLKeyShare> keyshare = SSLKeyShare::Create(group.group_id);
...
TEST(SSLTest, ESNI) {
  ...
  ASSERT_TRUE(SSL_add_esni_private_keys(server.get(), CBB_data(&key_struct),
                                        CBB_len(&key_struct), privs,
                                        privs_len));
