...
TEST(SSLTest, ParseTLS13NewSessionTicket) {
  ...
  const SSL_SESSION *session_without_ticket = SSL_get_session(client.get());
  ASSERT_FALSE(SSL_SESSION_has_ticket(session_without_ticket));
  ...
  SSL_SESSION *session_with_ticket = SSL_process_tls13_new_session_ticket(
      client.get(), kTicket, sizeof(kTicket));
  ASSERT_TRUE(session_with_ticket);
  bssl::UniquePtr<SSL_SESSION> session_with_ticket_free(session_with_ticket);
  ASSERT_TRUE(SSL_SESSION_has_ticket(session_with_ticket));

  uint8_t *session_buf = nullptr;
  size_t session_length = 0;
  ASSERT_TRUE(
      SSL_SESSION_to_bytes(session_with_ticket, &session_buf, &session_length));
