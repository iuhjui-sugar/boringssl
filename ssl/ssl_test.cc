...
TEST(SSLTest, Handoff) {
  ...
  bssl::UniquePtr<uint8_t> handoff_storage;
  ASSERT_TRUE(CBB_init(cbb.get(), 256));
  ASSERT_TRUE(SSL_serialize_handoff(server.get(), cbb.get()));
  ASSERT_TRUE(CBB_finish(&cbb, &handoff, &handoff_storage));
