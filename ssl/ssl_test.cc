...
TEST_P(SSLVersionTest, SequenceNumber) {
  ...
  if (is_dtls()) {
    if (version() == DTLS1_3_EXPERIMENTAL_VERSION) {
      ...
      // TODO(crbug.com/boringssl/715): The read sequences aren't checked
      // because the SSL_get_read_sequence API needs to be reworked for DTLS
      // 1.3.
    } else {
...
TEST_P(SSLVersionTest, RecordCallback) {
  if (version() == DTLS1_3_EXPERIMENTAL_VERSION) {
    ...
    // header format, so the record header sanity check doesn't make sense.
    return;
  }
