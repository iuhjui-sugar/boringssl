...
TEST(SSLTest, ESNI) {
  ...
  CBB key_struct, public_name, keys, kse_bytes, cipher_suites;
  ...
  const uint8_t *privs[1];
  privs[0] = CBB_data(&private_key);
  size_t privs_len[1];
  privs_len[0] = CBB_len(&private_key);
  // TODO(dmcardle): We are mistakenly passing the serialized SSLKeyShare object
  // as if it were a serialized ESNIKeys.
  ASSERT_TRUE(SSL_add_esni_private_keys(
      server.get(), CBB_data(&key_struct), CBB_len(&key_struct),
      privs, privs_len));
