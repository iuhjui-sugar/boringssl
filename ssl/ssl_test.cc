...
TEST(SSLTest, ECHServerConfigList) {
  // ech_config contains a serialized ECHConfig value.
  ...
  // private_key is a valid X25519 private key that corresponds to the public
  // value encoded in |ech_config|.
  ...
  EXPECT_EQ(SSL_R_ECH_SERVER_CONFIG_AND_PRIVATE_KEY_MISMATCH, ERR_GET_REASON(err));
  ERR_clear_error();

  // Adding an ECHConfig with the matching private key succeeds.
  ASSERT_TRUE(SSL_ECH_SERVER_CONFIG_LIST_add(config_list.get(), /*is_retry_config=*/1,
                                             ech_config, sizeof(ech_config),
  ...
  ASSERT_TRUE(SSL_ECH_SERVER_CONFIG_LIST_add(
      next_config_list.get(), /*is_retry_config=*/1, ech_config,
      sizeof(ech_config), private_key, sizeof(private_key)));
