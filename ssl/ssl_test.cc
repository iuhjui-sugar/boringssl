...
static const SSL_METHOD *kTLSMethod = TLS_method();
static const SSL_METHOD *kDTLSMethod = DTLS_method();

static const VersionParam kAllVersions[] = {
    {SSL3_VERSION, kTLSMethod, "SSL3"},
    {TLS1_VERSION, kTLSMethod, "TLS1"},
    {TLS1_1_VERSION, kTLSMethod, "TLS1_1"},
    {TLS1_2_VERSION, kTLSMethod, "TLS1_2"},
// TLS 1.3 requires RSA-PSS, which is disabled for Android system builds.
#if !defined(BORINGSSL_ANDROID_SYSTEM)
    {TLS1_3_VERSION, kTLSMethod, "TLS1_3"},
#endif
    {DTLS1_VERSION, kDTLSMethod, "DTLS1"},
    {DTLS1_2_VERSION, kDTLSMethod, "DTLS1_2"},
};
...
TEST_P(DefaultSessionTicketKeyTest, Initialization) {
  ...
  bssl::UniquePtr<SSL_CTX> server_ctx(SSL_CTX_new(v.method));
  ASSERT_TRUE(server_ctx);
  static const uint8_t zero_key[kTicketKeyLen] = {};
  uint8_t ticket_key[kTicketKeyLen];
  ASSERT_EQ(1, SSL_CTX_get_tlsext_ticket_keys(server_ctx.get(), ticket_key,
                                              kTicketKeyLen));
  ASSERT_NE(0, OPENSSL_memcmp(ticket_key, zero_key, kTicketKeyLen));
}

TEST_P(DefaultSessionTicketKeyTest, Rotation) {
  const VersionParam v = GetParam();
  if (v.version == SSL3_VERSION) {
    return;
  }
  ...
  bssl::UniquePtr<SSL_CTX> server_ctx(SSL_CTX_new(v.method));
  ASSERT_TRUE(server_ctx);
  bssl::UniquePtr<SSL_CTX> client_ctx(SSL_CTX_new(v.method));
  ASSERT_TRUE(client_ctx);
  ...
  SSL_CTX_set_session_psk_dhe_timeout(server_ctx.get(), UINT32_MAX);
  ...
  ASSERT_TRUE(SSL_CTX_use_certificate(server_ctx.get(), cert.get()));
  ASSERT_TRUE(SSL_CTX_use_PrivateKey(server_ctx.get(), key.get()));
  ASSERT_TRUE(SSL_CTX_set_min_proto_version(client_ctx.get(), v.version));
  ASSERT_TRUE(SSL_CTX_set_max_proto_version(client_ctx.get(), v.version));
  ASSERT_TRUE(SSL_CTX_set_min_proto_version(server_ctx.get(), v.version));
  ASSERT_TRUE(SSL_CTX_set_max_proto_version(server_ctx.get(), v.version));
...
static bool ForEachVersion(bool (*test_func)(bool is_dtls,
  ...
  for (auto version : kAllVersions) {
    bool is_dtls = version.method == kDTLSMethod;
    if (!test_func(is_dtls, version.method, version.version)) {
      if (is_dtls) {
