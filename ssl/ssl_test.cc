...
static bool TestSSLWriteRetry(bool is_dtls, const SSL_METHOD *method,
  ...
  for (bool enable_partial_write : std::vector<bool>{false, true}) {
    ...
    // write. Keep a count so we may reread it again later. "hello world" will
    // be written in two chunks, "hello" and " world".
    char data[] = "hello world";
    static const int kChunk1Len = 5;  // The length of "hello".
    static const int kChunk2Len = 6;  // The length of " world".
    static const int kTotalLen = kChunk1Len + kChunk2Len;
    unsigned count = 0;
    for (;;) {
      int ret = SSL_write(client.get(), data, kChunk1Len);
      if (ret <= 0) {
    ...
    if (SSL_get_error(client.get(), SSL_write(client.get(), data, kChunk1Len)) !=
    ...
    if (SSL_get_error(client.get(),
                      SSL_write(client.get(), data, kChunk1Len - 1)) !=
            SSL_ERROR_SSL ||
        !ExpectBadWriteRetry()) {
    ...
    if (SSL_get_error(client.get(),
                      SSL_write(client.get(), data2, kChunk1Len)) !=
        SSL_ERROR_WANT_WRITE) {
      fprintf(stderr, "SSL_write retry unexpectedly failed.\n");
      return false;
    }
    ...
    if (SSL_get_error(client.get(),
                      SSL_write(client.get(), data2, kChunk1Len - 1)) !=
            SSL_ERROR_SSL ||
        !ExpectBadWriteRetry()) {
      fprintf(stderr, "SSL_write retry did not fail as expected.\n");
      return false;
    ...
    if (SSL_get_error(client.get(), SSL_write(client.get(), data, kTotalLen)) !=
        SSL_ERROR_WANT_WRITE) {
      fprintf(stderr, "SSL_write retry unexpectedly failed.\n");
      return false;
    }
    ...
    for (unsigned i = 0; i < count; i++) {
      if (SSL_read(server.get(), buf, sizeof(buf)) != kChunk1Len ||
          OPENSSL_memcmp(buf, "hello", kChunk1Len) != 0) {
    ...
    char data3[] = "????? world";
    if (enable_partial_write) {
      if (SSL_write(client.get(), data3, kTotalLen) != kChunk1Len ||
          SSL_write(client.get(), data3 + kChunk1Len, kChunk2Len) !=
              kChunk2Len) {
        fprintf(stderr, "SSL_write retry failed.\n");
        return false;
      }
    } else if (SSL_write(client.get(), data3, kTotalLen) != kTotalLen) {
      fprintf(stderr, "SSL_write retry failed.\n");
      return false;
    }
    ...
    if (SSL_read(server.get(), buf, sizeof(buf)) != kChunk1Len ||
        OPENSSL_memcmp(buf, "hello", kChunk1Len) != 0 ||
        SSL_read(server.get(), buf, sizeof(buf)) != kChunk2Len ||
        OPENSSL_memcmp(buf, " world", kChunk2Len) != 0) {
