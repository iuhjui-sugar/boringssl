...
TEST(SSLTest, StreamMethod) {
  ...
  auto nopSetEncryptionKeys =
      [](SSL *ssl, enum ssl_encryption_level_t level, const uint8_t *read_key,
         const uint8_t *write_key, size_t secret_len) -> int { return 1; };
  ...
  ASSERT_TRUE(SSL_CTX_set_custom_quic_method(client_ctx.get(), &clientQuicMethod));
  ASSERT_TRUE(SSL_CTX_set_custom_quic_method(server_ctx.get(), &serverQuicMethod));
