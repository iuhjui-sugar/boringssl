...
bool SSL_serialize_handoff(const SSL *ssl, CBB *out) {
  ...
  if (!CBB_add_asn1(out, &seq, CBS_ASN1_SEQUENCE) ||
      ...
      // TODO: handle v2 ClientHello messges
...
bool SSL_apply_handoff(SSL *ssl, CBS *handoff) {
  ...
  if (!CBS_get_asn1(handoff, &seq, CBS_ASN1_SEQUENCE) ||
  ...
  CBS hs_buf;
  if (!CBS_get_asn1(&seq, &hs_buf, CBS_ASN1_OCTETSTRING)) {
    return false;
  }
