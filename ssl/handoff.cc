...
static bool apply_capabilities(SSL *ssl, Span<const uint8_t> capabilities) {
  CBS seq, capabilities_cbs(capabilities);
  if (!CBS_get_asn1(&capabilities_cbs, &seq, CBS_ASN1_SEQUENCE)) {
    return false;
  }

  if (!apply_remote_features(ssl, &seq)) {
    return false;
  }
...
int SSL_request_handshake_hints(SSL *ssl, const uint8_t *client_hello,
  ...
  if (!apply_capabilities(ssl, MakeConstSpan(capabilities, capabilities_len))) {
    return 0;
  }
