...
static bool serialize_features(CBB *out) {
  ...
  if (!CBB_add_asn1(out, &curves, CBS_ASN1_OCTETSTRING) ||
      !CBB_add_u16(&curves, SSL_CURVE_SECP224R1) ||
      !CBB_add_u16(&curves, SSL_CURVE_SECP256R1) ||
      !CBB_add_u16(&curves, SSL_CURVE_SECP384R1) ||
      !CBB_add_u16(&curves, SSL_CURVE_SECP521R1) ||
      !CBB_add_u16(&curves, SSL_CURVE_X25519)) {
    return false;
  }
...
static bool apply_remote_features(SSL *ssl, CBS *in) {
  ...
  supported_curves.Init(CBS_len(&curves) / 2);
  size_t idx = 0;
  ...
  new_configured_curves.Init(configured_curves.size());
  idx = 0;
  ...
  ssl->config->supported_group_list.CopyFrom(new_configured_curves);
