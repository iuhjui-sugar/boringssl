...
int tls13_server_handshake(SSL *ssl, SSL_HANDSHAKE *hs) {
  ...
  switch (hs->handshake_state) {
    ...
    case HS_STATE_CLIENT_CERTIFICATE:
       ...
       *
       * FIXME! This is wasteful. It assumes tls13_receive_certificate takes one
       * iteration which is not something the state machine gets to assume. Stop
       * requiring one state per message. */
