...
static enum ssl_hs_wait_t do_select_parameters(SSL_HANDSHAKE *hs) {
  ...
  OPENSSL_memcpy(hs->session_id, client_hello.session_id, client_hello.session_id_len);
...
static enum ssl_hs_wait_t do_send_server_hello(SSL_HANDSHAKE *hs) {
  ...
  if (!ssl->method->init_message(ssl, &cbb, &body, SSL3_MT_SERVER_HELLO) ||
      ...
      (ssl->version == TLS1_3_CCS_DRAFT_VERSION && !CBB_add_u8(&body, 0)) ||
      !CBB_add_u16_length_prefixed(&body, &extensions) ||
  ...
  if (ssl->version == TLS1_3_CCS_DRAFT_VERSION &&
...
static enum ssl_hs_wait_t do_process_end_of_early_data(SSL_HANDSHAKE *hs) {
  ...
  return hs->ssl->version == TLS1_3_CCS_DRAFT_VERSION
