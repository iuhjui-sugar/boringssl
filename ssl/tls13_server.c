...
static int tls13_send_encrypted_extensions(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  if (!HKDF_extract(ssl->s3->hs->xES, &ssl->s3->hs->key_len, digest,
                    ssl->s3->hs->premaster_secret, ssl->s3->hs->premaster_secret_len,
                    NULL, 0) ||
  ...
  if (!tls13_update_traffic_keys(ssl, tls13_type_handshake,
                                 ...
                                 hs_hash, hs_hash_len)) {
    OPENSSL_cleanse(ssl->s3->hs->premaster_secret, ssl->s3->hs->premaster_secret_len);
    OPENSSL_free(ssl->s3->hs->premaster_secret);
    return 0;
  }
