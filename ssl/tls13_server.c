...
static int tls13_receive_client_hello(SSL *ssl, SSL_HS_MESSAGE msg) {
  ...
  const uint8_t *draft_buf = NULL;
  const uint8_t *psk_buf = NULL;
  const uint8_t *key_share_buf = NULL;
  const uint8_t *early_data_buf = NULL;
  size_t draft_len = 0, psk_len = 0, key_share_len = 0, early_data_len = 0;
  CBS draft, psk, key_share, early_data;

  SSL_early_callback_ctx_extension_get(&early_ctx, TLSEXT_TYPE_draft_version,
                                       &draft_buf, &draft_len);
  CBS_init(&draft, draft_buf, draft_len);
  ...
  uint8_t out_alert;
  if (!ext_draft_version_parse_clienthello(ssl, &out_alert, &draft) ||
      !ext_pre_shared_key_parse_clienthello(ssl, &out_alert, &psk) ||
...
static int tls13_send_server_hello(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  if (!CBB_init(&cbb, 0) ||
      ...
      !ext_early_data_add_serverhello(ssl, &extensions) ||
      !CBB_finish(&cbb, &data, &length)) {
    goto err;
  }

  CBB_cleanup(&cbb);
  int ret = tls13_assemble_handshake_message(out, SSL3_MT_SERVER_HELLO, data,
                                             length);
  OPENSSL_free(data);
  return ret;
...
static int tls13_send_encrypted_extensions(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  CBB_cleanup(&cbb);
  int ret = tls13_assemble_handshake_message(out, SSL3_MT_ENCRYPTED_EXTENSIONS, data,
                                             length);
  OPENSSL_free(data);
  return ret;
...
static int tls13_send_certificate_request(SSL *ssl, SSL_HS_MESSAGE *out) {
  // TODO(IMPLEMENT)
  int ret = tls13_assemble_handshake_message(out, SSL3_MT_CERTIFICATE_REQUEST,
                                             NULL, 0);
  return ret;
}
