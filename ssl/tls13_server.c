...
static int tls13_receive_client_hello(SSL *ssl, SSL_HS_MESSAGE msg) {
  ...
  const uint8_t *psk_buf = NULL, key_share_buf = NULL, early_data_buf = NULL;
  size_t psk_len = 0, key_share_len = 0, early_data_len = 0;
  CBS psk, key_share, early_data;

  SSL_early_callback_ext_extension_get(&early_ctx, TLSEXT_TYPE_pre_shared_key,
                                       &psk_buf, &psk_len);
  CBS_init(&psk, psk_buf, psk_len);
  SSL_early_callback_ext_extension_get(&early_ctx, TLSEXT_TYPE_key_share,
                                       &key_share_buf, &key_share_len);
  CBS_init(&key_share, key_share_buf, key_share_len);
  SSL_early_callback_ext_extension_get(&early_ctx, TLSEXT_TYPE_early_data,
