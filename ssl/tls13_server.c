...
static enum ssl_hs_wait_t do_select_parameters(SSL *ssl, SSL_HANDSHAKE *hs) {
  /* Certificate-based authentication is only allowed for non-resumption
   * handshakes. */
  ...
  /* The PRF hash is now known. Set up the key schedule, hash in the
   * ClientHello, and incorporate the PSK into the running secret. */
