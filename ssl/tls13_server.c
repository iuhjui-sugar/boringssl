...
static int tls13_send_server_hello(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  if (!CBB_init(&cbb, 0) ||
      ...
      !CBB_add_u16(&cbb, ssl_cipher_get_value(ssl->hs->cipher)) ||
...
static int tls13_send_encrypted_extensions(SSL *ssl, SSL_HS_MESSAGE *out) {
  ...
  if (!HKDF_extract(ssl->hs->xES, &ssl->hs->key_len, digest,
                    ssl->hs->premaster_secret, ssl->hs->premaster_secret_len,
                    NULL, 0) ||
  ...
  if (!tls13_update_traffic_keys(ssl, tls13_type_handshake,
                                 ...
                                 hs_hash, hs_hash_len)) {
    OPENSSL_cleanse(ssl->hs->premaster_secret, ssl->hs->premaster_secret_len);
    OPENSSL_free(ssl->hs->premaster_secret);
    return 0;
  }
  ...
  // TODO(IMPLEMENT)
  int ret = assemble_handshake_message(out, SSL3_MT_ENCRYPTED_EXTENSIONS, NULL, 0);
  return ret;
}
...
int tls13_server_handshake(SSL *ssl) {
  ...
  switch (ssl->hs->handshake_state) {
    case HS_STATE_CLIENT_HELLO:
      if (ssl->hs->in_message->type != SSL3_MT_CLIENT_HELLO) {
        result = 0;
        break;
      }
      result = tls13_receive_client_hello(ssl, *ssl->hs->in_message);
    ...
    case HS_STATE_SERVER_HELLO:
      result = tls13_send_server_hello(ssl, ssl->hs->out_message);
    ...
    case HS_STATE_SERVER_ENCRYPTED_EXTENSIONS:
      result = tls13_send_encrypted_extensions(ssl, ssl->hs->out_message);
    ...
    case HS_STATE_SERVER_CERTIFICATE_REQUEST:
      result = tls13_send_certificate_request(ssl, ssl->hs->out_message);
      ssl->hs->handshake_state = HS_STATE_SERVER_CERTIFICATE;
      ssl->hs->handshake_interrupt = HS_NEED_WRITE;
      break;
