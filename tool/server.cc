...
bool Server(const std::vector<std::string> &args) {
  ...
  do {
    ...
    if (args_map.count("-echconfig-key") != 0 &&
        !SSL_add_ech_private_key(
            ssl.get(), CBS_data(&echconfigs_body), CBS_len(&echconfigs_body),
            echconfig_key_data.data(), echconfig_key_data.size())) {
      fprintf(stderr, "Error setting ECHConfig and private key.\n");
      result = false;
      continue;
    }
