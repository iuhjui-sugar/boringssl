...
bool Server(const std::vector<std::string> &args) {
  ...
  do {
    ...
    // TODO(dmcardle): Ideally, we would have argument specify (1) a file
    // containing one ESNIKeys and (2) a file containing the sequence of
    // corresponding private keys.

    if (args_map.count("-esnikeys") != 0 && args_map.count("-esniprivs") != 0) {
      SSL_set_enable_esni_grease(ssl.get(), 1);

      std::vector<uint8_t> esnikeys_base64, esnikeys;
      if (!ReadAllFilename(&esnikeys_base64, args_map["-esnikeys"]) ||
          !DecodeEsniKeys(esnikeys_base64, &esnikeys)) {
        return false;
      }

      std::vector<uint8_t> esniprivs_base64;
      std::vector<std::vector<uint8_t>> esniprivs;
      if (!ReadAllFilename(&esniprivs_base64, args_map["-esniprivs"]) ||
          !DecodeEsniPrivs(esniprivs_base64, &esniprivs)) {
        return false;
      }
      ...
      if (!SSL_add_esni_private_keys(ssl.get(), esnikeys.data(),
                                     esnikeys.size(), priv_ptrs.data(),
                                     priv_lens.data())) {
        return false;
      }
    }
