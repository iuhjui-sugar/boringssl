...
bool Server(const std::vector<std::string> &args) {
  ...
  do {
    ...
    if (args_map.count("-echconfig-key") != 0) {
      auto *configs = SSL_ECH_SERVER_CONFIGS_new();
      ...
      if (ret != 1) {
        ...
        PrintSSLError(stderr, "Error setting ECHConfig and private key.\n",
                      ssl_err, ret);
        result = false;
        continue;
      }

      SSL_CTX_set_ech_server_configs(ctx.get(), configs);
      SSL_ECH_SERVER_CONFIGS_free(configs);
    }
