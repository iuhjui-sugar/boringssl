...
static std::unique_ptr<EVP_PKEY> LoadPrivateKey(const std::string &file) {
  ...
  std::unique_ptr<EVP_PKEY> pkey(PEM_read_bio_PrivateKey(bio.get(), nullptr, nullptr,
                                              nullptr));
  return pkey;
}
...
static std::unique_ptr<BIO> session_out;
...
bool Client(const std::vector<std::string> &args) {
  ...
  std::unique_ptr<SSL_CTX> ctx(SSL_CTX_new(SSLv23_client_method()));
  ...
  if (args_map.count("-channel-id-key") != 0) {
    std::unique_ptr<EVP_PKEY> pkey = LoadPrivateKey(args_map["-channel-id-key"]);
  ...
  std::unique_ptr<BIO> bio(BIO_new_socket(sock, BIO_CLOSE));
  ...
  if (args_map.count("-session-in") != 0) {
    std::unique_ptr<BIO> in(BIO_new_file(args_map["-session-in"].c_str(), "rb"));
    if (!in) {
      fprintf(stderr, "Error reading session\n");
      ERR_print_errors_cb(PrintErrorCallback, stderr);
      return false;
    }
    std::unique_ptr<SSL_SESSION> session(PEM_read_bio_SSL_SESSION(in.get(), nullptr,
                                                       nullptr, nullptr));
    if (!session) {
      fprintf(stderr, "Error reading session\n");
      ERR_print_errors_cb(PrintErrorCallback, stderr);
