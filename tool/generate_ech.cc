...
bool GenerateECH(const std::vector<std::string> &args) {
  ...
  unsigned max_name_len_unsigned = 0;
  if (args_map.count("-max-name-length") != 0 &&
      !GetUnsigned(&max_name_len_unsigned, "-max-name-length", 0, args_map)) {
    fprintf(stderr, "Error parsing -max-name-length argument\n");
    return false;
  }
  const size_t max_name_len = static_cast<size_t>(max_name_len_unsigned);
  ...
  if (!SSL_marshal_ech_config(&ech_config, &ech_config_len, config_id,
                              key.get(), args_map["-public-name"].c_str(),
                              max_name_len)) {
