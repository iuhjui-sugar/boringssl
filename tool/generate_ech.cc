...
bool GenerateECH(const std::vector<std::string> &args) {
  ...
  unsigned config_id_u;
  if (!GetUnsigned(&config_id_u, "-config-id", 0, args_map) ||
      config_id_u > std::numeric_limits<uint8_t>::max()) {
    fprintf(stderr, "Error parsing -config-id argument\n");
    return false;
  }

  unsigned max_name_len_u = 0;
  if (args_map.count("-max-name-length") != 0 &&
      !GetUnsigned(&max_name_len_u, "-max-name-length", 0, args_map)) {
  ...
  if (!SSL_marshal_ech_config(&ech_config, &ech_config_len,
                              static_cast<uint8_t>(config_id_u), key.get(),
                              args_map["-public-name"].c_str(),
                              size_t{max_name_len_u})) {
