...
static const struct argument kArguments[] = {
    ...
    },
    {
        "-out-private-key",
        kRequiredArgument,
        "The path where the private key corresponding to the ECHConfig should "
        "be written.",
    },
    {
        "-public-name",
    ...
    },
    {
        "-config-id",
        kRequiredArgument,
        "The u8 config ID for the new ECHConfig, from 0 to 255. Config IDs may "
        "be reused, but should be unique among active configs on a server for "
        "performance.",
    },
    {
        "-maximum-name-length",
...
bool GenerateECH(const std::vector<std::string> &args) {
  ...
  unsigned config_id_unsigned;
  if (!GetUnsigned(&config_id_unsigned, "-config-id", 0, args_map) ||
      config_id_unsigned > std::numeric_limits<uint8_t>::max()) {
    fprintf(stderr, "Error parsing -config-id argument\n");
    return false;
  }
  const uint8_t config_id = static_cast<uint8_t>(config_id_unsigned);

  unsigned maximum_name_length_unsigned = 0;
  if (args_map.count("-maximum-name-length") != 0 &&
      (!GetUnsigned(&maximum_name_length_unsigned, "-maximum-name-length", 0,
                    args_map) ||
       maximum_name_length_unsigned > std::numeric_limits<uint16_t>::max())) {
    fprintf(stderr, "Error parsing -maximum-name-length argument\n");
    return false;
  }
  const uint16_t maximum_name_length =
      static_cast<uint16_t>(maximum_name_length_unsigned);
