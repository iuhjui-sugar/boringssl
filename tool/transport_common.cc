...
void PrintConnectionInfo(const SSL *ssl) {
  ...
  if (!SSL_is_server(ssl)) {
    ...
    const uint8_t *sct_staple;
    size_t sct_staple_len;
    SSL_get0_signed_cert_timestamp_list(ssl, &sct_staple, &sct_staple_len);
    fprintf(stderr, "  SCT staple: %s\n", sct_staple_len > 0 ? "yes" : "no");
  }
