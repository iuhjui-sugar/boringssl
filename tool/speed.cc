...
static bool SpeedMLDSA(const std::string &selected) {
  if (!selected.empty() && selected != "MLDSA") {
    return true;
  }

  TimeResults results;

  auto encoded_public_key = std::make_unique<uint8_t[]>(MLDSA_PUBLIC_KEY_BYTES);
  auto priv = std::make_unique<MLDSA_private_key>();
  if (!TimeFunctionParallel(&results, [&]() -> bool {
        if (!MLDSA_generate_key(encoded_public_key.get(), nullptr,
                                priv.get())) {
          fprintf(stderr, "Failure in MLDSA_generate_key.\n");
          return false;
        }
        return true;
      })) {
    fprintf(stderr, "Failed to time MLDSA_generate_key.\n");
    return false;
  }
  ...
  auto encoded_private_key =
      std::make_unique<uint8_t[]>(MLDSA_PRIVATE_KEY_BYTES);
  CBB cbb;
  CBB_init_fixed(&cbb, encoded_private_key.get(), MLDSA_PRIVATE_KEY_BYTES);
  MLDSA_marshal_private_key(&cbb, priv.get());

  if (!TimeFunctionParallel(&results, [&]() -> bool {
        CBS cbs;
        CBS_init(&cbs, encoded_private_key.get(), MLDSA_PRIVATE_KEY_BYTES);
        if (!MLDSA_parse_private_key(priv.get(), &cbs)) {
          fprintf(stderr, "Failure in MLDSA_parse_private_key.\n");
          return false;
        }
        return true;
      })) {
    fprintf(stderr, "Failed to time MLDSA_parse_private_key.\n");
    return false;
  }
  ...
  auto out_encoded_signature =
      std::make_unique<uint8_t[]>(MLDSA_SIGNATURE_BYTES);
  if (!TimeFunctionParallel(&results, [&]() -> bool {
        if (!MLDSA_sign(out_encoded_signature.get(), priv.get(),
                        (const uint8_t *)message, message_len, nullptr, 0)) {
          fprintf(stderr, "Failure in MLDSA_sign.\n");
          return false;
        }
        return true;
      })) {
    fprintf(stderr, "Failed to time MLDSA_sign.\n");
    return false;
  }
  ...
  if (!TimeFunctionParallel(&results, [&]() -> bool {
        CBS cbs;
        CBS_init(&cbs, encoded_public_key.get(), MLDSA_PUBLIC_KEY_BYTES);
        if (!MLDSA_parse_public_key(pub.get(), &cbs)) {
          fprintf(stderr, "Failure in MLDSA_parse_public_key.\n");
          return false;
        }
        return true;
      })) {
    fprintf(stderr, "Failed to time MLDSA_parse_public_key.\n");
    return false;
  }
  ...
  if (!TimeFunctionParallel(&results, [&]() -> bool {
        if (!MLDSA_verify(pub.get(), out_encoded_signature.get(),
                          MLDSA_SIGNATURE_BYTES, (const uint8_t *)message,
    ...
    fprintf(stderr, "Failed to time MLDSA_verify.\n");
    return false;
  }
...
static bool SpeedMLKEM(const std::string &selected) {
  ...
  if (!TimeFunctionParallel(&results, [&]() -> bool {
        ...
        MLKEM_generate_key(encoded_public_key, nullptr, &priv);
        uint8_t shared_secret[MLKEM_SHARED_SECRET_BYTES];
        MLKEM_decap(shared_secret, ciphertext, sizeof(ciphertext), &priv);
        return true;
      })) {
