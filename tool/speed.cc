...
static bool SpeedDilithium(const std::string &selected) {
  ...
  if (!TimeFunctionParallel(&results, [&]() -> bool {
        DILITHIUM_sign_deterministic(out_encoded_signature, &priv,
                                     (const uint8_t *)message, message_len);
        return true;
      })) {
    fprintf(stderr, "Failed to time DILITHIUM_sign_deterministic.\n");
    return false;
  }

  results.Print("Dilithium sign (deterministic)");
