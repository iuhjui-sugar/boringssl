...
// Changes the formatting of TimeResults during printing
static bool g_machine_readable = false;
...
struct TimeResults {
  ...
  void PrintMachineReadable(const std::string &description, size_t bytes_per_call) {
    printf("+%u;%u;%lu;%s\n", num_calls, us, bytes_per_call,
           description.c_str());
  }

  void Print(const std::string &description) {
    if (g_machine_readable) {
      PrintMachineReadable(description, 0);
    } else {
  ...
  void PrintWithBytes(const std::string &description, size_t bytes_per_call) {
    if (g_machine_readable) {
      PrintMachineReadable(description, bytes_per_call);
    } else {
...
static bool SpeedRSAKeyGen(const std::string &selected) {
  ...
  for (int size : kSizes) {
    ...
    TimeResults results = {num_calls, us};
    results.Print(description);
    const size_t n = durations.size();
    assert(n > 0);

    // Do not print this if g_machine_readable is set in order to have
    // consistent results with other test output.
    if (!g_machine_readable) {
...
static const struct argument kArguments[] = {
    ...
    },
    {
        "-machine-readable",
        kBooleanArgument,
        "If this flag is set, speed will print the output of each metric in "
        "the format (semicolon-separated): \"+[number of calls];[time to "
        "complete operations in us];[bytes per call];[description]\". Note "
        "that for operations with no information about bytes per call, this "
        "format will print 0.0 for [bytes per call].",
    },
    {
        "",
        kOptionalArgument,
...
bool Speed(const std::vector<std::string> &args) {
  ...
  if (args_map.count("-machine-readable") != 0) {
    g_machine_readable = true;
  }
