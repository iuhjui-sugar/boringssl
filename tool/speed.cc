...
static bool SpeedHashToCurve(const std::string &selected) {
  ...
  results.Print("hashtocurve P521_XMD:SHA-512_SSWU_RO_");
  ...
  results.Print("hashtoscalar P521_XMD:SHA-512");
...
static bool SpeedTrustToken(std::string name, size_t batchsize,
  ...
  bssl::UniquePtr<TRUST_TOKEN> token(sk_TRUST_TOKEN_pop(tokens.get()));
  ...
  if (!TimeFunction(&results, [&]() -> bool {
        ...
        int ok = TRUST_TOKEN_CLIENT_begin_redemption(
            client.get(), &redeem_msg, &redeem_msg_len, token.get(),
            kClientData, sizeof(kClientData) - 1, kRedemptionTime);
  ...
  if (!TRUST_TOKEN_CLIENT_begin_redemption(
          client.get(), &redeem_msg, &redeem_msg_len, token.get(), kClientData,
