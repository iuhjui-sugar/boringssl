...
static std::vector<size_t> g_chunk_lens = {16, 256, 1350, 8192};
...
static bool SpeedAEADChunk(const EVP_AEAD *aead, std::string name,
  ...
  name += " (" + std::to_string(chunk_len) + " bytes)";
...
static bool SpeedAEAD(const EVP_AEAD *aead, const std::string &name,
  ...
  for (size_t chunk_len : g_chunk_lens) {
...
bool Speed(const std::vector<std::string> &args) {
  ...
  if (args_map.count("-chunks") != 0) {
    ...
    while (start != end) {
      ...
      g_chunk_lens.push_back(static_cast<size_t>(val));
      start = ptr;
      if (start != end && *start == ',') {
        start++;
      }
