...
static bool SpeedNewHope(const std::string &selected) {
  ...
  if (!TimeFunction(&results, [sk]() -> bool {
    ...
    NEWHOPE_client_compute_key(servermsg, clientmsg, client_key);
    NEWHOPE_server_compute_key(sk, clientmsg, server_key);
    return memcmp(server_key, client_key, SHA256_DIGEST_LENGTH) == 0;
  })) {
    fprintf(stderr, "newhope failed.\n");
  }
