...
static bool SpeedECDHCurve(const std::string &name, int nid,
  ...
  if (!TimeFunction(&results, [nid]() -> bool {
        ...
        BIGNUM x, y;
        ScopedBIGNUM x_storage(&x);
        ScopedBIGNUM y_storage(&y);
        BN_init(&x);
        BN_init(&y);
        ...
        if (!point ||
            ...
            !EC_POINT_get_affine_coordinates_GFp(group, point.get(), &x, &y,
                                                 ctx.get())) {
          return false;
        }
...
static bool SpeedECDSACurve(const std::string &name, int nid,
  ...
  if (!TimeFunction(&results, [&key, &signature, &digest, &sig_len]() -> bool {
        return ECDSA_sign(0, digest, 20, signature, &sig_len, key.get());
      })) {
    return false;
  }
  ...
  if (!TimeFunction(&results, [&key, &signature, &digest, sig_len]() -> bool {
        return ECDSA_verify(0, digest, 20, signature, sig_len, key.get());
      })) {
    return false;
  }
