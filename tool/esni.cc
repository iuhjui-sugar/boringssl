...
static bool PrintBase64(const std::string &label, const std::string &filename,
  ...
  bssl::UniquePtr<BIO> bio_file(BIO_new_fp(stdout, 0));
  if (!filename.empty()) {
    print_labels = false;
    bio_file.reset(BIO_new_file(filename.c_str(), "w"));
    if (!bio_file)
      return false;
  }
  ...
  if (!EVP_EncodeBlock(encoded.data(), CBB_data(bytes), CBB_len(bytes)) ||
      (print_labels && BIO_printf(bio_file.get(), "%s [%zu]: ", label.c_str(),
                                  encoded.size()) < 0) ||
      BIO_write_all(bio_file.get(), encoded.data(), encoded.size()) < 0 ||
      (print_labels && BIO_printf(bio_file.get(), "\n") < 0)) {
    return false;
  }
...
static bool DecodeBase64(const std::vector<uint8_t> &encoded,
                         std::vector<uint8_t> *out) {
  size_t decoded_len, actual_decoded_len;

  std::vector<uint8_t> trimmed(encoded);
  while (!trimmed.empty() && trimmed.back() == '\0') {
    trimmed.pop_back();
  }

  if (!EVP_DecodedLength(&decoded_len, trimmed.size())) {
    return false;
  }
  out->resize(decoded_len);
  if (!EVP_DecodeBase64(out->data(), &actual_decoded_len, out->size(),
                        reinterpret_cast<const uint8_t *>(trimmed.data()),
                        trimmed.size())) {
    return false;
  }
  out->resize(actual_decoded_len);
  return true;
}

bool DecodeEsniKeys(const std::vector<uint8_t> &encoded, std::vector<uint8_t> *out) {
  return DecodeBase64(encoded, out);
}

bool DecodeEsniPrivs(const std::vector<uint8_t> &encoded,
                     std::vector<std::vector<uint8_t>> *out_privs) {
  std::vector<uint8_t> decoded;
  if (!DecodeBase64(encoded, &decoded)) {
    fprintf(stderr, "Failed to decode ESNI private keys\n");
    return false;
  }

  CBS privs;
  CBS_init(&privs, decoded.data(), decoded.size());
