...
int HKDF(uint8_t *out_key, size_t out_len,
  ...
  if (!HKDFExtract(prk, &prk_len, digest, secret, secret_len, salt,
                   salt_len) ||
      !HKDFExpand(out_key, out_len, digest, prk, prk_len, info,
                  info_len)) {
    return 0;
  }
...
int HKDFExtract(uint8_t *out_key, size_t *out_len, const EVP_MD *digest,
                const uint8_t *secret, size_t secret_len,
                const uint8_t *salt, size_t salt_len) {
  ...
  *out_len = 0;
  if (HMAC(digest, salt, salt_len, secret, secret_len, out_key,
           (unsigned *)out_len) == NULL) {
    goto out;
  }
...
int HKDFExpand(uint8_t *out_key, size_t out_len, const EVP_MD *digest,
