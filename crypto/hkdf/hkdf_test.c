...
int main(void) {
  ...
  for (i = 0; i < sizeof(kTests) / sizeof(kTests[0]); i++) {
    ...
    if (!HKDFExtract(prk, &prk_len, test->md_func(), test->ikm, test->ikm_len,
                     test->salt, test->salt_len)) {
      fprintf(stderr, "Call to HKDFExtract failed\n");
    ...
    if (prk_len != test->prk_len ||
      ...
      fprintf(stderr,
              "%zu: Resulting PRK does not match test vector\n", i);
      return 1;
    }
    if (!HKDFExpand(buf, test->out_len, test->md_func(), prk, prk_len,
                    test->info, test->info_len)) {
      fprintf(stderr, "Call to HKDFExpand failed\n");
      ERR_print_errors_fp(stderr);
      return 1;
    }
