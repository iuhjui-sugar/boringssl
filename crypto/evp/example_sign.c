...
int main(void) {
  ...
  if (!test_d2i_AutoPrivateKey(kExampleRSAKeyDER, sizeof(kExampleRSAKeyDER),
                               EVP_PKEY_RSA)) {
    fprintf(stderr, "test_d2i_AutoPrivateKey(kExampleRSAKeyDER) failed\n");
  }

  if (!test_d2i_AutoPrivateKey(kExampleRSAKeyPKCS8, sizeof(kExampleRSAKeyPKCS8),
                               EVP_PKEY_RSA)) {
    fprintf(stderr, "test_d2i_AutoPrivateKey(kExampleRSAKeyPKCS8) failed\n");
  }

  if (!test_d2i_AutoPrivateKey(kExampleECKeyDER, sizeof(kExampleECKeyDER),
                               EVP_PKEY_EC)) {
    fprintf(stderr, "test_d2i_AutoPrivateKey(kExampleECKeyDER) failed\n");
    return 1;
  }
