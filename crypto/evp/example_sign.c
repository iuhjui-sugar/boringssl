...
static int example_EVP_DigestSignAlgorithm(void) {
  int ret = 0;
  EVP_PKEY *pkey = NULL;
  EVP_MD_CTX md_ctx;
...
static int example_EVP_DigestVerifyInitFromAlgorithm(void) {
  ...
  /* Signatures are BIT STRINGs, but they have a multiple of 8 bytes, so there
   * is a leading 0. */
...
int main(void) {
  ...
  if (!example_EVP_DigestSignAlgorithm()) {
    fprintf(stderr, "EVP_DigestSignInit failed\n");
    return 1;
  }
