...
/* salsa208_word_specification implements the algorithm described in RFC 7914,
 * section 3. It modifies the block at |inout| in-place. */
...
/* xor_blocks sets |*c| to be |*a| XOR |*b|. */
static inline void xor_blocks(block_t *c, const block_t *a, const block_t *b) {
 ...
 * block or 2 * |r| Salsa20 blocks long. */
...
/* scryptROMix implements the function described in RFC 7914, section 5. |T|
 * and |V| are scratch space allocated by the caller. |T| must have space for
 * one scrypt block or 2 * |r| Salsa20 blocks. |V| must have space for |N|
 * scrypt blocks or 2 * |r| * |N| Salsa20 blocks. |B| is an array of 128 * |r|
 * bytes (an scrypt block) and is modified in-place. */
static void scryptROMix(block_t *B, uint64_t r, uint64_t N, block_t *T,
  ...
  for (uint64_t i = 0; i < N; i++) {
    /* Note this assumes |N| < 2^32. */
    uint32_t j = B[2 * r - 1].words[0] % N;
    for (size_t k = 0; k < 2 * r; k++) {
      xor_blocks(&T[k], &B[k], &V[2 * r * j + k]);
    }
...
int EVP_PBE_scrypt(const char *password, size_t password_len,
                   ...
                   uint64_t p, uint64_t max_mem, uint8_t *out_key,
                   size_t key_len) {
  if (r == 0 || p == 0 || p > SCRYPT_PR_MAX / r ||
      /* |N| must be a power of two. */
      N < 2 || (N & (N - 1)) ||
      /* We only support 32-bit |N| in |scryptROMix|. */
      N > UINT32_MAX ||
  ...
  OPENSSL_COMPILE_ASSERT(UINT64_MAX >= ((size_t)-1),
                         max_mem_may_not_fit_in_u64);
  ...
  /* Allocate and divide up the scratch space. */
  size_t B_blocks = p * 2 * r;
  size_t B_bytes = B_blocks * sizeof(block_t);
  size_t T_blocks = 2 * r;
  size_t V_blocks = N * 2 * r;
  size_t alloc_size = (B_blocks + T_blocks + V_blocks) * sizeof(block_t);
  block_t *B = OPENSSL_malloc(alloc_size);
  if (B == NULL) {
