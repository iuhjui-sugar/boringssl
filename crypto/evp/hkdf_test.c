...
int main(void) {
  ...
  for (i = 0; i < NUM_TESTS; i++) {
    ...
    if (!HKDF(test.ikm, test.ikm_len, test.salt, test.salt_len, test.info,
              test.info_len, test.md_func(), test.l, buf)) {
    ...
    if (memcmp(buf, test.okm, test.l) != 0) {
      fprintf(stderr, "Resulting key material does not match test vector %d\n",
              i);
      ERR_free_strings();
      return 1;
    }
