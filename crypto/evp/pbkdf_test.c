...
static int test_empty_password(void) {
  ...
  if (!test_PBKDF2("", 0, "salt", 4, 1, EVP_sha1(), sizeof(kKey), kKey)) {
    return 0;
  }
...
static int test_empty_salt(void) {
  ...
  if (!test_PBKDF2("password", 8, "", 0, 2, EVP_sha256(), sizeof(kKey), kKey)) {
    return 0;
  }
...
static int test_rfc6070_vectors(void) {
  ...
  if (!test_PBKDF2("password", 8, "salt", 4, 2, EVP_sha1(), sizeof(kKey2),
                   kKey2)) {
    return 0;
  }
  ...
  if (!test_PBKDF2("pass\0word", 9, "sa\0lt", 5, 4096, EVP_sha1(),
                   sizeof(kKey3), kKey3)) {
    return 0;
  }
...
static int test_sha2(void) {
  ...
  if (!test_PBKDF2("passwordPASSWORDpassword", 24,
