...
static std::unique_ptr<EVP_PKEY> LoadExampleRSAKey() {
  std::unique_ptr<RSA> rsa(RSA_private_key_from_bytes(kExampleRSAKeyDER,
...
static bool TestEVP_DigestSignInit(void) {
  std::unique_ptr<EVP_PKEY> pkey = LoadExampleRSAKey();
...
static bool TestVerifyRecover() {
  ...
  std::unique_ptr<RSA> rsa(EVP_PKEY_get1_RSA(pkey.get()));
  if (!rsa) {
    return false;
  }
  ...
  size_t out_len;
  std::unique_ptr<EVP_PKEY_CTX> ctx(EVP_PKEY_CTX_new(pkey.get(), nullptr));
...
static bool TestValidPrivateKey(const uint8_t *input, size_t input_len,
  ...
  std::unique_ptr<EVP_PKEY> pkey(d2i_AutoPrivateKey(NULL, &p, input_len));
...
static bool Testd2i_AutoPrivateKey() {
  ...
  std::unique_ptr<EVP_PKEY> pkey(d2i_AutoPrivateKey(NULL, &p, sizeof(kInvalidPrivateKey)));
  if (pkey) {
...
static bool TestEVP_PKCS82PKEY(void) {
  ...
  std::unique_ptr<PKCS8_PRIV_KEY_INFO> p8inf(
  ...
  std::unique_ptr<EVP_PKEY> pkey(EVP_PKCS82PKEY(p8inf.get()));
  if (pkey) {
...
static bool TestEVPMarshalEmptyPublicKey(void) {
  std::unique_ptr<EVP_PKEY> empty(EVP_PKEY_new());
  if (!empty) {
    return false;
  }
  ScopedCBB cbb;
...
static bool Testd2i_PrivateKey(void) {
  ...
  std::unique_ptr<EVP_PKEY> pkey(d2i_PrivateKey(EVP_PKEY_RSA, nullptr, &derp,
                                     sizeof(kExampleRSAKeyDER)));
