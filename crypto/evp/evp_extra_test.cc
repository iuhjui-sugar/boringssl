...
static bssl::unique_ptr<EVP_PKEY> LoadExampleRSAKey() {
  bssl::unique_ptr<RSA> rsa(RSA_private_key_from_bytes(kExampleRSAKeyDER,
                                           sizeof(kExampleRSAKeyDER)));
  if (!rsa) {
    return nullptr;
  }
  bssl::unique_ptr<EVP_PKEY> pkey(EVP_PKEY_new());
...
static bool TestEVP_DigestSignInit(void) {
  bssl::unique_ptr<EVP_PKEY> pkey = LoadExampleRSAKey();
...
static bool TestVerifyRecover() {
  ...
  bssl::unique_ptr<RSA> rsa(EVP_PKEY_get1_RSA(pkey.get()));
  if (!rsa) {
    return false;
  }
  ...
  size_t out_len;
  bssl::unique_ptr<EVP_PKEY_CTX> ctx(EVP_PKEY_CTX_new(pkey.get(), nullptr));
...
static bool TestValidPrivateKey(const uint8_t *input, size_t input_len,
  ...
  bssl::unique_ptr<EVP_PKEY> pkey(d2i_AutoPrivateKey(NULL, &p, input_len));
...
static bool Testd2i_AutoPrivateKey() {
  ...
  bssl::unique_ptr<EVP_PKEY> pkey(d2i_AutoPrivateKey(NULL, &p, sizeof(kInvalidPrivateKey)));
  if (pkey) {
...
static bool TestEVP_PKCS82PKEY(void) {
  ...
  bssl::unique_ptr<PKCS8_PRIV_KEY_INFO> p8inf(
  ...
  bssl::unique_ptr<EVP_PKEY> pkey(EVP_PKCS82PKEY(p8inf.get()));
  if (pkey) {
...
static bool TestEVPMarshalEmptyPublicKey(void) {
  bssl::unique_ptr<EVP_PKEY> empty(EVP_PKEY_new());
  if (!empty) {
    return false;
  }
  ScopedCBB cbb;
...
static bool Testd2i_PrivateKey(void) {
  ...
  bssl::unique_ptr<EVP_PKEY> pkey(d2i_PrivateKey(EVP_PKEY_RSA, nullptr, &derp,
                                     sizeof(kExampleRSAKeyDER)));
