...
TEST(EVPExtraTest, MarshalEmptyPublicKey) {
  ...
  EXPECT_TRUE(ErrorMatches(ERR_peek_last_error(), ERR_LIB_EVP, EVP_R_UNSUPPORTED_ALGORITHM));
}
...
TEST(EVPExtraTest, Ed25519) {
  ...
  EXPECT_TRUE(ErrorMatches(ERR_get_error(), ERR_LIB_EVP, EVP_R_BUFFER_TOO_SMALL));
  ERR_clear_error();

  // There is no private key.
  EXPECT_FALSE(EVP_PKEY_get_raw_private_key(pubkey.get(), nullptr, &len));
  EXPECT_TRUE(ErrorMatches(ERR_get_error(), ERR_LIB_EVP, EVP_R_NOT_A_PRIVATE_KEY));
  ERR_clear_error();
