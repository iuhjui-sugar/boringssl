...
typedef size_t CounterArray[fips_counter_max + 1];
...
static bool counter_delta_is_zero_except_for_a_one_at(CounterArray before,
                                                      CounterArray after,
                                                      fips_counter_t position) {
  for (fips_counter_t counter = static_cast<fips_counter_t>(0);
       counter <= fips_counter_max;
       counter = static_cast<fips_counter_t>(counter + 1)) {
    const size_t expected_delta = counter == position ? 1 : 0;
    if (after[counter] != before[counter] + expected_delta) {
      return false;
    }
  }
  return true;
...
TEST(CryptoTest, FIPSCountersEVP) {
  ...
  for (const auto &test : kTests) {
    ...
    ASSERT_TRUE(
        counter_delta_is_zero_except_for_a_one_at(before, after, test.counter));
  }
}
  ...
  constexpr struct {
    ...
    fips_counter_t counter;
  } kTests[] = {
      {
          ...
          16,
          fips_counter_evp_aead_aes_128_gcm,
      },
      {
          EVP_aead_aes_256_gcm,
          32,
          fips_counter_evp_aead_aes_256_gcm,
      },
  };
