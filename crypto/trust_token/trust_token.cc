...
bool TRUST_TOKEN_client_set_srr_key(TT_CTX *ctx, const CBS key) {
  // TODO: Implement.
  return true;
}

bool TRUST_TOKEN_issuer_set_srr_key(TT_CTX *ctx, const CBS key) {
  // TODO: Implement.
  return true;
}
...
bool TRUST_TOKEN_issuer_perform_redemption(
  ...
  CBS inner(request);
  uint16_t inner_len;
  if (!CBS_get_u16(&inner, &inner_len)) {
    return false;
  }
...
bool TRUST_TOKEN_client_finish_redemption(TT_CTX *ctx, bool *result, const CBS response) {
  CBS in(response);
  uint8_t res;
