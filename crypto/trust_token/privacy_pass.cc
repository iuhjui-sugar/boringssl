...
static bool privacy_pass_client_begin_issuance(TT_CTX *ctx, uint8_t **out, size_t *out_len, size_t count) {
  PP_CTX *cctx = (PP_CTX*)ctx->protocol;
  ...
  printf("PrivacyPass issuance begin.\n");

  return CBB_finish(&request, out, out_len);
}

static bool privacy_pass_issuer_do_issuance(TT_CTX *ctx, uint8_t **out, size_t *out_len, const CBS request) {
  PP_CTX *cctx = (PP_CTX*)ctx->protocol;
...
static bool privacy_pass_client_finish_issuance(TT_CTX *ctx, STACK_OF(TRUST_TOKEN) **out_tokens, const CBS response) {
  PP_CTX *cctx = (PP_CTX*)ctx->protocol;
...
static bool privacy_pass_client_begin_redemption(TT_CTX *ctx, uint8_t **out, size_t *out_len, const TRUST_TOKEN *token) {
  if (token->len != 2) {
    return false;
  }
  CBB request;
...
static bool privacy_pass_issuer_set_metadata(TT_CTX *ctx, uint8_t public_metadata, bool private_metadata) {
...
static bool privacy_pass_issuer_do_redemption(TT_CTX *ctx, bool *result, const CBS request) {
  PP_CTX *cctx = (PP_CTX*)ctx->protocol;
...
TT_CTX *TRUST_TOKEN_privacy_pass_init_issuer(
    uint16_t max_batchsize) {
  TT_CTX *ret = (TT_CTX *)OPENSSL_malloc(sizeof(TT_CTX));
  ret->method = TRUST_TOKEN_PrivacyPassProtocol();
