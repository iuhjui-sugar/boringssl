...
static int privacy_pass_init_client(TRUST_TOKEN_CLIENT *ctx) {
  ctx->protocol = (PRIVACY_PASS_CTX *)OPENSSL_malloc(sizeof(PRIVACY_PASS_CTX));
  // TODO: Make public_keys map.
  return 1;
}

static int privacy_pass_init_issuer(TRUST_TOKEN_ISSUER *ctx) {
  ctx->protocol = (PRIVACY_PASS_CTX *)OPENSSL_malloc(sizeof(PRIVACY_PASS_CTX));
  return 1;
}

static void privacy_pass_free_client(TRUST_TOKEN_CLIENT *ctx) {
  OPENSSL_free(ctx->protocol);
}

static void privacy_pass_free_issuer(TRUST_TOKEN_ISSUER *ctx) {
  OPENSSL_free(ctx->protocol);
}
...
static STACK_OF(TRUST_TOKEN) *
    privacy_pass_client_finish_issuance(TRUST_TOKEN_CLIENT *ctx, const CBS response) {
  PRIVACY_PASS_CTX *cctx = (PRIVACY_PASS_CTX*)ctx->protocol;
...
int TRUST_TOKEN_privacy_pass_init_key(uint8_t *out_priv_key,
                                      size_t *out_priv_key_len,
                                      size_t max_out_priv_key_len,
                                      uint8_t *out_pub_key,
                                      size_t *out_pub_key_len,
                                      size_t max_out_pub_key_len,
                                      uint16_t version) {
  EC_KEY *key = VOPRF_Setup(0x4242);
  CBB cbb;
  if (!CBB_init_fixed(&cbb, out_priv_key, max_out_priv_key_len) ||
  ...
  if (!CBB_init_fixed(&cbb, out_pub_key, max_out_pub_key_len) ||
...
// int TRUST_TOKEN_privacy_pass_init_private_metadata_key(
...
static const TRUST_TOKEN_CLIENT_METHOD kPrivacy_PassTrustTokenClientMethod = {
    privacy_pass_init_client,
    privacy_pass_free_client,
...
static const TRUST_TOKEN_ISSUER_METHOD kPrivacy_PassTrustTokenIssuerMethod = {
    privacy_pass_init_issuer,
    privacy_pass_free_issuer,
    privacy_pass_issuer_set_metadata,
    privacy_pass_issuer_issue,
    privacy_pass_issuer_redeem
};

TRUST_TOKEN_CLIENT *TRUST_TOKEN_CLIENT_new_privacy_pass(uint16_t max_batchsize) {
...
int TRUST_TOKEN_CLIENT_privacy_pass_add_key(TRUST_TOKEN_CLIENT *ctx,
                                            const uint8_t *key,
                                            size_t key_len) {
  PRIVACY_PASS_CTX *cctx = (PRIVACY_PASS_CTX*)ctx->protocol;
...
TRUST_TOKEN_ISSUER *TRUST_TOKEN_ISSUER_new_privacy_pass(uint16_t max_batchsize) {
...
int TRUST_TOKEN_ISSUER_privacy_pass_add_key(TRUST_TOKEN_ISSUER *ctx,
                                            const uint8_t *key,
                                            size_t key_len) {
  PRIVACY_PASS_CTX *cctx = (PRIVACY_PASS_CTX*)ctx->protocol;
  // TODO: Add key to TT_CTX->P_CTX
