...
static int generate_keypair_from_secret(const PMBTOKEN_METHOD *method,
                                        EC_SCALAR *out_x, EC_SCALAR *out_y,
                                        EC_RAW_POINT *out_pub,
                                        const uint8_t *secret,
                                        size_t secret_len, uint8_t pair_id) {
  static const uint8_t kKeygenXLabel[] = "TrustTokenPMBTokenKeyGenX";
  static const uint8_t kKeygenYLabel[] = "TrustTokenPMBTokenKeyGenY";
  ...
  size_t len;
  if (!CBB_init(&cbb, 0) ||
      !CBB_add_bytes(&cbb, kKeygenXLabel, sizeof(kKeygenXLabel)) ||
      !CBB_add_bytes(&cbb, secret, secret_len) ||
      !CBB_add_u8(&cbb, pair_id) ||
      !CBB_finish(&cbb, &buf, &len) ||
      !method->hash_to_scalar(method->group, out_x, buf, len)) {
  ...
  CBB_zero(&cbb);
  if (!CBB_init(&cbb, 0) ||
      !CBB_add_bytes(&cbb, kKeygenYLabel, sizeof(kKeygenYLabel)) ||
      !CBB_add_bytes(&cbb, secret, secret_len) ||
      !CBB_finish(&cbb, &buf, &len) ||
      !method->hash_to_scalar(method->group, out_y, buf, len)) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_KEYGEN_FAILURE);
    goto err;
  }


  if (!ec_point_mul_scalar_precomp(method->group, out_pub, &method->g_precomp,
                                   out_x, &method->h_precomp, out_y, NULL,
                                   NULL)) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
    goto err;
...
static int pmbtoken_generate_key_from_secret(const PMBTOKEN_METHOD *method,
                                             CBB *out_private, CBB *out_public,
                                             const uint8_t *secret,
                                             size_t secret_len) {
  const EC_GROUP *group = method->group;
  EC_RAW_POINT pub[3];
  EC_SCALAR x0, y0, x1, y1, xs, ys;
  if (!generate_keypair_from_secret(method, &x0, &y0, &pub[0], secret,
                                    secret_len, 0) ||
      !generate_keypair_from_secret(method, &x1, &y1, &pub[1], secret,
                                    secret_len, 1) ||
      !generate_keypair_from_secret(method, &xs, &ys, &pub[2], secret,
                                    secret_len, 2)) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_KEYGEN_FAILURE);
    return 0;
  }
...
int pmbtoken_exp1_generate_key_from_secret(CBB *out_private, CBB *out_public,
                                           const uint8_t *secret,
                                           size_t secret_len) {
  if (!pmbtoken_exp1_init_method()) {
    return 0;
  ...
  return pmbtoken_generate_key_from_secret(&pmbtoken_exp1_method, out_private,
...
int pmbtoken_exp2_generate_key_from_secret(CBB *out_private, CBB *out_public,
                                           const uint8_t *secret,
                                           size_t secret_len) {
  if (!pmbtoken_exp2_init_method()) {
    return 0;
  ...
  return pmbtoken_generate_key_from_secret(&pmbtoken_exp2_method, out_private,
                                           out_public, secret, secret_len);
}
