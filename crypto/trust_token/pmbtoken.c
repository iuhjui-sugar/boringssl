...
static int hash_to_curve(EC_GROUP *group, EC_RAW_POINT *out, uint8_t *in, size_t len) {
  ...
  uint8_t *tmp_buf = (uint8_t *)OPENSSL_malloc(len+4);
  if (tmp_buf == NULL) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
    return 0;
  }
  OPENSSL_memcpy(tmp_buf + 4, in, len);

  OPENSSL_memcpy(tmp_buf, "HTC1", 4);
  SHA512(tmp_buf, len+4, hash_buf);

  OPENSSL_memcpy(tmp_buf, "HTC2", 4);
  SHA512(tmp_buf, len+4, hash_buf+64);
...
int pmbtoken_sign(TRUST_TOKEN_ISSUER *ctx, uint8_t out_s[PMBTOKEN_NONCE_SIZE],
  ...
  for (size_t index = 0; index < ctx->key_index; index++) {
    if (ctx->keys[index].id == ctx->public_metadata) {
      key = ctx->keys[index];
    }
  }
