...
// |out_x| and |out_y| are set to the secret half of the keypair. It returns one
...
static int derive_keypair_from_secret(const PMBTOKEN_METHOD *method,
...
static int pmbtoken_generate_key(const PMBTOKEN_METHOD *method,
  ...
  if (!generate_keypair(method, &x0, &y0) ||
      !generate_keypair(method, &x1, &y1) ||
      !generate_keypair(method, &xs, &ys)) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_KEYGEN_FAILURE);
    return 0;
  }
...
static int pmbtoken_derive_key_from_secret(const PMBTOKEN_METHOD *method,
                                           ...
                                           size_t secret_len) {
  EC_SCALAR x0, y0, x1, y1, xs, ys;
  if (!derive_keypair_from_secret(method, &x0, &y0, secret, secret_len, 0) ||
      !derive_keypair_from_secret(method, &x1, &y1, secret, secret_len, 1) ||
      !derive_keypair_from_secret(method, &xs, &ys, secret, secret_len, 2)) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_KEYGEN_FAILURE);
    return 0;
  }
