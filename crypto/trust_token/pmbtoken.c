...
static int cbs_get_prefixed_point(CBS *cbs, const EC_GROUP *group,
  ...
  if (prefix_point) {
    ...
    if (!ec_point_from_uncompressed(group, out, CBS_data(cbs), plen) ||
        !CBS_skip(cbs, plen)) {
      return 0;
    }
  }
...
static int pmbtoken_client_key_from_bytes(const PMBTOKEN_METHOD *method,
  ...
  CBS cbs;
  CBS_init(&cbs, in, len);
  if (!cbs_get_prefixed_point(&cbs, method->group, &key->pub0, method->prefix_point) ||
      !cbs_get_prefixed_point(&cbs, method->group, &key->pub1, method->prefix_point) ||
      !cbs_get_prefixed_point(&cbs, method->group, &key->pubs, method->prefix_point) ||
      CBS_len(&cbs) != 0) {
...
static STACK_OF(TRUST_TOKEN) *
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (!CBS_copy_bytes(cbs, s, PMBTOKEN_NONCE_SIZE) ||
        ...
        !cbs_get_prefixed_point(cbs, group, &Wsp_affine, method->prefix_point)) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_DECODE_FAILURE);
      goto err;
    }
    ...
    if (!CBB_init(&token_cbb, 4 + PMBTOKEN_NONCE_SIZE + 3 * (2 + point_len)) ||
        ...
        !cbb_add_prefixed_point(&token_cbb, group, &affines[0], method->prefix_point) ||
        !cbb_add_prefixed_point(&token_cbb, group, &affines[1], method->prefix_point) ||
        !cbb_add_prefixed_point(&token_cbb, group, &affines[2], method->prefix_point) ||
        !CBB_flush(&token_cbb)) {
