...
static int hash_t(EC_GROUP *group, EC_RAW_POINT *out,
  ...
  const uint8_t kHashTLabel[9] = "PMBTHashT";
...
static int hash_s(EC_GROUP *group, EC_RAW_POINT *out, const EC_RAW_POINT *t,
  ...
  const uint8_t kHashSLabel[9] = "PMBTHashS";
  int ok = 1;
  CBB cbb, tmp_cbb;
  uint8_t *buf = NULL;
  size_t len;
...
static int do_scalar_mul(const EC_GROUP *group, EC_RAW_POINT *out,
...
int pmbtoken_read(const TRUST_TOKEN_ISSUER *ctx, uint8_t *out_private_metadata,
  ...
  if (!do_scalar_mul(group, &calculated, &T, &key.xs, &token->S, &key.ys) ||
      ec_GFp_simple_cmp(group, &calculated, &token->Ws) != 0) {
    return 0;
  }
  ...
  *out_private_metadata = 2;
  ...
  if (do_scalar_mul(group, &calculated, &T, &key.x0, &token->S, &key.y0) &&
      ec_GFp_simple_cmp(group, &calculated, &token->W) == 0) {
    *out_private_metadata = 0;
  }

  // Compare the private metadata bit against a value of '1'.
  if (do_scalar_mul(group, &calculated, &T, &key.x1, &token->S, &key.y1) &&
      ec_GFp_simple_cmp(group, &calculated, &token->W) == 0) {
