...
static bool clear_issuer_do_issuance(TT_CTX *ctx, uint8_t **out, size_t *out_len, const CBS request) {
  CLEAR_CTX *cctx = (CLEAR_CTX*)ctx->protocol;
...
static bool clear_client_finish_issuance(TT_CTX *ctx, STACK_OF(TRUST_TOKEN) **out_tokens, const CBS response) {
  CLEAR_CTX *cctx = (CLEAR_CTX*)ctx->protocol;
...
static bool clear_client_begin_redemption(TT_CTX *ctx, uint8_t **out, size_t *out_len, const TRUST_TOKEN *token) {
  if (token->len != 2) {
    return false;
  }
  CBB request;
...
static bool clear_issuer_set_metadata(TT_CTX *ctx, uint8_t public_metadata, bool private_metadata) {
  return false;
}
static bool clear_issuer_do_redemption(TT_CTX *ctx, bool *result, const CBS request) {
  CLEAR_CTX *cctx = (CLEAR_CTX*)ctx->protocol;
