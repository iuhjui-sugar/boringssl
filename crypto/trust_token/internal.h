...
int voprf_exp2_generate_key_from_secret(CBB *out_private, CBB *out_public,
                                        const uint8_t *secret,
                                        size_t secret_len);
...
struct trust_token_method_st {
  ...
  // generate_key_from_secret deterministically derives a keypair based on
  // |secret| and writes their serialized forms into |out_private| and
  // |out_public|. It returns one on success and zero on failure.
  int (*generate_key_from_secret)(CBB *out_private, CBB *out_public,
