...
// pmbtoken_sign signs |request| with |key| and a private metadata value of
// |private_metadata|. On success, it returns one and writes the response to
// |out_response| and sets |*out_proof| to a newly-allocated buffer containing
// the DLEQ proofs. Otherwise, it returns zero. The caller takes ownership of
// |*out_proof| and is responsible for freeing it using |OPENSSL_free|.
//
...
// pmbtoken_unblind processes an issuance response and accompanying proof.
// |pretoken| is the pre-token returned from the corresponding |pmbtoken_blind|
// call. On success, it returns one and writes the resulting token to
// |out_token|. Otherwise, it returns zero.
//
