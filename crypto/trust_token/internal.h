...
struct TRUST_TOKEN_CLIENT_METHOD {
  int (*init)(TRUST_TOKEN_CLIENT *ctx);
  void (*free)(TRUST_TOKEN_CLIENT *ctx);
  int (*begin_issuance)(TRUST_TOKEN_CLIENT *ctx, uint8_t **out, size_t *out_len,
                        size_t count);
  STACK_OF(TRUST_TOKEN) *
      (*finish_issuance)(TRUST_TOKEN_CLIENT *ctx, const CBS response);
...
struct TRUST_TOKEN_ISSUER_METHOD {
  int (*init)(TRUST_TOKEN_ISSUER *ctx);
  void (*free)(TRUST_TOKEN_ISSUER *ctx);
  ...
  int (*issue)(TRUST_TOKEN_ISSUER *ctx, uint8_t **out, size_t *out_len,
               const CBS request);
  int (*redeem)(TRUST_TOKEN_ISSUER *ctx, int *result, const CBS request);
};
