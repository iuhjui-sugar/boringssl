...
struct TRUST_TOKEN_METHOD {
  ...
  bool (*issuer_do_issuance)(TT_CTX *ctx, uint8_t **out, size_t *out_len, const CBS request);
  bool (*client_finish_issuance)(TT_CTX *ctx, STACK_OF(TRUST_TOKEN) **out_tokens, const CBS response);
  bool (*client_begin_redemption)(TT_CTX *ctx, uint8_t **out, size_t *out_len, const TRUST_TOKEN *token);
  bool (*issuer_set_metadata)(TT_CTX *ctx, uint8_t public_metadata, bool private_metadata);
