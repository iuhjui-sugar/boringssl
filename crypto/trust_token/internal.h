...
int pmbtoken_exp1_read(const TRUST_TOKEN_ISSUER_KEY *key,
                       ...
                       size_t token_len, const uint8_t *msg, size_t msg_len);
...
struct trust_token_method_st {
  ...
  // blind generates a new issuance request for |count| tokens. If |msg| is not
  // NULL, the nonce used for these tokens are derived from it. On success, it
  // returns a newly-allocated |STACK_OF(TRUST_TOKEN_PRETOKEN)| and writes a
  // request to the issuer to |cbb|. On failure, it returns NULL. The
  ...
  // read parses a token from |token| and verifies it using |key|. If |msg|
  // is not NULL, the nonce is derived from it and the salt in the token. On
  // success, it returns one and stores the nonce and private metadata
  // bit in |out_nonce| and |*out_private_metadata|. Otherwise, it
