...
int TRUST_TOKEN_CLIENT_begin_issuance(TRUST_TOKEN_CLIENT *ctx, uint8_t **out,
  ...
  for (size_t i = 0; i < count; i++) {
    uint8_t *ptr;
    if (!CBB_add_space(&request, &ptr, PMBTOKEN_REQUEST_SIZE)) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
      goto err;
    }
    PMBTOKEN_PRETOKEN *pretoken = pmbtoken_blind(ptr);
    if (pretoken == NULL ||
...
STACK_OF(TRUST_TOKEN) *TRUST_TOKEN_CLIENT_finish_issuance(
  ...
  size_t tokens_len;
  if (!pmbtoken_unblind(&key->key, NULL, &tokens_len, ctx->pretokens,
                        CBS_data(&in), CBS_len(&in), count)) {
    goto err;
  }
  tokens_ptr = OPENSSL_malloc(tokens_len);
  if (!tokens_ptr) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
    goto err;
  }
  if (!pmbtoken_unblind(&key->key, tokens_ptr, &tokens_len, ctx->pretokens,
                        CBS_data(&in), CBS_len(&in), count)) {
    goto err;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (!CBB_init(&token_cbb, 0) ||
        ...
        !CBB_add_bytes(&token_cbb, tokens_ptr + i * PMBTOKEN_TOKEN_SIZE,
                       PMBTOKEN_TOKEN_SIZE)) {
      goto token_err;
    }
...
static const struct trust_token_issuer_key_st *trust_token_issuer_get_key(
    const TRUST_TOKEN_ISSUER *ctx, uint32_t public_metadata) {
  for (size_t i = 0; i < ctx->num_keys; i++) {
...
int TRUST_TOKEN_ISSUER_issue(const TRUST_TOKEN_ISSUER *ctx, uint8_t **out,
  ...
  uint8_t *ptr;
  size_t response_len;
  if (!pmbtoken_sign(&key->key, NULL, &response_len, CBS_data(&in),
                     CBS_len(&in), count, private_metadata) ||
      !CBB_add_space(&response, &ptr, response_len) ||
      !pmbtoken_sign(&key->key, ptr, &response_len, CBS_data(&in), CBS_len(&in),
                     count, private_metadata)) {
    goto err;
  }
...
int TRUST_TOKEN_ISSUER_redeem(const TRUST_TOKEN_ISSUER *ctx, uint8_t **out,
  ...
  if (!CBS_get_u32(&token_cbs, &public_metadata) ||
      !CBS_get_bytes(&token_cbs, &pmbtoken, PMBTOKEN_TOKEN_SIZE) ||
      CBS_len(&token_cbs) != 0) {
  ...
  if (key == NULL ||
      !pmbtoken_read(&key->key, nonce, &private_metadata, CBS_data(&pmbtoken))) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_INVALID_TOKEN);
    return 0;
  }
