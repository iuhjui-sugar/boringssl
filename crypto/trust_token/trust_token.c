...
STACK_OF(TRUST_TOKEN) *
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (!pmbtoken_unblind(&token, s, &Wp, &Wsp, pretoken)) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_DECODE_FAILURE);
      goto err;
    }
    ...
    TRUST_TOKEN *trust_token = TRUST_TOKEN_new(CBB_data(&token_cbb), CBB_len(&token_cbb));
    if (trust_token == NULL) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
      goto token_err;
    }

    if (!sk_TRUST_TOKEN_push(tokens, trust_token)) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
      goto token_err;
    }
    token_ok = 1;
...
uint8_t TRUST_TOKEN_decode_private_metadata(const uint8_t *key, size_t key_len,
                                            const uint8_t *client_data,
                                            size_t client_data_len,
                                            uint8_t encrypted_bit) {
  uint8_t metadata_obfuscator =
      get_metadata_obfuscator(key, key_len, client_data, client_data_len);
  return encrypted_bit ^ metadata_obfuscator;
}
