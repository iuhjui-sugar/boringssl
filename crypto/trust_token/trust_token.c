...
int TRUST_TOKEN_ISSUER_issue(TRUST_TOKEN_ISSUER *ctx, uint8_t **out,
  ...
  return ctx->method->issue(ctx, out, out_len, out_tokens_issued, request,
                            request_len, max_issuance);
}
...
int TRUST_TOKEN_ISSUER_redeem(TRUST_TOKEN_ISSUER *ctx, uint8_t **out,
  ...
  if (result) {
    ...
    uint32_t public_id_u32 = (public_id[0] << 24) | (public_id[1] << 8) |
                             (public_id[2] << 16) | (public_id[3] << 0);

    CBB obfuscator_key;
    uint8_t *obfuscator_key_buf;
    size_t obfuscator_key_len;
    if (!CBB_init(&obfuscator_key, 0) ||
        !CBB_add_bytes(&obfuscator_key, ctx->metadata_key, ctx->metadata_key_len) ||
        !CBB_add_bytes(&obfuscator_key, CBS_data(&client_data), CBS_len(&client_data)) ||
