...
TRUST_TOKEN_ISSUER *TRUST_TOKEN_ISSUER_new(uint16_t max_batchsize) {
  ...
  ret->max_batchsize = max_batchsize;
  ret->num_keys = 0;
  ret->public_metadata = 0;
  ret->private_metadata = 0;
  return ret;
}
...
int TRUST_TOKEN_CLIENT_add_key(TRUST_TOKEN_CLIENT *ctx, const uint8_t *key,
  ...
  if (!CBS_get_u32(&cbs, &key_id) ||
      ...
      !EC_POINT_oct2point(group, key_s.pub0, CBS_data(&tmp), CBS_len(&tmp),
                          NULL) ||
      !CBS_get_u16_length_prefixed(&cbs, &tmp) ||
      !EC_POINT_oct2point(group, key_s.pub1, CBS_data(&tmp), CBS_len(&tmp),
                          NULL) ||
      !CBS_get_u16_length_prefixed(&cbs, &tmp) ||
      !EC_POINT_oct2point(group, key_s.pubs, CBS_data(&tmp), CBS_len(&tmp),
                          NULL) ||
      CBS_len(&cbs) != 0) {
...
int TRUST_TOKEN_ISSUER_issue(TRUST_TOKEN_ISSUER *ctx, uint8_t **out,
  ...
  if (!CBB_add_u16(&response, count) ||
      !CBB_add_u32(&response, ctx->public_metadata)) {
    OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
    return 0;
  }
...
int TRUST_TOKEN_ISSUER_redeem(TRUST_TOKEN_ISSUER *ctx, uint8_t **out,
  ...
  uint32_t public_metadata;
  uint16_t token_len;
  if (!CBS_get_u32(&inner, &public_metadata) ||
      !CBS_get_u16(&inner, &token_len)) {
    return 0;
  }

  TRUST_TOKEN *ret_token = OPENSSL_malloc(sizeof(TRUST_TOKEN));
  if (ret_token == NULL) {
    return 0;
  }
  ret_token->data = OPENSSL_malloc(token_len);
