...
static int cbb_add_raw_point(CBB *cbb, const EC_GROUP *group, const EC_RAW_POINT *point) {
  size_t len =
...
static int cbs_get_raw_point(CBS *cbs, EC_GROUP *group, EC_RAW_POINT *out) {
  CBS child;
...
STACK_OF(TRUST_TOKEN) *
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    PMBTOKEN_TOKEN *token = pmbtoken_unblind(
        ctx, s, &Wp, &Wsp, CBS_data(&proof), CBS_len(&proof), pretoken, key_id);
    if (token == NULL) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, TRUST_TOKEN_R_DECODE_FAILURE);
      goto err;
    }
    ...
    int token_ok = 0;
    TRUST_TOKEN *trust_token = OPENSSL_malloc(sizeof(TRUST_TOKEN));
    if (trust_token == NULL) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
      goto token_err;
    }
    ...
    if (!CBB_init(&token_cbb, 0) ||
        ...
        !CBB_finish(&token_cbb, &trust_token->data, &trust_token->len) ||
        !sk_TRUST_TOKEN_push(tokens, trust_token)) {
      OPENSSL_PUT_ERROR(TRUST_TOKEN, ERR_R_MALLOC_FAILURE);
      goto token_err;
    }
    token_ok = 1;
...
int TRUST_TOKEN_ISSUER_redeem(const TRUST_TOKEN_ISSUER *ctx, uint8_t **out,
  ...
  if (!SHA256_Init(&sha_ctx) ||
      !SHA256_Update(&sha_ctx, ctx->metadata_key, ctx->metadata_key_len) ||
      !SHA256_Update(&sha_ctx, CBS_data(&client_data), CBS_len(&client_data)) ||
