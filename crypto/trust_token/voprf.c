...
static int hash_to_scalar_challenge(const VOPRF_METHOD *method, EC_SCALAR *out,
  ...
  CBB cbb;
  CBB_zero(&cbb);
  uint8_t transcript[5 * EC_MAX_UNCOMPRESSED + 2 + sizeof(kChallengeLabel) - 1];
  size_t len;
  if (!CBB_init_fixed(&cbb, transcript, sizeof(transcript)) ||
      ...
      !CBB_add_u16(&cbb, sizeof(kChallengeLabel) - 1) ||
...
static void sha384_update_point_with_length(
  ...
  uint8_t buf[EC_MAX_UNCOMPRESSED];
  size_t len = ec_point_to_bytes(group, point, POINT_CONVERSION_UNCOMPRESSED,
                                 buf, sizeof(buf));
  assert(len > 0);
...
static int compute_composite_element(const VOPRF_METHOD *method,
  ...
  int ok = 0;
  CBB cbb;
  uint8_t transcript[2 + SHA384_DIGEST_LENGTH + 2 + 2 * EC_MAX_UNCOMPRESSED +
