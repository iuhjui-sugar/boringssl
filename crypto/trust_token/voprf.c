...
typedef int (*hash_to_scalar_func_t)(const EC_GROUP *group,
                                            EC_SCALAR *out, uint8_t *buf,
                                            size_t len);
...
typedef struct {
  ...
  // hash_to_group implements the hash to group operation for VOPRFs. It returns
  // one on success and zero on error.
  hash_to_group_func_t hash_to_group;
  // hash_to_scalar implements the hash to scalar operation for VOPRFs. It
...
static STACK_OF(TRUST_TOKEN_PRETOKEN) *
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    // We sample rinv in Montgomery form to simplify inverting.
    EC_SCALAR r;
