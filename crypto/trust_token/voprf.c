...
static int compute_composite_seed(const VOPRF_METHOD *method,
  ...
  if (!cbb_serialize_point(&seed_transcript_cbb, group, pub) ||
...
static int compute_composite_element(const VOPRF_METHOD *method,
  ...
  if (!CBB_add_u16(&composite_transcript_cbb, SHA384_DIGEST_LENGTH) ||
      !CBB_add_bytes(&composite_transcript_cbb, seed, SHA384_DIGEST_LENGTH) ||
      !CBB_add_u16(&composite_transcript_cbb, index) ||
      !cbb_serialize_point(&composite_transcript_cbb, group, C) ||
      !cbb_serialize_point(&composite_transcript_cbb, group, D) ||
      !CBB_add_bytes(&composite_transcript_cbb, kCompositeLabel,
                     sizeof(kCompositeLabel)) ||
...
static int voprf_sign(const VOPRF_METHOD *method,
  ...
  for (size_t i = 0; i < num_to_issue; i++) {
    ...
    BTs[i] = BT;
    Zs[i] = Z;
    if (!compute_composite_element(method, seed, &dis[i], i, &BT_affine, &Z_affine)) {
      goto err;
    }
  ...
  CBB proof;
  if (!CBB_add_u16_length_prefixed(cbb, &proof) ||
      !generate_proof(method, &proof, key, &M, &Z) ||
      !CBB_flush(cbb)) {
    goto err;
  }
