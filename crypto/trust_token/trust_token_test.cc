...
TEST_P(TrustTokenMetadataTest, SetAndGetMetadata) {
  ...
  for (TRUST_TOKEN *token : tokens.get()) {
    ...
    if (method()->use_token_hash) {
      ...
      ASSERT_EQ(OPENSSL_memcmp(token_hash, srr + 41, sizeof(token_hash)), 0);
      ...
      ASSERT_TRUE(sizeof(kExpectedSRRTokenHash) - 1 == srr_len);
      ASSERT_EQ(OPENSSL_memcmp(kExpectedSRRTokenHash, srr, srr_len), 0);
    } else {
      uint8_t decode_private_metadata;
      ASSERT_TRUE(TRUST_TOKEN_decode_private_metadata(
          method(), &decode_private_metadata, metadata_key,
      ...
      ASSERT_TRUE(sizeof(kExpectedSRRNoTokenHash) - 1 == srr_len);
      ASSERT_EQ(OPENSSL_memcmp(kExpectedSRRNoTokenHash, srr, srr_len), 0);
    }
  }
}
