...
TEST_F(TrustTokenProtocolTest, InvalidToken) {
  ...
  uint32_t public_metadata = 0x0001;
  uint32_t key_id;
  uint8_t tokens_issued;
  ASSERT_TRUE(
      TRUST_TOKEN_CLIENT_begin_issuance(client.get(), &issue_msg, &msg_len, 1));
  bssl::UniquePtr<uint8_t> free_imsg(issue_msg);
  ASSERT_TRUE(TRUST_TOKEN_ISSUER_issue(issuer.get(), &issue_resp, &resp_len,
                                       &tokens_issued, issue_msg, msg_len,
                                       public_metadata, /*private_metadata=*/1,
                                       /*max_issuance=*/10));
  bssl::UniquePtr<uint8_t> free_msg(issue_resp);
  bssl::UniquePtr<STACK_OF(TRUST_TOKEN)> tokens(
      TRUST_TOKEN_CLIENT_finish_issuance(client.get(), &key_id, issue_resp,
                                         resp_len));
  ASSERT_TRUE(tokens);
...
TEST_P(TrustTokenMetadataTest, SetAndGetMetadata) {
  ...
  for (TRUST_TOKEN *token : tokens.get()) {
    ...
    uint8_t private_metadata = TRUST_TOKEN_decode_private_metadata(
        metadata_key, sizeof(metadata_key), kClientData,
...
INSTANTIATE_TEST_SUITE_P(TrustTokenAllMetadataTest, TrustTokenMetadataTest,
