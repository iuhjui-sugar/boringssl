...
TEST_P(TrustTokenMetadataTest, TruncatedProof) {
  ...
  const EC_GROUP *group = EC_GROUP_new_by_curve_name(NID_secp384r1);
  size_t tokenLength = PMBTOKEN_NONCE_SIZE + 2 * (1 + 2 * BN_num_bytes(&group->field));
  if (method() == TRUST_TOKEN_experiment_v1())
    tokenLength += 4;
  for (size_t i = 0; i < count; i++) {
    ASSERT_TRUE(CBB_add_bytes(bad_response.get(), CBS_data(&real_response), tokenLength));
    ASSERT_TRUE(CBS_skip(&real_response, tokenLength));
  }
