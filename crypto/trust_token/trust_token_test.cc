...
TEST(TrustTokenTest, Protocol) {
  ...
  bssl::UniquePtr<EVP_PKEY> priv(raw);

  size_t pub_len;
  ASSERT_TRUE(EVP_PKEY_get_raw_public_key(priv.get(), NULL, &pub_len));
  uint8_t *pub_data = (uint8_t *)OPENSSL_malloc(pub_len);
  ASSERT_TRUE(EVP_PKEY_get_raw_public_key(priv.get(), pub_data, &pub_len));
