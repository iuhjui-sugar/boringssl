...
TEST(TrustTokenTest, Protocol) {
  ...
  uint8_t *msg, *resp, *rtoken;
  size_t msg_len, resp_len, rtoken_len;
  ...
  for (TRUST_TOKEN *token : tokens) {
    ...
    ASSERT_TRUE(TRUST_TOKEN_CLIENT_begin_redemption(client, &msg, &msg_len,
                                                    token, NULL, 0));
    ASSERT_TRUE(TRUST_TOKEN_ISSUER_redeem(issuer, &resp, &resp_len, &rtoken, &rtoken_len, msg, msg_len,
                                          time(nullptr) + i));
    int result = false;
    uint8_t *srr = NULL;
    size_t srr_len;
    ASSERT_TRUE(TRUST_TOKEN_CLIENT_finish_redemption(client, &result, &srr,
                                                     &srr_len, resp, resp_len));
