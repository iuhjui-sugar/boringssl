...
TEST_P(TrustTokenMetadataTest, TruncatedProof) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    ASSERT_TRUE(CBB_add_bytes(bad_response.get(), CBS_data(&tmp), CBS_len(&tmp)));
    ASSERT_TRUE(CBS_get_u16_length_prefixed(&real_response, &tmp));
    ASSERT_TRUE(CBB_add_u16(bad_response.get(), CBS_len(&tmp)));
    ASSERT_TRUE(CBB_add_bytes(bad_response.get(), CBS_data(&tmp), CBS_len(&tmp)));
    ASSERT_TRUE(CBS_get_u16_length_prefixed(&real_response, &tmp));
    ASSERT_TRUE(CBB_add_u16(bad_response.get(), CBS_len(&tmp) - 2));
    ASSERT_TRUE(CBB_add_bytes(bad_response.get(), CBS_data(&tmp), CBS_len(&tmp) - 2));
  }
...
TEST_P(TrustTokenBadKeyTest, BadKey) {
  ...
  struct trust_token_issuer_key_st *key =
      &issuer.get()->keys[std::get<0>(GetParam()) - 7];
  EC_SCALAR *scalars[] = {&key->x0, &key->y0, &key->x1,
                          &key->y1, &key->xs, &key->ys};
  int corrupted_key = std::get<2>(GetParam());
  ...
  if ((corrupted_key / 2 == 0 && std::get<1>(GetParam()) == true) ||
      (corrupted_key / 2 == 1 && std::get<1>(GetParam()) == false)) {
    ASSERT_TRUE(tokens);
  } else {
    ASSERT_FALSE(tokens);
  }
