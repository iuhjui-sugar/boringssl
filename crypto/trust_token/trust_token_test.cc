...
TEST(TrustTokenTest, ClearProtocol) {
  ...
  EVP_PKEY *pub = EVP_PKEY_new_raw_public_key(EVP_PKEY_ED25519, NULL, pub_data, pub_len);
...
TEST(TrustTokenTest, PrivacyPassProtocol) {
  ...
  for (TRUST_TOKEN *token : tokens) {
    ...
    CBS msg_cbs;
    CBS_init(&msg_cbs, msg, msg_len);
    ASSERT_TRUE(TRUST_TOKEN_issuer_perform_redemption(issuer, &resp, &resp_len, msg_cbs, time(nullptr)+i));
    bool result = false;
    CBS resp_cbs;
    uint8_t *srr = NULL;
    size_t srr_len;
