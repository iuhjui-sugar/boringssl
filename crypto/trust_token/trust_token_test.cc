...
static int ec_point_uncompressed_from_compressed(
  ...
  if (!EC_POINT_oct2point(group, point, in, len, nullptr)) {
    EC_POINT_free(point);
    return 0;
  }
  *out_len = EC_POINT_point2oct(group, point, POINT_CONVERSION_UNCOMPRESSED,
                                out, EC_MAX_UNCOMPRESSED, nullptr);
  EC_POINT_free(point);
  return 1;
}
...
TEST(TrustTokenTest, PSTV1VOPRFTestVector1) {
  ...
  ec_scalar_from_bytes(group, &key.xs, kPrivateKey, sizeof(kPrivateKey));

  bssl::UniquePtr<EC_POINT> pub(EC_POINT_new(group));
  ASSERT_TRUE(pub);
  ASSERT_TRUE(EC_POINT_oct2point(group, pub.get(), kPublicKey,
                                 sizeof(kPublicKey), nullptr));
  ASSERT_TRUE(ec_jacobian_to_affine(group, &key.pubs, &pub->raw));
  ...
  ASSERT_TRUE(voprf_pst1_sign_with_proof_scalar_for_testing(
      &key, response.get(), &sign_input, 1, 1, 0, kProofScalar,
      sizeof(kProofScalar)));
  uint8_t *output_buf;
  size_t output_len;
  ASSERT_TRUE(CBB_finish(response.get(), &output_buf, &output_len));
  ...
  size_t expected_len;
  ASSERT_TRUE(
      CBB_finish(expected_response.get(), &expected_buf, &expected_len));

  ASSERT_EQ(Bytes(expected_buf, expected_len), Bytes(output_buf, output_len));
}
...
TEST(TrustTokenTest, PSTV1VOPRFTestVector3) {
  ...
  ASSERT_TRUE(CBB_init(response.get(), 0));
  ASSERT_TRUE(voprf_pst1_sign_with_proof_scalar_for_testing(
      &key, response.get(), &sign_input, 2, 2, 0, kProofScalar,
      sizeof(kProofScalar)));
  uint8_t *output_buf;
  size_t output_len;
  ASSERT_TRUE(CBB_finish(response.get(), &output_buf, &output_len));
  ...
  ASSERT_TRUE(CBB_init(expected_response.get(), 0));
  ASSERT_TRUE(
      CBB_add_bytes(expected_response.get(), evaluated_buf, offset + evaluated_len));
  ASSERT_TRUE(CBB_add_u16(expected_response.get(), sizeof(kProof)));
  ASSERT_TRUE(CBB_add_bytes(expected_response.get(), kProof, sizeof(kProof)));
  ASSERT_TRUE(CBB_flush(expected_response.get()));
