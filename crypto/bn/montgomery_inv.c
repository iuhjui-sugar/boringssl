...
static uint64_t bn_neg_mod_inv_u64(uint64_t b);
...
uint64_t bn_mont_n0(const BIGNUM *n) {
  /* r == 2**(BN_MONT_CTX_N0_LIMBS * BN_BITS2) and kLgR == lg(r). This ensures
   * that we can do integer division by |r| by simply ignoring
  ...
  return bn_neg_mod_inv_u64(n_mod_r);
}

/* bn_neg_mod_inv_u64 calculates the negative inverse modulo |r|; i.e. it
 * calculates |v| such that |u*r - v*b == 1|. |r| is defined in |bn_mont_n0|.
 * |b| must be odd.
 *
...
static uint64_t bn_neg_mod_inv_u64(uint64_t b) {
  /* alpha == 2**(lg r - 1) == r / 2. */
  static const uint64_t alpha = (uint64_t)1 << (LG_LITTLE_R - 1);

  assert(b % 2 == 1);
  const uint64_t beta = b;

  /* |a /= 2|, if |a| were passed as a parameter. */
  uint64_t u = 1;
  uint64_t v = 0;

  /* The invariant maintained from here on is: r = u*2*alpha - v*beta. */
