...
func parseAttribute(s string) (key string, value string, err error) {
...
func (s *testScanner) addAttribute(line string) (key string, ok bool) {
	key, value, err := parseAttribute(s.scanner.Text())
	if err != nil {
		s.setError(err)
		return "", false
	}
	...
	if !ok {
		s.setError(fmt.Errorf("could not parse '%s'", value))
		return "", false
	}
	if _, dup := s.test.Values[key]; dup {
		s.setError(fmt.Errorf("duplicate key '%s'", key))
		return "", false
	}
...
func (s *testScanner) Scan() bool {
	s.test = test{}
	s.test.Values = make(map[string]*big.Int)
	...
	for s.scanLine() {
		...
		_, ok = s.addAttribute(s.scanner.Text())
		if !ok {
			return false
		}
	}
...
func checkKeys(t test, keys ...string) bool {
	...
	for _, k := range keys {
		if _, ok := t.Values[k]; !ok {
			fmt.Fprintf(os.Stderr, "Line %d: missing key '%s'.\n", t.LineNumber, k)
			foundErrors = true
		}
	}
	...
	for k, _ := range t.Values {
		...
		if !found {
			fmt.Fprintf(os.Stderr, "Line %d: unexpected key '%s'.\n", t.LineNumber, k)
			foundErrors = true
		}
	}
...
func main() {
	...
	for scanner.Scan() {
		...
		default:
			fmt.Fprintf(os.Stderr, "Line %d: unknown test type '%s'.\n", test.LineNumber, test.Type)
		}
	}
	if scanner.Err() != nil {
