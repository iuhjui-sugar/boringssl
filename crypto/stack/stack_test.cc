...
TEST(StackTest, Basic) {
  ...
  ASSERT_TRUE(value);
  ASSERT_TRUE(bssl::PushToStack(sk.get(), std::move(value)));
  value = TEST_INT_new(3);
  ASSERT_TRUE(value);
  ASSERT_TRUE(bssl::PushToStack(sk.get(), std::move(value)));
  value = TEST_INT_new(4);
  ASSERT_TRUE(value);
  ASSERT_TRUE(bssl::PushToStack(sk.get(), std::move(value)));
  value = TEST_INT_new(5);
  ASSERT_TRUE(value);
  ASSERT_TRUE(bssl::PushToStack(sk.get(), std::move(value)));
...
static uint64_t compare_count = 0;
...
TEST(StackTest, Sorted) {
  ...
  do {
    ...
    uint64_t old_compare_count = compare_count;
    sk_TEST_INT_sort(sk.get());
    EXPECT_EQ(old_compare_count, compare_count);
    EXPECT_TRUE(sk_TEST_INT_is_sorted(sk.get()));
    ExpectStackEquals(sk.get(), vec_sorted);
