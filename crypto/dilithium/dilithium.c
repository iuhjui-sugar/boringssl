...
static uint32_t abs_signed(uint32_t x) {
  ...
  return constant_time_select_int(constant_time_lt_w(x, 0x80000000), x, -x);
}
...
static uint32_t abs_mod_prime(uint32_t x) {
  ...
  return constant_time_select_int(constant_time_lt_w(kHalfPrime, x), kPrime - x, x);
}
...
static void power2_round(uint32_t *r1, uint32_t *r0, uint32_t r) {
  ...
  *r0 = r - (*r1 << kDroppedBits);
  ...
  crypto_word_t mask = constant_time_lt_w(1u << (kDroppedBits - 1), *r0);
