...
static int dilithium_sign_with_randomizer(
  ...
  for (size_t kappa = 0;; kappa += L) {
    ...
    vectork w;
    matrix_mult(&w, &values->a_ntt, &y_ntt);
    ...
    vectorl cs1;
    vectorl_mult_scalar(&cs1, &values->s1_ntt, &c_ntt);
    vectorl_inverse_ntt(&cs1);
    vectork cs2;
    vectork_mult_scalar(&cs2, &values->s2_ntt, &c_ntt);
    vectork_inverse_ntt(&cs2);

    vectorl_add(&values->sign.z, &y, &cs1);
    ...
    vectork_make_hint(&values->sign.h, &ct0, &cs2, &w);
