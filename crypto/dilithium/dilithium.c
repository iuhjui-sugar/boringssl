...
static const uint32_t kHalfPrime = (kPrime - 1) / 2;
static const uint32_t kGamma1 = 1 << 19;
static const uint32_t kGamma2 = (kPrime - 1) / 32;
...
static uint32_t abs_signed(uint32_t x) {
  // Mask is set iff the most-significant bit is set.
  uint32_t mask = 0u - ((x & 0x80000000) >> 31);
...
static uint32_t abs_mod_prime(uint32_t x) {
  assert(x < kPrime);
  // Mask is set iff x > kHalfPrime.
  uint32_t mask = 0u - ((kHalfPrime - x) >> 31);
  return (mask & (kPrime - x)) | (~mask & x);
}

// Returns the maximum of two values in constant time.
static uint32_t maximum(uint32_t x, uint32_t y) {
  // Mask is set iff x < y.
  uint32_t mask = 0u - ((x - y) >> 31);
...
static void power2_round(uint32_t *r1, uint32_t *r0, uint32_t r) {
  ...
  *r0 = r - (*r1 << kDroppedBits);
  ...
  uint32_t mask = 0u - (((1u << (kDroppedBits - 1)) - *r0) >> 31);

  *r0 = (mask & r0_adjusted) | (~mask & *r0);
  *r1 = (mask & r1_adjusted) | (~mask & *r1);
}
...
static uint32_t high_bits(uint32_t x) {
  // Formula taken from the reference implementation.
  uint32_t r1 = (x + 127) >> 7;
  r1 = (r1 * 1025 + (1 << 21)) >> 22;
  r1 &= 15;
  return r1;
...
static int dilithium_parse_signature(struct signature *sign, CBS *in) {
  ...
  CBS z_bytes;
  if (!CBS_get_bytes(in, &z_bytes, 640 * L) ||
      !vectorl_decode_signed(&sign->z, CBS_data(&z_bytes), 20, 1 << 19)) {
    return 0;
  };

  CBS hint_bytes;
  if (!CBS_get_bytes(in, &hint_bytes, OMEGA + K)) {
    return 0;
  }
  if (!hint_bit_unpack(&sign->h, CBS_data(&hint_bytes))) {
    return 0;
  }
...
void DILITHIUM_generate_key_external_entropy(
  ...
  OPENSSL_memcpy(pub.rho, rho, sizeof(pub.rho));
  ...
  vectork_power2_round(&pub.t1, &priv->t0, &t);
...
void DILITHIUM_sign_deterministic(
  ...
  uint8_t zeroes[32];
  OPENSSL_memset(zeroes, 0, 32);
  BORINGSSL_keccak_absorb(&keccak_ctx, zeroes, 32);
