...
static void scalar_sample_in_ball_vartime(scalar *out, const uint8_t *seed,
  ...
  int offset = 8;
  // TODO(davidben): SampleInBall implements a Fisher–Yates shuffle, which
  // unavoidably leaks where the zeros are by memory access pattern. For now, we
  // assume this is public information and declassify the shuffle samples.
  // However, it is unclear whether this is actually safe to leak because it
  // happens before leaky sigantures are rejected. Follow up with the Dilithium
  // designers to confirm this was not an oversight.
...
static int dilithium_sign_with_randomizer(
  ...
  for (size_t kappa = 0;; kappa += L) {
    ...
    // hide which of the |z_max| or |r0_max| bound failed.
