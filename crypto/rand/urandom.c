 ...
 * buffer, which is unsafe across |fork|.  To use it the buffer, the caller must
 * promise never to fork. */
...
/* urandom_fd_requested is set by |RAND_set_urandom_fd|. */
static int urandom_fd_requested = -2;

/* urandom_buffering_requested is set by |RAND_I_promise_not_to_fork|. */
 ...
 * |urandom_buffering|, which values may be read safely after calling the
 * once. */
static void init_once(void) {
  urandom_buffering = urandom_buffering_requested;
  if (urandom_fd_requested != -2) {
    urandom_fd = urandom_fd_requested;
  } else {
    int fd;
    do {
...
/* RAND_cleanup closes any cached file descriptor. */
void RAND_cleanup(void) {
  close(urandom_fd);
  /* Note that we're doing this without holding a lock.  But this function is
   * documented to be unsafe if other threads might still be calling
   * |RAND_bytes|. */
  urandom_fd = -2;
  /* Thread-local storage is freed for us. */
}
 ...
 * refilling it if necessary to satisfy the request.  It acquires its own
 * locks. */
static void read_from_buffer(struct rand_buffer *buf,
                             uint8_t *out, size_t requested) {
  size_t remaining = BUF_SIZE - buf->used;
  /* Loop until the buffer has enough bytes that we can fall through to the
   * fast path. */
