...
static char fill_with_entropy(int fd, uint8_t *out, size_t len) {
  ssize_t r;
...
static void read_from_buffer(struct rand_buffer *buf,
  ...
  while (requested > remaining) {
    ...
    if (!fill_with_entropy(urandom_fd, buf->rand, BUF_SIZE)) {
      abort();
      return;
    }
    buf->used = 0;
...
void CRYPTO_sysrand(uint8_t *out, size_t requested) {
  ...
  if (!fill_with_entropy(urandom_fd, out, requested)) {
    abort();
  }
}
