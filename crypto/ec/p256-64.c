...
static const EC_METHOD kMethod = {
    EC_FLAGS_DEFAULT_OCT, ec_GFp_nistp256_group_init,
    ec_GFp_simple_group_finish, ec_GFp_simple_group_clear_finish,
...
enum openssl_altimpl_result_t EC_P256_install_64bit(void) {
  return p256_method_set(&kMethod);
}

#else  /* 64_BIT && !WINDOWS */

enum openssl_altimpl_result_t EC_P256_install_64bit(void) {
  return OPENSSL_ALTIMPL_NO_SUPPORT;
}
