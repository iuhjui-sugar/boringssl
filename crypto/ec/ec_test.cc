...
// kECKey is an ECPrivateKey.
static const uint8_t kECKey[] = {
    0x30, 0x77, 0x02, 0x01, 0x01, 0x04, 0x20, 0xc6, 0xc1, 0xaa, 0xda,
    ...
    0x00, 0x04, 0xc8, 0x15, 0x61, 0xec, 0xf2, 0xe5, 0x4e, 0xde, 0xfe,
    0x66, 0x17, 0xdb, 0x1c, 0x7a, 0x34, 0xa7, 0x07, 0x44, 0xdd, 0xb2,
    0x61, 0xf2, 0x69, 0xb8, 0x3d, 0xac, 0xfc, 0xd2, 0xad, 0xe5, 0xa6,
    0x81, 0xe0, 0xe2, 0xaf, 0xa3, 0xf9, 0xb6, 0xab, 0xe4, 0xc6, 0x98,
    0xef, 0x64, 0x95, 0xf1, 0xbe, 0x49, 0xa3, 0x19, 0x6c, 0x50, 0x56,
    0xac, 0xb3, 0x76, 0x3f, 0xe4, 0x50, 0x7e, 0xec, 0x59, 0x6e, 0x88,
};

// kECKeyWithoutPublic is the above key with the optional publicKey field
// omitted.
...
static bool EncodeECPrivateKey(std::vector<uint8_t> *out, const EC_KEY *key) {
  ...
  if (!CBB_init(cbb.get(), 0) ||
      !EC_KEY_marshal_private_key(cbb.get(), key, 0 /* enc_flags */) ||
...
bool Testd2i_ECPrivateKey() {
  ...
  // The public key should be restored on re-encoding.
  if (std::vector<uint8_t>(kECKey, kECKey + sizeof(kECKey)) != out) {
    fprintf(stderr, "Serialisation of key is incorrect.\n");
    return false;
  }
...
static bool TestZeroPadding() {
  ...
  if (std::vector<uint8_t>(kECKeyWithZeros,
                           kECKeyWithZeros + sizeof(kECKeyWithZeros)) != out) {
    fprintf(stderr, "Serialisation of key was incorrectz.\n");
    return false;
  }
