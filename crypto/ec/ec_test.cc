...
static bssl::unique_ptr<EC_KEY> DecodeECPrivateKey(const uint8_t *in, size_t in_len) {
  CBS cbs;
  CBS_init(&cbs, in, in_len);
  bssl::unique_ptr<EC_KEY> ret(EC_KEY_parse_private_key(&cbs, NULL));
...
static bool Testd2i_ECPrivateKey() {
  bssl::unique_ptr<EC_KEY> key = DecodeECPrivateKey(kECKeyWithoutPublic,
  ...
  bssl::unique_ptr<BIGNUM> x(BN_new());
  bssl::unique_ptr<BIGNUM> y(BN_new());
  if (!x || !y) {
    return false;
  }
  ...
  bssl::unique_ptr<char> x_hex(BN_bn2hex(x.get()));
  bssl::unique_ptr<char> y_hex(BN_bn2hex(y.get()));
  if (!x_hex || !y_hex) {
    return false;
  }
  if (0 != strcmp(
...
static bool TestZeroPadding() {
  ...
  bssl::unique_ptr<EC_KEY> key = DecodeECPrivateKey(kECKeyWithZeros,
...
static bool TestSetAffine(const int nid) {
  bssl::unique_ptr<EC_KEY> key(EC_KEY_new_by_curve_name(nid));
  if (!key) {
    return false;
  }
  ...
  auto point = bssl::unique_ptr<EC_POINT>(EC_POINT_new(group));
  if (!point) {
    return false;
  }
  ...
  bssl::unique_ptr<EC_POINT> invalid_point(EC_POINT_new(group));
  if (!invalid_point) {
    return false;
  }
...
static bool TestArbitraryCurve() {
  ...
  bssl::unique_ptr<EC_KEY> key(EC_KEY_new_by_curve_name(NID_X9_62_prime256v1));
  ...
  bssl::unique_ptr<BIGNUM> p(BN_bin2bn(kP, sizeof(kP), nullptr));
  bssl::unique_ptr<BIGNUM> a(BN_bin2bn(kA, sizeof(kA), nullptr));
  bssl::unique_ptr<BIGNUM> b(BN_bin2bn(kB, sizeof(kB), nullptr));
  bssl::unique_ptr<BIGNUM> gx(BN_bin2bn(kX, sizeof(kX), nullptr));
  bssl::unique_ptr<BIGNUM> gy(BN_bin2bn(kY, sizeof(kY), nullptr));
  bssl::unique_ptr<BIGNUM> order(BN_bin2bn(kOrder, sizeof(kOrder), nullptr));
  bssl::unique_ptr<BIGNUM> cofactor(BN_new());
  if (!ctx || !p || !a || !b || !gx || !gy || !order || !cofactor ||
      !BN_set_word(cofactor.get(), 1)) {
    return false;
  }

  bssl::unique_ptr<EC_GROUP> group(
      EC_GROUP_new_curve_GFp(p.get(), a.get(), b.get(), ctx.get()));
  if (!group) {
    return false;
  }
  bssl::unique_ptr<EC_POINT> generator(EC_POINT_new(group.get()));
  if (!generator ||
  ...
  bssl::unique_ptr<EC_KEY> key2(EC_KEY_new());
  bssl::unique_ptr<EC_POINT> point(EC_POINT_new(group.get()));
  bssl::unique_ptr<BIGNUM> x(BN_new()), y(BN_new());
  if (!key2 || !point || !x || !y ||
...
static bool TestAddingEqualPoints(int nid) {
  ...
  bssl::unique_ptr<EC_POINT> p1(EC_POINT_new(group));
  bssl::unique_ptr<EC_POINT> p2(EC_POINT_new(group));
  bssl::unique_ptr<EC_POINT> double_p1(EC_POINT_new(group));
  bssl::unique_ptr<EC_POINT> p1_plus_p2(EC_POINT_new(group));
