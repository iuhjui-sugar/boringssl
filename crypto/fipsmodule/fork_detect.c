...
static void init_fork_detect(void) {
  if (secure_getenv("BORINGSSL_IGNORE_MADV_WIPEONFORK")) {
    return;
  }
...
uint64_t CRYPTO_get_fork_generation(void) {
  ...
  // concurrently.
  //
  // If a multi-threaded process forks, only a single thread exists in the new
  // process. That thread may only call async-signal-safe functions prior to
  // execve(), which excludes calling any BoringSSL functions. Thus the cleared
  // flag byte can only be observed in a single-threaded context.
  //
  // However, if a single-threaded process forks and the child goes
  // multi-threaded, those threads can race to increment the fork-detection
  // count. Thus the generation counter and flag byte are protected by a lock.

  CRYPTO_once(g_fork_detect_once_bss_get(), init_fork_detect);
  char *const flag_ptr = *g_fork_detect_addr_bss_get();
  if (flag_ptr == NULL) {
