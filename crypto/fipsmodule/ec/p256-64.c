...
static const uint64_t rrmodp[NLIMBS] = {3, 0xfffffffbffffffff, 0xfffffffffffffffe, 0x4fffffffd};
#endif
#if defined(OPENSSL_32_BIT)
#define NLIMBS 8
typedef uint64_t fe[NLIMBS];
static const fe fe_one = {1, 0, 0, 0xffffffff, 0xffffffff, 0xffffffff, 0xfffffffe, 0};
static const uint64_t rrmodp[NLIMBS] = {3, 0, 0xffffffff, 0xfffffffb, 0xfffffffe, 0xffffffff, 0xfffffffd, 4};
#endif
...
static void fe_cmovznz(uint64_t out[NLIMBS], uint64_t t, const uint64_t z[NLIMBS], const uint64_t nz[NLIMBS]) {
  for (int i = 0; i < NLIMBS; i++) { out[i] = cmovznz_u64(t, z[i], nz[i]); }
}
