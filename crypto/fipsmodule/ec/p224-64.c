...
static void bin28_to_felem(p224_felem out, const u8 in[28]) {
...
static void felem_to_bin28(u8 out[28], const p224_felem in) {
...
static void p224_flip_endian(u8 *out, const u8 *in, size_t len) {
 ...
 * The scalars we actually use are small, so results fit without overflow */
static void widefelem_scalar(p224_widefelem out, const p224_widelimb scalar) {
  out[0] *= scalar;
  out[1] *= scalar;
  out[2] *= scalar;
  out[3] *= scalar;
 ...
 * copies it to out. */
static void p224_select_point(const u64 idx, size_t size,
...
static void p224_batch_mul(p224_felem x_out, p224_felem y_out, p224_felem z_out,
                           const u8 *p_scalar, const u8 *g_scalar,
