...
func writeCurveData(w *columnWriter, curve elliptic.Curve, includeNonMontgomery bool) error {
	...
	if err := writeWordsDecl(w, "BN_ULONG", fmt.Sprintf("k%sField", cName), bigIntToU64s(curve, params.P), formatBN); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "BN_ULONG", fmt.Sprintf("k%sOrder", cName), bigIntToU64s(curve, params.N), formatBN); err != nil {
		return err
	}
	...
	if includeNonMontgomery {
		if err := writeWordsDecl(w, "BN_ULONG", fmt.Sprintf("k%sB", cName), bigIntToU64s(curve, params.B), formatBN); err != nil {
			return err
		}
		if err := writeWordsDecl(w, "BN_ULONG", fmt.Sprintf("k%sGX", cName), bigIntToU64s(curve, params.Gx), formatBN); err != nil {
			return err
		}
		if err := writeWordsDecl(w, "BN_ULONG", fmt.Sprintf("k%sGY", cName), bigIntToU64s(curve, params.Gy), formatBN); err != nil {
			return err
		}
	}
	...
	if err := writeWordsDecl(w, "uint64_t", fmt.Sprintf("k%sFieldR", cName), bigIntToU64s(curve, montgomeryR(params.P, 64)), formatU64); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint64_t", fmt.Sprintf("k%sFieldRR", cName), bigIntToU64s(curve, montgomeryRR(params.P, 64)), formatU64); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint64_t", fmt.Sprintf("k%sOrderRR", cName), bigIntToU64s(curve, montgomeryRR(params.N, 64)), formatU64); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint64_t", fmt.Sprintf("k%sMontB", cName), bigIntToU64s(curve, toMontgomery(params.B, params.P, 64)), formatU64); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint64_t", fmt.Sprintf("k%sMontGX", cName), bigIntToU64s(curve, toMontgomery(params.Gx, params.P, 64)), formatU64); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint64_t", fmt.Sprintf("k%sMontGY", cName), bigIntToU64s(curve, toMontgomery(params.Gy, params.P, 64)), formatU64); err != nil {
		return err
	}
	if _, err := io.WriteString(w, "#elif defined(OPENSSL_32_BIT)\n"); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint32_t", fmt.Sprintf("k%sFieldR", cName), bigIntToU32s(curve, montgomeryR(params.P, 32)), formatU32); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint32_t", fmt.Sprintf("k%sFieldRR", cName), bigIntToU32s(curve, montgomeryRR(params.P, 32)), formatU32); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint32_t", fmt.Sprintf("k%sOrderRR", cName), bigIntToU32s(curve, montgomeryRR(params.N, 32)), formatU32); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint32_t", fmt.Sprintf("k%sMontB", cName), bigIntToU32s(curve, toMontgomery(params.B, params.P, 32)), formatU32); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint32_t", fmt.Sprintf("k%sMontGX", cName), bigIntToU32s(curve, toMontgomery(params.Gx, params.P, 32)), formatU32); err != nil {
		return err
	}
	if err := writeWordsDecl(w, "uint32_t", fmt.Sprintf("k%sMontGY", cName), bigIntToU32s(curve, toMontgomery(params.Gy, params.P, 32)), formatU32); err != nil {
		return err
	}
...
func writeWordsDecl[Word any](w *columnWriter, typ, decl string, words []Word, format func(Word) string) error {
	if _, err := fmt.Fprintf(w, "OPENSSL_UNUSED static const %s %s[] = {\n    ", typ, decl); err != nil {
		return err
	}
	if err := writeWords(w, words, format); err != nil {
		return err
