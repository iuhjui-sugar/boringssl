...
//
// Using this function, or a similar one for other window sizes, the non-trivial
// doubling case in single-point scalar multiplication may occur if and only the
// 2^(w-1) bit of the group order is zero. If it is one, this case may be
// ignored.
//
...
// PROOF:
//
// Let w be a window width (in this function, w = 5). Let n be a prime group
// order much larger than 2^w. Let l be the number of bits needed to represent
// n. Assume there exists some 0 <= k < n such that signed w-bit windowed
