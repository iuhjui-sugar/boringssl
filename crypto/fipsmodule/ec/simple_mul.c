...
static unsigned ec_GFp_mont_comb_width(const EC_GROUP *group) {
...
int ec_GFp_mont_init_precomp(const EC_GROUP *group, EC_PRECOMP *out,
  ...
  // b4 * 2^(4*width) + b3 * 2^(3*width) + ... + b0 * 2^(0*width). width here is
  // |ec_GFp_mont_comb_width|. We store at index i - 1 because the 0th comb
  // entry is always infinity.
  EC_RAW_POINT comb[(1 << EC_MONT_PRECOMP_COMB_SIZE) - 1];
  unsigned comb_width = ec_GFp_mont_comb_width(group);
  ...
  for (unsigned i = 1; i < EC_MONT_PRECOMP_COMB_SIZE; i++) {
    // Compute entry 2^i by doubling the entry for 2^(i-1) |comb_width| times.
    unsigned bit = 1 << i;
    ec_GFp_mont_dbl(group, &comb[bit - 1], &comb[bit / 2 - 1]);
    for (unsigned j = 1; j < comb_width; j++) {
...
static void ec_GFp_mont_get_comb_window(const EC_GROUP *group,
  ...
  for (unsigned j = 0; j < EC_MONT_PRECOMP_COMB_SIZE; j++) {
    window |= bn_is_bit_set_words(scalar->words, width, j * comb_width + i)
              << j;
  }
...
void ec_GFp_mont_mul_precomp(const EC_GROUP *group, EC_RAW_POINT *r,
  ...
  unsigned comb_width = ec_GFp_mont_comb_width(group);
  int r_is_at_infinity = 1;
  for (unsigned i = comb_width - 1; i < comb_width; i--) {
    if (!r_is_at_infinity) {
      ec_GFp_mont_dbl(group, r, r);
    }
