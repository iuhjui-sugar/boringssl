...
static void ec_GFp_simple_mul_single(const EC_GROUP *group, EC_RAW_POINT *r,
  ...
  // This implementation is intended as a constant-time generic implementation
  // for curves which are not common enough to warrant a tuned
  // implementation. Accordingly, it uses unsigned digits so the doubling
  // special-case in |ec_GFp_simple_add| is unreachable for all curves.

  // Compute a table of the first 16 multiples of |p| (and infinity).
