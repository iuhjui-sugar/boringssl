...
// Return 1 if r*z^2 = x
// |z| is in the Montogomery domain, |r| and |x| are not.
...
static int ecp_nistz256_cmp_x_coordinate(const EC_GROUP *group,
                                         ...
                                         BN_CTX *ctx) {
  // P_N is the difference between the field order (p) and the group order (N).
  ...
  // The group order. This value is not in the montgomery domain.
  ...
  // < p.x < P. in that case we need not only to compare r but also
  // r+group_order to p.x. Otherwise we could reject a valid signature.
