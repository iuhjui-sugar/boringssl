...
static unsigned calc_first_wvalue(unsigned* index, uint8_t p_str[33])
{
...
static unsigned calc_wvalue(unsigned* index, uint8_t p_str[33])
{
  static const unsigned kWindowSize = 7;
  static const unsigned kMask = (1 << (7 /* kWindowSize */ + 1)) - 1;
...
static void mul_p_add_and_store(const EC_GROUP *group, 
                                EC_RAW_POINT *r,
                                                   ...
                                                   const EC_SCALAR *in)
{
  ...
  if (!beeu_mod_inverse_vartime(out->words, in->words, o_felem)) {
    return 0;
  }
...
static int is_greater(const BN_ULONG a[P256_LIMBS], 
...
static int cmp_x_and_r(const BN_ULONG r[P256_LIMBS], 
...
DEFINE_METHOD_FUNCTION(EC_METHOD, EC_GFp_nistz256_method) {
  ...
  out->scalar_vartime_inv_mont = ecp_nistz256_mont_inv_mod_ord_vartime;
