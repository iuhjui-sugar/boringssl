...
func transform(lines []string, symbols map[string]bool) (ret []string) {
	...
	for {
		...
		if strings.Contains(line, ",") {
			line = strings.Replace(line, "BORINGSSL_bcm_text_dummy_hash", "BORINGSSL_bcm_text_hash", -1)
		}
...
func extractSection(extracted, lines []string, source *lineSource) ([]string, []string) {
	for {
		line, ok := source.Next()
		if !ok {
