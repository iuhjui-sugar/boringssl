	...
	for _, arg := range argNodes {
		...
		case ruleMemoryRef:
			...
			if knownSymbol {
				...
				changed = true
			} else if len(symbol) > 0 && !changed && !isSynthesized(symbol) {
				changed = true
			...
			case "toc@ha":
				...
				if ok {
					...
					d.target.leaSym(d.output, target, symbol, false)
				...
				if ok {
					...
					d.target.leaSym(d.output, target, symbol, true)
					d.tocLoaders[symbol] = struct{}{}
...
func transform(w stringWriter, target targetPlatform, inputs []inputFile) error {
	...
	// TOC loaders only exist on ppc64le so we can emit assembly directly.
...
type targetPlatform interface {
	...
	leaSym(w stringWriter, target, source string, deref bool)
}
...
func (x86_64) leaSym(w stringWriter, target, source string, deref bool) {
	panic("x86-64 doesn't have a TOC")
}
...
func (ppc64le) leaSym(w stringWriter, target, source string, deref bool) {
	temp := "3"
	if temp == target {
		temp = "4"
	}
