...
func transform(lines []string, symbols map[string]bool) (ret []string) {
	...
	for lineNo, line := range lines {
		...
		case ".comm":
			...
			name := p[0]
			bssAccessors = append(bssAccessors, name)
		...
		case ".section":
			...
			case ".data.rel.ro.local":
				...
				// that contains function pointers. These vary
				// at run-time so are not allowed.
				panic("rel.ro section found")
