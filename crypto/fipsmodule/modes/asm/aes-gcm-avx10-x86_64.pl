...
sub _begin_func {
    ...
    my $seh_startproc = $uses_seh ? ".seh_startproc" : "";
    return <<___;
.globl $funcname
...
# void gcm_init_##suffix(u128 Htable[16], const uint64_t Xi[2]);
#
# Initialize |Htable| with powers of the GHASH subkey |Xi|.
#
...
sub _aes_gcm_init {
    my $local_label_suffix = ++$g_init_macro_expansion_count;

    # Function arguments
    my ( $HTABLE, $XI ) = @argregs[ 0 .. 1 ];
    ...
    vmovq           8($XI), $H_CUR_XMM
    vpinsrq         \$1, ($XI), $H_CUR_XMM, $H_CUR_XMM
...
#
# |ivec| must give the current counter in big-endian format.  For a new
# message, the low word of the counter must be 2.  This function loads the
# counter from |ivec| and increments the loaded counter as needed, but it does
# *not* store the updated counter back to |ivec|.  The caller must update
# |ivec| if any more data segments follow.  Internally, only the low 32-bit
# word of the counter is incremented, following the GCM standard.
sub _aes_gcm_update {
    my $local_label_suffix = ++$g_update_macro_expansion_count;
    ...
    ( $RNDKEYLAST0, $RNDKEYLAST1, $RNDKEYLAST2, $RNDKEYLAST3 ) =
      ( $V20, $V21, $V22, $V23 );

    # GHASHTMP[0-2] are temporary variables used by _ghash_step_4x.  These
    # coincide with anything used for AES encryption, since for
...
___
    ...
    if ( $VL == 32 ) {
        $code .= "vbroadcasti32x4 .Linc_2blocks(%rip), $LE_CTR_INC\n";
    }
    elsif ( $VL == 64 ) {
        $code .= "vbroadcasti32x4 .Linc_4blocks(%rip), $LE_CTR_INC\n";
    }
    else {
        die "Unsupported vector length";
    }
...
___
    ...
    if ($enc) {
        ...
        @{[ _ctr_begin_4x ]}
        lea             16($AESKEY), %rax
.Lvaesenc_4x_loop_1_$local_label_suffix:
        vbroadcasti32x4 (%rax), $RNDKEY
        @{[ _vaesenc_4x $RNDKEY ]}
        add             \$16, %rax
        cmp             %rax, $RNDKEYLAST_PTR
        jne             .Lvaesenc_4x_loop_1_$local_label_suffix
...
___
    ...
    if ($enc) {
        $code .= ".Lghash_last_ciphertext_4x$local_label_suffix:\n";
...
.Lcrypt_loop_1x$local_label_suffix:
    ...
    vpxord          $RNDKEY0, $V0, $V0
    lea             16($AESKEY), %rax
.Lvaesenc_1x_loop_1_$local_label_suffix:
    vbroadcasti32x4 (%rax), $RNDKEY
    vaesenc         $RNDKEY, $V0, $V0
    add             \$16, %rax
    cmp             %rax, $RNDKEYLAST_PTR
    jne             .Lvaesenc_1x_loop_1_$local_label_suffix
...
___
    if ($enc) {
        $code .= "vpshufb $BSWAP_MASK, $V0, $V0\n";
    }
    else {
        $code .= "vpshufb $BSWAP_MASK, $V1, $V0\n";
    }
    $code .= <<___;
...
___
    ...
    vpxord          $RNDKEY0, $V0, $V0
    lea             16($AESKEY), %rax
.Lvaesenc_1x_loop_2_$local_label_suffix:
    vbroadcasti32x4 (%rax), $RNDKEY
    vaesenc         $RNDKEY, $V0, $V0
    add             \$16, %rax
    cmp             %rax, $RNDKEYLAST_PTR
    jne             .Lvaesenc_1x_loop_2_$local_label_suffix
    vaesenclast     $RNDKEYLAST, $V0, $V0
...
# void gcm_ghash_vpclmulqdq_avx10(uint8_t Xi[16], const u128 h_powers[16],
#                                 const uint8_t *in, size_t len);
#
# This function processes the AAD (Additional Authenticated Data) in GCM.
# Using the key |h_powers|, it updates the GHASH accumulator |Xi| with the data
# given by |in| and |len|.  On the first call, |Xi| must be all zeroes.  |len|
...
# which uses 128-bit vectors (xmm registers) and only has a 1x-wide loop.  This
# keeps the code size down, and it enables some micro-optimizations, e.g. using
# VEX-coded instructions instead of EVEX-coded to save some instruction bytes.
# To optimize for large amounts of AAD, we could implement a 4x-wide loop and
# provide a version using 512-bit vectors, but that doesn't seem to be useful.
$code .= _begin_func "gcm_ghash_vpclmulqdq_avx10", 1;
{
    # Function arguments
    ...
    my ( $BSWAP_MASK, $GFPOLY, $GHASH_ACC, $H_POW1 ) =
      map( "%xmm$_", ( 4 .. 7 ) );
    ...
    # Update GHASH with 16 bytes of AAD at a time.
    test            $AADLEN, $AADLEN
    jz              .Laad_done
    vmovdqu8        $OFFSETOFEND_H_POWERS-16($H_POWERS), $H_POW1
.Laad_loop_1x:
    vmovdqu         ($AAD), %xmm0
