...
static void CheckGenerationInChild(const char *name,
...
TEST(ForkDetect, Test) {
  ...
  if (child == 0) {
    ...
    for (int i = 0; i < 2; i++) {
      ForkInChild([&] { CheckGenerationInChild("Grandchild", child_generation + 1); });
    }
    ...
    ForkInChild([&] {
      ...
      for (int i = 0; i < 2; i++) {
        for (auto &t : threads) {
          t = std::thread(
              [&] { CheckGenerationInChild("Grandchild thread", child_generation + 1); });
        }
        for (auto &t : threads) {
          t.join();
        }
