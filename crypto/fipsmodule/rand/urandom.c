...
DEFINE_BSS_GET(struct CRYPTO_STATIC_MUTEX, rand_lock);
...
DEFINE_BSS_GET(CRYPTO_once_t, rand_once);

/* message writes |msg_len| bytes from |msg| to stderr. We use this because
 * referencing |stderr| with |fprintf| generates relocations, which is a
 * problem inside the FIPS module. */
static void message(const char *msg, size_t msg_len) {
  ssize_t r;
  do {
    r = write(2, msg, msg_len);
