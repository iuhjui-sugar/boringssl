...
void RAND_bytes_with_additional_data(uint8_t *out, size_t out_len,
  ...
  if (state->calls >= kReseedInterval) {
     ...
     * bug on ppc64le. glibc sometimes implements pthread locks by wrapping the
     * user's critical section in a hardware transaction, but, on some older
     * versions of glibc and the kernel, syscalls made with |syscall| did not
