...
#elsif defined (OPENSSL_WINDOWS) || defined (OPENSSL_TRUSTY)

// These platformes are guaranteed not to fork, and therefore do not require
// fork detection support. Returning a constant non zero value makes BoringSSL
// assume address space duplication is not a concern and re-seeding for these
// cases is not needed.
uint64_t CRYPTO_get_fork_generation(void) { return 0xc0ffee; }

#else

// These platformes may fork, but we do not have a mitigation mechanism in
// place.  Returning a constant zero value makes BoringSSL assume that address
// space duplication could have occured on any call and re-seeding should always
// happen.
