...
static void we_are_forked(void) {
  // Immediately after a fork, the proces must be single-threaded.
  uint32_t value = *g_atfork_fork_generation_bss_get();
  if (value++ == 0) {
    value++;
  }
  *g_atfork_fork_generation_bss_get() = value;
  // Clear the unsafe buffering flag, since we have forked.
...
#else
  // Use the pthread_atfork generation number instead of madvise.
