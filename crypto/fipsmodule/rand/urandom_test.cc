...
static std::vector<Event> TestFunctionPRNGModel(unsigned flags) {
  ...
  if (!have_rdrand()) {
    if (!sysrand(false, kAdditionalDataLength) ||
        // Initialise CRNGT.
      ...
      switch (event.type) {
        case Event::Syscall::kGetRandom:
          if ((event.flags & GRND_NONBLOCK) == 0) {
            ADD_FAILURE() << "Blocking getrandom found with RDRAND: " << ToString(events);
          }
          break;

        case Event::Syscall::kUrandomIoctl:
          ADD_FAILURE() << "Urandom polling found with RDRAND: " << ToString(events);
          break;
