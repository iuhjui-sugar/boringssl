...
static bool regs_read(struct regs *out_regs, int child_pid) {
  ...
  if (ptrace(PTRACE_GETREGSET, child_pid, NT_PRSTATUS, &io) !=
      0) {
    return false;
  }
...
#error "This code probably doesn't work"
    ...
    word.u64 = ptrace(PTRACE_PEEKDATA, child_pid,
                      reinterpret_cast<void *>(aligned_addr), 0);
    return word.u8[offset];
}

static char *get_string_from_remote(int child_pid, uint64_t ptr) {
  bssl::ScopedCBB cbb;
  ...
  uint8_t *str;
  size_t str_len;
  if (!CBB_finish(cbb.get(), &str, &str_len)) {
    abort();
  }
  return (char *) str;
}
  ...
  for (;;) {
    ...
    switch (regs.syscall) {
...
#endif
      {
        ...
        char *filename = get_string_from_remote(child_pid, filename_ptr);
        if (strstr(filename, "/dev/__properties__/") == filename) {
