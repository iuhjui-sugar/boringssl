...
#if defined(OPENSSL_X86_64) && defined(OPENSSL_URANDOM)
...
struct Event {
  ...
  static Event UrandomIoctl() {
    Event e;
    e.type = Syscall::kUrandomIoctl;
    return e;
  }

  static Event Abort() {
    Event e;
    e.type = Syscall::kAbort;
    return e;
  }
...
static void GetTrace(std::vector<Event> *out_trace, unsigned flags,
  ...
  for (;;) {
    ...
    bool inject_einval = false;
    bool is_urandom_ioctl = false;
    intptr_t ioctl_output_addr;

    switch (syscall_number) {
      case __NR_getrandom:
        if (flags & NO_GETRANDOM) {
          // Set the syscall number to nonsense to get -ENOSYS.
          regs.orig_rax = 0xfffff;
    ...
    if (is_urandom_ioctl) {
      ...
      // ptrace always works with ill-defined "words", which I think are 64-bit
      ...
      uint64_t word =
          ptrace(PTRACE_PEEKDATA, child_pid,
                 reinterpret_cast<void *>(ioctl_output_addr), nullptr);
      word >>= 32;
      word <<= 32;
      word |= result;
      ASSERT_EQ(0, ptrace(PTRACE_POKEDATA, child_pid,
                          reinterpret_cast<void *>(ioctl_output_addr),
...
#define TRACE_FLAG(flag)                                         \
  ...
  for (unsigned flags = 0; flags < NEXT_FLAG; flags++) {
    ...
    if (expected_trace != actual_trace) {
      const std::string trace_str = ToString(actual_trace);
      const std::string expected_str = ToString(expected_trace);
