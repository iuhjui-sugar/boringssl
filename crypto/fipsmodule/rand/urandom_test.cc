...
//       one.
//   args[0..2]: syscall arguments, if registers were read just beforing 
//       entering one.
...
static bool regs_set_ret(int child_pid, const struct regs *orig_regs, int ret);
...
static bool regs_set_ret(int child_pid, const struct regs *orig_regs, int ret) {
  return regs_set(child_pid, orig_regs, ret);
}
...
// remote_memcpy copies |n| bytes from |in_src| in the local address space, to
// |dest| in the address space of |child_pid|.
static void remote_memcpy(int child_pid, uint64_t dest, const void *in_src,
                          size_t n) {
...
static void GetTrace(std::vector<Event> *out_trace, unsigned flags,
  ...
  for (;;) {
      ...
      case __NR_connect: {
        ...
        if (sock_fd >= 0 && connect_fd == sock_fd) {
          ...
          if (flags & CONNECT_ERROR) {
            ...
            // The test system might not have an entropy daemon running, just
    ...
    if (force_result.has_value()) {
      ASSERT_TRUE(regs_read(&regs, child_pid));
      ASSERT_TRUE(regs_set_ret(child_pid, &regs, force_result.get()));
      ...
      remote_memcpy(child_pid, ioctl_output_addr, &result, sizeof(result));
      ...
      remote_memcpy(child_pid, regs.args[1], entropy, socket_read_bytes);

      ASSERT_TRUE(regs_read(&regs, child_pid));
      ASSERT_TRUE(regs_set_ret(child_pid, &regs, socket_read_bytes));
    }
  }
}
