...
// Define SHA{n}[_{variant}]_ASM iif SHA{n}_block_data_order[_{variant}] is
// defined in assembly.
...
#define SHA1_ASM_HW
OPENSSL_INLINE int sha1_hw_capable(void) {
  return CRYPTO_is_SSSE3_capable() && ((OPENSSL_get_ia32cap(2) & (1 << 9)) != 0);
}

#define SHA1_ASM_AVX2
OPENSSL_INLINE int sha1_avx2_capable(void) {
  return CRYPTO_is_SSSE3_capable() && CRYPTO_is_AVX2_capable() && CRYPTO_is_BMI1_capable() && CRYPTO_is_BMI2_capable();
}
...
OPENSSL_INLINE int sha1_avx_capable(void) {
  return CRYPTO_is_SSSE3_capable() && CRYPTO_is_AVX_capable() && CRYPTO_is_intel_cpu();
}
...
#define SHA256_ASM_HW
OPENSSL_INLINE int sha256_hw_capable(void) {
  return (OPENSSL_get_ia32cap(2) & (1 << 29)) != 0;
}

#define SHA256_ASM_AVX
OPENSSL_INLINE int sha256_avx_capable(void) {
  return CRYPTO_is_AVX_capable() && CRYPTO_is_SSSE3_capable() && CRYPTO_is_intel_cpu();
}
