...
#define SHA1_ASM_HW
OPENSSL_INLINE int sha1_hw_capable(void) {
  return CRYPTO_is_SSSE3_capable() && CRYPTO_is_x86_SHA_capable();
}
...
OPENSSL_INLINE int sha1_avx2_capable(void) {
  ...
  //  * sha1_block_data_order_avx2 does not seem to actually use any SSSE3
  //    instructions.
...
OPENSSL_INLINE int sha1_avx_capable(void) {
  ...
  //  * Lift the restriction to Intel CPUs. The restruction seems to be based on
...
OPENSSL_INLINE int sha256_avx_capable(void) {
  // TODO: Simplify this logic, which was extracted from the assembly:
  //  * Does AVX imply SSSE3?
  //  * sha256_block_data_order_avx does not actually use an SSSE3 instructions
  //  * Lift the restriction to Intel CPUs. The restruction seems to be based on
  //    possibly-outdated assumptions of the performance of SHLD and SHRD
  //    described in sha1-586.pl.
  return CRYPTO_is_AVX_capable() && CRYPTO_is_SSSE3_capable() &&
...
OPENSSL_INLINE int sha512_avx_capable(void) {
  // Additionally check for Intel CPUs. This function makes some assumptions on
  // the performance of SHLD and SHRD. See discussion in sha1-586.pl.
  return CRYPTO_is_AVX_capable() && CRYPTO_is_SSSE3_capable() &&
         CRYPTO_is_intel_cpu();
}
