...
1.  OpenSSL depends on the link order and inserts two object files, `fips_start.o` and `fips_end.o`, in order to establish the `module_start` and `module_end` values. BoringSSL adds labels at the correct places in the assembly.
...
1.  OpenSSL doesn't run the power-on test until `FIPS_module_mode_set` is called. BoringSSL does it in a constructor function. Failure of the test is non-fatal in OpenSSL, BoringSSL will crash.
