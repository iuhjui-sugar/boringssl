...
// |aes_hw_and|, etc., should be used instead. Note |aes_nohw_shift_left| and
...
static void aes_nohw_inv_mix_columns(AES_NOHW_BATCH *batch) {
  ...
  //
  // First, apply the [5 0 4 0] matrix. Multiplying by 4 in F_8 is described by
  // the following bit equations:
  //
  //   b0 = a6
  //   b1 = a6 ^ a7
  //   b2 = a0 ^ a7
...
static void aes_nohw_setup_key_192(AES_KEY *key, const uint8_t in[24]) {
  ...
  for (size_t i = 0; i < 4; i++) {
    ...
    // previous loop iteration but was not yet outputted.
...
void aes_nohw_ctr32_encrypt_blocks(const uint8_t *in, uint8_t *out,
  ...
  alignas(sizeof(aes_word_t)) union {
...
void aes_nohw_cbc_encrypt(const uint8_t *in, uint8_t *out, size_t len,
  ...
  AES_NOHW_SCHEDULE sched;
  aes_nohw_expand_round_keys(&sched, key);
  alignas(sizeof(aes_word_t)) uint8_t iv[16];
  memcpy(iv, ivec, 16);
  ...
  while (blocks > 0) {
    ...
    alignas(sizeof(aes_word_t)) uint8_t copy[AES_NOHW_BATCH_SIZE * 16];
