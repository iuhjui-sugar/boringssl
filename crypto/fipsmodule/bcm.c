...
static void BORINGSSL_bcm_power_on_self_test(void) {
  ...
  if (!EVP_AEAD_CTX_init_with_direction(&aead_ctx, EVP_aead_aes_128_gcm(),
                                        kAESKey, sizeof(kAESKey), 0,
                                        evp_aead_seal) ||
      !EVP_AEAD_CTX_seal(&aead_ctx, output, &out_len, sizeof(output), nonce,
  ...
  if (!EVP_AEAD_CTX_init_with_direction(&aead_ctx, EVP_aead_aes_128_gcm(),
                                        kAESKey, sizeof(kAESKey), 0,
                                        evp_aead_open) ||
      !EVP_AEAD_CTX_open(&aead_ctx, output, &out_len, sizeof(output), nonce,
                         EVP_AEAD_nonce_length(EVP_aead_aes_128_gcm()),
