...
int bn_mont_ctx_set_RR_consttime(BN_MONT_CTX *mont, BN_CTX *ctx) {
  ...
  //
  // The square steps take 2^n R to 2^(2n) R. This is equivalent to doubling n
  // times. If the bit of the exponent is set, the multiply step takes
  // the result of the squaring from 2^(2n) R to 2^(2n + n) R. This is
  // equivalent to doubling n/2 times. When n is below some threshold, doubling
  // n times is faster than squaring and multiplying. When above, squaring is
  // faster.
  //
  ...
  // all the remaining bits of the exponent are zero, so no multiplications will
  // be needed in the square-and-multiply stage.
