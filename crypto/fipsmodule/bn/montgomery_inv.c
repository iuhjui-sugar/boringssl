...
int bn_mont_ctx_set_RR_consttime(BN_MONT_CTX *mont, BN_CTX *ctx) {
  ...
  // RR is R, or 2^lgBigR, in the Montgomery domain. Thus we can first compute 2
  // in the Montgomery domain, 2R or 2^(lgBigR+1), and then use Montgomery
  // square-and-multiply to exponentiate.
  //
  // The multiply steps take 2^n R to 2^(n+1) R. It is more efficient to double
  // the value instead. The square steps take 2^n R to 2^(2n) R. This is
  // equivalent to doubling n times. For sufficiently small n, it is again more
  // efficient to double.
  //
  // We iterative double to this threshold, then switch to Montgomery squaring.
  // From benchmarking various 32-bit and 64-bit architectures, the word count
  // seems to work well as a threshold. (Doubling scales linearly by word count
  // and Montgomery reduction scales quadratically, so the threshold should
