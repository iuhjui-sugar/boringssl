...
// FIPS builds on Android will attempt to write flag files to /dev/boringssl/
// named after the module hash. If the flag file exists, it's assumed that
// self-tests have already passed and thus do not need to be repeated.
#define BORINGSSL_FIPS_SELF_TEST_FLAG_FILE
static const char kFlagPrefix[] = "/dev/boringssl/";
#endif
...
int BORINGSSL_self_test(
  ...
  if (flag_path_valid && access(flag_path, R_OK) == 0) {
