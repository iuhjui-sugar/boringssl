...
static int serialize_ecdsa_sig(uint8_t *out, size_t max_out, size_t *out_len,
                               const ECDSA_SIG *sig) {
  if ((max_out & 1) ||  //
      !BN_bn2bin_padded(out, max_out / 2, sig->r) ||
      !BN_bn2bin_padded(out + max_out / 2, max_out / 2, sig->s)) {
    return 0;
  }
  return 1;
}
...
static int boringssl_self_test_slow(void) {
  ...
  if (sig == NULL ||
      !serialize_ecdsa_sig(ecdsa_sign_output, sizeof(ecdsa_sign_output),
                           &ecdsa_sign_output_len, sig) ||
