...
static int boringssl_self_test_slow(void) {
  ...
  ECDSA_SIG *sig = NULL;
  uint8_t *ecdsa_sign_output = NULL;
  DH *dh = NULL;
  ...
  static const uint8_t kECDSASignSig[70] = {
      0x30, 0x44, 0x02, 0x20, 0x67, 0x80, 0xc5, 0xfc, 0x70, 0x27, 0x5e, 0x2c,
      0x70, 0x61, 0xa0, 0xe7, 0x87, 0x7b, 0xb1, 0x74, 0xde, 0xad, 0xeb, 0x98,
      0x87, 0x02, 0x7f, 0x3f, 0xa8, 0x36, 0x54, 0x15, 0x8b, 0xa7, 0xf5, 0x0c,
      0x02, 0x20, 0x68, 0x04, 0x73, 0x40, 0x94, 0xb2, 0xd1, 0x90, 0xac, 0x2d,
      0x0c, 0xd7, 0xa5, 0x7f, 0x2f, 0x2e, 0xb2, 0x62, 0xb0, 0x09, 0x16, 0xe1,
      0xa6, 0x70, 0xb5, 0xbb, 0x0d, 0xfd, 0x8e, 0x0c, 0x02, 0x3f,
  };
  ...
  if (sig == NULL ||
      !ECDSA_SIG_to_bytes(&ecdsa_sign_output, &ecdsa_sign_output_len, sig) ||
      ecdsa_sign_output_len != sizeof(kECDSASignSig) ||
      !check_test(kECDSASignSig, ecdsa_sign_output, ecdsa_sign_output_len,
  ...
  static const uint8_t kECDSAVerifySig[70] = {
      0x30, 0x44, 0x02, 0x20, 0x67, 0x80, 0xc5, 0xfc, 0x70, 0x27, 0x5e, 0x2c,
      0x70, 0x61, 0xa0, 0xe7, 0x87, 0x7b, 0xb1, 0x74, 0xde, 0xad, 0xeb, 0x98,
      0x87, 0x02, 0x7f, 0x3f, 0xa8, 0x36, 0x54, 0x15, 0x8b, 0xa7, 0xf5, 0x0c,
      0x02, 0x20, 0x2d, 0x36, 0xe5, 0x79, 0x97, 0x90, 0xbf, 0xbe, 0x21, 0x83,
      0xd3, 0x3e, 0x96, 0xf3, 0xc5, 0x1f, 0x6a, 0x23, 0x2f, 0x2a, 0x24, 0x48,
      0x8c, 0x8e, 0x5f, 0x64, 0xc3, 0x7e, 0xa2, 0xcf, 0x05, 0x29,
  };

  ECDSA_SIG_free(sig);
  sig = ECDSA_SIG_from_bytes(kECDSAVerifySig, sizeof(kECDSAVerifySig));

  if (!ECDSA_do_verify(kECDSAVerifyDigest, sizeof(kECDSAVerifyDigest), sig,
                       ec_key)) {
...
err:
  ...
  BN_free(ec_scalar);
  ECDSA_SIG_free(sig);
  OPENSSL_free(ecdsa_sign_output);
  DH_free(dh);
