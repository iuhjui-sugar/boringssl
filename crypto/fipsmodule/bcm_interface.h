...
enum bcm_status_t {
  ...
  bcm_status_failure = -1,  // Generic, does not map to an OpenSSL error.
};
...
OPENSSL_INLINE int bcm_success(bcm_status status) {
  return status >= BCM_SUCCESS_APPROVED;
}
...
// the CPU hardware random number generator if one is
// present. bcm_status_approved is returned on success, and a failure status is
// returned otherwise.
