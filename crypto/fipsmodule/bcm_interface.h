...
#define BCM_SUCCESS_APPROVED (0)
#define BCM_SUCCESS_NOT_APPROVED (1)
...
// FIPS service indictor. For the moment, the official service indiacator
...
enum bcm_infallible_t {
  bcm_infallible_approved = BCM_SUCCESS_APPROVED,
  bcm_infallible_not_approved = BCM_SUCCESS_NOT_APPROVED,
};

enum bcm_status_t {
  bcm_status_approved = BCM_SUCCESS_APPROVED,
  bcm_status_not_approved = BCM_SUCCESS_NOT_APPROVED,

  // Failure codes
  bcm_status_failure = 2,
};
...
OPENSSL_INLINE int bcm_success(bcm_status status) {
  return status == BCM_SUCCESS_APPROVED || status == BCM_SUCCESS_NOT_APPROVED;
}
...
// returns bcm_infallible_approved.
...
// non-zero this will only succeed if the CPU hwrng is thought to be "fast".
// bcm_status_approvedD is returned on success, and a failure status is
// returned otherwise.
bcm_status BCM_RAND_bytes_hwrng(uint8_t *out, size_t len, int fast);

// RAND_bytes_with_additional_data samples from the RNG after mixing 32 bytes
// from |user_additional_data| in.
void BCM_RAND_bytes_with_additional_data(
