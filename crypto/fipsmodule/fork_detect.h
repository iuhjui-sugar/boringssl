...
// is a non-zero integer which, if queried in an address space and a
// subsequently forked copy, will be different.
//
// This function may be used to clear cached single-use values across a fork.
// When initializing a cache, record the fork generation. Before using the
// cache, check if the fork generation has changed. If so, drop the cache and
// update the save fork generation. Note this logic transparently handles
// platforms which always return zero.
//
