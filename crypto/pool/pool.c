...
int CRYPTO_BUFFER_up_ref(CRYPTO_BUFFER *buf) {
  /* This is safe in the that |buf->pool| is NULL because it's just standard
   * reference counting in that case.
   *
   * This is also safe is |buf->pool| is non-NULL because, if it were racing
