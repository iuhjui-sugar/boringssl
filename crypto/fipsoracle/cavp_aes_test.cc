...
static bool MonteCarlo(const TestCtx *ctx, FileTest *t,
  ...
  std::vector<uint8_t> prev_result, result, empty_iv;
  for (int i = 0; i < 100; i++) {
    printf("COUNT = %d\r\nKEY = %s\r\n", i,
    ...
    if (!ctx->has_iv) {  // ECB mode
      for (int j = 0; j < 1000; j++) {
        prev_result = result;
        if (!CipherOperation(cipher, &result, encrypt, key, empty_iv, in)) {
          return false;
        }
        in = result;
      }
      ...
      if (!EVP_CipherInit_ex(cipher_ctx.get(), cipher, nullptr, nullptr,
                             nullptr, encrypt ? 1 : 0) ||
          !EVP_CIPHER_CTX_set_key_length(cipher_ctx.get(), key.size()) ||
          !EVP_CipherInit_ex(cipher_ctx.get(), nullptr, nullptr, key.data(), iv.data(),
                             -1) ||
      ...
      for (int j = 0; j < 1000; j++) {
        prev_result = result;
        if (j == 0) {
          if (!CipherOperationCtx(cipher, cipher_ctx.get(), &result, encrypt,
                                  key, iv, in)) {
            return false;
          }
          in = iv;
