...
static bool TestAEADEncrypt(FileTest *t, void *arg) {
  ...
  std::string count;
  std::vector<uint8_t> key, iv, pt, aad, tag, result;
  if (!t->GetAttribute(&count, "Count") || !t->GetBytes(&key, "Key") ||
      !t->GetBytes(&aad, "AAD") || !t->GetBytes(&pt, "PT") ||
    ...
    t->PrintLine("Invalid test case B.");
    return false;
  }

  printf("%s", t->CurrentTestToString().c_str());
  size_t tag_len = strtoul(tag_len_str.c_str(), nullptr, 0);
  if (AEADEncrypt(ctx->aead, key, pt, aad, &iv, &tag, tag_len / 8, &result)) {
    printf("IV = %s\n", EncodeHex(iv.data(), result.size()).c_str());
  ...
  if (ctx->response_sample) {
    ...
    if (!ctx->response_sample->GetAttribute(&expected_count, "Count") ||
        ...
        !t->ExpectBytesEqual(expected_ct.data(), expected_ct.size(),
...
static bool TestAEADDecrypt(FileTest *t, void *arg) {
  ...
  if (!t->GetAttribute(&count, "Count") || !t->GetBytes(&key, "Key") ||
      ...
      ct.size() != pt_len || aad.size() != aad_len ||
  ...
  printf("%s", t->CurrentTestToString().c_str());
  bool aead_result =
      AEADDecrypt(ctx->aead, key, ct, tag, iv, &aad, aad_len, &result, pt_len);
  if (aead_result) {
    printf("PT = %s\n\n", EncodeHex(result.data(), result.size()).c_str());
  } else {
    printf("FAIL\n\n");
