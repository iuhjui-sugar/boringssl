...
static int test_decode(void) {
  ...
  for (i = 0; i < kNumTests; i++) {
    ...
    if (!EVP_DecodeBase64(&len, out, sizeof(out),
  ...
  if (EVP_DecodeBase64(&len, out, sizeof(out), (const uint8_t*)"a!bc", 4)) {
  ...
  if (EVP_DecodeBase64(&len, out, sizeof(out), (const uint8_t*)"abc", 4)) {
