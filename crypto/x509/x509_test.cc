...
TEST(X509Test, ServerGatedCryptoEKUs) {
  ...
  for (X509 *leaf : {ms_sgc.get(), ns_sgc.get()}) {
  ...
  // also included. The explicit "any" EKU should also work and, lastly, not
  // specifying an EKU is also valid.
  for (X509 *leaf : {server_eku.get(), server_eku_plus_ms_sgc.get(),
                     any_eku.get(), no_eku.get()}) {
