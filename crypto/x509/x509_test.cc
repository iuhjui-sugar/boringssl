...
static bssl::UniquePtr<EVP_PKEY> PrivateKeyFromPEM(const char *pem) {
  bssl::UniquePtr<BIO> bio(BIO_new_mem_buf(const_cast<char *>(pem), strlen(pem)));
...
static bool Verify(X509 *leaf, const std::vector<X509 *> &roots,
  ...
  bssl::UniquePtr<STACK_OF(X509)> intermediates_stack(CertsToStack(intermediates));
  if (!roots_stack ||
      !intermediates_stack) {
    return false;
  }
...
static bool TestVerify() {
  ...
  bssl::UniquePtr<X509> intermediate_self_signed(CertFromPEM(kIntermediateSelfSignedPEM));
