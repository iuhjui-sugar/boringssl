...
TEST(X509Test, TestCRL) {
  ...
  ASSERT_EQ(X509_V_ERR_CERT_REVOKED,
  ...
  ASSERT_EQ(X509_V_ERR_UNABLE_TO_GET_CRL,
            Verify(leaf.get(), {root.get()}, {root.get()}, empty_crls,
                   X509_V_FLAG_CRL_CHECK));
  ASSERT_EQ(X509_V_ERR_UNABLE_TO_GET_CRL,
            Verify(leaf.get(), {root.get()}, {root.get()}, {bad_issuer_crl.get()},
                   X509_V_FLAG_CRL_CHECK));
  ASSERT_EQ(X509_V_OK,
            Verify(leaf.get(), {root.get()}, {root.get()}, {known_critical_crl.get()},
                   X509_V_FLAG_CRL_CHECK));
  ASSERT_EQ(X509_V_ERR_UNHANDLED_CRITICAL_CRL_EXTENSION,
            Verify(leaf.get(), {root.get()}, {root.get()}, {unknown_critical_crl.get()},
                   X509_V_FLAG_CRL_CHECK));
  ASSERT_EQ(X509_V_ERR_UNHANDLED_CRITICAL_CRL_EXTENSION,
            Verify(leaf.get(), {root.get()}, {root.get()}, {unknown_critical_crl2.get()},
                   X509_V_FLAG_CRL_CHECK));
}
...
TEST(X509Test, TestSignCtx) {
  ...
  ASSERT_TRUE(EVP_DigestSignInit(md_ctx.get(), NULL, EVP_sha256(), NULL, pkey.get()));
...
TEST(X509Test, TestPrintUTCTIME) {
  ...
  for (auto t : asn1_utctime_tests) {
    ...
    ASSERT_EQ(ok, (strcmp(t.want, "Bad time value") != 0));
    ASSERT_EQ(len, strlen(t.want));
    ASSERT_FALSE(memcmp(contents, t.want, len));
  }
}
