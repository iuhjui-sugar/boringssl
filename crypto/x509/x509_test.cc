...
// |X509*|.
static bssl::unique_ptr<X509> CertFromPEM(const char *pem) {
  bssl::unique_ptr<BIO> bio(BIO_new_mem_buf(pem, strlen(pem)));
...
// |EVP_PKEY*|.
static bssl::unique_ptr<EVP_PKEY> PrivateKeyFromPEM(const char *pem) {
  bssl::unique_ptr<BIO> bio(BIO_new_mem_buf(const_cast<char *>(pem), strlen(pem)));
...
static STACK_OF(X509)* CertsToStack(const std::vector<X509*> &certs) {
  bssl::unique_ptr<STACK_OF(X509)> stack(sk_X509_new_null());
  if (!stack) {
    return nullptr;
  }
  for (auto cert : certs) {
...
static bool Verify(X509 *leaf, const std::vector<X509 *> &roots,
  ...
  bssl::unique_ptr<STACK_OF(X509)> roots_stack(CertsToStack(roots));
  bssl::unique_ptr<STACK_OF(X509)> intermediates_stack(CertsToStack(intermediates));
  if (!roots_stack ||
      !intermediates_stack) {
    return false;
  }

  bssl::unique_ptr<X509_STORE_CTX> ctx(X509_STORE_CTX_new());
  if (!ctx) {
    return false;
  }
...
static bool TestVerify() {
  bssl::unique_ptr<X509> cross_signing_root(CertFromPEM(kCrossSigningRootPEM));
  bssl::unique_ptr<X509> root(CertFromPEM(kRootCAPEM));
  bssl::unique_ptr<X509> root_cross_signed(CertFromPEM(kRootCrossSignedPEM));
  bssl::unique_ptr<X509> intermediate(CertFromPEM(kIntermediatePEM));
  bssl::unique_ptr<X509> intermediate_self_signed(CertFromPEM(kIntermediateSelfSignedPEM));
  bssl::unique_ptr<X509> leaf(CertFromPEM(kLeafPEM));
  bssl::unique_ptr<X509> leaf_no_key_usage(CertFromPEM(kLeafNoKeyUsagePEM));
  bssl::unique_ptr<X509> forgery(CertFromPEM(kForgeryPEM));
...
static bool TestPSS() {
  bssl::unique_ptr<X509> cert(CertFromPEM(kExamplePSSCert));
  if (!cert) {
    return false;
  }

  bssl::unique_ptr<EVP_PKEY> pkey(X509_get_pubkey(cert.get()));
  if (!pkey) {
    return false;
  }
...
static bool TestBadPSSParameters() {
  bssl::unique_ptr<X509> cert(CertFromPEM(kBadPSSCertPEM));
  if (!cert) {
    return false;
  }
...
static bool SignatureRoundTrips(EVP_MD_CTX *md_ctx, EVP_PKEY *pkey) {
  ...
  bssl::unique_ptr<X509> cert(CertFromPEM(kLeafPEM));
...
static bool TestSignCtx() {
  bssl::unique_ptr<EVP_PKEY> pkey(PrivateKeyFromPEM(kRSAKey));
  if (!pkey) {
    return false;
  }
