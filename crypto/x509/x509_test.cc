...
TEST(X509Test, ZeroLengthsWithX509PARAM) {
  ...
  for (size_t i = 0; i < kTests.size(); i++) {
    ...
    ASSERT_EQ(X509_V_OK, Verify(leaf.get(), {root.get()}, {}, empty_crls, 0,
                                false, [&test](X509_VERIFY_PARAM *param) {
                                  if (!test.func(param, test.correct_value,
    ...
    ASSERT_EQ(test.mismatch_error,
              Verify(leaf.get(), {root.get()}, {}, empty_crls, 0, false,
                     [&test](X509_VERIFY_PARAM *param) {
                       if (!test.func(param, test.incorrect_value,
                                      test.incorrect_value_len)) {
                         abort();
                       }
                     }));

    // Passing zero as the length, for this API, is supported for compatibility
    // with OpenSSL.
    ASSERT_EQ(X509_V_OK,
              Verify(leaf.get(), {root.get()}, {}, empty_crls, 0, false,
                     [&test](X509_VERIFY_PARAM *param) {
                       if (!test.func(param, test.correct_value, 0)) {
                         abort();
                       }
                     }));

    // Passing zero should not be a no-op.
    ASSERT_EQ(test.mismatch_error,
              Verify(leaf.get(), {root.get()}, {}, empty_crls, 0, false,
                     [&test](X509_VERIFY_PARAM *param) {
                       if (!test.func(param, test.incorrect_value, 0)) {
                         abort();
                       }
                     }));
  ...
  ASSERT_EQ(X509_V_OK,
            Verify(leaf.get(), {root.get()}, {}, empty_crls, 0, false,
                   [](X509_VERIFY_PARAM *param) {
                     if (!X509_VERIFY_PARAM_set1_ip(param, kIP, sizeof(kIP))) {
                       abort();
                     }
                   }));
  ...
  ASSERT_EQ(X509_V_ERR_IP_ADDRESS_MISMATCH,
            Verify(leaf.get(), {root.get()}, {}, empty_crls, 0, false,
                   [](X509_VERIFY_PARAM *param) {
                     if (!X509_VERIFY_PARAM_set1_ip(param, kWrongIP,
                                                    sizeof(kWrongIP))) {
                       abort();
                     }
                   }));
  ...
  ASSERT_EQ(X509_V_ERR_INVALID_CALL,
            Verify(leaf.get(), {root.get()}, {}, empty_crls, 0, false,
                   [](X509_VERIFY_PARAM *param) {
                     if (X509_VERIFY_PARAM_set1_ip(param, kIP, 0)) {
                       abort();
                     }
                   }));
  ...
  ASSERT_EQ(X509_V_ERR_INVALID_CALL,
            Verify(leaf.get(), {root.get()}, {}, empty_crls, 0, false,
                   [](X509_VERIFY_PARAM *param) {
                     if (X509_VERIFY_PARAM_set1_ip(param, nullptr, 0)) {
                       abort();
                     }
                   }));
