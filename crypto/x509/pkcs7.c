...
int PKCS7_get_PEM_certificates(STACK_OF(X509) *out_certs, BIO *pem_bio) {
  ...
  /* Even though we pass "PKCS7" as the expected PEM type here, PEM internally
   * will actually allow several other values too, including "CERTIFICATE". */
  if (!PEM_bytes_read_bio(&data, &len, NULL /* PEM type output */, "PKCS7",
                          pem_bio, NULL /* password callback */,
