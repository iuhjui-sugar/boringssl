...
template <typename PUBLIC_KEY, size_t PUBLIC_KEY_BYTES, typename PRIVATE_KEY,
          size_t PRIVATE_KEY_BYTES, void (*GENERATE)(uint8_t *, PRIVATE_KEY *),
...
void BasicTest() {
  ...
  GENERATE(encoded_public_key, priv.get());
...
TEST(MLKEMTest, Basic768) {
  BasicTest<MLKEM_public_key, MLKEM_PUBLIC_KEY_BYTES, MLKEM_private_key,
            ...
            MLKEM_public_from_private, MLKEM_parse_public_key,
            MLKEM_marshal_public_key, MLKEM_parse_private_key,
            MLKEM_marshal_private_key, MLKEM_CIPHERTEXT_BYTES, MLKEM_encap,
            MLKEM_decap>();
}
...
TEST(MLKEMTest, Basic1024) {
  BasicTest<MLKEM1024_public_key, MLKEM1024_PUBLIC_KEY_BYTES,
            ...
            MLKEM1024_generate_key, MLKEM1024_public_from_private,
            MLKEM1024_parse_public_key, MLKEM1024_marshal_public_key,
            MLKEM1024_parse_private_key, MLKEM1024_marshal_private_key,
            MLKEM1024_CIPHERTEXT_BYTES, MLKEM1024_encap, MLKEM1024_decap>();
}
...
void IteratedTest(uint8_t out[32]) {
  ...
  for (int i = 0; i < 10000; i++) {
    uint8_t generate_entropy[MLKEM_GENERATE_KEY_ENTROPY];
...
TEST(MLKEMTest, Iterate768) {
  ...
  // Value from
  // https://github.com/C2SP/CCTV/tree/main/ML-KEM#accumulated-pq-crystals-vectors
  const uint8_t kExpected[32] = {
      0xf7, 0xdb, 0x26, 0x0e, 0x11, 0x37, 0xa7, 0x42, 0xe0, 0x5f, 0xe0,
      0xdb, 0x95, 0x25, 0x01, 0x28, 0x12, 0xb0, 0x04, 0xd2, 0x90, 0x40,
      0xa5, 0xb6, 0x06, 0xaa, 0xd3, 0xd1, 0x34, 0xb5, 0x48, 0xd3};
...
TEST(MLKEMTest, Iterate1024) {
  ...
  // https://github.com/C2SP/CCTV/tree/main/ML-KEM#accumulated-pq-crystals-vectors
  const uint8_t kExpected[32] = {
      0x47, 0xac, 0x88, 0x8f, 0xe6, 0x15, 0x44, 0xef, 0xc0, 0x51, 0x8f,
      0x46, 0x09, 0x4b, 0x4f, 0x8a, 0x60, 0x09, 0x65, 0xfc, 0x89, 0x82,
      0x2a, 0xcb, 0x06, 0xdc, 0x71, 0x69, 0xd2, 0x4f, 0x35, 0x43};
  EXPECT_EQ(Bytes(result), Bytes(kExpected));
}
}  // namespace
