...
static_assert(encoded_public_key_size(RANK768) == MLKEM_PUBLIC_KEY_BYTES, "");
...
static_assert(ciphertext_size(RANK768) == MLKEM_CIPHERTEXT_BYTES, "");
...
static struct public_key<RANK768> *public_key_768_from_external(
    const struct MLKEM_public_key *external) {
  static_assert(sizeof(struct MLKEM_public_key) >=
                    sizeof(struct public_key<RANK768>),
                "MLKEM public key is too small");
  static_assert(alignof(struct MLKEM_public_key) >=
...
static struct private_key<RANK768> *private_key_768_from_external(
    const struct MLKEM_private_key *external) {
  static_assert(sizeof(struct MLKEM_private_key) >=
                    sizeof(struct private_key<RANK768>),
                "MLKEM private key too small");
  static_assert(alignof(struct MLKEM_private_key) >=
...
void MLKEM_generate_key(uint8_t out_encoded_public_key[MLKEM_PUBLIC_KEY_BYTES],
  ...
  if (optional_out_seed) {
    OPENSSL_memcpy(optional_out_seed, entropy, sizeof(entropy));
  }
  MLKEM_generate_key_external_entropy(out_encoded_public_key, out_private_key,
                                      entropy);
}

void MLKEM_private_key_from_seed(struct MLKEM_private_key *out_private_key,
                                 const uint8_t seed[MLKEM_SEED_BYTES]) {
  uint8_t public_key_bytes[MLKEM_PUBLIC_KEY_BYTES];
  MLKEM_generate_key_external_entropy(public_key_bytes, out_private_key, seed);
}
...
void MLKEM1024_private_key_from_seed(
    struct MLKEM1024_private_key *out_private_key,
    const uint8_t seed[MLKEM_SEED_BYTES]) {
  uint8_t public_key_bytes[MLKEM1024_PUBLIC_KEY_BYTES];
  MLKEM1024_generate_key_external_entropy(public_key_bytes, out_private_key,
                                          seed);
}
...
template <int RANK>
void mlkem_generate_key_external_entropy(
    uint8_t *out_encoded_public_key, private_key<RANK> *priv,
    const uint8_t entropy[MLKEM_SEED_BYTES]) {
  uint8_t augmented_entropy[33];
  OPENSSL_memcpy(augmented_entropy, entropy, 32);
  augmented_entropy[32] = RANK;

  uint8_t hashed[64];
  hash_g(hashed, augmented_entropy, sizeof(augmented_entropy));
...
void MLKEM_generate_key_external_entropy(
  ...
  mlkem_generate_key_external_entropy(out_encoded_public_key, priv, entropy);
}
...
void MLKEM_public_from_private(struct MLKEM_public_key *out_public_key,
...
// Calls MLKEM_encap_external_entropy| with random bytes from |RAND_bytes|
void MLKEM_encap(uint8_t out_ciphertext[MLKEM_CIPHERTEXT_BYTES],
  ...
  MLKEM_encap_external_entropy(out_ciphertext, out_shared_secret, public_key,
                               entropy);
}
...
void MLKEM1024_decap(uint8_t out_shared_secret[MLKEM_SHARED_SECRET_BYTES],
...
int MLKEM_marshal_public_key(CBB *out,
                             const struct MLKEM_public_key *public_key) {
...
int MLKEM_parse_public_key(struct MLKEM_public_key *public_key, CBS *in) {
...
int MLKEM_marshal_private_key(CBB *out,
                              const struct MLKEM_private_key *private_key) {
  const struct ::private_key<RANK768> *const priv =
      private_key_768_from_external(private_key);
...
int MLKEM_parse_private_key(struct MLKEM_private_key *out_private_key,
                            CBS *in) {
