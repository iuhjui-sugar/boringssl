...
TEST(ASN1Test, OptionalAndDefaultBooleans) {
  ...
  EXPECT_EQ(obj->optional, -1);
  EXPECT_EQ(obj->default_true, 0xff);
  EXPECT_EQ(obj->default_false, 0);
  ...
  obj->optional = 0;
  obj->default_true = 0;
  obj->default_false = 0xff;
  TestSerialize(obj.get(), i2d_BOOLEANS, kFieldsIncluded);

  der = kFieldsIncluded;
  obj.reset(d2i_BOOLEANS(nullptr, &der, sizeof(kFieldsIncluded)));
  ASSERT_TRUE(obj);
  EXPECT_EQ(obj->required, 0xff);
  EXPECT_EQ(obj->optional, 0);
  EXPECT_EQ(obj->default_true, 0);
  EXPECT_EQ(obj->default_false, 0xff);
