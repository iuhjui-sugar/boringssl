...
TEST(ASN1Test, TimeOverflow) {
  ...
  // It is annoying this is a long for historical reasons.
  uint64_t positive_range = LONG_MAX;
  uint64_t negative_range = LONG_MIN;
  if (sizeof(long) == sizeof(int64_t)) {
    const int64_t kValidTimeRange = 315569519999;
    positive_range = kValidTimeRange;
    negative_range = -kValidTimeRange;
  }
  copy = min_time;
  EXPECT_TRUE(OPENSSL_gmtime_adj(&copy, /*offset_day=*/0, positive_range));
  EXPECT_EQ(TimeToTuple(copy), TimeToTuple(max_time));
  EXPECT_TRUE(OPENSSL_gmtime_adj(&copy, /*offset_day=*/0, negative_range));
  EXPECT_EQ(TimeToTuple(copy), TimeToTuple(min_time));
  ...
  EXPECT_TRUE(OPENSSL_gmtime_adj(&copy, /*offset_day=*/-1,
                                 positive_range + 24 * 3600));
  EXPECT_EQ(TimeToTuple(copy), TimeToTuple(max_time));
  EXPECT_TRUE(OPENSSL_gmtime_adj(&copy, /*offset_day=*/1,
                                 negative_range - 24 * 3600));
  EXPECT_EQ(TimeToTuple(copy), TimeToTuple(min_time));
