...
// Is a time_t representing a time within our expected range
...
// outside of the years 0000 to 9999. Returns 1 on success, 0 on
// failure.
//
// If time_t is integer sized, we return 0 if the value is not
// representable in an integer sized time_t.
int OPENSSL_timegm(const struct tm *tm, time_t *out) {
  static_assert(sizeof(time_t) == sizeof(int32_t) ||
                sizeof(time_t) == sizeof(int64_t), "time_t is broken");
...
struct tm *OPENSSL_gmtime(const time_t *time, struct tm *out_tm) {
  ...
  if (!utc_from_posix_time(time64, &out_tm->tm_year, &out_tm->tm_mon,
