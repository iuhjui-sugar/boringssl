...
#if defined(OPENSSL_WINDOWS)
  fd_set read_fds, write_rds;
  FD_ZERO(&read_fds);
  FD_ZERO(&write_fds);
  FD_SET(sock, wait_type == WaitType::kRead ? &read_fds : &write_fds);
  timeval timeout;
  timeout.tv_sec = kTimeoutSeconds;
  timeout.tv_usec = 0;
  if (select(0, &read_fds, &write_fds, nullptr, &timeout) <= 0) {
    return false;
  }
  return FD_ISSET(sock, wait_type == WaitType::kRead ? &read_fds : &write_fds);
#else
