...
static bool WaitForReadable(Socket sock) {
  ...
  timeout.tv_usec = 0;
  return select(0, &fds, nullptr, nullptr, &timeout) == 1 &&
         FD_ISSET(sock, &fds);
#else
  pollfd fd = {/*fd=*/sock, /*events=*/POLLIN, /*revents=*/0};
  return poll(&fd, 1, kTimeoutSeconds * 1000) == 1 && (fd.revents & POLLIN);
#endif
}
...
TEST(BIOTest, SocketNonBlocking) {
  ...
  // |accept_bio| should readable. Drain it.
  int bytes_read = 0;
  while (bytes_read < bytes_written) {
    ASSERT_TRUE(WaitForReadable(accept_sock.get())) << LastSocketError();
    ret = BIO_read(accept_bio.get(), buf, sizeof(buf));
    ASSERT_GT(ret, 0);
    bytes_read += ret;
  }

  // |connect_bio| should be writeable again.
  ret = BIO_write(connect_bio.get(), kTestMessage, sizeof(kTestMessage));
