...
static bool TestSocketConnect() {
  ...
  BIO_snprintf(hostname, sizeof(hostname), "%s:%d", "127.0.0.1",
               ntohs(sin.sin_port));
  bssl::unique_ptr<BIO> bio(BIO_new_connect(hostname));
  if (!bio) {
...
static bool TestZeroCopyBioPairs() {
  ...
  for (size_t i = 0; i < OPENSSL_ARRAY_SIZE(kLengths); i++) {
    for (size_t j = 0; j < OPENSSL_ARRAY_SIZE(kPartialLengths); j++) {
      ...
      bssl::unique_ptr<BIO> bio1_scoper(bio1);
      bssl::unique_ptr<BIO> bio2_scoper(bio2);
...
static bool ReadASN1(bool should_succeed, const uint8_t *data, size_t data_len,
  ...
  bssl::unique_ptr<BIO> bio(BIO_new_mem_buf(data, data_len));
  ...
  bssl::unique_ptr<uint8_t> out_storage(out);
...
static bool TestASN1() {
  ...
  bssl::unique_ptr<uint8_t> large(reinterpret_cast<uint8_t *>(
