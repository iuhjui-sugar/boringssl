...
// pkcs7_bundle writes a PKCS#7, SignedData structure to |out|. While doing so
// it makes callbacks to let the caller fill in parts of the structure. All
// callbacks are ignored if NULL and return one on success or zero on error.
//
...
int pkcs7_bundle(CBB *out, int (*digest_algos_cb)(CBB *out, const void *arg),
