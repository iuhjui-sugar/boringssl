...
int PKCS7_bundle_certificates(CBB *out, const STACK_OF(X509) *certs) {
  return pkcs7_bundle(out, /*digest_algos_cb=*/NULL,
                      pkcs7_bundle_certificates_cb, /*signer_infos_cb=*/NULL,
                      certs);
}
...
int PKCS7_bundle_CRLs(CBB *out, const STACK_OF(X509_CRL) *crls) {
  return pkcs7_bundle(out, /*digest_algos_cb=*/NULL, pkcs7_bundle_crls_cb,
...
// |digest_algos_set|.
static int write_sha256_ai(CBB *digest_algos_set, const void *) {
  CBB seq;
...
PKCS7 *PKCS7_sign(X509 *sign_cert, EVP_PKEY *pkey, STACK_OF(X509) *certs,
  ...
  if (sign_cert == NULL && pkey == NULL && flags == PKCS7_DETACHED) {
    ...
    if (!si_data.signature ||
        ...
        !pkcs7_bundle(&cbb, write_sha256_ai, /*cert_crl_cb=*/NULL,
