...
template<typename T> const void *Func(T func) {
  static_assert(std::is_function<typename std::remove_pointer<T>::type>::value,
                "argument to Func is not a function pointer");
  return reinterpret_cast<const char *>(func);
}

static bool g_sde_detected = false;
static CRYPTO_once_t g_sde_detected_once = CRYPTO_ONCE_INIT;

static void g_sde_detected_init() {
  ...
  constexpr size_t kBufSize = 1024 * 32;
  std::unique_ptr<uint8_t[]> buf(new uint8_t[kBufSize]);
  size_t len = 0;
  ssize_t n;

  do {
    n = read(fd, buf.get() + len, kBufSize - len);
    if (n > 0) {
      len += n;
    }
  } while (len < kBufSize && (n > 0 || (n < 0 && errno == EINTR)));
  close(fd);
  ...
  g_sde_detected = (memmem(buf.get(), len, "/pinbin\n", 8) != nullptr);
}
...
class ImplDispatchTest : public ::testing::Test {
 protected:
  ...
  void AssertBreakpoints(
      ...
      std::function<void()> f) {
    CRYPTO_once(&g_sde_detected_once, g_sde_detected_init);
    if (g_sde_detected) {
      // This trick does not work under SDE, sadly.
      printf("Skipping because SDE was detected.\n");
      return;
    }
...
TEST_F(ImplDispatchTest, AESNI) {
  const bool aesni = OPENSSL_ia32cap_P[1] & (1 << (57 - 32));
  const bool avx_movbe = ((OPENSSL_ia32cap_P[1] >> 22) & 0x41) == 0x41;

  AssertBreakpoints(
      {
          {Func(CRYPTO_gcm128_encrypt_ctr32), true},
          {Func(aes_hw_ctr32_encrypt_blocks), aesni},
          {Func(aes_hw_encrypt), aesni},
          {Func(aesni_gcm_encrypt), aesni && avx_movbe},
      },
      [] {
