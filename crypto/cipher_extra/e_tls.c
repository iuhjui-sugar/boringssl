...
static int aead_tls_seal(const EVP_AEAD_CTX *ctx, uint8_t *out, size_t *out_len,
  ...
  size_t out_tag_len;
  if (!aead_tls_seal_scatter(ctx, out, out + cipher_len, &out_tag_len,
                             max_out_len - cipher_len, nonce, nonce_len, in,
                             in_len, ad, ad_len)) {
    return 0;
  }
