...
static int aead_chacha20_poly1305_seal_scatter(
  ...
  if (max_out_tag_len < c20_ctx->tag_len) {
  ...
  OPENSSL_memcpy(out_tag, tag, c20_ctx->tag_len);
  *out_tag_len = c20_ctx->tag_len;
  return 1;
}
...
static int aead_chacha20_poly1305_seal(const EVP_AEAD_CTX *ctx, uint8_t *out,
  ...
  size_t out_tag_len;
  if (!aead_chacha20_poly1305_seal_scatter(ctx, out, out + in_len, &out_tag_len,
                                           max_out_len - in_len, nonce,
...
static int aead_chacha20_poly1305_open_gather(
  ...
  if (CRYPTO_memcmp(tag, in_tag, c20_ctx->tag_len) != 0) {
    OPENSSL_PUT_ERROR(CIPHER, CIPHER_R_BAD_DECRYPT);
    return 0;
  }
...
static int aead_chacha20_poly1305_open(const EVP_AEAD_CTX *ctx, uint8_t *out,
                                       ...
                                       const uint8_t *ad, size_t ad_len) {
  const struct aead_chacha20_poly1305_ctx *c20_ctx = ctx->aead_state;
  size_t pt_len = in_len - c20_ctx->tag_len;
  ...
  if (!aead_chacha20_poly1305_open_gather(ctx, out, nonce, nonce_len, in,
                                          pt_len, in + pt_len, in_len - pt_len,
                                          ad, ad_len)) {
    return 0;
  }
  *out_len = pt_len;
