...
OPENSSL_COMPILE_ASSERT(alignof(((EVP_AEAD_CTX *)NULL)->state) >= 8,
...
static struct aead_aes_gcm_siv_asm_ctx *asm_ctx_from_ctx(
  ...
  uintptr_t p = (uintptr_t)&ctx->state;
  p = p + (p & 8);
  return (struct aead_aes_gcm_siv_asm_ctx *) p;
}
...
OPENSSL_COMPILE_ASSERT(sizeof(((EVP_AEAD_CTX *)NULL)->state) >=
                       ...
                       AEAD_state_too_small);
#if defined(__GNUC__) || defined(__clang__)
OPENSSL_COMPILE_ASSERT(alignof(((EVP_AEAD_CTX *)NULL)->state) >=
