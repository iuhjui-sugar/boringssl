...
static void CheckRepresentation(T value) {
  ...
  // Convert to the corresponding two's-complement unsigned value. If |T| is
  // already unsigned, this is a no-op.
  //
  // If |T| is signed, conversion to unsigned is defined to repeatedly adding or
  // subtracting (numerically, not within |T|) one more than the unsigned type's
  // maximum value until it fits (this must be a power of two).
