...
static const MD md4 = { "MD4", &EVP_md4, NULL };
...
static const MD md5_sha1 = { "MD5-SHA1", &EVP_md5_sha1, NULL };
...
static int test_digest(const TEST_VECTOR *test) {
  ...
  if (!EVP_DigestInit_ex(&ctx, test->md->func(), NULL)) {
  ...
  if (test->md->one_shot_func && test->repeat == 1) {
    ...
    if (!compare_digest(test, digest, EVP_MD_size(test->md->func()))) {
      goto done;
    }
    ...
    if (!compare_digest(test, out, EVP_MD_size(test->md->func()))) {
      goto done;
    }
  }
