...
static void cbb_init_toplevel(CBB *cbb, uint8_t *buf, size_t cap,
                              int can_resize) {
  cbb->is_child = 0;
  cbb->child = NULL;
  cbb->u.base.buf = buf;
...
int CBB_init(CBB *cbb, size_t initial_capacity) {
  ...
  cbb_init_toplevel(cbb, buf, initial_capacity, /*can_resize=*/1);
  return 1;
}

int CBB_init_fixed(CBB *cbb, uint8_t *buf, size_t len) {
  CBB_zero(cbb);
  cbb_init_toplevel(cbb, buf, len, /*can_resize=*/0);
  return 1;
}
...
static int cbb_add_child(CBB *cbb, CBB *out_child, uint8_t len_len,
  ...
  OPENSSL_memset(out_child, 0, sizeof(CBB));
...
int CBB_add_bytes(CBB *cbb, const uint8_t *data, size_t len) {
  ...
  if (!CBB_flush(cbb) ||
      !cbb_buffer_add(cbb_get_base(cbb), &dest, len)) {
    return 0;
  }
...
int CBB_add_u8(CBB *cbb, uint8_t value) {
  ...
  return cbb_buffer_add_u(cbb_get_base(cbb), value, 1);
}
...
int CBB_add_u16(CBB *cbb, uint16_t value) {
  ...
  return cbb_buffer_add_u(cbb_get_base(cbb), value, 2);
}
...
int CBB_add_u24(CBB *cbb, uint32_t value) {
  ...
  return cbb_buffer_add_u(cbb_get_base(cbb), value, 3);
}
...
int CBB_add_u32(CBB *cbb, uint32_t value) {
  ...
  return cbb_buffer_add_u(cbb_get_base(cbb), value, 4);
}
...
int CBB_add_u64(CBB *cbb, uint64_t value) {
  ...
  return cbb_buffer_add_u(cbb_get_base(cbb), value, 8);
}
