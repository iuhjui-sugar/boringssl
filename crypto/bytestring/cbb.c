...
static void cbb_on_error(CBB *cbb) {
  ...
  // convention. Probably the thing to do is allow |CBB_cleanup| on child |CBB|s
  // and make every child's |CBB_cleanup| set the error bit if unflushed. That
  // will be convenient for C++ callers, but very tedious C callers. So C
  // callers perhaps should get a |CBB_on_error| function that can be, less
  // tediously, stuck in a |goto err| block.
