...
static void cbb_init(CBB *cbb, uint8_t *buf, size_t cap) {
  cbb->is_child = 0;
  cbb->child = NULL;
  cbb->u.base.buf = buf;
  cbb->u.base.len = 0;
  cbb->u.base.cap = cap;
  cbb->u.base.can_resize = 1;
  cbb->u.base.error = 0;
}
...
int CBB_init(CBB *cbb, size_t initial_capacity) {
  ...
  cbb_init(cbb, buf, initial_capacity);
  return 1;
}

int CBB_init_fixed(CBB *cbb, uint8_t *buf, size_t len) {
  CBB_zero(cbb);
  cbb_init(cbb, buf, len);
  cbb->u.base.can_resize = 0;
  return 1;
}
