 ...
 * |CBS_is_valid_string(cbs)| to check for NUL bytes. */
int CBS_strdup(const CBS *cbs, char **out_ptr);

/* CBS_is_valid_string returns one if the current contents |cbs| do not
   contain NUL bytes and zero otherwise. */
int CBS_is_valid_string(const CBS *cbs);

/* CBS_mem_equal compares the current contents of |cbs| with the |len|
 * bytes starting at |data|. If they're equal, it returns one,
 * otherwise zero. This is not a constant-time comparison. */
