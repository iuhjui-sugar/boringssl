...
static int cbs_get_length_prefixed(CBS *cbs, CBS *out, size_t len_len) {
  uint64_t len64;
  if (!cbs_get_u(cbs, &len64, len_len)) {
    return 0;
  }
  size_t len = len64;
  if (len64 != (uint64_t)len) {
    // len64 should fit into size_t
    // for call to CBS_get_bytes
    return 0;
  }
...
int CBS_get_bytes_length_prefixed(CBS *cbs, CBS *out, size_t len_len) {
  return cbs_get_length_prefixed(cbs, out, len_len);
}
