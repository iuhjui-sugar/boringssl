...
static int cbs_get_any_asn1_element(CBS *cbs, CBS *out, unsigned *out_tag,
   ...
   * If the first byte is 31 (0x1f, the largets value that fits in the allotted
   * bits), then the tag is more than one byte long and the continuation bytes
   * contain the tag number. This parser only supports tag numbers less than 31
   * (and thus single-byte tags). */
  if ((tag & 0x1f) == 0x1f) {
    return 0;
  }
