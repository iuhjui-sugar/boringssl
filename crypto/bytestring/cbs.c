...
int CBS_parse_rfc5280_time(const CBS *cbs, int type, struct tm *out_tm) {
  ...
  // Allow for a four digit timzone offset to be specified.  This is
  // not allowed in RFC 5280, but exists out there in a few hard to
  // get rid of places where artisinally rolled long lived
  // certificates were baked into places after being generated with
  // the openssl command that permissively allowed the creation of
  // certificates with specified timezones.
  //
  // TODO(bbe) This has been expunged from public web-pki, find a
  // way to get rid of this or make it off by default.
  if (offset_sign != 0) {
