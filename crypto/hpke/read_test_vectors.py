...
def rename_odict_keys(odict, subst):
  """Rename each of |odict|'s keys by looking up in |subst|.

  Preserves order of key-value pairs.
  """
  return collections.OrderedDict([(subst.get(k, k), odict[k]) for k in odict])
...
def read_test_vectors_and_generate_code(json_file_in_path, test_file_out_path):
  ...
  kHpkeModeBase = 0
  kHpkeDhkemX25519Sha256 = 0x0020

  test_vectors = filter(lambda v: \
      v["mode"] == kHpkeModeBase and \
      v["kemID"] == kHpkeDhkemX25519Sha256, test_vectors)
  ...
  with open(test_file_out_path, "w") as file_out:
    for t in test_vectors:
      t = rename_odict_keys(t, {"nonce": "outerNonce"})
