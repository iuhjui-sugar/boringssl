...
// SHA256). Despite only supporting this one KEM, any supported KDF can be used
...
// EVP_HPKE_derive_x25519_keypair is used internally and for tests. This
// function derives an X25519 keypair from the |ikm| bytestring.
OPENSSL_EXPORT int EVP_HPKE_derive_x25519_keypair(uint8_t *out_private,
                                                  uint8_t *out_public,
                                                  const uint8_t *ikm,
                                                  size_t ikm_len);
...
// EVP_HPKE_CTX_free calls |EVP_HPKE_CTX_cleanup| and then frees |ctx| itself.
...
//   * The |kdf_id| parameter selects which KDF is used for non-KEM HPKE
//     operations. Its value must be one of the |HPKE_KDF_*| constants.
//   * The |aead_id| parameter controls which AEAD is used for the "open" and
//     "seal" operations. Its value must be one of the |HPKE_AEAD_*| constants.
//
// For all other EVP_HPKE_CTX_* functions:
//   * The |hpke| parameter must be non-NULL and already initialized.
...
// EVP_HPKE_CTX_open uses the HPKE context |hpke| to (a) authenticate |in_len|
// bytes from |in| and |ad_len| bytes from |ad| and (b) to decrypt at most
...
OPENSSL_EXPORT int EVP_HPKE_CTX_export(const EVP_HPKE_CTX *hpke,
