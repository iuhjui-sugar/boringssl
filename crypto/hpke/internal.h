...
//
// Both |out_enc_len| and |peer_public_value_len| must be at least
...
// sender key.
//
// |peer_public_value_len| must be at least |X25519_PUBLIC_VALUE_LEN|.
// |ephemeral_private_len| must be at least |X25519_PRIVATE_KEY_LEN|.
// |ephemeral_public_len| must be at least |X25519_PUBLIC_VALUE_LEN|.
...
//
// |enc_len| must be at least |X25519_PUBLIC_VALUE_LEN|.
// |public_key_len| must be at least |X25519_PUBLIC_VALUE_LEN|.
// |private_key_len| must be at least |X25519_PRIVATE_KEY_LEN|.
...
//
// |out_enc_len| must be at least |X25519_PUBLIC_VALUE_LEN|.
// |peer_public_value_len| must be at least |X25519_PUBLIC_VALUE_LEN|.
//
...
// EVP_HPKE_CTX_get_aead_id returns the |hpke| context's configured AEAD. The
// returned value is one of the |EVP_HPKE_AEAD_*| constants, or zero if the
// context has not been set up.
OPENSSL_EXPORT uint16_t EVP_HPKE_CTX_get_aead_id(const EVP_HPKE_CTX *hpke);

// EVP_HPKE_CTX_get_aead_id returns the |hpke| context's configured KDF. The
// returned value is one of the |EVP_HPKE_HKDF_*| constants, or zero if the
// context has not been set up.
