...
// constants."
//
// When the sender and receiver are set up with the |EVP_HPKE_CTX_setup_base*|
// functions, the sender's encrypts messages for the recipient's public key, and
// the recipient decrypts the messages with no authentication.
//
// When the sender and receiver are set up with the |EVP_HPKE_CTX_setup_psk*|
// functions, the recipient additionally authenticates that the sender possesses
// the given pre-shared key (PSK).
...
// shared secret from the sender. Note that this function may fail if |enc| is
// invalid.
