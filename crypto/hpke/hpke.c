...
static int hpke_key_schedule(EVP_HPKE_CTX *hpke, const uint8_t *zz,
  ...
  if (!EVP_AEAD_CTX_init(&hpke->aead, aead, key, sizeof(key),
...
static int hpke_increment_seq(EVP_HPKE_CTX *hpke) {
  if (hpke->seq >= (1 << AES_GCM_128_NN) - 1) {
    return 1;
  }
  hpke->seq += 1;
  return 0;
...
static int hpke_decap(const EVP_HPKE_CTX *hpke,
  ...
  OPENSSL_memcpy(kem_context + X25519_PUBLIC_VALUE_LEN, secret_key_r,
                 sizeof(public_key));
...
// TODO(dmcardle): Make generic for other AEADs?
static void hpke_nonce(EVP_HPKE_CTX *hpke, uint8_t out_nonce[AES_GCM_128_NN]) {
  // Encode |hpke->seq| as an AES_GCM_128_NN-byte big-endian integer.
  uint64_t seq = hpke->seq;
  uint8_t big_endian_seq[AES_GCM_128_NN] = {0};
  for (int i = 0; i < AES_GCM_128_NN; i++) {
    big_endian_seq[AES_GCM_128_NN - i - 1] = seq & 0xff;
    seq >>= 8;
  }
...
int EVP_HPKE_CTX_open(EVP_HPKE_CTX *hpke, uint8_t *out, size_t *out_len,
  ...
  uint8_t nonce[AES_GCM_128_NN];
  hpke_nonce(hpke, nonce);

  if (!EVP_AEAD_CTX_open(&hpke->aead, out, out_len, max_out_len, nonce,
                         EVP_AEAD_nonce_length(hpke->aead.aead), in, in_len, ad,
                         ad_len)) {
    return 0;
  }
  if (!hpke_increment_seq(hpke)) {
    return 1;
  }
  return 1;
}
...
int EVP_HPKE_CTX_seal(EVP_HPKE_CTX *hpke, uint8_t *out, size_t *out_len,
  ...
  if (!EVP_AEAD_CTX_seal(&hpke->aead, out, out_len, max_out_len, nonce,
                         EVP_AEAD_nonce_length(hpke->aead.aead), in, in_len, ad,
                         ad_len)) {
    return 0;
  }
