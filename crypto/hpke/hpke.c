...
static const uint8_t kHpkeRfcId[] = "RFCXXXX ";
...
static int hpke_labeled_extract(EVP_HPKE_CTX *hpke, uint8_t *out_key,
                                ...
                                size_t ikm_len) {
  CBB labeled_ikm;
  if (!CBB_init(&labeled_ikm, strlen(kHpkeRfcId) + label_len + ikm_len)) {
    return 0;
  }
  if (!CBB_add_bytes(&labeled_ikm, kHpkeRfcId, strlen(kHpkeRfcId)) ||
...
static int hpke_labeled_expand(EVP_HPKE_CTX *hpke, uint8_t *out_key,
  ...
  if (!CBB_init(&labeled_info,
                sizeof(uint16_t) + strlen(kHpkeRfcId) + label_len + info_len)) {
    return 0;
  }
...
static int hpke_key_schedule(EVP_HPKE_CTX *hpke, const uint8_t *zz,
  ...
  const uint8_t kPskIdHashLabel[] = "pskID_hash";
  ...
  const uint8_t kInfoHashLabel[] = "info";
  ...
  const uint8_t kPskHashLabel[] = "psk_hash";
