...
static const unsigned char kHpkeRfcId[] = "RFCXXXX ";
...
static int hpke_labeled_extract(const EVP_HPKE_CTX *hpke, uint8_t *out_key,
  ...
  CBB labeled_ikm;
  int ok =
      (CBB_init(&labeled_ikm, strlen(kHpkeRfcId) + label_len + ikm_len) &&
       CBB_add_bytes(&labeled_ikm, kHpkeRfcId, strlen(kHpkeRfcId)) &&
...
static int hpke_labeled_expand(const EVP_HPKE_CTX *hpke, uint8_t *out_key,
  ...
  int ok = (CBB_init(&labeled_info, sizeof(uint16_t) + strlen(kHpkeRfcId) +
            ...
            CBB_add_bytes(&labeled_info, kHpkeRfcId, strlen(kHpkeRfcId)) &&
...
static int hpke_key_schedule(EVP_HPKE_CTX *hpke, uint16_t aead_id,
  ...
  static const unsigned char kPskIdHashLabel[] = "pskID_hash";
  ...
  static const unsigned char kInfoHashLabel[] = "info_hash";
  ...
  static const unsigned char kKeyExpandLabel[] = "key";
  uint8_t key[EVP_AEAD_MAX_KEY_LENGTH];
  size_t key_len = EVP_AEAD_key_length(aead);
  if (!hpke_labeled_expand(hpke, key, key_len, secret, secret_len,
                           kKeyExpandLabel, strlen(kKeyExpandLabel),
  ...
  static const unsigned char kNonceExpandLabel[] = "nonce";
  ...
  static const unsigned char kExporterSecretExpandLabel[] = "exp";
...
int EVP_HPKE_CTX_export(EVP_HPKE_CTX *hpke, uint8_t *secret_out,
                        ...
                        size_t context_len) {
  static const unsigned char kExportExpandLabel[] = "sec";
  if (!hpke_labeled_expand(hpke, secret_out, secret_len, hpke->exporter_secret,
                           SHA256_DIGEST_LENGTH, kExportExpandLabel,
