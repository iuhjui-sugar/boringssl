...
int EVP_HPKE_derive_x25519_keypair(uint8_t out_private[X25519_PRIVATE_KEY_LEN],
  ...
  const char *kDkpPrkLabel = "dkp_prk";
  const char *kSkExpandLabel = "sk";
  uint8_t dkp_prk[EVP_MAX_MD_SIZE];
  size_t dkp_prk_len;
  if (!hpke_labeled_extract(EVP_sha256(), dkp_prk, &dkp_prk_len, NULL, 0,
                            kKemSuiteId, sizeof(kKemSuiteId),
                            (const uint8_t *)kDkpPrkLabel, strlen(kDkpPrkLabel),
                            ikm, ikm_len) ||
      !hpke_labeled_expand(EVP_sha256(), out_private, X25519_PRIVATE_KEY_LEN,
                           dkp_prk, dkp_prk_len, kKemSuiteId,
                           sizeof(kKemSuiteId), (const uint8_t *)kSkExpandLabel,
...
static int hpke_key_schedule(EVP_HPKE_CTX *hpke,
  ...
  if (!CBB_init_fixed(&context_cbb, context, sizeof(context)) ||
      ...
      !CBB_add_u16(&context_cbb, hpke->kdf_id) ||
      !CBB_add_u16(&context_cbb, hpke->aead_id) ||
