...
int EVP_HPKE_KEY_init(EVP_HPKE_KEY *key, const EVP_HPKE_KEM *kem,
                      const uint8_t *priv_key, size_t priv_key_len) {
  return kem->init_key(key, kem, priv_key, priv_key_len);
}
...
int EVP_HPKE_CTX_setup_base_s(EVP_HPKE_CTX *hpke, uint8_t *out_enc,
                              size_t *out_enc_len, size_t max_enc,
...
int EVP_HPKE_CTX_setup_base_s_with_seed_for_testing(
  ...
  return kem->encap_with_seed(kem, shared_secret, &shared_secret_len, out_enc,
...
int EVP_HPKE_CTX_setup_base_r(EVP_HPKE_CTX *hpke, const EVP_HPKE_KEY *key,
  ...
  uint8_t shared_secret[MAX_SHARED_SECRET_LEN];
  size_t shared_secret_len;
  return key->kem->decap(key, shared_secret, &shared_secret_len, enc,
                         enc_len) &&
