...
static int hpke_key_schedule(EVP_HPKE_CTX *hpke, uint16_t aead_id,
  ...
  // TODO(dmcardle) draft-draft-irtf-cfrg-hpke-04 says "info", but reference implementation uses "info_hash"
  //const uint8_t kInfoHashLabel[] = {'i', 'n', 'f', 'o'};
  const uint8_t kInfoHashLabel[] = {'i', 'n', 'f', 'o', '_', 'h', 'a', 's', 'h'};
...
static int hpke_increment_seq(EVP_HPKE_CTX *hpke) {
  ...
  int ok =
      BN_lshift(&max_seq_value, BN_value_one(), nonce_len_bits) &&
...
static int hpke_decap(const EVP_HPKE_CTX *hpke,
  ...
  OPENSSL_memcpy(kem_context + X25519_PUBLIC_VALUE_LEN, public_key,
                 sizeof(public_key));
  if (!hpke_extract_and_expand(hpke, out_zz, SHA256_DIGEST_LENGTH, dh, kem_context)) {
    return 0;
  }
  return 1;
}
