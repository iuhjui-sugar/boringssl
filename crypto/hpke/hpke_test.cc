...
class HPKETestVector {
 public:
  ...
  void Verify() const {
    ...
    size_t enc_len;
    ASSERT_TRUE(EVP_HPKE_CTX_setup_base_s_with_seed_for_testing(
    ...
    EVP_HPKE_KEY key;
    ASSERT_TRUE(EVP_HPKE_KEY_init(&key, kem, secret_key_r_.data(),
                                  secret_key_r_.size()));
    ...
    ASSERT_TRUE(EVP_HPKE_CTX_setup_base_r(receiver_ctx.get(), &key, kdf, aead,
                                          enc, enc_len, info_.data(),
                                          info_.size()));
...
TEST(HPKETest, RoundTrip) {
  ...
  EVP_HPKE_KEY key;
  ASSERT_TRUE(EVP_HPKE_KEY_init(&key, EVP_hpke_x25519_hkdf_sha256(),
                                secret_key_r, sizeof(secret_key_r)));
  uint8_t public_key_r[X25519_PUBLIC_VALUE_LEN];
  size_t public_key_r_len;
  ASSERT_TRUE(EVP_HPKE_KEY_public_key(&key, public_key_r, &public_key_r_len,
                                      sizeof(public_key_r)));
    ...
    for (const auto aead : kAllAEADs) {
      ...
      for (const Span<const uint8_t> &info : info_values) {
        ...
        for (const Span<const uint8_t> &ad : ad_values) {
          ...
          size_t enc_len;
          ASSERT_TRUE(EVP_HPKE_CTX_setup_base_s(
          ...
          // Set up the receiver.
          ScopedEVP_HPKE_CTX receiver_ctx;
          ASSERT_TRUE(EVP_HPKE_CTX_setup_base_r(receiver_ctx.get(), &key, kdf(),
                                                aead(), enc, enc_len,
                                                info.data(), info.size()));
...
TEST(HPKETest, X25519EncapSmallOrderPoint) {
  ...
  for (const auto kdf : kAllKDFs) {
    ...
    for (const auto aead : kAllAEADs) {
      ...
      uint8_t enc[X25519_PUBLIC_VALUE_LEN];
      size_t enc_len;
      ASSERT_FALSE(EVP_HPKE_CTX_setup_base_s(
          sender_ctx.get(), enc, &enc_len, sizeof(enc),
      ...
      ScopedEVP_HPKE_CTX receiver_ctx;
      ASSERT_FALSE(EVP_HPKE_CTX_setup_base_r(
          receiver_ctx.get(), &key, kdf(), aead(), kSmallOrderPoint,
          sizeof(kSmallOrderPoint), nullptr, 0));
    }
  }
}
...
TEST(HPKETest, ReceiverInvalidSeal) {
  ...
  // Set up the receiver.
  ScopedEVP_HPKE_CTX receiver_ctx;
  ASSERT_TRUE(EVP_HPKE_CTX_setup_base_r(
      receiver_ctx.get(), &key, EVP_hpke_hkdf_sha256(), EVP_hpke_aes_128_gcm(),
...
TEST(HPKETest, SetupSenderBufferTooLarge) {
  ...
  uint8_t enc[X25519_PUBLIC_VALUE_LEN + 1];
  size_t enc_len;
  EXPECT_TRUE(EVP_HPKE_CTX_setup_base_s(
      sender_ctx.get(), enc, &enc_len, sizeof(enc),
      EVP_hpke_x25519_hkdf_sha256(), EVP_hpke_hkdf_sha256(),
      EVP_hpke_aes_128_gcm(), public_key_r, sizeof(public_key_r), nullptr, 0));
...
TEST(HPKETest, SetupReceiverWrongLengthEnc) {
  ...
  ASSERT_FALSE(EVP_HPKE_CTX_setup_base_r(
      receiver_ctx.get(), &key, EVP_hpke_hkdf_sha256(), EVP_hpke_aes_128_gcm(),
      bogus_enc, sizeof(bogus_enc), nullptr, 0));
  uint32_t err = ERR_get_error();
  EXPECT_EQ(ERR_LIB_EVP, ERR_GET_LIB(err));
...
TEST(HPKETest, InvalidReceiverKey) {
  ...
  EVP_HPKE_KEY key;
  EXPECT_FALSE(EVP_HPKE_KEY_init(&key, EVP_hpke_x25519_hkdf_sha256(),
                                 private_key, sizeof(private_key)));
}
