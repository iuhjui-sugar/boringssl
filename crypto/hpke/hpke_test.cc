...
class HpkeTestVector {
 public:
  ...
  void Verify() const {
    ...
    EXPECT_TRUE(EVP_HPKE_CTX_setup_base_x25519_s_for_test(
    ...
    EXPECT_TRUE(EVP_HPKE_CTX_setup_base_x25519_r(
        ...
        secret_key_r_.data(), info_.data(), info_.size()));
 ...
 private:
  void SanityCheck() const {
    // Recover the public key corresponding to |secret_key_e_| and compare it
    // with |public_key_e|.
    uint8_t recovered_public_key[X25519_PUBLIC_VALUE_LEN];
    X25519_public_from_private(recovered_public_key, secret_key_e_.data());
    EXPECT_EQ(Bytes(recovered_public_key), Bytes(public_key_e_));
  }
...
HpkeTestVector::HpkeTestVector(FileTest *t) {
  ...
  EXPECT_TRUE(t->GetBytes(&secret_key_r_, "skRm"));
  EXPECT_TRUE(t->GetBytes(&public_key_r_, "pkRm"));
  EXPECT_TRUE(t->GetBytes(&secret_key_e_, "skEm"));
  EXPECT_TRUE(t->GetBytes(&public_key_e_, "pkEm"));
...
// Test HPKE without injecting an ephemeral keypair via
// |EVP_HPKE_CTX_setup_base_x25519_s_for_test|. If not for this test,
// |EVP_HPKE_CTX_setup_base_x25519_s| would have no coverage.
TEST(HPKETest, ActualUsage) {
  uint16_t kdf_ids[] = {HPKE_HKDF_SHA256, HPKE_HKDF_SHA384, HPKE_HKDF_SHA512};
  uint16_t aead_ids[] = {HPKE_AEAD_AES_GCM_128, HPKE_AEAD_AES_GCM_256,
                         HPKE_AEAD_CHACHA20POLY1305};

  uint8_t info_a[] = {1, 1, 2, 3, 5, 8};
  uint8_t info_b[] = {42, 42, 42};
  uint8_t ad_a[] = {1, 2, 4, 8, 16};
  uint8_t ad_b[] = {7};
  Span<uint8_t> info_values[] = {{nullptr, 0}, info_a, info_b};
  Span<uint8_t> ad_values[] = {{nullptr, 0}, ad_a, ad_b};
  ...
  for (uint16_t kdf_id : kdf_ids) {
    for (uint16_t aead_id : aead_ids) {
      for (const Span<uint8_t> &info : info_values) {
        for (const Span<uint8_t> &ad : ad_values) {
          // Set up the sender.
          ScopedEVP_HPKE_CTX sender_ctx;
          ...
          ScopedEVP_HPKE_CTX receiver_ctx;
          EXPECT_TRUE(EVP_HPKE_CTX_setup_base_x25519_r(
              receiver_ctx.get(), kdf_id, aead_id, enc, secret_key_r,
              info.data(), info.size()));

          const char kCleartextPayload[] = "foobar";

          // Have sender encrypt message for the receiver.
          uint8_t ciphertext[sizeof(kCleartextPayload) + EVP_AEAD_MAX_OVERHEAD];
          size_t ciphertext_len;
          EXPECT_TRUE(EVP_HPKE_CTX_seal(
              sender_ctx.get(), ciphertext, &ciphertext_len, sizeof(ciphertext),
          ...
          uint8_t cleartext[sizeof(ciphertext)];
          size_t cleartext_len;
          EXPECT_TRUE(EVP_HPKE_CTX_open(
              receiver_ctx.get(), cleartext, &cleartext_len, sizeof(cleartext),
              ciphertext, ciphertext_len, ad.data(), ad.size()));

          // Verify that decrypted message matches the original.
          EXPECT_EQ(Bytes(cleartext, cleartext_len),
