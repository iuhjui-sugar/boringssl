...
TEST(HPKETest, ReadFileTest) {
  FileTestGTest("crypto/hpke/hpke_test_vectors.txt", [](FileTest *t) {
    ...
    ASSERT_TRUE(t->GetBytes(&test_vec.secret_key_r, "skRm"));
    ASSERT_TRUE(t->GetBytes(&test_vec.secret_key_e, "skEm"));
    ASSERT_TRUE(t->GetBytes(&test_vec.public_key_e, "pkEm"));
    ASSERT_TRUE(t->GetBytes(&test_vec.public_key_r, "pkRm"));
    ASSERT_TRUE(t->GetBytes(&test_vec.enc, "enc"));
    ASSERT_TRUE(t->GetBytes(&test_vec.zz, "zz"));
    ASSERT_TRUE(t->GetBytes(&test_vec.context, "key_schedule_context"));
