...
TEST(HPKETest, ReadFileTest) {
  FileTestGTest("crypto/hpke/hpke_test_vectors.txt", [](FileTest *t) {
    ...
    CHECK(t->GetBytes(&test_vec.info, "info"));
    CHECK(t->GetBytes(&test_vec.secret_key_r, "skRm"));
    CHECK(t->GetBytes(&test_vec.secret_key_e, "skEm"));
    CHECK(t->GetBytes(&test_vec.public_key_e, "pkEm"));
    CHECK(t->GetBytes(&test_vec.public_key_r, "pkRm"));
    CHECK(t->GetBytes(&test_vec.enc, "enc"));
    CHECK(t->GetBytes(&test_vec.zz, "zz"));
    CHECK(t->GetBytes(&test_vec.context, "key_schedule_context"));
    CHECK(t->GetBytes(&test_vec.secret, "secret"));
    CHECK(t->GetBytes(&test_vec.key, "key"));
    CHECK(t->GetBytes(&test_vec.nonce, "outer_nonce"));
    CHECK(t->GetBytes(&test_vec.exporter_secret, "exporterSecret"));

    for (int i = 1; t->HasAttribute(BuildAttrName("aad", i)); i++) {
      HpkeTestVector::Encryption encryption;
      CHECK(t->GetBytes(&encryption.aad, BuildAttrName("aad", i)));
      CHECK(
          t->GetBytes(&encryption.ciphertext, BuildAttrName("ciphertext", i)));
      CHECK(t->GetBytes(&encryption.nonce, BuildAttrName("nonce", i)));
      CHECK(t->GetBytes(&encryption.plaintext, BuildAttrName("plaintext", i)));
