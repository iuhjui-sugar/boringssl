...
TEST(HPKETest, SetupSenderWrongLengthEnc) {
  // No need for a real X25519 key because the function should fail early.
  const uint8_t public_key_r[X25519_PUBLIC_VALUE_LEN] = {0xff};
...
TEST(HPKETest, SetupReceiverWrongLengthEnc) {
  // No need for a real X25519 keypair because the function should fail early.
  const uint8_t public_key[X25519_PUBLIC_VALUE_LEN] = {0xff};
  const uint8_t private_key[X25519_PUBLIC_VALUE_LEN] = {0xff};
...
TEST(HPKETest, SetupReceiverWrongLengthKeys) {
  ...
  const uint8_t enc[X25519_PUBLIC_VALUE_LEN] = {0xff};
