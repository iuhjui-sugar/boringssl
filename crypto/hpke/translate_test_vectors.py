...
def dict_modify(d, key, new_value):
...
def read_test_vectors_and_generate_code(json_file_in_path, test_file_out_path):
    ...
    with open(json_file_in_path) as file_in:
        test_vecs = json.load(
            file_in, object_pairs_hook=collections.OrderedDict)
    ...
    test_vecs = [t for t in test_vecs if t["kemID"]
    ...
    key_mapping = {
        # We only test the public API, so we can drop internal values.
        "nonce": None,
        "keyScheduleContext": None,
    ...
    # Drop "nonce" field from every encryption object in each test vector.
    test_vecs = map(lambda t: dict_modify(t, "encryptions", [rename_odict_keys(
        {"nonce": None}, e) for e in t["encryptions"]]), test_vecs)
