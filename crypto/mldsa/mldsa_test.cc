...
TEST(MLDSATest, DISABLED_BitFlips) {
  ...
  EXPECT_TRUE(MLDSA_generate_key(encoded_public_key.data(), priv.get()));
  ...
  EXPECT_EQ(MLDSA_verify(pub.get(), encoded_signature.data(), kMessage,
                         sizeof(kMessage)),
            1);
...
TEST(MLDSATest, SignatureIsRandomized) {
  ...
  EXPECT_EQ(MLDSA_verify(pub.get(), encoded_signature1.data(), kMessage,
                         sizeof(kMessage)),
            1);
  EXPECT_EQ(MLDSA_verify(pub.get(), encoded_signature2.data(), kMessage,
                         sizeof(kMessage)),
            1);
}
...
static void MLDSAFileTest(FileTest *t) {
  ...
  uint8_t gen_key_entropy[MLDSA_GENERATE_KEY_ENTROPY];
  ...
  ASSERT_EQ(seed.size(), size_t{CTR_DRBG_ENTROPY_LEN});
  {
    ...
    ASSERT_TRUE(CTR_DRBG_generate(state.get(), gen_key_entropy,
                                  MLDSA_GENERATE_KEY_ENTROPY, nullptr, 0));
  ...
  auto parsed_pub = std::make_unique<MLDSA_public_key>();
  auto priv = std::make_unique<MLDSA_private_key>();
  std::vector<uint8_t> encoded_private_key(MLDSA_PRIVATE_KEY_BYTES);
  std::vector<uint8_t> encoded_public_key(MLDSA_PUBLIC_KEY_BYTES);

  EXPECT_TRUE(MLDSA_generate_key_external_entropy(encoded_public_key.data(),
                                                  priv.get(), gen_key_entropy));
  ...
  // Reproduce signature.
  std::vector<uint8_t> encoded_signature(MLDSA_SIGNATURE_BYTES);
  EXPECT_TRUE(MLDSA_sign_deterministic(encoded_signature.data(), priv.get(),
                                       message.data(), message.size()));

  ASSERT_GE(signed_message_expected.size(), size_t{MLDSA_SIGNATURE_BYTES});
  EXPECT_EQ(Bytes(encoded_signature),
            Bytes(bssl::MakeConstSpan(signed_message_expected)
                      .first(MLDSA_SIGNATURE_BYTES)));
  EXPECT_EQ(Bytes(message), Bytes(bssl::MakeConstSpan(signed_message_expected)
                                      .subspan(MLDSA_SIGNATURE_BYTES)));

  // Check that verification matches.
  CBS cbs;
  CBS_init(&cbs, encoded_public_key.data(), encoded_public_key.size());
  ASSERT_TRUE(MLDSA_parse_public_key(parsed_pub.get(), &cbs));
  EXPECT_EQ(MLDSA_verify(parsed_pub.get(), encoded_signature.data(),
                         message.data(), message.size()),
            1);
  ...
  ASSERT_TRUE(MLDSA_parse_private_key(parsed_priv.get(), &cbs));

  EXPECT_TRUE(MLDSA_sign_deterministic(encoded_signature2.data(),
  ...
  std::vector<uint8_t> encoded_private_key2(MLDSA_PRIVATE_KEY_BYTES);
  std::vector<uint8_t> encoded_public_key2(MLDSA_PUBLIC_KEY_BYTES);

  CBB_init_fixed(&cbb, encoded_private_key2.data(),
                 encoded_private_key2.size());
  ASSERT_TRUE(MLDSA_marshal_private_key(&cbb, parsed_priv.get()));
  CBB_init_fixed(&cbb, encoded_public_key2.data(), encoded_public_key2.size());
  ASSERT_TRUE(MLDSA_marshal_public_key(&cbb, parsed_pub.get()));

  EXPECT_EQ(Bytes(encoded_public_key2), Bytes(encoded_public_key));
  EXPECT_EQ(Bytes(encoded_private_key2), Bytes(encoded_private_key));
}

static void MLDSASignFileTest(FileTest *t) {
  ...
  if (!MLDSA_parse_private_key(&priv, &cbs)) {
  ...
  // Reproduce signature.
  uint8_t encoded_signature[MLDSA_SIGNATURE_BYTES];
  MLDSA_sign_deterministic(encoded_signature, &priv, message.data(),
                           message.size());

  ASSERT_GE(signed_message_expected.size(), size_t{MLDSA_SIGNATURE_BYTES});
  EXPECT_EQ(Bytes(encoded_signature),
            Bytes(signed_message_expected.data(), MLDSA_SIGNATURE_BYTES))
      << "Unexpected signing result for edge case: " << description;
  EXPECT_EQ(Bytes(message),
            Bytes(&signed_message_expected[MLDSA_SIGNATURE_BYTES],
                  signed_message_expected.size() - MLDSA_SIGNATURE_BYTES))
      << "Unexpected signing result for edge case: " << description;
  EXPECT_TRUE(is_valid) << "Unexpected signing result for edge case: "
                        << description;
}

static void MLDSAVerifyFileTest(FileTest *t) {
  std::string description, valid;
  ...
  if (!MLDSA_parse_public_key(&pub, &cbs)) {
  ...
  // Verify signature.
  ASSERT_GE(signed_message.size(), size_t{MLDSA_SIGNATURE_BYTES});
  EXPECT_EQ(MLDSA_verify(&pub, signed_message.data(),
                         &signed_message[MLDSA_SIGNATURE_BYTES],
                         signed_message.size() - MLDSA_SIGNATURE_BYTES),
            is_valid)
      << "Unexpected verification result for edge case: " << description;
}

TEST(MLDSATest, TestVectors) {
  FileTestGTest("crypto/dilithium/dilithium_tests.txt", MLDSAFileTest);
}

TEST(MLDSATest, EdgeCaseSigningTests) {
  FileTestGTest("crypto/dilithium/edge_cases_draft_dilithium3_sign.txt",
                MLDSASignFileTest);
}

TEST(MLDSATest, EdgeCaseVerifyTests) {
  FileTestGTest("crypto/dilithium/edge_cases_draft_dilithium3_verify.txt",
                MLDSAVerifyFileTest);
}

TEST(MLDSATest, KeyGenerationHardCodedNIST) {
  // Published on
  ...
  static const uint8_t expected_public_key[MLDSA_PUBLIC_KEY_BYTES] = {
  ...
  static const uint8_t expected_private_key[MLDSA_PRIVATE_KEY_BYTES] = {
      0x88, 0xC9, 0x3D, 0xEB, 0x90, 0x1A, 0x24, 0xA9, 0x45, 0xFA, 0x94, 0xAE,
      0x54, 0xDB, 0xAF, 0x46, 0xD8, 0x1C, 0xAC, 0xA9, 0x43, 0xFB, 0xE3, 0x20,
  ...
  bssl::ScopedCBB cbb;
  EXPECT_TRUE(CBB_init(cbb.get(), MLDSA_PRIVATE_KEY_BYTES));
  EXPECT_TRUE(MLDSA_marshal_private_key(cbb.get(), priv.get()));

  EXPECT_EQ(CBB_len(cbb.get()), size_t{MLDSA_PRIVATE_KEY_BYTES});
  EXPECT_EQ(Bytes(CBB_data(cbb.get()), CBB_len(cbb.get())),
            Bytes(expected_private_key));
}

TEST(MLDSATest, SignatureHardCodedNIST) {
  // Published on
  // https://csrc.nist.gov/Projects/post-quantum-cryptography/post-quantum-cryptography-standardization/example-files
  static const uint8_t encoded_private_key[MLDSA_PRIVATE_KEY_BYTES] = {
  ...
  ASSERT_TRUE(MLDSA_parse_private_key(priv.get(), &cbs));
  ...
  static const uint8_t expected_signature[MLDSA_SIGNATURE_BYTES] = {
