...
TEST(MLDSATest, Basic) {
  ...
  EXPECT_TRUE(MLDSA_private_key_from_seed(priv2.get(), seed, sizeof(seed)));

  EXPECT_EQ(Bytes(Marshal(MLDSA_marshal_private_key, priv.get())),
            Bytes(Marshal(MLDSA_marshal_private_key, priv2.get())));
}
...
static void MLDSAWycheproofVerifyTest(FileTest *t) {
  ...
  const int sig_ok =
      MLDSA_verify(pub.get(), signature.data(), signature.size(), msg.data(),
