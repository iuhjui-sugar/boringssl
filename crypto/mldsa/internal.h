...
// MLDSA_sign_internal signs |msg| using |private_key| and writes the signature
// to |out_encoded_signature|. The |context_prefix| and |context| are prefixed
// to the message, in that order, before signing. The |randomizer| value can be
// set to zero bytes in order to make a deterministic signature, or else filled
// with entropy for the usual |MLDSA_sign| behavior. It returns 1 on success and
// 0 on error.
OPENSSL_EXPORT int MLDSA_sign_internal(
...
// MLDSA_verify_internal verifies that |encoded_signature| is a valid signature
// of |msg| by |public_key|. The |context_prefix| and |context| are prefixed to
// the message before verification, in that order. It returns 1 on success and 0
// on error.
OPENSSL_EXPORT int MLDSA_verify_internal(
