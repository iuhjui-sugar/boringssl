...
static void scalar_inverse_ntt(scalar *s) {
  ...
  for (int offset = 1; offset < DEGREE; offset <<= 1) {
    ...
    for (int i = 0; i < step; i++) {
      ...
      declassify_assert(step_root < kPrime);
      for (int j = k; j < k + offset; j++) {
        uint32_t even = s->c[j];
        declassify_assert(even < kPrime);
        uint32_t odd = s->c[j + offset];
        declassify_assert(odd < kPrime);
        s->c[j] = reduce_once(odd + even);

        // |reduce_montgomery| works on values up to kPrime*R and R > 2*kPrime.
        // kPrime + even < 2*kPrime because even < kPrime, as asserted above.
        // Thus kPrime + even - odd < 2*kPrime because odd >= 0, because it's
        // unsigned and less than kPrime, as asserted above. Lastly
        // step_root < kPrime, as asserted above.
...
// 2^19.
static void scalar_encode_signed_4_eta(uint8_t *out, const scalar *s) {
  // Every two elements lands on a byte boundary.
  static_assert(DEGREE % 2 == 0, "DEGREE must be a multiple of 2");
  for (int i = 0; i < DEGREE / 2; i++) {
    uint32_t a = reduce_once(kPrime + ETA - s->c[2 * i]);
    uint32_t b = reduce_once(kPrime + ETA - s->c[2 * i + 1]);
    declassify_assert(a < 16);
    declassify_assert(b < 16);
    out[i] = a | (b << 4);
  }
...
// 2^12.
static void scalar_encode_signed_13_12(uint8_t *out, const scalar *s) {
  static const uint32_t kMax = 1u << 12;
  // Every two elements lands on a byte boundary.
  static_assert(DEGREE % 8 == 0, "DEGREE must be a multiple of 8");
  for (int i = 0; i < DEGREE / 8; i++) {
    uint32_t a = reduce_once(kPrime + kMax - s->c[8 * i]);
    uint32_t b = reduce_once(kPrime + kMax - s->c[8 * i + 1]);
    uint32_t c = reduce_once(kPrime + kMax - s->c[8 * i + 2]);
    uint32_t d = reduce_once(kPrime + kMax - s->c[8 * i + 3]);
    uint32_t e = reduce_once(kPrime + kMax - s->c[8 * i + 4]);
    uint32_t f = reduce_once(kPrime + kMax - s->c[8 * i + 5]);
    uint32_t g = reduce_once(kPrime + kMax - s->c[8 * i + 6]);
    uint32_t h = reduce_once(kPrime + kMax - s->c[8 * i + 7]);
...
// 2^19.
static void scalar_encode_signed_20_19(uint8_t *out, const scalar *s) {
  static const uint32_t kMax = 1u << 19;
  // Every two elements lands on a byte boundary.
  static_assert(DEGREE % 4 == 0, "DEGREE must be a multiple of 4");
  for (int i = 0; i < DEGREE / 4; i++) {
    uint32_t a = reduce_once(kPrime + kMax - s->c[4 * i]);
    uint32_t b = reduce_once(kPrime + kMax - s->c[4 * i + 1]);
    uint32_t c = reduce_once(kPrime + kMax - s->c[4 * i + 2]);
    uint32_t d = reduce_once(kPrime + kMax - s->c[4 * i + 3]);
...
static void scalar_decode_10(scalar *out, const uint8_t *in) {
  static_assert(DEGREE % 4 == 0, "DEGREE must be a multiple of 4");
  uint32_t v;
  for (int i = 0; i < DEGREE / 4; i++) {
...
// eta.
static int scalar_decode_signed_4_eta(scalar *out, const uint8_t *in) {
  static_assert(DEGREE % 8 == 0, "DEGREE must be a multiple of 8");
  uint32_t v;
  for (int i = 0; i < DEGREE / 8; i++) {
    ...
    uint32_t msbs = v & 0x88888888u;
    // For each nibble where the MSB is set, mask all the other bits.
    uint32_t mask = msbs >> 1;
    mask |= mask >> 1;
    mask |= mask >> 1;
    ...
    out->c[i * 8] = reduce_once(kPrime + ETA - (v & 15));
    out->c[i * 8 + 1] = reduce_once(kPrime + ETA - ((v >> 4) & 15));
    out->c[i * 8 + 2] = reduce_once(kPrime + ETA - ((v >> 8) & 15));
    out->c[i * 8 + 3] = reduce_once(kPrime + ETA - ((v >> 12) & 15));
    out->c[i * 8 + 4] = reduce_once(kPrime + ETA - ((v >> 16) & 15));
    out->c[i * 8 + 5] = reduce_once(kPrime + ETA - ((v >> 20) & 15));
    out->c[i * 8 + 6] = reduce_once(kPrime + ETA - ((v >> 24) & 15));
    out->c[i * 8 + 7] = reduce_once(kPrime + ETA - (v >> 28));
  }
  return 1;
}
...
// 2^12.
static void scalar_decode_signed_13_12(scalar *out, const uint8_t *in) {
  static const uint32_t kMax = 1u << 12;
  ...
  for (int i = 0; i < DEGREE / 8; i++) {
    ...
    // 2^12.
    out->c[i * 8] = reduce_once(kPrime + kMax - (a & k13Bits));
    out->c[i * 8 + 1] = reduce_once(kPrime + kMax - ((a >> 13) & k13Bits));
    out->c[i * 8 + 2] =
        reduce_once(kPrime + kMax - ((a >> 26) | ((b & k7Bits) << 6)));
    out->c[i * 8 + 3] = reduce_once(kPrime + kMax - ((b >> 7) & k13Bits));
    out->c[i * 8 + 4] =
        reduce_once(kPrime + kMax - ((b >> 20) | ((c & 1) << 12)));
    out->c[i * 8 + 5] = reduce_once(kPrime + kMax - ((c >> 1) & k13Bits));
    out->c[i * 8 + 6] = reduce_once(kPrime + kMax - ((c >> 14) & k13Bits));
    out->c[i * 8 + 7] =
        reduce_once(kPrime + kMax - ((c >> 27) | ((uint32_t)d) << 5));
  }
}

// FIPS 204, Algorithm 19 (`BitUnpack`). Specialized to bitlen(a+b) = 20 and b =
// 2^19.
static void scalar_decode_signed_20_19(scalar *out, const uint8_t *in) {
  static const uint32_t kMax = 1u << 19;
  ...
  for (int i = 0; i < DEGREE / 4; i++) {
    ...
    // 2^19.
    out->c[i * 4] = reduce_once(kPrime + kMax - (a & k20Bits));
    out->c[i * 4 + 1] =
        reduce_once(kPrime + kMax - ((a >> 20) | ((b & 0xff) << 12)));
    out->c[i * 4 + 2] = reduce_once(kPrime + kMax - ((b >> 8) & k20Bits));
    out->c[i * 4 + 3] =
        reduce_once(kPrime + kMax - ((b >> 28) | ((uint32_t)c) << 4));
  }
}
...
static struct private_key *private_key_from_external(
    const struct MLDSA_private_key *external) {
  static_assert(sizeof(struct MLDSA_private_key) == sizeof(struct private_key),
                "Kyber private key size incorrect");
  static_assert(
      alignof(struct MLDSA_private_key) == alignof(struct private_key),
      "Kyber private key align incorrect");
  return (struct private_key *)external;
}

static struct public_key *public_key_from_external(
    const struct MLDSA_public_key *external) {
  static_assert(sizeof(struct MLDSA_public_key) == sizeof(struct public_key),
                "mldsa public key size incorrect");
  static_assert(alignof(struct MLDSA_public_key) == alignof(struct public_key),
...
int MLDSA_generate_key(uint8_t out_encoded_public_key[MLDSA_PUBLIC_KEY_BYTES],
  ...
  return MLDSA_generate_key_external_entropy(out_encoded_public_key,
                                             out_private_key, entropy);
}

int MLDSA_private_key_from_seed(struct MLDSA_private_key *out_private_key,
                                const uint8_t *seed, size_t seed_len) {
  if (seed_len != MLDSA_SEED_BYTES) {
    return 0;
  }
  uint8_t public_key[MLDSA_PUBLIC_KEY_BYTES];
  return MLDSA_generate_key_external_entropy(public_key, out_private_key, seed);
}
...
int MLDSA_generate_key_external_entropy(
  ...
  CBB cbb;
  CBB_init_fixed(&cbb, out_encoded_public_key, MLDSA_PUBLIC_KEY_BYTES);
  if (!mldsa_marshal_public_key(&cbb, &values->pub)) {
    goto err;
  }

  BORINGSSL_keccak(priv->public_key_hash, sizeof(priv->public_key_hash),
                   out_encoded_public_key, MLDSA_PUBLIC_KEY_BYTES,
                   boringssl_shake256);
...
int MLDSA_public_from_private(struct MLDSA_public_key *out_public_key,
...
int MLDSA_sign_internal(
  ...
  for (size_t kappa = 0;; kappa += L) {
    ...
    CBB cbb;
    CBB_init_fixed(&cbb, out_encoded_signature, MLDSA_SIGNATURE_BYTES);
    if (!mldsa_marshal_signature(&cbb, &values->sign)) {
      goto err;
    }

    BSSL_CHECK(CBB_len(&cbb) == MLDSA_SIGNATURE_BYTES);
    ret = 1;
    break;
  }
...
// |RAND_bytes|. Returns 1 on success and 0 on failure.
int MLDSA_sign(uint8_t out_encoded_signature[MLDSA_SIGNATURE_BYTES],
               const struct MLDSA_private_key *private_key, const uint8_t *msg,
  ...
  return MLDSA_sign_internal(out_encoded_signature, private_key, msg, msg_len,
                             context_prefix, sizeof(context_prefix), context,
                             context_len, randomizer);
}

// FIPS 204, Algorithm 3 (`ML-DSA.Verify`).
int MLDSA_verify(const struct MLDSA_public_key *public_key,
                 const uint8_t *signature, size_t signature_len,
                 const uint8_t *msg, size_t msg_len, const uint8_t *context,
                 size_t context_len) {
  if (context_len > 255 || signature_len != MLDSA_SIGNATURE_BYTES) {
    return 0;
  }

  const uint8_t context_prefix[2] = {0, context_len};
  return MLDSA_verify_internal(public_key, signature, msg, msg_len,
                               context_prefix, sizeof(context_prefix), context,
                               context_len);
}

// FIPS 204, Algorithm 8 (`ML-DSA.Verify_internal`).
int MLDSA_verify_internal(
    const struct MLDSA_public_key *public_key,
    const uint8_t encoded_signature[MLDSA_SIGNATURE_BYTES], const uint8_t *msg,
    size_t msg_len, const uint8_t *context_prefix, size_t context_prefix_len,
    const uint8_t *context, size_t context_len) {
  int ret = 0;
  ...
  CBS cbs;
  CBS_init(&cbs, encoded_signature, MLDSA_SIGNATURE_BYTES);
...
int MLDSA_marshal_public_key(CBB *out,
                             const struct MLDSA_public_key *public_key) {
  return mldsa_marshal_public_key(out, public_key_from_external(public_key));
}

int MLDSA_parse_public_key(struct MLDSA_public_key *public_key, CBS *in) {
...
int MLDSA_marshal_private_key(CBB *out,
                              const struct MLDSA_private_key *private_key) {
  return mldsa_marshal_private_key(out, private_key_from_external(private_key));
}

int MLDSA_parse_private_key(struct MLDSA_private_key *private_key, CBS *in) {
