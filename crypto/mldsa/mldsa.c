...
int MLDSA_generate_key(uint8_t out_encoded_public_key[MLDSA_PUBLIC_KEY_BYTES],
  ...
  uint8_t entropy[MLDSA_GENERATE_KEY_ENTROPY];
...
int MLDSA_generate_key_external_entropy(
  ...
  BORINGSSL_keccak(expanded_seed, sizeof(expanded_seed), entropy,
                   MLDSA_GENERATE_KEY_ENTROPY, boringssl_shake256);
...
static int mldsa_sign_with_randomizer(
...
// mldsa signature in deterministic mode. Returns 1 on success and 0 on
// failure.
int MLDSA_sign_deterministic(
    uint8_t out_encoded_signature[MLDSA_SIGNATURE_BYTES],
    const struct MLDSA_private_key *private_key, const uint8_t *msg,
    size_t msg_len) {
  uint8_t randomizer[MLDSA_SIGNATURE_RANDOMIZER_BYTES];
  OPENSSL_memset(randomizer, 0, sizeof(randomizer));
  return mldsa_sign_with_randomizer(out_encoded_signature, private_key, msg,
                                    msg_len, randomizer);
}
...
int MLDSA_verify(const struct MLDSA_public_key *public_key,
                 const uint8_t encoded_signature[MLDSA_SIGNATURE_BYTES],
