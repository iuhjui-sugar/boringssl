...
TEST(Ed25519Test, TestVectors) {
  FileTestGTest("crypto/curve25519/ed25519_tests.txt", [](FileTest *t) {
    ...
    BORINGSSL_SECRET(private_key.data(), private_key.size());
    BORINGSSL_SECRET(message.data(), message.size());
    uint8_t signature[64];
    ASSERT_TRUE(ED25519_sign(signature, message.data(), message.size(),
                             private_key.data()));
    BORINGSSL_DECLASSIFY(signature, sizeof(signature));
    BORINGSSL_DECLASSIFY(message.data(), message.size());
...
TEST(Ed25519Test, KeypairFromSeed) {
  ...
  BORINGSSL_SECRET(seed, sizeof(seed));

  uint8_t public_key2[32], private_key2[64];
  ED25519_keypair_from_seed(public_key2, private_key2, seed);
  BORINGSSL_DECLASSIFY(public_key2, sizeof(public_key2));
  BORINGSSL_DECLASSIFY(private_key2, sizeof(private_key2));
