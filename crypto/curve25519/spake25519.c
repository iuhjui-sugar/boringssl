...
void SPAKE2_commitment(
  ...
  // Generate key pair (out_private_key, P)
  // XXX ask agl: does this need the bit/setting clearing
  // as in the EdDSA computation of a? Tests fail without them.
  RAND_bytes(out_state->private_key, 32);
  out_state->private_key[0] &= 248;
  out_state->private_key[31] &= 127;
  out_state->private_key[31] |= 64;

  ge_scalarmult_base(&P, out_state->private_key);

  // mask = h(password) * <N or M>
  SHA256(password, password_length, out_state->pwd);
  ge_scalarmult_small_precomp(&mask, out_state->pwd,
      role == kSpakeRoleInitiator
        ? kSpakeMSmallPrecomp
        : kSpakeNSmallPrecomp);
  ...
  ge_tobytes(out_state->our_commitment, &Pstar_proj);
}
...
int SPAKE2_get_key(
  ...
  // XXX ask agl about negation
  peers_commitment[31] ^= 0x80;  // because the below decoding negates
  if (0 != ge_frombytes_negate_vartime(&Qstar, peers_commitment)) {
    // Point received from peeir was not on the curve
    return -1;
  }
  peers_commitment[31] ^= 0x80;  // restore for hash below

  // Unmask peer's commitment
  ge_scalarmult_small_precomp(&peers_mask, state->pwd,
      state->role == kSpakeRoleInitiator
        ? kSpakeNSmallPrecomp
        : kSpakeMSmallPrecomp);
  ...
  ge_p1p1 dh_secret;
  ge_scalarmult_slow(&dh_secret, state->private_key, &Q_ext);

  ge_p2 dh_secret_proj;
  uint8_t dh_secret_encoded[32];
  ge_p1p1_to_p2(&dh_secret_proj, &dh_secret);
  ge_tobytes(dh_secret_encoded, &dh_secret_proj);

  SHA256_CTX sha;
  SHA256_Init(&sha);
  if (state->role == kSpakeRoleInitiator) {
    SHA256_Update(&sha, state->our_commitment, 32);
    SHA256_Update(&sha, peers_commitment, 32);
  } else {
    SHA256_Update(&sha, peers_commitment, 32);
    SHA256_Update(&sha, state->our_commitment, 32);
  }
  SHA256_Update(&sha, state->pwd, 32);
  SHA256_Update(&sha, dh_secret_encoded, 32);
