...
// kOrder is the order of the prime-order subgroup of Ed25519 in little-endian
// order.
...
static void scalar_cmov(scalar *dest, const scalar *src, crypto_word_t mask) {
  for (size_t i = 0; i < 8; i++) {
    dest->words[i] = constant_time_select_w(mask, src->words[i], dest->words[i]);
  }
}
...
static void scalar_add(scalar *dest, const scalar *src) {
  ...
  for (size_t i = 0; i < 8; i++) {
    const uint32_t tmp = dest->words[i] + src->words[i];
    uint32_t carry_out = (tmp < dest->words[i]);
    const uint32_t tmp2 = tmp + carry;
    carry_out += (tmp2 < tmp);
    dest->words[i] = tmp2;
    carry = carry_out;
  }
...
int SPAKE2_generate_msg(SPAKE2_CTX *ctx, uint8_t *out, size_t *out_len,
  ...
  if (!ctx->disable_password_scalar_hack) {
    ...
    scalar_cmov(&tmp, &order, constant_time_eq_w(password_scalar.bytes[0] & 1, 1));
    scalar_add(&password_scalar, &tmp);

    scalar_double(&order);
    OPENSSL_memset(&tmp, 0, sizeof(tmp));
    scalar_cmov(&tmp, &order, constant_time_eq_w(password_scalar.bytes[0] & 2, 2));
    scalar_add(&password_scalar, &tmp);

    scalar_double(&order);
    OPENSSL_memset(&tmp, 0, sizeof(tmp));
    scalar_cmov(&tmp, &order, constant_time_eq_w(password_scalar.bytes[0] & 4, 4));
    scalar_add(&password_scalar, &tmp);

    assert((password_scalar.bytes[0] & 7) == 0);
  }

  OPENSSL_memcpy(ctx->password_scalar, &password_scalar.bytes[0],
