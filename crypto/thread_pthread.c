...
#if defined(__GNUC__) || defined(__clang__)
// main_thread_destructor is called when the library is unloaded with dlclose or
...
static void main_thread_destructor(void) {
  if (g_thread_local_key_created) {
    ...
    //
    // This is marked "dangerous" because it can cause multi-threaded processes
    // to crash (even if they don't use dlclose): if the destructor runs while
    // other threads are still executing then they may end up using an invalid
    // key to access thread-local variables.
    //
