...
FileTest::ReadResult FileTest::ReadNext() {
  ...
  while (true) {
    ...
    if (buf[0] == '\n' || buf[0] == '\r' || buf[0] == '\0') {
      ...
      const char *inst = kv.c_str();
      const size_t inst_len = kv.size();
      const char *delimiter = FindDelimiter(inst);
      std::string key, value;
      if (delimiter == nullptr) {
        key = StripSpace(inst, inst_len);
      } else {
        key = StripSpace(inst, delimiter - inst);
        value = StripSpace(delimiter + 1, inst + inst_len - delimiter - 1);
      }
      instructions_[key] = value;
    } else if (buf[0] != '#') {  // Comment lines are ignored.
      if (in_instruction_block) {
        // Instructions should be separate blocks.
        fprintf(stderr, "Line %u is test case in an instruction block.\n",
                line_);
        return kReadError;
      }
      ...
      std::string value =
          StripSpace(delimiter + 1, buf.get() + len - delimiter - 1);
