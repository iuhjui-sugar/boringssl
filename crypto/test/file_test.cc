...
const std::string &FileTest::GetName() {
  OnKeyUsed(name_);
  return name_;
}

const std::string &FileTest::GetValue() {
  OnKeyUsed(name_);
  return value_;
}
...
std::string FileTest::GetAttribute(const std::string &key) {
  OnKeyUsed(key);
...
bool FileTest::DecodeBytes(std::vector<uint8_t> *out, const std::string &in) {
  if (in.size() >= 2 && in[0] == '"' && in[in.size() - 1] == '"') {
    out->assign(in.begin() + 1, in.end() - 1);
    return true;
  }

  if (in.size() % 2 != 0) {
    PrintLine("Error decoding value: %s", in.c_str());
    return false;
  }
