...
// line. Blank lines are otherwise ignored. For tests which process multiple
// kinds of test cases, the first attribute is parsed out separately. Otherwise,
// attributes are unordered. Tests which do not dispatch may ignore this
// mechanism.
//
// Functions in this module freely output to |stderr| on failure. It is
// recommended that tests to include the input line number in output.
// |PrintLine| does this automatically.
//
...
class FileTest {
 public:
  ...
  // GetName returns the name of the first attribute of the current test.
  const std::string &GetName();
  // GetValue returns the value of the first attribute of the current test.
  ...
  // GetAttribute returns the attribute with value |key| or the empty string if
  // it was unset.
  ...
  // DecodeBytes decodes |in| as a byte string and writes the result to |*out|.
  // |in| may be either a hexadecimal string or a quoted ASCII string. It
  // returns true on success and returns false with an error to |stderr| on
  // failure.
  bool DecodeBytes(std::vector<uint8_t> *out, const std::string &in);
 ...
 private:
  ...
  // name_ is the name of the first attribute of the test.
  std::string name_;
  // value_ is the value of the first attribute.
  std::string value_;
