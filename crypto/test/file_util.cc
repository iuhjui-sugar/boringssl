...
static std::string GetTempPathCandidate() {
  ...
  return dir + "bssl_test_tmp_" + EncodeHex(buf);
}
...
bool TemporaryFile::Init(bssl::Span<const uint8_t> content) {
  // For simplicity, just try the first candidate and assume it works. 96 bits
  // of cryptographically secure randomness is plenty.
  path_ = GetTempPathCandidate();
  if (path_.empty()) {
    return false;
  }
...
bool TemporaryDirectory::Init() {
  // For simplicity, just try the first candidate and assume it works. 96 bits
  // of cryptographically secure randomness is plenty.
  std::string path = GetTempPathCandidate();
  if (path.empty()) {
    return false;
  }
