...
//
// - This is a debug build. We can instrument release builds at all, but this
//   ensures we cover the code uninstrumented as well. See the comment in
//   |CHECK_ABI|. Note ABI testing is only meaningful for assembly, which is not
//   affected by compiler optimizations.
...
template <typename R, typename... Args>
inline R CheckGTest(const char *func_str, const char *args_str,
                    const char *file, int line, R (*func)(Args...),
                    typename DeductionGuard<Args>::Type... args) {
  Result result;
  crypto_word_t ret = CheckImpl(&result, func, args...);
  if (!result.ok()) {
    testing::Message msg;
    msg << "ABI failures in " << func_str << "(" << args_str << "):\n";
...
// CHECK_ABI cannot itself be defined as CHECK_ABI(func, ...) because, if there
// are no arguments, __VA_ARGS__ will expand to nothing and the CheckGTest call
// will have a trailing comma.
#define CHECK_ABI_FUNC_AS_STRING_(func, ...) #func
#define CHECK_ABI_ARGS_AS_STRING_(func, ...) #__VA_ARGS__
...
// non-fatal GTest failure if |func| does not satisfy ABI requirements.
//
...
#define CHECK_ABI(...)                                                   \
  abi_test::internal::CheckGTest(CHECK_ABI_FUNC_AS_STRING_(__VA_ARGS__), \
                                 CHECK_ABI_ARGS_AS_STRING_(__VA_ARGS__), \
