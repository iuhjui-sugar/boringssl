...
class UnwindCursor {
 public:
  ...
  // GetSP returns the instruction pointer at the current frame.
...
class UnwindCursor {
 public:
  ...
  UnwindStatusOr<crypto_word_t> GetIP() {
    unw_word_t ip;
    int ret = unw_get_reg(&cursor_, UNW_REG_IP, &ip);
    if (ret < 0) {
      return UNWError(ret);
    }
    return ip;
  ...
  UnwindStatusOr<crypto_word_t> GetSP() {
    unw_word_t sp;
    int ret = unw_get_reg(&cursor_, UNW_REG_SP, &sp);
    if (ret < 0) {
      return UNWError(ret);
    }
    return sp;
  ...
  // GetCallerState returns the callee-saved registers at the current frame.
  UnwindStatusOr<CallerState> GetCallerState() {
    // |CallerState| uses |crypto_word_t|, while libunwind uses |unw_word_t|,
    // but both are defined as |uint*_t| from stdint.h, so we can assume the
    // types match.
    CallerState state;
    int ret = 0;
#if defined(OPENSSL_X86_64)
    ret = ret < 0 ? ret : unw_get_reg(&cursor_, UNW_X86_64_RBX, &state.rbx);
    ret = ret < 0 ? ret : unw_get_reg(&cursor_, UNW_X86_64_RBP, &state.rbp);
    ret = ret < 0 ? ret : unw_get_reg(&cursor_, UNW_X86_64_R12, &state.r12);
    ret = ret < 0 ? ret : unw_get_reg(&cursor_, UNW_X86_64_R13, &state.r13);
    ret = ret < 0 ? ret : unw_get_reg(&cursor_, UNW_X86_64_R14, &state.r14);
    ret = ret < 0 ? ret : unw_get_reg(&cursor_, UNW_X86_64_R15, &state.r15);
#else
#error "unknown architecture"
#endif
    if (ret < 0) {
