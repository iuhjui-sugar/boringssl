...
static bool unwind_tests_enabled = false;
...
crypto_word_t RunTrampoline(Result *out, crypto_word_t func,
  ...
  unwind = unwind && unwind_tests_enabled;
...
// in_trampoline is true if we are in an instrumented |abi_test_trampoline|
// call, in the region that triggers |SIGTRAP|.
static bool in_trampoline = false;
// unwind_function_done, if |in_trampoline| is true, is whether the function
// under test has returned. It is undefined otherwise.
static bool unwind_function_done;
// trampoline_state, if |in_trampoline| is true, is the state the function under
// test must preserve. It is undefined otherwise.
static CallerState trampoline_state;
// trampoline_sp, if |in_trampoline| is true, is the stack pointer of the
// trampoline frame. It is undefined otherwise.
static unw_word_t trampoline_sp;
...
static void TrapHandler(int sig) {
  ...
  if (!in_trampoline) {
    ...
    in_trampoline = true;
    unwind_function_done = false;
    trampoline_sp = sp;
    ret = CallerStateFromUNWCursor(&trampoline_state, &cursor);
    if (ret < 0) {
      FatalError("Error getting initial caller state: ", unw_strerror(ret));
    }
  } else {
    if (sp == trampoline_sp || unwind_function_done) {
      // |unwind_function_done| should imply |sp| is |trampoline_sp|, but
      ...
      if (ip == kReturnAddress && sp == trampoline_sp) {
        unwind_function_done = true;
      }
      if (ip == kStopAddress && sp == trampoline_sp) {
        // |SIGTRAP| is fatal again.
        in_trampoline = false;
      }
    } else if (IsAncestorStackFrame(sp, trampoline_sp)) {
      // This should never happen. We went past |trampoline_sp| without stopping
      // at |kStopAddress|.
      ...
      for (;;) {
        ...
        if (IsAncestorStackFrame(cur_sp, trampoline_sp)) {
          AddUnwindError("unwound past starting frame from ",
                         IPToString(ip, &ctx).data());
          break;
        }
        if (cur_sp == trampoline_sp) {
          ...
          if (ret < 0) {
            ...
            ForEachMismatch(state, trampoline_state, [&](const char *reg) {
...
bool UnwindTestsEnabled() { return internal::unwind_tests_enabled; }
