...
static int TestTruncatedTags(const EVP_AEAD *aead) {
  ...
  if (nonce_len > sizeof(nonce)) {
    fprintf(stderr, "Nonce length of AEAD too long.\n");
    return 0;
  }

  EVP_AEAD_CTX ctx;
  if (!EVP_AEAD_CTX_init(&ctx, aead, key, key_len, 1 /* one byte tag */,
                         NULL /* ENGINE */)) {
    /* Not all AEADs accept truncated tags. */
    ERR_clear_error();
    return 1;
  }
  ...
  memset(ciphertext, kSentinel, sizeof(ciphertext));

  if (!EVP_AEAD_CTX_seal(&ctx, ciphertext, &ciphertext_len, sizeof(ciphertext),
                         nonce, nonce_len, plaintext, sizeof(plaintext),
                         nullptr /* ad */, 0)) {
  ...
  if (overhead_used >= EVP_AEAD_max_overhead(aead)) {
    fprintf(stderr, "AEAD is probably ignoring request to truncate tags.\n");
    return 0;
  }

  uint8_t plaintext2[sizeof(plaintext) + 16];
  memset(plaintext2, kSentinel, sizeof(plaintext2));

  size_t plaintext2_len;
  if (!EVP_AEAD_CTX_open(&ctx, plaintext2, &plaintext2_len, sizeof(plaintext2),
