...
// rc4Stream is an io.Reader that produces a deterministic stream of
// pseudorandom bytes. This is so generating test data can round-trip
// properly.
...
func makeTestCase(length int, extraPadding, wrongBytes, noPadding bool) (*testCase, error) {
...
func main() {
	...
	if *bulkCipher != "rc4" {
		...
		t, err := makeTestCase(5, true, false, false)
		if err != nil {
		...
		t, err = makeTestCase(5, false, true, false)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s\n", err)
			os.Exit(1)
		}
		...
		t, err = makeTestCase(64 - hash.Size(), false, false, true)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s\n", err)
			os.Exit(1)
		}
	...
	for l := 0; l < 500; l += 5 {
		t, err := makeTestCase(l, false, false, false)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s\n", err)
			os.Exit(1)
		}
