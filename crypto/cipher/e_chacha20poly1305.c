...
static int open_p(aead_poly1305_update poly1305_update, const EVP_AEAD_CTX *ctx,
                  uint8_t *out, size_t *out_len, size_t max_out_len,
                  const uint8_t nonce[12], const uint8_t *in, size_t in_len,
                  const uint8_t *ad, size_t ad_len) {
  const struct aead_chacha20_poly1305_ctx *c20_ctx = ctx->aead_state;
...
static int aead_chacha20_poly1305_open(const EVP_AEAD_CTX *ctx, uint8_t *out,
  ...
  return open_p(poly1305_update, ctx, out, out_len, max_out_len, nonce, in,
                in_len, ad, ad_len);
}
...
static int aead_chacha20_poly1305_old_open(
  ...
  memset(nonce_96, 0, 4);
  memcpy(nonce_96 + 4, nonce, 8);
  return open_p(poly1305_update_old, ctx, out, out_len, max_out_len, nonce_96,
                in, in_len, ad, ad_len);
}
