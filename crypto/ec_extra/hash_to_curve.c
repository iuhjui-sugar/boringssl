...
static int hash_to_field2(const EC_GROUP *group, const EVP_MD *md,
  ...
  //
  // Note: there is a bug in the reference implementation used to generate the
  // P-521 test vectors. It computes L = 96, which would match a 512-bit prime.
  // A 521-bit prime with k = 256 gives L = 98.
  // |ec_hash_to_curve_p521_xmd_sha512_sswu_ref_for_testing| matches the
...
static inline void mul_minus_A(const EC_GROUP *group, EC_FELEM *out,
  ...
  ec_felem_add(group, &tmp, in, in);      // tmp = 2*in
  ec_felem_add(group, &tmp, &tmp, &tmp);  // tmp = 4*in
  ec_felem_sub(group, out, &tmp, in);     // out = 3*in
}
...
int ec_hash_to_curve_p521_xmd_sha512_sswu_ref_for_testing(
  ...
  // Using k = 240 reproduces the L = 96 value used in the reference
  // implementation. See the discussion in |hash_to_field2|.
  return hash_to_curve_p521_xmd_sswu(group, out, dst, dst_len, EVP_sha512(),
                                     /*k=*/240, msg, msg_len);
}
