...
OPENSSL_INLINE int ChaCha20_ctr32_ssse3_4x_capable(size_t len) {
  ...
  // 4x measured to be faster than 1x for larger messages on all CPUs. On Atom
  // 1x was measured to be faster for for short messages; the 1x path was
  // optimized for in-order Atom performance. Assume the CPU is Atom if it
  // supports MOVBE but not XSAVE.
  int faster =
      (len >= 192) || (CRYPTO_is_MOVBE_capable() && !CRYPTO_is_XSAVE_capable());
