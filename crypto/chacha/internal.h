...
OPENSSL_INLINE int ChaCha20_ctr32_avx2_capable(size_t len) {
  return (len >= 128) && CRYPTO_is_AVX2_capable(); // TODO
}
...
OPENSSL_INLINE int ChaCha20_ctr32_ssse3_4x_capable(size_t len) {
  // !MOVBE && XSAVE implies Atom
  return (len >= 192) && CRYPTO_is_SSSE3_capable() && CRYPTO_is_MOVBE_capable() &&
         CRYPTO_is_XSAVE_capable();
}
