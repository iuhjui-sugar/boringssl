...
static void zero_scalar(scalar *out) { OPENSSL_memset(out, 0, sizeof(*out)); }

static void zero_vector(vector *out) { OPENSSL_memset(out, 0, sizeof(*out)); }
...
// numbers.
static void multiply(scalar *out, const scalar *lhs, const scalar *rhs) {
...
static void matrix_multiply(vector *out, const matrix *m, const vector *a) {
  zero_vector(out);
  for (int i = 0; i < RANK; i++) {
    for (int j = 0; j < RANK; j++) {
      scalar product;
      multiply(&product, &m->v[i][j], &a->v[j]);
      add_accumulate_scalar(&out->v[i], &product);
    }
  }
}

static void matrix_multiply_transpose(vector *out, const matrix *m,
                                      const vector *a) {
  zero_vector(out);
  for (int i = 0; i < RANK; i++) {
    for (int j = 0; j < RANK; j++) {
      scalar product;
      multiply(&product, &m->v[j][i], &a->v[j]);
      add_accumulate_scalar(&out->v[i], &product);
    }
  }
}

static void inner_product(scalar *out, const vector *lhs, const vector *rhs) {
  zero_scalar(out);
  for (int i = 0; i < RANK; i++) {
    scalar product;
    multiply(&product, &lhs->v[i], &rhs->v[i]);
...
static void parse(scalar *out, struct BORINGSSL_keccak_st *ctx) {
  int index = 0;
  uint8_t bytes[3];
  while (index < DEGREE) {
...
// probability 3/8.
static void centered_binomial_distribution(scalar *out, int eta,
                                           uint8_t *entropy) {
  for (int i = 0; i < DEGREE; i++) {
    int32_t value = 0;
    for (int j = 0; j < eta; j++) {
...
static void generate_secret_vector(vector *out, int eta, uint8_t *counter,
                                   uint8_t *seed) {
  ...
  for (int i = 0; i < RANK; i++) {
    ...
    centered_binomial_distribution(&out->v[i], eta, entropy);
  }
}

// Expands the matrix of a seed for key generation and for encaps-CPA.
static void expand_matrix(matrix *out, uint8_t *rho) {
  uint8_t input[34];
...
// |bits| many bits.
static void encode_scalar(uint8_t *out, const scalar *s, int bits) {
  int written = 0;
  *out = 0;
  for (int i = 0; i < DEGREE; i++) {
    int left = bits;
...
static void encode_vector(uint8_t *out, const vector *a, int bits) {
  for (int i = 0; i < RANK; i++) {
    encode_scalar(out + i * bits * DEGREE / 8, &a->v[i], bits);
  }
}

// Algorithm 3 of the Kyber spec.
static void decode_scalar(scalar *out, uint8_t *in, int bits) {
  int remaining = 8;
  for (int i = 0; i < DEGREE; i++) {
    out->c[i] = 0;
    int left = bits;
...
static void decode_vector(vector *out, uint8_t *in, int bits) {
  for (int i = 0; i < RANK; i++) {
    decode_scalar(&out->v[i], in + i * bits * DEGREE / 8, bits);
  }
}
...
static void encrypt_cpa(uint8_t *out, uint8_t *public_key, uint8_t *message,
                        uint8_t *randomness) {
  vector rhs;
  decode_vector(&rhs, public_key, LOG2PRIME);
  matrix m;
  expand_matrix(&m, public_key + PUBLIC_KEY_RHO_OFFSET);
  uint8_t counter = 0;
  vector secret;
  generate_secret_vector(&secret, ETA1, &counter, randomness);
  number_theoretic_transform_vector(&secret);
  vector error;
  generate_secret_vector(&error, ETA2, &counter, randomness);
  ...
  encode_scalar(out + COMPRESSED_VECTOR_SIZE, &v, DV);
}
...
static void decrypt_cpa(uint8_t *out, uint8_t *private_key,
                        uint8_t *ciphertext) {
  vector u;
  decode_vector(&u, ciphertext, DU);
  decompress_vector(&u, DU);
  number_theoretic_transform_vector(&u);
  scalar v;
  decode_scalar(&v, ciphertext + COMPRESSED_VECTOR_SIZE, DV);
  decompress_scalar(&v, DV);
  vector secret;
  decode_vector(&secret, private_key, LOG2PRIME);
  scalar mask;
  inner_product(&mask, &secret, &u);
  inverse_number_theoretic_transform_scalar(&mask);
  subtract_accumulate_scalar(&v, &mask);
  compress_scalar(&v, 1);
  encode_scalar(out, &v, 1);
}
...
void KYBER_generate_key_external_entropy(
  ...
  uint8_t counter = 0;
  vector secret;
  generate_secret_vector(&secret, ETA1, &counter, sigma);
  number_theoretic_transform_vector(&secret);
  vector error;
  generate_secret_vector(&error, ETA1, &counter, sigma);
  number_theoretic_transform_vector(&error);
  vector rhs;
  matrix_multiply_transpose(&rhs, &m, &secret);
