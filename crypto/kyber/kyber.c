...
static const int kEta1 = 2;
static const int kEta2 = 2;
static const uint16_t kPrime = 3329;
static const int kLog2Prime = 12;
static const uint16_t kHalfPrime = (kPrime - 1) / 2;
static const int kDegree = 256;
static const int kRank = 3;
static const int kDU = 10;
static const int kDV = 4;
static const uint16_t kInverseDegree = 3303;
static const size_t kEncodedScalarSize = kLog2Prime * kDegree / 8;
static const size_t kEncodedVectorSize = kEncodedScalarSize * kRank;
static const size_t kCompressedVectorSize = kDU * kRank * kDegree / 8;
...
// reduce_simple reduces 0 <= x < 2*kPrime, mod kPrime.
static uint16_t reduce_simple(uint16_t x) {
  assert(x < 2 * kPrime);
  const uint16_t subtracted = x - kPrime;
  uint16_t mask = ((int16_t)subtracted) >> 15;
  // On Aarch64, omitting a |value_barrier_u16| results in a 2x speedup of Kyber
  // overall and Clang still produces constant-time code using `csel`.
  return (mask & x) | (~mask & subtracted);
}

// constant time reduce x mod kPrime using Barrett reduction.
static uint16_t reduce(uint32_t x) {
  assert(x < kPrime + 2 * kPrime * kPrime);
...
static void scalar_ntt(scalar *s) {
  int offset = kDegree;
  for (int step = 1; step < kDegree / 2; step <<= 1) {
    offset >>= 1;
    int k = 0;
    for (int i = 0; i < step; i++) {
      ...
      for (int j = k; j < k + offset; j++) {
        ...
        s->c[j + offset] = reduce_simple(even - odd + kPrime);
      }
      k += 2 * offset;
    }
  }
...
static void scalar_inverse_ntt(scalar *s) {
  int step = kDegree / 2;
  for (int offset = 2; offset < kDegree; offset <<= 1) {
    step >>= 1;
    int k = 0;
    for (int i = 0; i < step; i++) {
...
static void scalar_sub(scalar *lhs, const scalar *rhs) {
  for (int i = 0; i < kDegree; i++) {
    lhs->c[i] = reduce_simple(lhs->c[i] - rhs->c[i] + kPrime);
  }
}
...
static void scalar_mult(scalar *out, const scalar *lhs, const scalar *rhs) {
  for (int i = 0; i < kDegree / 2; i++) {
    uint16_t real_real = reduce(lhs->c[2 * i] * rhs->c[2 * i]);
    uint16_t img_img = reduce(rhs->c[2 * i + 1] * lhs->c[2 * i + 1]);
    uint16_t real_img = reduce(lhs->c[2 * i] * rhs->c[2 * i + 1]);
    uint16_t img_real = reduce(lhs->c[2 * i + 1] * rhs->c[2 * i]);
    out->c[2 * i] =
        reduce_simple(real_real + reduce((uint32_t)img_img * kModRoots[i]));
...
static void vector_generate_secret(vector *out, int eta, uint8_t *counter,
  ...
  for (int i = 0; i < kRank; i++) {
    input[32] = (*counter)++;
    uint8_t entropy[128];
    BORINGSSL_keccak(entropy, size, input, sizeof(input), boringssl_shake256);
...
// |bits| many bits. This results in |kDegree|*|bits|/8 bytes of output.
...
// Encodes an entire vector into 32*|kRank|*|bits| bytes. Note that since 256
...
static void vector_encode(uint8_t *out, const vector *a, int bits) {
  for (int i = 0; i < kRank; i++) {
    scalar_encode(out + i * bits * kDegree / 8, &a->v[i], bits);
  }
}

// Algorithm 3 of the Kyber spec. Reads |kDegree|*|bits|/8 bytes from |in|.
...
// Decodes 32*|kRank|*|bits| bytes from |in| into |out|.
static void vector_decode(vector *out, const uint8_t *in, int bits) {
  for (int i = 0; i < kRank; i++) {
    scalar_decode(&out->v[i], in + i * bits * kDegree / 8, bits);
  }
}
...
static uint16_t compress(uint16_t x, int bits) {
  ...
  assert(remainder >= 0 && remainder < 2 * kPrime);
  // Adjusting quotient if necessary.
  quotient = constant_time_select_w(constant_time_ge_w(remainder, kPrime),
                                    quotient + 1, quotient);
  ...
  remainder = product - quotient * kPrime;
  // The rounding logic compares the number to (kPrime - 1)/2 and conditionally
  // increments the quotient. The final mod 2^|bits| step is achieves through
  // bitwise operations.
  return constant_time_select_w(constant_time_ge_w(kHalfPrime, remainder),
                                quotient, quotient + 1) &
         ((1 << bits) - 1);
}
...
void KYBER_generate_key_external_entropy(
  ...
  uint8_t counter = 0;
  vector_generate_secret(&priv->s, kEta1, &counter, sigma);
  vector_ntt(&priv->s);
  vector error;
  vector_generate_secret(&error, kEta1, &counter, sigma);
  vector_ntt(&error);
...
static void encrypt_cpa(uint8_t out[KYBER_CIPHERTEXT_BYTES],
                        const struct public_key *pub, const uint8_t message[64],
                        const uint8_t randomness[32]) {
  uint8_t counter = 0;
  vector secret;
  vector_generate_secret(&secret, kEta1, &counter, randomness);
  vector_ntt(&secret);
  vector error;
  vector_generate_secret(&error, kEta2, &counter, randomness);
  int size = 2 * kEta2 * kDegree / 8;
  assert(size <= 128);
  uint8_t input[33];
  ...
  BORINGSSL_keccak(entropy, size, input, sizeof(input), boringssl_shake256);
  scalar scalar_error;
  scalar_centered_binomial_distribution(&scalar_error, kEta2, entropy);
  vector u;
...
void KYBER_encap_external_entropy(
  ...
  encrypt_cpa(out_ciphertext, pub, input, prekey_and_randomness + 32);
...
void KYBER_decap(uint8_t *out_shared_secret, size_t out_shared_secret_len,
  ...
  for (int i = 0; i < KYBER_CIPHERTEXT_BYTES; i++) {
    accumulator |= ciphertext[i] ^ expected_ciphertext[i];
  }
  uint8_t mask = constant_time_is_zero_8(accumulator);
  const uint8_t *const fo_failure_secret = priv->fo_failure_secret;
  uint8_t input[64];
  for (int i = 0; i < 32; i++) {
...
int KYBER_public_key_from_bytes(struct KYBER_public_key *public_key,
                                const uint8_t *in, size_t in_len) {
  CBS cbs;
  CBS_init(&cbs, in, in_len);
  return KYBER_parse_public_key(public_key, &cbs);
}
...
int KYBER_private_key_from_bytes(struct KYBER_private_key *out_private_key,
                                 const uint8_t *in, size_t in_len) {
  CBS cbs;
  CBS_init(&cbs, in, in_len);
  return KYBER_parse_private_key(out_private_key, &cbs);
}
