...
static void encrypt_cpa(uint8_t *out,
...
static void decrypt_cpa(uint8_t *out, const uint8_t *private_key,
...
void KYBER_generate_key_external_entropy(
  ...
  OPENSSL_memcpy(out_private_key + FO_FAILUE_OFFSET, entropy + 32, 32);
}
...
void KYBER_encap_external_entropy(
  ...
  BORINGSSL_keccak(input + 32, 32, public_key, KYBER_PUBLIC_KEY_BYTES,
                   boringssl_sha3_256);
...
void KYBER_decap(uint8_t *out_shared_secret, size_t out_shared_secret_len,
  ...
  for (int i = 0; i < 32; i++) {
    input[i] = constant_time_select_8(mask, prekey_and_randomness[i],
                                      (private_key + FO_FAILUE_OFFSET)[i]);
  }
  BORINGSSL_keccak(input + 32, 32, ciphertext, KYBER_CIPHERTEXT_BYTES,
                   boringssl_sha3_256);
  BORINGSSL_keccak(out_shared_secret, out_shared_secret_len, input, sizeof(input),
                   boringssl_shake256);
}
