...
static void vector_generate_secret(vector *out, uint8_t *counter,
...
void KYBER_generate_key_external_entropy(
  ...
  uint8_t counter = 0;
  vector_generate_secret(&priv->s, &counter, sigma);
  vector_ntt(&priv->s);
  vector error;
  vector_generate_secret(&error, &counter, sigma);
  vector_ntt(&error);
...
static void encrypt_cpa(uint8_t out[KYBER_CIPHERTEXT_BYTES],
  ...
  uint8_t counter = 0;
  vector secret;
  vector_generate_secret(&secret, &counter, randomness);
  vector_ntt(&secret);
  vector error;
  vector_generate_secret(&error, &counter, randomness);
  uint8_t input[33];
