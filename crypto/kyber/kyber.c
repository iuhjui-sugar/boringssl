...
#define FO_FAILURE_OFFSET (2 * ENCODED_VECTOR_SIZE + 2 * 32)

typedef struct scalar {
  // On every function entry and exit, 0 <= c < PRIME.
  uint16_t c[DEGREE];
} scalar;
...
static uint16_t reduce(uint32_t x) {
  assert(x >= 0 && x < PRIME + 2 * PRIME * PRIME);
  uint64_t product = (uint64_t)x * BARRETT_MULTIPLIER;
  uint32_t quotient = product >> BARRETT_SHIFT;
  uint32_t remainder = x - quotient * PRIME;
...
static uint16_t compress(uint16_t x, int bits) {
  ...
  uint32_t quotient = ((uint64_t)product * BARRETT_MULTIPLIER) >> BARRETT_SHIFT;
  uint32_t remainder = product - quotient * PRIME;
...
static uint16_t decompress(uint16_t x, int bits) {
  uint32_t product = (uint32_t)x * PRIME;
...
static void decrypt_cpa(uint8_t out[32],
...
void KYBER_generate_key_external_entropy(
  ...
  vector_encode(out_public_key, &rhs, LOG2PRIME);
  OPENSSL_memcpy(out_public_key + ENCODED_VECTOR_SIZE, rho, 32);
  uint8_t public_key_hash[32];
  BORINGSSL_keccak(public_key_hash, sizeof(public_key_hash), out_public_key,
                   KYBER_PUBLIC_KEY_BYTES, boringssl_sha3_256);
  vector_encode(out_private_key, &secret, LOG2PRIME);
  OPENSSL_memcpy(out_private_key + PUBLIC_KEY_OFFSET, out_public_key,
                 KYBER_PUBLIC_KEY_BYTES);
  OPENSSL_memcpy(out_private_key + PUBLIC_KEY_HASH_OFFSET, public_key_hash,
                 sizeof(public_key_hash));
  OPENSSL_memcpy(out_private_key + FO_FAILURE_OFFSET, entropy + 32, 32);
}
...
void KYBER_encap_external_entropy(
  ...
  BORINGSSL_keccak(input + KYBER_ENCAP_ENTROPY,
                   sizeof(input) - KYBER_ENCAP_ENTROPY, public_key,
                   KYBER_PUBLIC_KEY_BYTES, boringssl_sha3_256);
  ...
  encrypt_cpa(out_ciphertext, public_key, input, prekey_and_randomness + 32);
...
void KYBER_decap(uint8_t *out_shared_secret, size_t out_shared_secret_len,
  ...
  decrypt_cpa(decrypted, private_key, ciphertext);
  OPENSSL_memcpy(decrypted + 32, private_key + PUBLIC_KEY_HASH_OFFSET, 32);
  uint8_t prekey_and_randomness[64];
  ...
  encrypt_cpa(expected_ciphertext, private_key + PUBLIC_KEY_OFFSET, decrypted,
  ...
  for (int i = 0; i < 32; i++) {
    input[i] = constant_time_select_8(mask, prekey_and_randomness[i],
                                      (private_key + FO_FAILURE_OFFSET)[i]);
  }
