...
static void KyberFileTest(FileTest *t) {
  ...
  KYBER_encap_external_entropy(ciphertext, encapsulated_key, 32, public_key,
                               encap_entropy);
  KYBER_decap(decapsulated_key, 32, ciphertext, private_key);
  ...
  std::memcpy(corrupted_ciphertext, ciphertext, KYBER_CIPHERTEXT_BYTES);
  corrupted_ciphertext[3] ^= 0x40;
  KYBER_decap(corrupted_decapsulated_key, 32, corrupted_ciphertext, private_key);
