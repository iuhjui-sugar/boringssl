...
void BORINGSSL_keccak(uint8_t *out, size_t out_len, const uint8_t *in,
                      size_t in_len, enum boringssl_keccak_config config) {
  size_t required_out_len = 0;
  ...
  const size_t rate_bytes = 200 - capacity_bytes;
  ...
  do {
    ...
    if (in_len == 0) {
      state.u8[todo] ^= terminator;
      state.u8[rate_bytes - 1] ^= 0x80;
    }
  } while (in_len);
  ...
  while (out_len) {
    ...
    keccak_f(state.u64);
    OPENSSL_memcpy(out, state.u8, todo);
    out += todo;
    out_len -= todo;
  }
}
