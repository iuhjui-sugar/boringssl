...
if __name__ == '__main__':
    p("# This file was generated by poly_rq_mul.py")
    p(".text")
    p(".align 32")
    ...
    p(".cfi_startproc")
    p("push %rbp")
    p(".cfi_adjust_cfa_offset 8")
    p(".cfi_offset rbp, -16")
    p("movq %rsp, %rbp")
    p(".cfi_def_cfa_register rbp")
    ...
    for coeff in range(3):
        ...
        for j in range(8): # for each 16 (or 12) coefficient chunk
            ...
            def store_limb(limbreg, i, j):
                ...
                if coeff == 2:
                    offset = (i*176 + j * 44 + coeff*16) * 2
                    if offset >= 701*2:
                        return

                    p("vpand mask_mod8192(%rip), %ymm{}, %ymm{}".format(limbreg, limbreg))
                    p("vmovdqu %xmm{}, {}({})".format(limbreg, offset, r_real))
                    p("vextracti128 $1, %ymm{}, %xmm{}".format(limbreg, limbreg, limbreg))
                    if offset + 32 >= 701*2:
                        # An xmm write would spill off the end of the output buffer.
                        assert(offset == 1384)  # this is the only case.
                        p("vpextrw $0, %xmm{}, {}({})".format(limbreg, offset + 16, r_real))
                    else:
                        p("vmovq %xmm{}, {}({})".format(limbreg, offset + 16, r_real))
            ...
            if j < 8:
                for i in range(-1, 3):
                    ...
                    p("vpand {}(%rip), %ymm{}, %ymm{}".format(mask, h[i+4], temp))
