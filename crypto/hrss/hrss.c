...
static void poly3_mul_aux(const struct poly3_span *out,
                          ...
                          const struct poly3_span *b, size_t n) {
  if (n == 1) {
    word_t r_s_low, r_s_high = 0, r_a_low, r_a_high = 0;
...
static void poly3_mul(struct poly3 *out, const struct poly3 *x,
  ...
  word_t scratch_s[WORDS_PER_POLY * 3];  // FIXME: make this exact.
  word_t scratch_a[WORDS_PER_POLY * 3];
...
static void poly_mul_novec(struct poly *out, const struct poly *x,
                           const struct poly *y) {
  uint16_t prod[2 * N];
  uint16_t scratch[2 * N];  // FIXME make exact
