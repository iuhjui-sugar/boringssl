...
// POLY_MUL_SCRATCH contains the working variables needed by |poly_mul|. An
// instance of this object must be passed into |poly_mul| for it to use. The
// contents afterwards may be discarded, but the object may also be reused in
// future |poly_mul| calls to save heap allocations.
//
...
struct POLY_MUL_SCRATCH {
  union {
    ...
    struct {
      uint8_t space[POLY_MUL_RQ_SCRATCH_SPACE];
    } rq;
#endif
  } u;
};
...
static void poly_mul(struct POLY_MUL_SCRATCH *scratch, struct poly *r,
  ...
  if (has_avx2) {
    poly_Rq_mul(r->v, a->v, b->v, scratch);
    return;
  }
#endif
