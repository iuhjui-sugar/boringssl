...
// vec_set takes either |CONSTTIME_TRUE_W| or |CONSTTIME_FALSE_W| and returns a
// vector with all bits set or none set, respectively.
static inline vec_t vec_set(crypto_word_t v) { return _mm_set1_epi32(v); }
...
static inline vec_t vec_set(crypto_word_t v) { return vdupq_n_u16(v); }
...
// poly3_fmadd sets |out| to |out| + |in|×m, where m is (ms, ma). The last word
// past |BITS_IN_LAST_WORD| is not cleared.
static void poly3_fmadd(struct poly3 *RESTRICT out,
  ...
  for (size_t i = 0; i < WORDS_PER_POLY; i++) {
    ...
    const crypto_word_t orig_s = out->s.v[i];
    const crypto_word_t orig_a = out->a.v[i];
    const crypto_word_t t1 = ~(orig_s | orig_a);
    const crypto_word_t t2 = ~(product_s | product_a);
...
// poly3_clear_top_bits clears the bits in the final word that are only for
// alignment.
static void poly3_clear_top_bits(struct poly3 *p) {
...
static void poly3_mod_phiN(struct poly3 *p) {
  ...
  const crypto_word_t t2 = ~(factor_s | factor_a);

  for (size_t i = 0; i < WORDS_PER_POLY; i++) {
    const crypto_word_t s = p->s.v[i];
    const crypto_word_t a = p->a.v[i];
    const crypto_word_t t1 = ~(s | a);
    p->s.v[i] = (a & factor_a) ^ (t1 & factor_s) ^ (t2 & s);
    p->a.v[i] = (s & factor_s) ^ (t1 & factor_a) ^ (t2 & a);
  }

  poly3_clear_top_bits(p);
}
...
static void poly3_word_add(crypto_word_t *out_s, crypto_word_t *out_a,
  ...
  const crypto_word_t t1 = ~(s1 | a1);
  const crypto_word_t t2 = ~(s2 | a2);
...
static inline crypto_word_t word_from_vec(vec_t v) {
  crypto_word_t ret;
  memcpy(&ret, &v, sizeof(ret));
  return ret;
}

// poly3_invert_vec sets |*out| to |in|^-1, i.e. such that |out|×|in| == 1.
static void poly3_invert_vec(struct poly3 *out, const struct poly3 *in) {
  ...
  for (unsigned i = 0; i < 2 * (N - 1) - 1; i++) {
    ...
    const crypto_word_t should_swap_word = word_from_vec(should_swap);
    const crypto_word_t still_going_word = word_from_vec(still_going);

    const crypto_word_t deg_sum = should_swap_word & (deg_f ^ deg_g);
    deg_f ^= deg_sum;
    deg_g ^= deg_sum;

    deg_f -= 1 & still_going_word;
    k += 1 & still_going_word;
    const vec_t f0_is_nonzero = vec_broadcast_bit(f_s[0] | f_a[0]);
    const crypto_word_t f0_is_nonzero_word = word_from_vec(f0_is_nonzero);

    rotation = constant_time_select_w(still_going_word & f0_is_nonzero_word, k,
                                      rotation);
    const vec_t f0s_sum = (still_going & f0_is_nonzero) & (f_s[0] ^ f0s);
    f0s ^= f0s_sum;
    const vec_t f0a_sum = (still_going & f0_is_nonzero) & (f_a[0] ^ f0a);
    f0a ^= f0a_sum;
...
// HRSS_poly3_invert sets |*out| to |in|^-1, i.e. such that |out|×|in| == 1.
void HRSS_poly3_invert(struct poly3 *out, const struct poly3 *in) {
  ...
  for (unsigned i = 0; i < 2 * (N - 1) - 1; i++) {
    ...
    rotation = constant_time_select_w(still_going & f0_is_nonzero, k, rotation);
    f0s = constant_time_select_w(still_going & f0_is_nonzero, f.s.v[0], f0s);
    f0a = constant_time_select_w(still_going & f0_is_nonzero, f.a.v[0], f0a);
