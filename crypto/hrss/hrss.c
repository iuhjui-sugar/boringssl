...
static void poly3_fmsub(struct poly3 *RESTRICT out,
  ...
  for (size_t i = 0; i < WORDS_PER_POLY; i++) {
    ...
    const crypto_word_t product_a = a & ma;
    const crypto_word_t product_s = (s ^ ms) & product_a;

    const crypto_word_t out_s = out->s.v[i];
    const crypto_word_t out_a = out->a.v[i];
    const crypto_word_t t = out_a ^ product_a;
    out->s.v[i] = (out_s ^ product_a) & (t ^ product_s);
    out->a.v[i] = t | (out_s ^ product_s);
  }
}
...
static void poly3_mul_aux(const struct poly3_span *out,
  ...
  if (n == 1) {
    ...
    for (size_t i = 0; i < BITS_PER_WORD; i++) {
      ...
      const crypto_word_t m_a = a_a & v_a;
      const crypto_word_t m_s = (a_s ^ v_s) & m_a;
