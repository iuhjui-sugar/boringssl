...
// vec3_rshift1 right-shifts the 24 uint16_t's in |v| by one.
static inline void vec3_rshift1(vec_t v[3]) {
...
// vec4_rshift1 right-shifts the 32 uint16_t's in |v| by one.
static inline void vec4_rshift1(vec_t v[4]) {
  const __m128i carry0 = _mm_srli_si128(v[0], 14);
  v[0] = _mm_slli_si128(v[0], 2);
...
static inline vec_t vec_broadcast_bit(vec_t a) {
  ...
  return vdupq_lane_u16(*(uint16x4_t *)&a, 0);
}
...
static inline void poly3_vec_lshift1(vec_t a_s[6], vec_t a_a[6]) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    a_s[i] <<= 1;
    vec_t t = vextq_u16(kZero, next_carry_s, 7);
    a_s[i] |= vextq_u16(kZero, next_carry_s, 7);
    a_s[i] |= carry_s;
    t = vextq_u16(next_carry_s, kZero, 7);
...
static int poly_parse_mod3(struct poly *out,
                           const uint8_t in[HRSS_POLY3_BYTES]) {
  uint16_t *coeffs = out->v;
  for (size_t i = 0; i < HRSS_POLY3_BYTES; i++) {
    if (in[i] > 242) {
      return 0;
    }
...
void HRSS_generate_key(
  ...
  OPENSSL_STATIC_ASSERT(
      sizeof(struct HRSS_public_key) >= sizeof(struct public_key) + 8,
      "HRSS public key too small");
  OPENSSL_STATIC_ASSERT(
      sizeof(struct HRSS_private_key) >= sizeof(struct private_key) + 8,
      "HRSS private key too small");
#if defined(__GNUC__) || defined(__clang__)
  OPENSSL_STATIC_ASSERT(__alignof__(struct HRSS_public_key) >= 8,
                        "HRSS public key incorrect alignment");
  OPENSSL_STATIC_ASSERT(__alignof__(struct HRSS_private_key) >= 8,
                        "HRSS private key incorrect alignment");
#endif
...
void HRSS_marshal_private_key(uint8_t out[HRSS_PRIVATE_KEY_BYTES],
                              const struct HRSS_private_key *in_priv) {
  const struct private_key *priv =
      private_key_from_external((struct HRSS_private_key *)in_priv);
  struct poly f, f_inverse;
  poly_from_poly3(&f, &priv->f);
  poly_marshal_mod3(out, &f);
  out += HRSS_POLY3_BYTES;
  poly_from_poly3(&f_inverse, &priv->f_inverse);
  poly_marshal_mod3(out, &f_inverse);
  out += HRSS_POLY3_BYTES;
  poly_marshal(out, &priv->ph_inverse);
  out += POLY_BYTES;

  // poly_marshal only marshals 700 coefficients because, in the normal context,
  // that's the correct thing for it to do. We don't want a second copy of the
  // marshaling code so we just include the final coefficient as a little-endian
  // number:
  out[0] = priv->ph_inverse.v[N - 1];
  out[1] = priv->ph_inverse.v[N - 1] >> 8;
  out += 2;

  OPENSSL_memcpy(out, priv->hmac_key, sizeof(priv->hmac_key));
}

int HRSS_parse_private_key(struct HRSS_private_key *out_priv,
                           const uint8_t in[HRSS_PRIVATE_KEY_BYTES]) {
  struct private_key *priv = private_key_from_external(out_priv);
  struct poly f, f_inverse;

  if (!poly_parse_mod3(&f, in) ||
      !poly_parse_mod3(&f_inverse, in + HRSS_POLY3_BYTES) ||
      !poly_unmarshal(&priv->ph_inverse, in + 2 * HRSS_POLY3_BYTES, 0)) {
    return 0;
  }

  priv->ph_inverse.v[N - 1] =
      in[2 * HRSS_POLY3_BYTES + POLY_BYTES] |
      ((uint16_t)in[2 * HRSS_POLY3_BYTES + POLY_BYTES + 1]) << 8;

  poly3_from_poly(&priv->f, &f);
  poly3_from_poly(&priv->f_inverse, &f_inverse);
  OPENSSL_memcpy(&priv->hmac_key, in + 2 * HRSS_POLY3_BYTES + POLY_BYTES + 2,
                 sizeof(priv->hmac_key));
  return 1;
}
