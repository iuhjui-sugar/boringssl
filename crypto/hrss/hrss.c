...
static void poly2_reverse_700(struct poly2 *out, const struct poly2 *in) {
  assert(((in->v[WORDS_PER_POLY-1] >> (700 % BITS_PER_WORD)) & 1) == 0);
...
static void poly3_invert_vec(struct poly3 *out, const struct poly3 *in) {
  ...
  // r = 1
  memset(&r_s, 0, sizeof(v_s));
  memset(&r_a, 0, sizeof(v_a));
  r_a[0] = kOne;
  // f = all ones.
  memset(f_s, 0, sizeof(g_s));
  ...
  for (size_t i = 0; i < (2*(N-1)) - 1; i++) {
    ...
    const crypto_word_t delta_is_non_negative = ~delta_sign_bit;
    const crypto_word_t delta_is_positive = constant_time_ge_w(delta, 1);
    const vec_t g_has_constant_term = vec_broadcast_bit(g_a[0] & kOne);
    const vec_t mask_w =
        {delta_is_non_negative & delta_is_positive};
  ...
  poly3_mul_const(out, lsb_to_all(f_s[0][0]), lsb_to_all(f_a[0][0]));
...
void HRSS_poly3_invert(struct poly3 *out, const struct poly3 *in) {
  ...
  for (size_t i = 0; i < (2*(N-1)) - 1; i++) {
    ...
    const crypto_word_t delta_is_positive = constant_time_ge_w(delta, 1);
    const crypto_word_t g_has_constant_term =
        ~constant_time_is_zero_w(g.a.v[0] & 1);
    const crypto_word_t mask =
        g_has_constant_term & delta_is_non_negative & delta_is_positive;
...
static void poly_invert_mod2(struct poly *out, const struct poly *in) {
  ...
  for (size_t i = 0; i < (2*(N-1)) - 1; i++) {
    ...
    const crypto_word_t delta_is_positive = constant_time_ge_w(delta, 1);
    const crypto_word_t g_has_constant_term =
        ~constant_time_is_zero_w(g.v[0] & 1);
    const crypto_word_t mask =
        g_has_constant_term & delta_is_non_negative & delta_is_positive;
