...
TEST(HRSS, Poly3Invert) {
  ...
  for (size_t i = 0; i < 500; i++) {
    poly3 r;
    RAND_bytes(reinterpret_cast<uint8_t*>(&r), sizeof(p));
    r.s.v[WORDS_PER_POLY - 1] &= (UINT64_C(1) << BITS_IN_LAST_WORD) - 1;
    r.a.v[WORDS_PER_POLY - 1] &= (UINT64_C(1) << BITS_IN_LAST_WORD) - 1;
    // (s, a) = (1, 1) is invalid. Map those to one.
    for (size_t j = 0; j < WORDS_PER_POLY; j++) {
      r.s.v[j] ^= r.s.v[j] & r.a.v[j];
    }
