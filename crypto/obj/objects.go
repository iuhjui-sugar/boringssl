...
func readNumbers(path string) (nameToNID map[string]int, numNIDs int, err error) {
	in, err := os.Open(path)
	if err != nil {
		return nil, 0, fmt.Errorf("error opening %s: %s", path, err)
	}
	defer in.Close()
	...
	for scanner.Scan() {
		...
		if _, ok := nameToNID[name]; ok {
			return nil, 0, withLine(fmt.Errorf("duplicate name '%s'", name))
		}
...
func readObjects(numPath, objectsPath string) (*objects, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error opening %s: %s", objectsPath, err)
	}
	defer in.Close()

	// Implicitly define NID_undef.
	objs := &objects{}
	objs.byNID = make([]object, numNIDs)
	objs.nameToNID = make(map[string]int)
...
func writeHeader(path string, objs *objects) error {
	var b bytes.Buffer
	fmt.Fprintf(&b, `/* Copyright (c) 2016, Google Inc.
 *
 * Permission to use, copy, modify, and/or distribute this software for any
 * purpose with or without fee is hereby granted, provided that the above
 * copyright notice and this permission notice appear in all copies.
