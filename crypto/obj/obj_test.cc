...
static bool ExpectObj2Txt(const uint8_t *der, size_t der_len,
  ...
  int len = OBJ_obj2txt(nullptr, 0, &obj, dont_return_name ? 1 : 0);
  if (len != expected_len) {
    fprintf(stderr,
  ...
  len = OBJ_obj2txt(short_buf, sizeof(short_buf), &obj, dont_return_name ? 1 : 0);
  if (len != expected_len) {
    fprintf(stderr,
  ...
  char buf[256];
  len = OBJ_obj2txt(buf, sizeof(buf), &obj, dont_return_name ? 1 : 0);
  if (len != expected_len) {
    fprintf(stderr,
