...
static bool ExpectObj2Txt(const uint8_t *der, size_t der_len,
                          bool ignore_names, const char *expected) {
  ASN1_OBJECT obj;
  ...
  int len = OBJ_obj2txt(nullptr, 0, &obj, ignore_names);
  if (len != expected_len) {
    fprintf(stderr,
  ...
  len = OBJ_obj2txt(short_buf, sizeof(short_buf), &obj, ignore_names);
  if (len != expected_len) {
    fprintf(stderr,
  ...
  char buf[256];
  len = OBJ_obj2txt(buf, sizeof(buf), &obj, ignore_names);
  if (len != expected_len) {
    fprintf(stderr,
