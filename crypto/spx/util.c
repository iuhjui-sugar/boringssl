...
// Converts the integer value of input to out_len bytes in big-endian order.
void to_bytes(uint8_t *output, size_t out_len, uint64_t input) {
...
uint64_t to_uint64(const uint8_t *input, int input_len) {
  uint64_t tmp = 0;
