...
TEST(SpxTest, KeyGeneration) {
  ...
  EXPECT_EQ(memcmp(pk, expected_pk, sizeof(expected_pk)), 0);
  EXPECT_EQ(memcmp(sk, expected_sk, sizeof(expected_pk)), 0);
}
...
TEST(SpxTest, BasicSignVerify) {
  ...
  EXPECT_EQ(spx_verify(signature, pk, message, sizeof(message)), 0);
}
...
static void SpxFileTest(FileTest *t) {
  ...
  EXPECT_NE(spx_verify(signature.data(), public_key.data(), message.data(),
                       message.size()),
            0);
}
