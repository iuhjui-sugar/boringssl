...
{ package seh_directive;
    ...
    # for details on Windows's unwind mechanisms. The one difference from MASM
    # is there is no .seh_endprolog directive. Instead, the last prolog
    # directive is implicitly the end of the prolog.
    #
    ...
    # annotate epilogs, so we'd need to combine parts from both. Synthesizing
    # CFI from SEH requires restricting ourselves to a subset of CFI.
    #
    # Additionally, this only supports @abi-omnipotent functions (i.e. it is
    # incompatible with the automatic calling convention conversion). The main
    # compliation is the current scheme modifies RDI and RSI, which are
    # non-volatile on Windows, and stashes them in the parameter stack area.
    # This can be expressed with .seh_savereg, but .seh_savereg can only be used
    # fairly late in the prolog. However, unwind information gives enough data
    # to locate the parameter stack area at any point in the function, so we can
    # defer conversion or implement other schemes.
	...
	my $func = $current_function->{name};
	if ($func ne $prev_label_func) {
	    $prev_label_func = $func;
	    $next_label = 0;
	}
