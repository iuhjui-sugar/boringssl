...
// that the probability of a false positive is negligible.
...
static bool ForkAndRand(bssl::Span<uint8_t> out) {
  ...
  // and there currently are no threads, so this is safe.
  pid_t child = fork();
  if (child < 0) {
    perror("fork");
    close(pipefds[0]);
...
TEST(RandTest, Fork) {
  ...
  // Draw a little entropy to initialize any internally PRNG buffering.
  uint8_t byte;
  RAND_bytes(&byte, 1);
