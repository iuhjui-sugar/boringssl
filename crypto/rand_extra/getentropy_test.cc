...
TEST(GetEntropyTest, NotObviouslyBroken) {
  ...
  errno = 0;
  uint8_t toobig[257];
  // getentropy should fail returning -1 and setting errno to EIO if you
  // request more than 256 bytes of entropy
  // MacOS lies about it in its man page and returns EINVAL, so we accept either
  // as close enough on MacOS.
...
#endif
  errno = 0;
  // getentropy should fail with EINVAL when passed and invalid address.
  // again, MacOS again lies and returns EFAULT instead, although arguably
  // NULL is a valid address. so we just ensure this should fail and set errno.
