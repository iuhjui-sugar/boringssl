...
/* This is a PKCS#8 encrypted private key. It was generated with:
 *
...
static int test(const char *name, const uint8_t *der, size_t der_len) {
  ...
  X509_SIG *x509 = d2i_X509_SIG(NULL, &data, der_len);
  if (!x509) {
    fprintf(stderr, "d2i_X509_SIG failed.\n");
    return 0;
  }

  const char *password = "testing";
  PKCS8_PRIV_KEY_INFO *keypair = PKCS8_decrypt_pbe(x509, (const uint8_t *) password, strlen(password));
  X509_SIG_free(x509);
  if (!keypair) {
    fprintf(stderr, "PKCS8_decrypt failed.\n");
    return 0;
  }
  PKCS8_PRIV_KEY_INFO_free(keypair);
  return 1;
}
...
int main(int argc, char **argv) {
  ...
  if (!test("pkcs8", kDER, sizeof(kDER))) {
    return 1;
  }
