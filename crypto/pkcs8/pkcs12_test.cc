...
static bool Test(const char *name, const uint8_t *der, size_t der_len) {
  ...
  std::unique_ptr<EVP_PKEY> delete_key(key);
...
static bool TestCompat(const uint8_t *der, size_t der_len) {
  std::unique_ptr<BIO> bio(BIO_new_mem_buf(der, der_len));
  if (!bio) {
    return false;
  }

  std::unique_ptr<PKCS12> p12(d2i_PKCS12_bio(bio.get(), nullptr));
  if (!p12) {
  ...
  std::unique_ptr<EVP_PKEY> delete_key(key);
  std::unique_ptr<X509> delete_cert(cert);
