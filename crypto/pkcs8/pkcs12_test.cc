...
TEST(PKCS12Test, CreateWithAlias) {
  ...
  int res = X509_alias_set1(cert1.get(), reinterpret_cast<const unsigned char *>(alias.data()), alias.size());
  ASSERT_EQ(res, 1);
  ...
  STACK_OF(X509) *ca_certs = nullptr;
  ASSERT_TRUE(PKCS12_parse(p12.get(), kPassword, &parsed_key, &parsed_cert, &ca_certs));
  ...
  int alias_len = 0;
  unsigned char *parsed_alias = X509_alias_get0(sk_X509_value(ca_certs, 0), &alias_len);
  ASSERT_TRUE(parsed_alias);
  ASSERT_EQ(alias, std::string(reinterpret_cast<const char *>(parsed_alias), static_cast<size_t>(alias_len)));
}
