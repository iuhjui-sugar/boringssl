...
// stringTree is a map from uint32 -> string which can output data for a binary
// tree as C literals.
type stringTree struct {
	...
	// top 17 bits.
	entries         []uint32
	// internedStrings conatins the same strings as are in |stringData|,
...
// writeTree is a recursive function that takes a sorted slice of values and
// appends a binary tree to |*out|. See the comment in err.c:err_string_lookup
...
func (st *stringTree) buildTree() []uint32 {
	...
	out := make([]uint32, 0, len(st.entries)*2)
