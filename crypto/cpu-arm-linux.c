...
static int extract_cpuinfo_field(STRING_PIECE *out, const STRING_PIECE *in,
  ...
  STRING_PIECE in_copy = *in, line;
  while (STRING_PIECE_split(&line, &in_copy, &in_copy, '\n')) {
...
static int has_list_item(const STRING_PIECE *list, const char *item) {
  STRING_PIECE list_copy = *list, feature;
  while (STRING_PIECE_split(&list_copy, &feature, &list_copy, ' ')) {
...
static unsigned long get_hwcap_cpuinfo(const STRING_PIECE *cpuinfo) {
  STRING_PIECE arch;
  if (extract_cpuinfo_field(&arch, cpuinfo, "CPU architecture") &&
      STRING_PIECE_equals(&arch, "8")) {
     ...
     * "Features" line does not work. (We assume all Android releases running
     * on future ARM hardware will have a functional |getauxval| and not need
     * this function.) */
    return HWCAP_NEON;
  }
...
static int has_broken_neon(const STRING_PIECE *cpuinfo) {
  STRING_PIECE field;
  return extract_cpuinfo_field(&field, cpuinfo, "CPU implementer") &&
         STRING_PIECE_equals(&field, "0x51") &&
         extract_cpuinfo_field(&field, cpuinfo, "CPU architecture") &&
         STRING_PIECE_equals(&field, "7") &&
         extract_cpuinfo_field(&field, cpuinfo, "CPU variant") &&
         STRING_PIECE_equals(&field, "0x1") &&
         extract_cpuinfo_field(&field, cpuinfo, "CPU part") &&
         STRING_PIECE_equals(&field, "0x04d") &&
         extract_cpuinfo_field(&field, cpuinfo, "CPU revision") &&
         STRING_PIECE_equals(&field, "0");
}
