...
// cavp_keywrap_test processes a NIST CAVP AES test vector request file and emits
...
static bool AESKeyWrap(std::vector<uint8_t> *out, bool encrypt,
                       std::vector<uint8_t> &key, std::vector<uint8_t> &in) {
...
int main(int argc, char **argv) {
  ...
  const size_t key_bits = strtoul(argv[2], nullptr, 0);
  if (key_bits != 128 && key_bits != 256) {
    return usage(argv[0]);
  }

  TestCtx ctx = {encrypt, key_bits, nullptr};
