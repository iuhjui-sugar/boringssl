...
static bool TestRSA2SigGen(FileTest *t, void *arg) {
  ...
  bssl::UniquePtr<EVP_MD_CTX> evp_ctx(EVP_MD_CTX_create());
  bssl::UniquePtr<EVP_PKEY> pkey(EVP_PKEY_new());
  if (evp_ctx == NULL ||
      pkey == NULL ||
      !EVP_PKEY_set1_RSA(pkey.get(), ctx->key.get()) ||
      !EVP_DigestSignInit(evp_ctx.get(), NULL,
                          EVP_get_digestbyname(hash.c_str()), NULL,
                          pkey.get()) ||
      !EVP_DigestSignUpdate(evp_ctx.get(), msg.data(), msg.size())) {
    return false;
  }

  std::vector<uint8_t> sig(RSA_size(ctx->key.get()));
  size_t sig_len;
  if (!EVP_DigestSignFinal(evp_ctx.get(), sig.data(), &sig_len)) {
    return false;
  }
