...
static bool TestRSA2KeyGen(FileTest *t, void *arg) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (n == NULL ||
        !BN_bn2bin_padded(n_bytes.data(), n_bytes.size(), n) ||
        e == NULL ||
        !BN_bn2bin_padded(e_bytes.data(), e_bytes.size(), e) ||
        d == NULL ||
        !BN_bn2bin_padded(d_bytes.data(), d_bytes.size(), d) ||
        p == NULL ||
        !BN_bn2bin_padded(p_bytes.data(), p_bytes.size(), p) ||
        q == NULL ||
        !BN_bn2bin_padded(q_bytes.data(), q_bytes.size(), q)) {
      return false;
    }
