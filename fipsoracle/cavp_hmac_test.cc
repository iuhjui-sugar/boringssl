...
static bool TestHMAC(FileTest *t, void *arg) {
  std::string md_len_str, sha_alg;
  if (!t->GetInstruction(&md_len_str, "L") ||
      !t->GetInstruction(&sha_alg, "SHAAlg")) {
    return false;
  }
  ...
  if (HMAC(md, key.data(), k_len, msg.data(), msg.size(), out.data(), &out_len) == NULL) {
    return false;
  }
  ...
  printf("Mac = %s\r\n\r\n", EncodeHex(out.data(), t_len).c_str());
