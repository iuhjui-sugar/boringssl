...
function(perlasm var arch dest src)
  ...
  elseif(arch STREQUAL "x86")
    ...
    append_to_parent_scope("${var}_ASM" "${dest}-apple.S" "${dest}-linux.S")
    if(BORINGSSL_ASM_DIALECT MATCHES "MASM")
      add_perlasm_target("${dest}-win.asm" ${src} win32 -DOPENSSL_IA32_SSE2)
      append_to_parent_scope("${var}_MASM" "${dest}-win.asm")
    else()
  ...
  elseif(arch STREQUAL "x86_64")
    ...
    append_to_parent_scope("${var}_ASM" "${dest}-apple.S" "${dest}-linux.S")
    if(BORINGSSL_ASM_DIALECT MATCHES "MASM")
      add_perlasm_target("${dest}-win.asm" ${src} win32)
      append_to_parent_scope("${var}_MASM" "${dest}-win.asm")
    else()
      add_perlasm_target("${dest}-win.asm" ${src} win32n)
      append_to_parent_scope("${var}_NASM" "${dest}-win.asm")
    endif()
  else()
