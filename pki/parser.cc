...
bool Parser::ReadRawTLV(Input* out) {
  ...
  if (!CBS_get_any_asn1_element(&cbs_, &tmp_out, nullptr, nullptr)) {
    std::cerr << "readrawtlv get_any_asn1 failed" << std::endl;
    return false;
  }
  *out = Input(CBS_data(&tmp_out), CBS_len(&tmp_out));
  std::cerr << "readrawtlv succeeds" << std::endl;
  return true;
}
...
bool Parser::ReadTag(Tag tag, Input* out) {
  ...
  Input value;
  if (!PeekTagAndValue(&actual_tag, &value)) {
    std::cerr << "PeekTagAndValue barfed" << std::endl;
    return false;
  }
  if (actual_tag != tag) {
    std::cerr << "actual tag of "<< actual_tag << "is not " << tag  << std::endl;
    return false;
  }
...
bool Parser::ReadConstructed(Tag tag, Parser* out) {
  if (!IsConstructed(tag)) {
    std::cerr << "IsConstruted failed" << std::endl;;
    return false;
  }
  Input data;
  if (!ReadTag(tag, &data)) {
    std::cerr << "ReadTag failed" << std::endl;;
    return false;
  }
  *out = Parser(data);
  return true;
