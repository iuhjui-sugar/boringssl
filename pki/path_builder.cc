...
void CertIssuersIter::AddIssuers(ParsedCertificateList new_issuers) {
  for (std::shared_ptr<const ParsedCertificate> &issuer : new_issuers) {
    if (present_issuers_.find(BytesAsString(issuer->der_cert())) !=
        present_issuers_.end()) {
      continue;
    }
    present_issuers_.insert(BytesAsString(issuer->der_cert()));
...
class CertIssuerIterPath {
 ...
 private:
  ...
  static Key GetKey(const ParsedCertificate *cert) {
    ...
    return Key(BytesAsString(cert->normalized_subject()),
               BytesAsString(cert->subject_alt_names_extension().value),
               BytesAsString(cert->tbs().spki_tlv));
  }
