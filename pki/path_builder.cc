...
VerifyError CertPathBuilderResultPath::GetVerifyError() const{
  ...
  // Similarly, for the deadline and limit cases, there will often
  // be other errors that we probably do not care about, since
  // path building aborted. Surface these errors instead of having
  // them hidden in the multiple error case.
  //
  // This is still fallible if we are calling this on the "best"
  // path, since these errors could have happened on *another*
  // path, so currently these conditions need to be checked
  // for in the path builder result.
  //
  // In theory we could consider moving this functionality
  // to the path builder result, but then this becomes a bit
  // confusing since we are only returning the error from
  // potentially the best path, so for the moment it stays
  // here and callers must also check for these conditions
  // occuring on any other path than the best one (unless
  // they do not care about special casing these)
