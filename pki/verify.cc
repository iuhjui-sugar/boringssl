...
#include <algorithm>  // for move, binary_search
#include <chrono>
...
#include <variant>  // I'm going to hell for this.
...
Error::Error(Code code, size_t offset, std::string_view diagnostic)
    : offset(offset), code(code), diagnostic(diagnostic) {}
...
std::string Error::DiagnosticString() const { return diagnostic; }

std::string Error::AsString() const {
  std::string location =
      offset ? "Certificate " + std::to_string(offset) + " of the best path"
             : "Leaf certificate";
  switch (code) {
    ...
    case Error::Code::ITERATION_COUNT_EXCEEDED:
      return location + " took more than the permitted number of iterations";
    case Error::Code::DEADLINE_EXCEEDED:
      return location + " took more than the permitted amount of time";
    case Error::Code::DEPTH_LIMIT_REACHED:
      return location + " no path was found within the depth limit";
...
// PathBuilderDelegateImpl implements revocation checking.  This is done by
// checking each certificate in each path for revocation, and inserting an error
// if a revocation is found.  Path selection can thereby avoid paths with
// revoked certs in favor of paths that do not have revoked certs, if that
// choice is available.
class PathBuilderDelegateImpl : public SimplePathBuilderDelegate {
 public:
  PathBuilderDelegateImpl(
      ...
      SignatureVerifyCache *cache, bool check_revocation)
...
std::optional<std::vector<std::vector<std::string>>> VerifyInternal(
  ...
  std::optional<std::shared_ptr<const ParsedCertificate>> maybe_leaf =
      bssl::InternalParseCertificate(opts.leaf_cert, &out_error->diagnostic);

  if (!maybe_leaf.has_value()) {
    out_error->code = Error::Code::CERTIFICATE_REJECTED;
    out_error->offset = 0;
    return {};
  }
  ...
  PathBuilderDelegateImpl path_builder_delegate(
      ...
      opts.signature_verify_cache, opts.check_revocation);

  bssl::KeyPurpose key_purpose;
  ...
  for (const auto cert : opts.intermediates) {
