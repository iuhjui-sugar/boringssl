...
std::optional<std::vector<std::vector<std::string>>> VerifyInternal(
  ...
  if (!bssl::der::EncodePosixTimeAsGeneralizedTime(now, &verification_time)) {
    *out_error = {VerifyError::StatusCode::UNKNOWN_ERROR, 0,
  ...
  if (result.HasValidPath()) {
    ...
    if (!all_paths) {
      ...
      *out_error = best_path->GetVerifyError();
    ...
    for (const auto &path : result.paths) {
      ...
      // Currently we do not return a VerifyError for multiple valid paths,
      // In theory, we could change this API to return one for each
      // path (although this only will show PATH_VALID potentially
      // with some warnings in the diagnostic.
      //
      // Were we to add something to return all paths (valid or invalid)
      // we could return a VerifyError for each one.
  ...
  if (result.exceeded_iteration_limit) {
    *out_error = {VerifyError::StatusCode::PATH_ITERATION_COUNT_EXCEEDED, 0,
                  "Iteration count exceeded, could not find a trusted path."};
    return {};
  }
  if (result.exceeded_deadline) {
    *out_error = {VerifyError::StatusCode::PATH_DEADLINE_EXCEEDED, 0,
                  "Deadline exceeded, could not find a trusted path."};
    return {};
  }
  if (result.AnyPathContainsError(cert_errors::kDepthLimitExceeded)) {
    *out_error = {VerifyError::StatusCode::PATH_DEPTH_LIMIT_REACHED, 0,
