...
TEST(CertIssuerSourceStaticTest, AddCertsGetCertsAndClear) {
  ...
  std::shared_ptr<const ParsedCertificate> cert1 = ReadCertFromFile(test_dir + "root.pem");
  ASSERT_TRUE(cert1);
  std::shared_ptr<const ParsedCertificate> cert2 = ReadCertFromFile(test_dir + "i1_1.pem");
  ASSERT_TRUE(cert2);
  std::shared_ptr<const ParsedCertificate> cert3 = ReadCertFromFile(test_dir + "i1_2.pem");
  ASSERT_TRUE(cert3);
  ...
  EXPECT_THAT(source.Certs(), testing::UnorderedElementsAre(cert1, cert2, cert3));
