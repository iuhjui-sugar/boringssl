...
void VerifyTimeValidity(const ParsedCertificate &cert,
  ...
  der::GeneralizedTime bounded_time = time;
  int64_t posix_validation_time =
      INT64_MIN;  // Guaranteed before any validation time.
  if (bound_verification_time &&
      GeneralizedTimeToPosixTime(time, &posix_validation_time)) {
    int64_t bounded_posix_time =
        bound_verification_time->BoundTime(posix_validation_time);
    if (bounded_posix_time != posix_validation_time) {
      if (!EncodePosixTimeAsGeneralizedTime(bounded_posix_time,
                                            &bounded_time)) {
        // This should be impossible since GeneralizedTime can't be out of
        // range.
        bounded_time = time;
      } else {
        posix_validation_time = bounded_posix_time;
      }
    }
  }
  if (bounded_time < cert.tbs().validity_not_before) {
    errors->AddError(cert_errors::kValidityFailedNotBefore);
  }

  if (cert.tbs().validity_not_after < bounded_time) {
