...
void VerifyTimeValidity(const ParsedCertificate &cert,
  ...
  if (bound_verification_time) {
    std::optional<int64_t> posix_verification_time;
    posix_verification_time = bound_verification_time->TrustedTime();
    if (!posix_verification_time.has_value()) {
    ...
    // If we have a replacement time value, encode it.
    if (posix_verification_time.has_value() &&
        !EncodePosixTimeAsGeneralizedTime(posix_verification_time.value(),
                                          &verification_time)) {
