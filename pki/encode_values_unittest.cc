...
TEST(EncodeValuesTest, EncodeTimeAsGeneralizedTime) {
  ...
  absl::Time time = absl::UnixEpoch() + absl::Seconds(1466787894);
...
// although absl::Time can represent times from before the Windows epoch and
// after the 32-bit time_t maximum, the conversion between absl::Time and
...
TEST(EncodeValuesTest, EncodeTimeFromBeforeWindowsEpoch) {
  ...
  const absl::Time kStartOfYear1570 =
      absl::UnixEpoch() - absl::Seconds(12622780800);
  GeneralizedTime generalized_time;
...
TEST(EncodeValuesTest, EncodeTimeAfterTimeTMax) {
  ...
  absl::Time time;
  if (!fillins::FromUTCExploded(exploded, &time))
    return;
...
TEST(EncodeValuesTest, GeneralizedTimeToTime) {
  ...
  EXPECT_EQ(absl::UnixEpoch() + absl::Seconds(1466787894), time);
}
...
TEST(EncodeValuesTest, GeneralizedTimeToTimeBeforeWindowsEpoch) {
  ...
  bool platform_can_represent_time =
      fillins::FromUTCExploded(exploded, &expected_time);
  ...
  if (platform_can_represent_time)
    ...
    EXPECT_EQ(absl::InfinitePast(), time);
...
TEST(EncodeValuesTest, GeneralizedTimeToTimeAfter32BitPosixMaxYear) {
  ...
  if (platform_can_represent_time)
    ...
    EXPECT_EQ(absl::InfiniteFuture(), time);
