...
bool EncodeTimeAsGeneralizedTime(const absl::Time& time,
  ...
  fillins::UTCExplode(time, &exploded);
...
bool GeneralizedTimeToTime(const der::GeneralizedTime& generalized,
  ...
  if (fillins::FromUTCExploded(exploded, result))
    return true;
  ...
  if (static_cast<int>(generalized.year) > absl::Time::kExplodedMaxYear) {
    *result = absl::InfiniteFuture();
    return true;
  }
  if (static_cast<int>(generalized.year) < absl::Time::kExplodedMinYear) {
    *result = absl::InfinitePast();
    return true;
  }
  return false;
}
