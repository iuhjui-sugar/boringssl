...
// read.
[[nodiscard]] bool ParseBool(const Input& in, bool* out);
...
[[nodiscard]] bool ParseBoolRelaxed(const Input& in, bool* out);
...
[[nodiscard]] bool IsValidInteger(const Input& in, bool* negative);
...
// integer.
[[nodiscard]] bool ParseUint64(const Input& in, uint64_t* out);

// Same as ParseUint64() but for a uint8_t.
[[nodiscard]] bool ParseUint8(const Input& in, uint8_t* out);
...
[[nodiscard]] std::optional<BitString> ParseBitString(
    const Input& in);
...
bool operator<(const GeneralizedTime& lhs,
...
[[nodiscard]] bool ParseUTCTime(const Input& in,
                                           GeneralizedTime* out);
...
[[nodiscard]] bool ParsePrintableString(Input in, std::string* out);
...
[[nodiscard]] bool ParseTeletexStringAsLatin1(Input in,
                                                         std::string* out);
...
[[nodiscard]] bool ParseUniversalString(Input in, std::string* out);

// Reads a DER-encoded ASN.1 BMPString value from |in| and stores the
// result in |out| as UTF-8, returning true if successful.
[[nodiscard]] bool ParseBmpString(Input in, std::string* out);
