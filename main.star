...
def ci_builder(name, host, *, recipe="boringssl", category=None,
               short_name=None, include_cq=True, include_ci=True):
...
#
# TODO(davidben): As there is more capacity, switch the VS2015 builders to
# Windows 10 when unifying the CI and CQ configuration. We can then add
# some post-commit Windows 7 coverage if we need it.
