...
static bool WriteReply(int fd, size_t num_spans, ...) {
  if (num_spans == 0 || num_spans > kMaxArgs) {
    abort();
  }
  ...
  for (size_t i = 0; i < num_spans; i++) {
    Span<const uint8_t> span = va_arg(args, Span<const uint8_t>);
    nums[i + 1] = span.size();
  ...
  iovs[0].iov_len = sizeof(uint32_t) * (1 + num_spans);

  const size_t num_iov = num_spans + 1;
  size_t iov_done = 0;
...
static bool Hash(const Span<const uint8_t> args[]) {
  ...
  return WriteReply(STDOUT_FILENO, 1, Span<const uint8_t>(digest));
}
