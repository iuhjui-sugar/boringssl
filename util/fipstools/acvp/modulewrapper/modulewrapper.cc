...
static bool AESKeyWrapSeal(const Span<const uint8_t> args[]) {
  ...
  AES_KEY aes;
  if (!AESKeyWrapSetup(&aes, false, key, plaintext) ||
...
static bool AESKeyWrapOpen(const Span<const uint8_t> args[]) {
  ...
  AES_KEY aes;
  if (!AESKeyWrapSetup(&aes, true, key, ciphertext) || ciphertext.size() < 8 ||
...
static bool AESPaddedKeyWrapSeal(const Span<const uint8_t> args[]) {
  ...
  AES_KEY aes;
  if (!AESPaddedKeyWrapSetup(&aes, false, key) || plaintext.size() + 15 < 15) {
    return false;
  }
...
static bool AESPaddedKeyWrapOpen(const Span<const uint8_t> args[]) {
  ...
  AES_KEY aes;
  if (!AESPaddedKeyWrapSetup(&aes, true, key) || ciphertext.size() % 8) {
    return false;
  }
