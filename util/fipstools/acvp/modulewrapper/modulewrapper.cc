...
static bool HKDF(const Span<const uint8_t> args[], ReplyCallback write_reply) {
  ...
  OPENSSL_memcpy(&out_len, out_len_bytes.data(), sizeof(out_len));
  if (out_len > (1 << 24)) {
    return false;
  }
...
template <const EVP_MD *HashFunc()>
static bool HKDFExtract(const Span<const uint8_t> args[], ReplyCallback write_reply) {
  const EVP_MD *const md = HashFunc();
...
template <const EVP_MD *HashFunc()>
static bool HKDFExpandLabel(const Span<const uint8_t> args[], ReplyCallback write_reply) {
  const EVP_MD *const md = HashFunc();
