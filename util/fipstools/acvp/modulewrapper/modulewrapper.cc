...
static bool AESGCMCommon(const EVP_AEAD **out_aead, size_t *out_tag_len,
                         const Span<const uint8_t> &tag_len_span,
...
static bool AESGCMSeal(const Span<const uint8_t> args[]) {
  const Span<const uint8_t> &tag_len_span = args[0];
  const Span<const uint8_t> &key = args[1];
  const Span<const uint8_t> &plaintext = args[2];
  const Span<const uint8_t> &nonce = args[3];
  const Span<const uint8_t> &ad = args[4];

  const EVP_AEAD *aead;
  size_t tag_len;
  if (!AESGCMCommon(&aead, &tag_len, tag_len_span, key)) {
    return false;
  }
...
static bool AESGCMOpen(const Span<const uint8_t> args[]) {
  const Span<const uint8_t> &tag_len_span = args[0];
  const Span<const uint8_t> &key = args[1];
  const Span<const uint8_t> &ciphertext = args[2];
  const Span<const uint8_t> &nonce = args[3];
  const Span<const uint8_t> &ad = args[4];
