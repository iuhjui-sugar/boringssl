...
size_t Args::TotalArgLength() const {
  size_t total = 0;
...
std::unique_ptr<Args> Args::ParseFromFd(int fd) {
  ...
  args->num_args_ = nums[0];
  if (args->num_args_ == 0) {
    LOGE("Invalid, zero-argument operation requested.\n");
    return nullptr;
  } else if (args->num_args_ > kMaxArgs) {
    LOGE("Operation requested with %zu args, but %zu is the limit.\n", args->num_args_,
         kMaxArgs);
    return nullptr;
  }

  if (args->num_args_ > 1 &&
      !ReadAll(fd, &nums[2], sizeof(uint32_t) * (args->num_args_ - 1))) {
    return nullptr;
  }

  size_t need = 0;
  for (size_t i = 0; i < args->num_args_; i++) {
    const size_t arg_length = nums[i + 1];
    if (i == 0 && arg_length > kMaxNameLength) {
      LOGE("Operation with name of length %zu exceeded limit of %zu.\n",
           arg_length, kMaxNameLength);
      return nullptr;
    } else if (arg_length > kMaxArgLength) {
      LOGE("Operation with argument of length %zu exceeded limit of %zu.\n",
  ...
  if (need > args->buf_.size()) {
    size_t alloced = need + (need >> 1);
    if (alloced < need) {
      abort();
    }
    args->buf_.resize(alloced);
  }

  if (!ReadAll(fd, args->buf_.data(), need)) {
    return nullptr;
  }

  size_t offset = 0;
  for (size_t i = 0; i < args->num_args_; i++) {
    args->args_[i] = Span<const uint8_t>(&args->buf_[offset], nums[i + 1]);
    offset += nums[i + 1];
  }
...
bool AcvpTool::WriteReply(std::vector<Span<const uint8_t>> spans) {
  ...
  while (iov_done < num_iov) {
    ssize_t r;
    do {
      r = writev(STDOUT_FILENO, &iovs[iov_done], num_iov - iov_done);
    } while (r == -1 && errno == EINTR);
...
static bool GetConfig(AcvpTool &tool, const Span<const uint8_t> args[]) {
...
static uint32_t GetIterations(const Span<const uint8_t> iterations_bytes) {
  ...
  if (iterations == 0 || iterations == UINT32_MAX) {
    LOGE("Invalid number of iterations: %x.\n",
...
static bool AES(AcvpTool &tool, const Span<const uint8_t> args[]) {
  AES_KEY key;
  ...
  return tool.WriteReply(
      {Span<const uint8_t>(result), Span<const uint8_t>(prev_result)});
}

template <int (*SetKey)(const uint8_t *key, unsigned bits, AES_KEY *out),
          int Direction>
static bool AES_CBC(AcvpTool &tool, const Span<const uint8_t> args[]) {
  AES_KEY key;
  if (SetKey(args[0].data(), args[0].size() * 8, &key) != 0) {
    return false;
  }
...
static bool AES_CTR(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  if (args[3].size() != sizeof(kOneIteration) ||
    ...
    LOGE("Only a single iteration supported with AES-CTR\n");
    return false;
  }
  ...
  if (GetIterations(args[3]) != 1) {
    LOGE("Multiple iterations of AES-CTR is not supported.\n");
    return false;
  }
  ...
  AES_ctr128_encrypt(args[1].data(), out.data(), args[1].size(), &key, iv,
                     ecount_buf, &num);
  return tool.WriteReply({Span<const uint8_t>(out)});
}
...
static bool AESGCMSetup(EVP_AEAD_CTX *ctx, Span<const uint8_t> tag_len_span,
  ...
  if (tag_len_span.size() != sizeof(tag_len_32)) {
    LOGE("Tag size value is %u bytes, not an uint32_t\n",
...
static bool AESCCMSetup(EVP_AEAD_CTX *ctx, Span<const uint8_t> tag_len_span,
  ...
  memcpy(&tag_len_32, tag_len_span.data(), sizeof(tag_len_32));
  if (tag_len_32 != 4) {
    LOGE("AES-CCM only supports 4-byte tags, but %u was requested\n",
         static_cast<unsigned>(tag_len_32));
    return false;
  }

  if (key.size() != 16) {
    LOGE("AES-CCM only supports 128-bit keys, but %u bits were given\n",
  ...
  if (!EVP_AEAD_CTX_init(ctx, EVP_aead_aes_128_ccm_bluetooth(), key.data(),
    ...
    LOGE("Failed to setup AES-CCM with tag length %u\n",
         static_cast<unsigned>(tag_len_32));
    return false;
  }
...
template <bool (*SetupFunc)(EVP_AEAD_CTX *ctx, Span<const uint8_t> tag_len_span,
                            Span<const uint8_t> key)>
static bool AEADSeal(AcvpTool &tool, const Span<const uint8_t> args[]) {
...
template <bool (*SetupFunc)(EVP_AEAD_CTX *ctx, Span<const uint8_t> tag_len_span,
                            Span<const uint8_t> key)>
static bool AEADOpen(AcvpTool &tool, const Span<const uint8_t> args[]) {
  Span<const uint8_t> tag_len_span = args[0];
  Span<const uint8_t> key = args[1];
...
static bool AESPaddedKeyWrapSetup(AES_KEY *out, bool decrypt,
  ...
  if ((decrypt ? AES_set_decrypt_key : AES_set_encrypt_key)(
    ...
    LOGE("Invalid AES key length for AES-KW(P): %u\n",
         static_cast<unsigned>(key.size()));
    return false;
  }
  return true;
...
static bool AESKeyWrapSetup(AES_KEY *out, bool decrypt, Span<const uint8_t> key,
  ...
  if (input.size() % 8) {
    LOGE("Invalid AES-KW input length: %u\n",
...
static bool AESKeyWrapSeal(AcvpTool &tool, const Span<const uint8_t> args[]) {
  Span<const uint8_t> key = args[1];
  Span<const uint8_t> plaintext = args[2];
...
static bool AESKeyWrapOpen(AcvpTool &tool, const Span<const uint8_t> args[]) {
  Span<const uint8_t> key = args[1];
  Span<const uint8_t> ciphertext = args[2];
...
static bool AESPaddedKeyWrapSeal(AcvpTool &tool, const Span<const uint8_t> args[]) {
  Span<const uint8_t> key = args[1];
  Span<const uint8_t> plaintext = args[2];
...
static bool AESPaddedKeyWrapOpen(AcvpTool &tool, const Span<const uint8_t> args[]) {
  Span<const uint8_t> key = args[1];
  Span<const uint8_t> ciphertext = args[2];
...
static bool TDES(AcvpTool &tool, const Span<const uint8_t> args[]) {
  const EVP_CIPHER *cipher = EVP_des_ede3();

  if (args[0].size() != 24) {
    LOGE("Bad key length %u for 3DES.\n",
  ...
  if (args[1].size() % 8) {
    LOGE("Bad input length %u for 3DES.\n",
  ...
  return tool.WriteReply({Span<const uint8_t>(result),
                     Span<const uint8_t>(prev_result),
                     Span<const uint8_t>(prev_prev_result)});
}

template <bool Encrypt>
static bool TDES_CBC(AcvpTool &tool, const Span<const uint8_t> args[]) {
...
static bool HMAC(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  return tool.WriteReply({Span<const uint8_t>(digest, digest_len)});
}

static bool DRBG(AcvpTool &tool, const Span<const uint8_t> args[]) {
...
static bool ECDSAKeyGen(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  return tool.WriteReply({Span<const uint8_t>(d_bytes),
...
static bool ECDSAKeyVer(AcvpTool &tool, const Span<const uint8_t> args[]) {
  bssl::UniquePtr<EC_KEY> key = ECKeyFromName(args[0]);
  if (!key) {
    return false;
  }
  ...
  return tool.WriteReply({Span<const uint8_t>(reply)});
}
...
static bool ECDSASigGen(AcvpTool &tool, const Span<const uint8_t> args[]) {
  bssl::UniquePtr<EC_KEY> key = ECKeyFromName(args[0]);
...
static bool ECDSASigVer(AcvpTool &tool, const Span<const uint8_t> args[]) {
  bssl::UniquePtr<EC_KEY> key = ECKeyFromName(args[0]);
...
static bool CMAC_AES(AcvpTool &tool, const Span<const uint8_t> args[]) {
  uint8_t mac[16];
  ...
  return tool.WriteReply({Span<const uint8_t>(mac, mac_len)});
}

static bool CMAC_AESVerify(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  return tool.WriteReply({Span<const uint8_t>(&ok, sizeof(ok))});
}
...
static bool RSAKeyGen(AcvpTool &tool, const Span<const uint8_t> args[]) {
  uint32_t bits;
  ...
  bssl::UniquePtr<RSA> key(RSA_new());
  if (!RSA_generate_key_fips(key.get(), bits, nullptr)) {
    LOGE("RSA_generate_key_fips failed for modulus length %u.\n", bits);
    return false;
  }
  ...
  if (!tool.WriteReply({BIGNUMBytes(e), BIGNUMBytes(p), BIGNUMBytes(q),
...
static bool RSASigGen(AcvpTool &tool, const Span<const uint8_t> args[]) {
  uint32_t bits;
  if (args[0].size() != sizeof(bits)) {
    return false;
  }
...
template <const EVP_MD *(MDFunc)(), bool UsePSS>
static bool RSASigVer(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  return tool.WriteReply({Span<const uint8_t>(&ok, 1)});
}

template <const EVP_MD *(MDFunc)()>
static bool TLSKDF(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  return tool.WriteReply({out});
}

template <int Nid>
static bool ECDH(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  if (!EC_POINT_set_affine_coordinates_GFp(
    ...
    LOGE("Invalid peer point for ECDH.\n");
    return false;
  }
  ...
  if (!private_key.empty()) {
    ...
    if (!EC_KEY_set_private_key(ec_key.get(), our_k.get())) {
      LOGE("EC_KEY_set_private_key failed.\n");
      return false;
    }
    ...
    if (!EC_POINT_mul(group, our_pub.get(), our_k.get(), nullptr, nullptr,
      ...
      LOGE("Calculating public key failed.\n");
      return false;
    }
  } else if (!EC_KEY_generate_key_fips(ec_key.get())) {
    LOGE("EC_KEY_generate_key_fips failed.\n");
    return false;
  }
  ...
  if (out_len < 0) {
    LOGE("ECDH_compute_key failed.\n");
    return false;
  } else if (static_cast<size_t>(out_len) == output.size()) {
    LOGE("ECDH_compute_key output may have been truncated.\n");
    return false;
  }
  ...
  return tool.WriteReply({BIGNUMBytes(x.get()), BIGNUMBytes(y.get()), output});
}

static bool FFDH(AcvpTool &tool, const Span<const uint8_t> args[]) {
  ...
  if (DH_compute_key_padded(z.data(), their_pub.get(), dh.get()) !=
    ...
    LOGE("DH_compute_key_hashed failed.\n");
    return false;
  }

  return tool.WriteReply({BIGNUMBytes(DH_get0_pub_key(dh.get())), z});
}
