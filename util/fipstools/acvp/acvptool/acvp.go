...
func encodeSessionTokenCacheKey(key string) string {
	...
	for i := range key {
		...
		if b == '/' {
			builder.WriteByte('_')
		} else if b == '_' || b == '%' || b < 48 || b > 125 {
			builder.WriteByte('%')
			builder.WriteByte(hexTable[b>>4])
			builder.WriteByte(hexTable[b&4])
		} else {
			builder.WriteByte(b)
		}
...
func decodeSessionTokenCacheKey(key string) string {
	var builder bytes.Buffer
	...
	for i := 0; i < len(key); i++ {
		...
		case '%':
			if i+2 < len(key) {
				b, err := strconv.ParseUint(key[i+1:i+3], 16, 8)
				if err == nil {
					builder.WriteByte(uint8(b))
					i += 2
					continue
				}
			}
...
func loadCachedSessionTokens(server *acvp.Server, cachePath string) error {
	...
	for _, name := range names {
		...
		server.PrefixTokens[decodeSessionTokenCacheKey(name[:len(name)-6])] = string(contents)
		loaded++
	}
...
func main() {
	...
	if token := result.AccessToken; len(token) > 0 {
		...
		if len(sessionTokensCacheDir) > 0 {
			ioutil.WriteFile(filepath.Join(sessionTokensCacheDir, encodeSessionTokenCacheKey(url))+".token", []byte(token), 0600)
		}
	}
	...
	for _, setURL := range result.VectorSetURLs {
		...
		for {
			...
			if resultSize >= server.SizeLimit {
				...
				req.Header.Add("Authorization", "Bearer "+largeResponse.AccessToken)
