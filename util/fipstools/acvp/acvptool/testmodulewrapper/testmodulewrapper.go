// testmodulewrapper is modulewrapper binary that works with acvptool and
...
func kdf(args [][]byte) error {
	if len(args) != 6 {
		return fmt.Errorf("KDF received %d args", len(args))
	}

	mode, prf, counterLocation, key, counterBits32, outputBits32 := args[0], args[1], args[2], args[3], args[4], args[5]
	counterBits := binary.LittleEndian.Uint32(counterBits32)
	outputBits := binary.LittleEndian.Uint32(outputBits32)

	if !bytes.Equal(mode, []byte("counter")) {
		return fmt.Errorf("KDF received unsupported mode %q", string(mode))
	}
	...
	if outputBits%8 != 0 {
		return fmt.Errorf("KDF received unsupported output bits %d", outputBits)
	}
	...
	if outputBits+255 < outputBits {
		return fmt.Errorf("KDF received excessive output length %d", outputBits)
	}

	n := (outputBits + 255) / 256
	...
	return reply(key, input[4:], result[:outputBits/8])
}
	...
	for {
		...
		for i := uint32(0); i < numArgs; i++ {
			...
			if i == 0 && argLength > maxNameLength {
				...
				return fmt.Errorf("Operation with name of length %d exceeded limit of %d", argLength, maxArgLength)
			}
