...
func New(path string) (*Subprocess, error) {
	...
	if _, err := os.Stat("/tmp/fifo_stdin"); err != nil {
		//Create a named pipe for sending data to middleware
		err = syscall.Mkfifo("/tmp/fifo_stdin", syscall.S_IRUSR|syscall.S_IWUSR|syscall.S_IRGRP)
		if err != nil {
			return nil, err
		}
		//Create a named pipe for receiving data from middleware
	...
	// Open write end of the pipe for sending data to middleware
	stdin, err := os.OpenFile("/tmp/fifo_stdin", os.O_WRONLY, syscall.S_IRUSR|syscall.S_IWUSR|syscall.S_IRGRP)
	if err != nil {
		return nil, err
	}
	// Open read end of the pipe for receiving data from middleware
...
func NewWithIO(cmd *exec.Cmd, in *os.File, out *os.File) *Subprocess {
	...
	m.primitives = map[string]primitive{
		...
		"ACVP-AES-XTS":      &xts{},
		"ACVP-AES-GCM":      &aead{"AES-GCM", false, false},
		"ACVP-AES-GMAC":     &aead{"AES-GCM", false, true},
		"ACVP-AES-CCM":      &aead{"AES-CCM", true, false},
		"ACVP-AES-KW":       &aead{"AES-KW", false, false},
		"ACVP-AES-KWP":      &aead{"AES-KWP", false, false},
