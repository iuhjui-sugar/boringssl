...
func (k *kdfPrimitive) Process(vectorSet []byte, m Transactable) (interface{}, error) {
	...
	for _, group := range parsed.Groups {
		...
		outputBits := uint32le(group.OutputBits)
		...
		for _, test := range group.Tests {
			...
			resp, err := m.Transact(k.algo, 3, []byte(group.KDFMode), []byte(group.MACMode), []byte(group.CounterLocation), key, counterBits, outputBits)
			if err != nil {
