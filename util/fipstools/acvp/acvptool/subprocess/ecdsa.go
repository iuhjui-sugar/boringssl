...
func (e *ecdsa) Process(vectorSet []byte) (interface{}, error) {
	...
	for _, group := range parsed.Groups {
		if _, ok := e.curves[group.Curve]; !ok {
			return nil, fmt.Errorf("curve in test group %d not supported: %q", group.ID, group.Curve)
		}
		...
		for _, test := range group.Tests {
			...
			case "keyVer":
				...
				//result should be one byte, 00 if false, 01 if true
				switch {
			...
			case "sigGen":
				p, ok := e.m.primitives[group.HashAlgo]
				h, ok := p.(*hashPrimitive)
				if !ok {
					return nil, fmt.Errorf("unsupported hash algorithm in test group %d: %q", group.ID, group.HashAlgo)
				}
				...
				result, err := e.m.transact(op, 2, []byte(group.Curve), []byte(group.HashAlgo), msg)
				if err != nil {
			...
			default:
				return nil, fmt.Errorf("invalid mode in ECDSA vector set: %q", parsed.Mode)
			}
