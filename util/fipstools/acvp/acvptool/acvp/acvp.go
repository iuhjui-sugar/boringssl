...
func (query Query) toURLParams() string {
	...
	for i, conj := range query {
		for _, cond := range conj {
			...
			ret += fmt.Sprintf("%s[%d]=%s:%s", cond.Param, i, cond.Relation.String(), url.QueryEscape(cond.Value))
		}
	}
...
func (server *Server) Get(out interface{}, endPoint string) error {
	...
	req, err := http.NewRequest("GET", server.prefix+endPoint, nil)
	if err != nil {
		return err
	}
	if len(token) != 0 {
...
func (server *Server) write(method string, reply interface{}, endPoint string, contents []byte) error {
	...
	req, err := http.NewRequest("POST", server.prefix+endPoint, &buf)
	if err != nil {
		return err
	}
	if len(token) != 0 {
...
func (server *Server) Delete(endPoint string) error {
	...
	req, err := http.NewRequest("DELETE", server.prefix+endPoint, nil)
	if err != nil {
		return err
	}
	if len(token) != 0 {
