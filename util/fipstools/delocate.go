...
func (d *delocation) processDirective(statement, directive *node32) (*node32, error) {
	...
	case "data":
		// Some versions of MSAN are adding a .data section, and adding
		// references to symbols within it to the code. We will have to
...
[PABI]: “64-bit ELF v2 ABI Specification. Power Architechture.” March 21st, 2017
...
Since it jumps directly to memcpy@PLT, it can't restore r2 on return. Thus calls
must be followed by a nop. If the call ends up going out-of-module, the linker
will rewrite that nop to load r2 from the stack.
