...
func (d *delocation) processAarch64Instruction(statement, instruction *node32) (*node32, error) {
	...
	case "adrp":
		...
		// time. Thus all adrp are turned into loading the PC-relative addresses. This
		// means that the module is limited to 1MiB offsets.
			...
			case ruleARMBaseIndexScale:
				...
				if parts != nil {
					if parts.pegRule == ruleARMGOTLow12 {
						...
						// Suppress instruction. The GOT helper already dereferenced the
						// entry.
						d.writeCommentedNode(statement)
						return statement, nil
