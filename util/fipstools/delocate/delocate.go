...
Args:
	for i, arg := range argNodes {
		...
		case ruleGOTLocation:
			...
			if len(argNodes) != 2 {
				return nil, fmt.Errorf("movabs of _GLOBAL_OFFSET_TABLE_ didn't have exactly two arguments")
			}
		...
		case ruleGOTSymbolOffset:
			...
			if len(argNodes) != 2 {
				return nil, fmt.Errorf("movabs of _GLOBAL_OFFSET_TABLE_ offset didn't have exactly two arguments")
			}
