...
func preprocess(cppCommand []string, source string) (string, error) {
	cpp := exec.Command(cppCommand[0], cppCommand[1:]...)
	cpp.Stdin = bytes.NewBufferString(source)
...
func parseInputs(inputs []inputFile, cppCommand []string) error {
	for i, input := range inputs {
		...
		if len(cppCommand) > 0 {
			var err error
			if contents, err = preprocess(cppCommand, contents); err != nil {
				return err
			}
		}
...
func main() {
	...
	if len(*ccPath) > 0 {
		...
		// -E requests only preprocessing and "-" means to read from stdin.
