...
def ExtractVariablesFromCMakeFile(cmakefile):
  ...
  with open(cmakefile) as f:
    for line in f:
      line = line.strip()
      if line == '# BEGIN EXPORTED VARIABLES':
        in_export_block = True
        continue
      if not in_export_block:
        continue

      if line == '# END EXPORTED VARIABLES':
        if in_set_command:
  ...
  if in_export_block:
    raise ValueError('Unfinished export block')
  raise ValueError('No export block found')
...
def main(platforms):
  cmake = ExtractVariablesFromCMakeFile(os.path.join('src', 'CMakeLists.txt'))
