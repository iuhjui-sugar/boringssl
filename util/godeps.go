...
func main() {
	...
	var files []string
	var recurse func(pkgName, src string) error
	recurse = func(pkgName, src string) error {
		pkg, err := build.Default.Import(pkgName, src, 0)
		if err != nil {
			return err
		}
		...
		// Recurse into imports. Note the source directory must be
		// updated for relative imports to work correctly.
		for _, importName := range pkg.Imports {
			if err := recurse(importName, pkg.Dir); err != nil {
				return err
			}
		}
		return nil
