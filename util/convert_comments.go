...
//       each line.
//
// The conversion proceeds by:
//
//   1. Replace '/*' in the initial line with '//'.
//
//   2. Replace ' *' in subsequent lines with '//'.
//
//   3. Remove the '*/'.
//
//   4. If a single-line comment, add spaces as needed before '/*' to ensure
//      there are at least two spaces before the comment.
//
//   5. Remove trailing whitespace from any transformed lines.
//
...
func switchComments(in []byte) []byte {
...
func main() {
	for _, arg := range os.Args[1:] {
		...
		if err := ioutil.WriteFile(arg, switchComments(in), 0666); err != nil {
			panic(err)
		}
	}
}
