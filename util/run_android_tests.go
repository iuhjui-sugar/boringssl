...
func main() {
	...
	if *allTestsOutput != "" {
		if err := adb("pull", "-p", "/data/local/tmp/boringssl-tmp/all_tests.json", *allTestsOutput); err != nil {
	...
	if err := adb("shell", fmt.Sprintf("cd /data/local/tmp/boringssl-tmp/ssl/test/runner && ./runner -json-output ../../../runner.json %s", *runnerArgs)); err != nil {
		fmt.Printf("Failed to run SSL tests: %s\n", err)
		os.Exit(1)
	}
	if *runnerOutput != "" {
		if err := adb("pull", "-p", "/data/local/tmp/boringssl-tmp/runner.json", *runnerOutput); err != nil {
