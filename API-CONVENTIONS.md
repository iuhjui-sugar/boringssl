...
transferring. Callers should also take special note of functions documented to
only conditionally take ownership on success.
...
API documentation describe more complex obligations. For instance, an object may
borrow a pointer for longer than the duration of a single function call, in
which case the caller must ensure the lifetime extends accordingly.
