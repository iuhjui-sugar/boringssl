...
## Table of `*_ctrl` functions.

When porting code which uses `SSL_CTX_ctrl` or `SSL_ctrl`, use the replacement
functions below. For brevity, if a function has both `SSL_CTX` and `SSL`
variants, only the `SSL_CTX` version is listed. Note some values correspond to
multiple functions depending on the `larg` value. Refer to the documention or
the OpenSSL macro definitions to determine which is equivalent.
