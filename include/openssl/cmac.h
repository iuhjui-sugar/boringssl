...
/* CMAC calculates the 16-byte, CMAC authenticator of |in_len| bytes of |in|
 * and writes it to |out|. The |key_len| may be 16 or 32 bytes to select
 * between AES-128 and AES-256. If |key_len| is any other value, |CMAC| will
 * call abort(). */
OPENSSL_EXPORT void CMAC(uint8_t out[16], const uint8_t *key, size_t key_len,
