...
/* NEWHOPE_SERVERMSG_init initializes |out_msg| and |out_sk| for a new key
 * exchange. |msg| must have room for |NEWHOPE_SERVERMSG_LENGTH|. Neither output
 * may be cached. */
OPENSSL_EXPORT void NEWHOPE_keygen(uint8_t* out_msg, NEWHOPE_POLY* out_sk);
...
OPENSSL_EXPORT void NEWHOPE_server_compute_key(uint8_t* out_key,
 ...
 * |NEWHOPE_CLIENTMSG_LENGTH| bytes. */
OPENSSL_EXPORT void NEWHOPE_client_compute_key(uint8_t* out_key,
                                               uint8_t* out_msg,
                                               const uint8_t* msg);
