...
class OPENSSL_EXPORT VerifyError {
 public:
  ...
  enum class StatusCode {
    ...
    // CERTIFICATE_INVALID means that the certificate was formally invalid, and
    // was not considered.
    CERTIFICATE_INVALID,
    ...
    // CERTIFICATE_SELF_SIGNED is a special case of PATH_NOT_FOUND
    // It should not be special, but sadly some software cares and
  ...
  VerifyError(StatusCode code, ssize_t offset, std::string_view diagnostic);
 ...
 private:
  ssize_t offset_ = -1;
  StatusCode code_ = StatusCode::UNKNOWN_ERROR;
