...
class OPENSSL_EXPORT VerifyError {
 public:
  ...
  enum class StatusCode {
    ...
    // CERTIFICATE_INVALID means that the certificate was rejected as invalid.
    // There are many possible reasons for a certificate to be deemed invalid.
    CERTIFICATE_INVALID,
    ...
    // CERTIFICATE_UNSUPPOERTED_KEY means that the certificate's key type and/or
    // size is not supported.
    CERTIFICATE_UNSUPPORTED_KEY,

    // CERTIFICATE_UNSUPPOERTED_SIGNATURE ALGORITHM means that the signature
    ...
    // errors present on the certificate chain.
