...
// KYBER_public_key_from_bytes is a wrapper function around
// |KYBER_parse_public_key| which takes a buffer explicitly. It returns one on
// success or zero on error or if there is trailing data in |in|.
OPENSSL_EXPORT int KYBER_public_key_from_bytes(
    struct KYBER_public_key *public_key, const uint8_t *in, size_t in_len);
...
// KYBER_private_key_from_bytes is a wrapper function around
// |KYBER_parse_private_key| which takes a buffer explicitly. It returns one on
// success or zero on error or if there is trailing data in |in|.
OPENSSL_EXPORT int KYBER_private_key_from_bytes(
    struct KYBER_private_key *out_private_key, const uint8_t *in,
    size_t in_len);
