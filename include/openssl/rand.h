...
// RAND_bytes writes |len| bytes of random data to |buf| and returns
// one. In the event that sufficient random data can not be obtained,
// abort() is called.
OPENSSL_EXPORT int RAND_bytes(uint8_t *buf, size_t len);

// RAND_get_system_entropy_for_custom_prng writes |len| bytes of
// random data from a system entropy source to |buf|. The maximum
// length of entropy which may be requested is 256 bytes. If more than
// 256 bytes of data is requested, or if sufficient random data can
// not be obtained, abort() is called. RAND_bytes should normally be
// used instead of this function. This function should only be used
// for seed values or where malloc() can not be called. This function
