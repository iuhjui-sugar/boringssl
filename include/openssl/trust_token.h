...
// and extracts the SRR, verifying its integrity and storing it into a
// newly-allocated buffer, setting |*out_srr| to that buffer and |*out_srr_len|
// to its length. The signature of the SRR is stored into a newly-allocated
// buffer, setting |*out_sig| to that buffer and |*out_sig_len| to its length.
...
//
// These functions implement the issuer half of the Trust Token protocol. An
// instance of a |TRUST_TOKEN_ISSUER| can be reused across multiple protocol
// operations. It may be used concurrently on multiple threads by non-mutating
// functions, provided no other thread is concurrently calling a mutating
// function. Functions which take a |const| pointer are non-mutating and
...
// token is stored into a newly-allocated buffer and stored in |*out_token|. The
// extracted client data is stored into a newly-allocated buffer and stored in
// |*out_client_data|. The extracted redemption time is stored in
// |*out_redemption_time|. The caller takes ownership of buffer and must call
// |OPENSSL_free| when done. The caller should use |*out_token| to determine
// whether this token has been reused and whether to send a response to the
// requesting client. It returns one on success or zero on error.
...
// |*out_value| is set to the decrypted value, a value of zero or one indicating
// the bit is valid, while any other value indicates it was invalid. It returns
...
#define TRUST_TOKEN_R_INVALID_PROOF 112
