...
// TRUST_TOKEN_new creates a new Trust Token from |data|.
...
//
// These functions implements the client half of the Trust Token protocol. An
...
// operations. A given object may be used concurrently on multiple threads by
// non-mutating functions, provided no other thread is concurrently calling a
// mutating function. Unless otherwise documented, functions which take a
// |const| pointer are non-mutating and functions which take a non-|const|
// pointer are mutating.
...
// verifies the token. If the token is valid, a SRR is produced, signing over
// the requested data from the request and the value of the token, storing the
// result into a newly-allocated buffer and setting |*out| to that buffer and
// |*out_len| to its length. The extracted token is stored into a
// newly-allocated buffer and stored in |*out_token|. The extracted client data
// is stored into a newly-allocated buffer and stored in |*out_client_data|. The
