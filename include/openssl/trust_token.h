...
// of the list and must call |sk_TRUST_TOKEN_pop_free| when done. The list is
// empty if issuance fails.
...
// stored into a newly-allocated buffer and stored in |*out_token| with
// |*out_token_len| set to its length. The caller takes ownership of the buffer
...
OPENSSL_EXPORT int TRUST_TOKEN_ISSUER_redeem(TRUST_TOKEN_ISSUER *ctx,
                                             ...
                                             const uint8_t *request,
                                             size_t request_len, uint64_t time);
