...
// extracts the tokens, returning a list of tokens and the keyid of the key used to
...
OPENSSL_EXPORT uint8_t TRUST_TOKEN_decode_private_metadata(
    const uint8_t *key, size_t key_len, const uint8_t *client_data,
    size_t client_data_len, uint8_t encrypted_bit);
