...
OPENSSL_EXPORT bool TRUST_TOKEN_client_set_srr_key(TT_CTX *ctx, const EVP_PKEY *key);

// TRUST_TOKEN_issuer_set_srr_key sets the private key used to sign the SRR.
OPENSSL_EXPORT bool TRUST_TOKEN_issuer_set_srr_key(TT_CTX *ctx, const EVP_PKEY *key);
...
OPENSSL_EXPORT bool TRUST_TOKEN_client_finish_redemption(
    TT_CTX *ctx, bool *result, uint8_t **out_srr, size_t *out_srr_len, const CBS response);
