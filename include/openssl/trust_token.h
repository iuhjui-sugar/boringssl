...
OPENSSL_EXPORT bool TRUST_TOKEN_privacy_pass_init_key(
...
// TRUST_TOKEN_privacy_pass_client_add_key configures the |ctx| to support the public
// PrivacyPass key |key|. It returns true on success or false on error if the |key|
// can't be parsed or too many keys have been configured.
OPENSSL_EXPORT bool TRUST_TOKEN_privacy_pass_client_add_key(TT_CTX *ctx, const CBS key);
...
// TRUST_TOKEN_privacy_pass_issuer_add_key configures the |ctx| to support the private
// PrivacyPass key |key|. It may either be a basic PrivacyPass key returned by
// |TRUST_TOKEN_privacy_pass_init_key| or a private metadata key returned by
// |TRUST_TOKEN_privacy_pass_init_private_metadata_key|. It returns true on success or
// false on error if the |key| can't be parsed or too many keys have been configured.
OPENSSL_EXPORT bool TRUST_TOKEN_privacy_pass_issuer_add_key(TT_CTX *ctx, const CBS key);
...
// TRUST_TOKEN_issuer_perform_issuance ingests a |request| for token issuance and
...
// call |OPENSSL_free| when done. It returns true on success or false on error.
...
// TODO: TRUST_TOKEN_generate_srr_key - Generate a public/private keypair for signing SRR.
// TODO: TRUST_TOKEN_generate_client_keypair - Generate a public / private keypair for signing request data.
// TODO: TRUST_TOKEN_SignData(PrivateKey, const uint8_t* data, size_t len) → signature - Generates a signature over the given data.
// TODO: TRUST_TOKEN_parse_srr - Parse a SRR.
// TODO: Handle private metadata in SRR.
