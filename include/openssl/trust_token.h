...
// token specified by a |token| buffer of length |token_len|. |*out_value is set
// to the decrypted value, either zero or one. It returns one on success and
// zero on error.
OPENSSL_EXPORT int TRUST_TOKEN_decode_private_metadata(
    uint8_t *out_value, const uint8_t *key, size_t key_len,
    const uint8_t *token, size_t token_len, uint8_t encrypted_bit);
