...
OPENSSL_EXPORT bool TRUST_TOKEN_PrivacyPass_InitKey(
    uint16_t ciphersuite, uint16_t version, std::vector<uint8_t> *priv_key,
    std::vector<uint8_t> *pub_key);

// Initialize a client using the 'PrivacyPass' protocol and public key
// |public_key|.
OPENSSL_EXPORT TT_CTX *TRUST_TOKEN_PrivacyPass_InitClient(
...
// |private_key|.
OPENSSL_EXPORT TT_CTX *TRUST_TOKEN_PrivacyPass_InitIssuer(
    uint16_t ciphersuite, uint16_t max_batchsize, std::vector<uint8_t> key);
