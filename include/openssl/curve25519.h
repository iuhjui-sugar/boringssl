...
// ED25519ph_sign sets |out_sig| to be a signature of |sha512_digest| using
// |private_key|. |sha512_digest| must be the 64-byte result of hashing the
// input message with SHA-512. |context| and |context_len| allow for domain
// separation; see https://datatracker.ietf.org/doc/html/rfc8032#section-8.3.
// It returns one on success, or zero on allocation failure or if |context_len|
// is greater than 255.
OPENSSL_EXPORT int ED25519ph_sign(uint8_t out_sig[64],  //
                                  const uint8_t *context, size_t context_len,
                                  const uint8_t sha512_digest[64],
                                  const uint8_t private_key[64]);

// ED25519ph_verify returns one iff |signature| is a valid signature, by
// |public_key|, of |sha512_digest|. If returns zero otherwise. |sha512_digest|
// must be the 64-byte result of hashing the input message with SHA-512.
// |context| and |context_len| allow for domain separation; see
// https://datatracker.ietf.org/doc/html/rfc8032#section-8.3.
OPENSSL_EXPORT int ED25519ph_verify(const uint8_t *context, size_t context_len,
                                    const uint8_t sha512_digest[64],
                                    const uint8_t signature[64],
                                    const uint8_t public_key[32]);
