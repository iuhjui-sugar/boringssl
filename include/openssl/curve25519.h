 ...
 * |their_msg|, writes at most |max_out_key| bytes to |out_key| and sets
 * |*out_key_len| to the number of bytes written.
 *
 * The resulting keying material is suitable for:
 *   a) Using directly in a key-conformation step: i.e. each side could
 *      transmit a hash of their role and the key material to prove to the other
 *      side that they know the shared key.
 *   b) Using as input keying material to HKDF to generate a variety of subkeys
 *      for encryption etc.
 *
 * If |max_out_key| is smaller than the amount of key material generated then
 * the key is silently truncated. If you want to ensure that no truncation
...
OPENSSL_EXPORT int SPAKE2_process_msg(SPAKE2_CTX *ctx, uint8_t *out_key,
                                      size_t *out_key_len, size_t max_out_key,
