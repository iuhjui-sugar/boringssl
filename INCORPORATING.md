...
[Bazel](http://bazel.io/) and make to build BoringSSL, without too much pain.
...
You cannot link multiple versions of BoringSSL/OpenSSL into a single binary
without dealing with symbol conflicts. In a static link there's not a lot that
can be done because C doesn't have a module system.

When building shared objects (see [BUILDING.md](/BUILDING.md) about defines
that you might need to set for this) BoringSSL sets visibility for exposed API
and has hidden visibility by default. But once incorporated into a larger code
base, you may wish to use a [linker
script](https://sourceware.org/binutils/docs/ld/Scripts.html) to hide the
BoringSSL symbols. This will prevent any collisions with other versions that
may be included in order shared objects.
